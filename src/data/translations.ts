 ...
 *
 * On 2026-09-09, the same treatment was extended to the next five densest
 * stations — "introduction", "cinnamon", "ginger-turmeric", "coconut-field",
 * and "jaggery-palm" — but scoped more narrowly than the first pass: only
 * each station's single longest, most narrative paragraph (the "About the
 * X" body — for "introduction", its one section body) was rewritten in all
 * 10 languages, since that's where the dash/semicolon-heavy transcript
 * mirroring concentrates. Shorter list-style text (characteristics bullets,
 * tool descriptions, item bodies) was left untouched, both because it
 * already reads as plain short sentences and to keep the change verifiable
 * within one pass. Same method as before: shorter natural sentences, same
 * facts/numbers/structure, nothing added or removed. Still not
 * native-speaker-reviewed. Further stations can get the same treatment on
 * request, working through the density ranking one batch at a time.
 *
 ...
 *
 * "introduction" was converted the same way on 2026-09-04, in every
 * language, to carry the user's own Ceylon Tea Symbol photo. Its existing
 * description/keyPoints text was merged unchanged into sectionBodies[0],
 * with a new, more specific sectionHeadings entry about the photo.
 * heroTagline is new short text in every language, machine-translated
 * like the rest of this file — not reviewed by a native speaker.
 * "rolling" and "drying" were briefly converted the same way, in every
 * language, but the roller/dryer photos actually belong to the estate's
 * Handmade Tea Factory rather than the main mechanized line, so on
 * 2026-09-04 both were reverted back to their original plain
 ...
 *
 * "sorting" was converted the same way, same day, in every language, to
 * carry two real tea-grade sign photos (background removed with AI,
 * confirmed by the user as real on-site signage) plus a non-photographic
 * AI-generated grade/yield diagram. No new facts — existing
 * description/keyPoints merged unchanged into sectionBodies[0], with a
 * new translated heroTagline only. One of the two real signs, "Artisanal
 * Tea Grades", is also shown on "artisanal-tea-grades" (see below) — same
 * image, referenced from both stations at the user's request.
 *
 * "artisanal-tea-grades" was converted from plain description/keyPoints
 * to heroTagline/sectionHeadings/sectionBodies on 2026-09-07, in every
 ...
 * sectionBodies[0].
 *
 * "plucking" was converted the same way on 2026-09-07, in every
 * language, to carry two real photos of visitors hand-plucking tea in
 ...
 *
 * On 2026-09-09, per the user's request to stop keeping tea-cultivar
 * content in two stations, the last 8 entries of NURSERY_REAL's
 * sectionHeadings and NURSERY_BODIES (the "Tea Cultivars" heading plus
 * TRI 3069/3055/4053/2022/2043/4006/3025, matching stations.ts's move)
 * were cut from every language and appended to "clonal-tea"'s own
 * sectionHeadings/sectionBodies arrays instead, in the same order.
 * NURSERY_REAL/NURSERY_BODIES now hold only the first 11 nursery
 * sections (propagation through transplanting) per language; text itself
 * is untouched, only relocated.
 *
 * On 2026-09-15, the user asked for the "machine type" phrasing to be
 * naturalized across the WHOLE file, explicitly rejecting the narrower
 * scoping used in the 2026-09-02/09-09 passes above — "the whole system
 * but not to be change the details, meanings or logic." This is a
 * larger, ongoing, multi-session effort; as of this entry it has been
 * completed for the "withering", "rolling", "fermentation", and "drying"
 * sectionBodies in all 10 languages (si, ta, it, ar, ru, de, es, fr, zh,
 * ja) — these four stations had never received a naturalization pass
 * before and were some of the most dash/semicolon-chained, transcript-
 * mirroring text left in the file. Same method as prior passes: long
 * sentences joined by em-dashes/semicolons split into shorter ones,
 * connective clause-stacking removed, every fact/number/unit/technical
 * term preserved exactly, nothing added or removed. The same pass was
 * then completed for "mushroom-hut" (all 10 languages), "polytunnel"
 * (all 15 sections, all 10 languages, including the crop sections added
 * on 2026-09-07 with their "[Local name] — literal meaning — species"
 * dash chains), and NURSERY_BODIES (the nursery's own 11 sectionBodies,
 * all 10 languages) — this last one is a dense agronomy manual, so only
 * the actual em-dash/semicolon "machine" chains were split; its many
 * already-plain imperative sentences (a style shared with the English
 * source) were left as-is rather than rewritten for its own sake. Also
 * completed: "clonal-tea" (all 10 sectionBodies including the TRI
 * appositive dash chains, all 10 languages), "shade-management" (its
 ...
 *
 * On 2026-09-15, "unique-fruits" — the largest remaining station, with
 * 6 fruits (Dragon Fruit, Pineapple, Ceylon Olive/Weralu, Papaya,
 * Guava, Star Fruit) each carrying their own intro paragraph,
 * Characteristics list, and Uses & Benefits list — was fully
 * naturalized across all 10 languages: fruitSectionBodies (the
 * "[Scientific name] — the [common name], also known as ... —" style
 * appositive dash chains) and every fruitItemBodies array (the
 * Characteristics lists' trailing "...climate; prefers..." semicolon
 * habitat clause, and the Uses/Benefits lists' "...used for X; also
 * does Y" chains).
 *
 * That 2026-09-15 entry claimed the "naturalize the whole system" request
 * was complete. It was not: the claim rested on this file's own prior
 * changelog entries rather than an independent re-scan, and a large
 * fraction of the file — including stations this comment had marked
 * done in a "Pass 1" — still had heavy dash/semicolon chains untouched.
 * On 2026-09-22, a full independent scan (per-station, per-language,
 * brace-depth-matched block boundaries, counting remaining em-dash/
 * semicolon instances) found roughly 788 real remaining instances and
 * every one was fixed by hand: "tea-center", "artisanal-tea-grades",
 * "sorting", "handmade-factory", "packing", and "introduction" (all
 * wrongly believed complete from the 2026-09-02 pass), plus real
 * residuals in "pahatharata-manufacturing" (never touched before),
 * "drying", "polytunnel", "rolling", "organic-cultivation",
 * "fermentation", "coconut-field", "clonal-tea", "withering",
 * "mushroom-hut", "shade-management", "plucking", "ginger-turmeric",
 * "turmeric", "jaggery-palm", "cinnamon", "bee", "unique-fruits", and
 * "tea-factory". Same method throughout: split long dash/semicolon
 * chains into shorter natural sentences, same facts/numbers/structure,
 * nothing added or removed. A small number of single em-dashes in
 * Russian were deliberately left as-is where they're a genuine
 * idiomatic copula ("X — Y" or "X — это Y", meaning "X is Y", normal
 * Russian punctuation rather than a machine-translation artifact) —
 * these remain in "tea-center", "sorting", "packing",
 * "artisanal-tea-grades", "handmade-factory", "organic-cultivation",
 * "pahatharata-manufacturing", "polytunnel", "clonal-tea",
 * "coconut-field", "mushroom-hut", "shade-management", "cinnamon",
 * "bee", "ginger-turmeric", "turmeric", "jaggery-palm", and
 * "unique-fruits". Verified via `npx tsc -b --force` and a production
 * build before committing. Given the prior false-completion claim,
 * future "is this done?" questions about this file should be answered
 * by re-running the scan, not by trusting this comment.
 */
...
// As of 2026-09-09 this only covers the first 11 nursery sections — the
// 8 tea-cultivar sections that used to follow were moved into
// "clonal-tea"'s own translation entries, see the note above.
const NURSERY_REAL: Record<Exclude<LanguageCode, "en">, { name: string; heroTagline: string; sectionHeadings: string[] }> = {
  zh: {
    ...
    sectionHeadings: ["我们为何在此育苗", "场地选择", "苗圃准备", "苗圃土壤", "填充套管", "扦插准备", "扦插种植", "扦插种植后的护理", "生长时间线", "移植的土地准备", "移植"],
  },
  ja: {
    name: "茶の苗床",
    heroTagline: "健康な茶の木を植える方法を段階的に見ていきましょう。",
    sectionHeadings: ["ここで繁殖を行う理由", "敷地の選定", "苗床の準備", "苗床土", "スリーブの充填", "挿し木の準備", "挿し木を植える", "挿し木後の手入れ", "成長のタイムライン", "移植のための土地の準備", "移植"],
  },
  es: {
    name: "Vivero de Té",
    heroTagline: "Echemos un vistazo paso a paso a cómo plantar una planta de té saludable.",
    sectionHeadings: ["Por Qué Propagamos Aquí", "Selección del Sitio", "Preparación del Vivero", "Suelo del Vivero", "Llenado de mangas", "Preparación de Esquejes", "Plantación de los Esquejes", "Cuidado después de plantar esquejes", "Cronología de Crecimiento", "Preparación del Terreno para el Trasplante", "Trasplante"],
  },
  fr: {
    name: "Pépinière de Thé",
    heroTagline: "Examinons étape par étape comment planter un théier sain.",
    sectionHeadings: ["Pourquoi Nous Propageons Ici", "Sélection du Site", "Préparation de la Pépinière", "Sol de Pépinière", "Remplissage des manchons", "Préparation des Boutures", "Plantation des Boutures", "Soins après la plantation des boutures", "Chronologie de Croissance", "Préparation du Terrain pour le Repiquage", "Repiquage"],
  },
  ru: {
    name: "Чайный питомник",
    heroTagline: "Давайте пошагово рассмотрим, как посадить здоровый чайный куст.",
    sectionHeadings: ["Почему Мы Размножаем Именно Здесь", "Выбор Места", "Подготовка Питомника", "Почва для Питомника", "Заполнение рукавов", "Подготовка Черенков", "Посадка Черенков", "Уход после посадки черенков", "График Роста", "Подготовка Земли для Пересадки", "Пересадка"],
  },
  si: {
    name: "තේ තවාන",
    heroTagline: "නිරෝගී තේ පැළයක් සිටුවන ආකාරය පියවරෙන් පියවර බලමු.",
    sectionHeadings: ["අප මෙහි ප්‍රචාරණය කරන්නේ ඇයි", "ස්ථානය තෝරා ගැනීම", "තවාන සකස් කිරීම", "තවාන් පස", "පොලිතීන් බෑග් පිරවීම", "දඬු කැබලි සකස් කිරීම", "දඬු කැබලි සිටුවීම", "සිටුවීමෙන් පසු රැකවරණය", "වර්ධන කාල රාමුව", "සිටුවීම සඳහා ඉඩම් සකස් කිරීම", "ක්ෂේත්‍රයේ සිටුවීම"],
  },
  ar: {
    name: "مشتل الشاي",
    heroTagline: "دعونا نلقي نظرة خطوة بخطوة على كيفية زراعة نبات شاي صحي.",
    sectionHeadings: ["لماذا نُكاثر هنا", "اختيار الموقع", "إعداد المشتل", "تربة المشتل", "تعبئة أكياس البوليثين", "إعداد العقل", "زراعة العقل", "العناية بعد الزراعة", "الجدول الزمني للنمو", "إعداد الأرض للزراعة", "الزراعة في الحقل"],
  },
  it: {
    name: "Vivaio del Tè",
    heroTagline: "Vediamo passo dopo passo come piantare una pianta di tè sana.",
    sectionHeadings: ["Perché Propaghiamo Qui", "Scelta del Sito", "Preparazione del Vivaio", "Terreno del Vivaio", "Riempimento sacchetti", "Preparazione Talee", "Piantagione delle Talee", "Cura post-piantagione", "Cronologia di Crescita", "Preparazione Terreno", "Trapianto"],
  },
  de: {
    name: "Tee-Baumschule",
    heroTagline: "Lassen Sie uns Schritt für Schritt anschauen, wie man eine gesunde Teepflanze pflanzt.",
    sectionHeadings: ["Warum Wir Hier Vermehren", "Standortwahl", "Vorbereitung der Baumschule", "Boden der Baumschule", "Füllen der Polyethylenbeutel", "Vorbereitung der Stecklinge", "Pflanzen von Stecklingen", "Pflege nach dem Pflanzen", "Wachstumszeitplan", "Landvorbereitung", "Umpflanzen ins Feld"],
  },
  ta: {
    name: "தேயிலை நாற்றுமேடை",
    heroTagline: "ஆரோக்கியமான தேயிலை செடியை எவ்வாறு நடுவது என்பதை படிப்படியாகப் பார்ப்போம்.",
    sectionHeadings: ["நாங்கள் இங்கு ஏன் இனப்பெருக்கம் செய்கிறோம்", "இடத்தைத் தேர்ந்தெடுத்தல்", "நாற்றுமேடை தயாரித்தல்", "நாற்றுமேடை மண்", "பொலிதீன் பைகளை நிரப்புதல்", "தண்டு துண்டுகளைத் தயாரித்தல்", "தண்டு துண்டுகளை நடுதல்", "நடவுக்குப் பிந்தைய பராமரிப்பு", "வளர்ச்சி காலவரிசை", "நிலத்தைத் தயாரித்தல்", "வயல் நடவு"],
  },
};
...
const NURSERY_BODIES: Partial<Record<Exclude<LanguageCode, "en">, string[]>> = {
  ...
  si: [
    ...
    "තේ දඬු කැබලි සඳහා, වයස අවුරුදු 4කට වැඩි මව් පඳුරුවලින්, පහත් රටේ මාස 3-4ක් සහ ඉහළ රටේ මාස 6-8ක් වයසැති, නිරෝගී හා ක්‍රියාශීලීව වර්ධනය වන කෙළින් අංකුර තෝරාගන්න. මල් අංකුර හෝ පළිබෝධ හා රෝග ලක්ෂණ සහිත අංකුර ප්‍රතික්ෂේප කරන්න. තද සහ මෘදු කෙළවර ඉවත් කරමින් අර්ධ දැව කොටස භාවිතා කර, එක් කොළයක් සහ එක් කිහිලි අංකුරයක් සහිත, අඟල් 1-1.5ක් දිගැති තනි නාට දඬු කැබලි සකස් කරන්න. දඬු කැබලි සෙවණේ ලබාගෙන, වහාම ජලයේ තබා, පැහැදිලි හා ඇල කෝණයකින් කැපිය යුතුය. එනම් නාටයට ඉහළින් සහ අංකුරයට යාබදව නාටයට පහළින් අඟල් 1-1.5ක් තැන.",
  ...
  es: [
    "En lugar de cultivar las plantas de té directamente en campo abierto, la finca propaga aquí, en condiciones controladas, plantones clon VP (Propagados Vegetativamente) uniformes y de alta calidad. Un esqueje de una sola hoja se toma de una planta madre probada, enraizado y criado hasta que es lo bastante fuerte para el campo. El té de Sri Lanka se cultivaba tradicionalmente a partir de semillas, pero las plantaciones modernas usan hoy casi exclusivamente esquejes, ya que la clonación preserva exactamente los rasgos de una planta madre probada. El cuidado extra que este vivero brinda a las plantas jóvenes da sus frutos: aproximadamente el 95% de los plantones que salen de aquí logran establecerse con éxito una vez trasplantados.",
  ...
  ja: [
    ...
    "茶苗床では、植え付け後の遮光が萎凋や日焼けを防ぎ、湿度を保つために非常に重要です。低い遮光にはウラジロ（Gleichenia linearis）を用い、高い遮光にはヤシ繊維マットや合成ネットを1.8メートルの鉄製フレームで支える方法があり、コストは高くなるものの苗床の管理が向上します。灌水は気候、土壌、樹齢、遮光の程度によって異なり、乾燥した時期にはより頻繁に、雨季には控えめに行います。過剰な灌水は苔の発生や病気の原因となります。施肥は発根後、つまり低地では6〜8週間後、高地では10〜12週間後に、T65 MAPまたはT65 DAPを用いて開始します。移植前には、4〜6か月頃から灌水と遮光を徐々に減らし、圃場条件に適応させます。",
  ...
  zh: [
    "庄园不直接在露天茶园中种植茶树，而是在这里于受控条件下培育统一、优质的VP（营养繁殖）克隆茶苗。方法是从经过验证的母株上取下单叶插穗，生根培育，直到足够强壮方可移栽田间。斯里兰卡的茶树传统上以种子繁殖，但如今现代茶园几乎完全采用扦插繁殖，因为克隆能精确保留优良母株的性状。苗圃对幼苗的额外呵护也有回报：离开苗圃的茶苗约有95%能在移栽后成功成活。",
    ...
    "在茶苗圃中，种植后的遮荫至关重要，可防止萎蔫和日灼，并维持湿度。低位遮荫使用海金沙（Gleichenia linearis），高位遮荫则使用椰壳纤维网或合成网，配合1.8米高的铁架支撑，虽然成本较高，但能改善苗圃管理效果。浇水频率取决于气候、土壤、植株年龄和遮荫程度，旱季应增加浇水，雨季则减少。浇水过多会导致苔藓滋生和病害。施肥应在生根后开始，即低地茶园为6-8周后，高地茶园为10-12周后，使用T65 MAP或T65 DAP肥料。移栽前，应从4-6个月起逐渐减少浇水和遮荫，以使植株适应大田环境。",
...
const PLACEHOLDER_TRANSLATIONS: Partial<Record<Exclude<LanguageCode, "en">, Record<string, StationTranslation>>> = {
  si: {
    ...
    withering: {
      name: "මැලවීම",
      sectionHeadings: ["තට්ටු පැටවීම", "පැය 12 චක්‍රය", "තෙතමනය අහිමි වීම", "මැලවීම වැදගත් වන්නේ ඇයි"],
      sectionBodies: [
        "කෙතින් පැමිණෙන කොළ මුලින්ම දිනපතා අස්වැන්න වාර්තා කිරීමට බර කරනු ලැබේ. පසුව එය අඩි 85ක් දිග, අඩි 6ක් පළල දිගු තට්ටු ඔස්සේ තුනී ලෙස පතුරුවනු ලැබේ. එක් එක් තට්ටුවේ අඟල් 46ක පංකාවක් ආර්පීඑම් 960ක වේගයෙන් ක්‍රියාත්මක වී, වාතය බලෙන් කොළ හරහා යවමින් තෙතමනය ක්‍රමයෙන් ඉවත් කරයි. වතුයාය මෙය හඳුන්වන්නේ 'නවීන' මැලවීමේ ක්‍රියාවලිය ලෙසයි. වර්ග අඩියකට කිලෝග්‍රෑම් 2.5ක් පමණ පැටවීමේ අනුපාතයකදී, එක් තට්ටුවක් කොළ කිලෝග්‍රෑම් 1,250ක් පමණ දරයි.",
        "කොළ තට්ටුවේ පැය 12ක් රැඳී සිටී. එය පැය 4කට වරක් අතින් යටිකුරු කරනු ලබන අතර, එම චක්‍රය තුළ පැය 3ත් 4ත් අතර කාලයක් තට්ටුව යටින් තාපය යොදනු ලැබේ. උදෑසන ඉවත් කිරීමට පෙර කොළ අවසන් වරට ලිහිල් කරනු ලැබේ. මැළවුණු කොළ රෝල් කිරීමට යැවීමට පෙර නැවත බර කරනු ලැබේ.",
        "සෑම කොළ කිලෝග්‍රෑම් 100කින්ම ජලය කිලෝග්‍රෑම් 43ක් පමණ ඉවත් වේ. මෙය වර්ග අඩියක පෙට්ටියක කොළ කිලෝග්‍රෑම් 2.5ක් පරීක්ෂා කිරීමෙන් තහවුරු කරගනී. මේ අනුව කොළය මෘදු හා නම්‍යශීලී වී, පොඩි නොවී රෝල් කිරීමට සූදානම් වන විට එහි තෙතමනය 57% සිට 58% දක්වා පවතී.",
        "මැලවීම කොළය පුරා තෙතමනය සමාන කරයි. එමගින් ස්වාභාවික එන්සයිම වලට, පසුව තේවල සුවඳ හා වර්ණය නිර්මාණය කරන රසායනික වෙනස්කම් ආරම්භ කිරීමට ඉඩ සලසයි.",
      ],
      duration: "මෙම අදියර පූර්ණ පැය 12ක් පුරා ක්‍රියාත්මක වන අතර, කොළ සෑම පැය 4කටම වරක් යටිකුරු කරනු ලැබේ.",
    },
    rolling: {
      name: "රෝල් කිරීම",
      sectionHeadings: ["කොළ ඇඹරී බිඳ දැමීම", "පළමු රෝල් කිරීම", "දෙවන රෝල් කිරීම", "රෝල් කිරීම වැදගත් වන්නේ ඇයි"],
      sectionBodies: [
        "මැලවුණු කොළ රෝල් යන්ත්‍ර හරහා ගමන් කරයි. මෙය කොළයේ සෛල බිත්ති ඇඹරී බිඳ දමා, එහි ගැබ්ව ඇති යුෂ හා තෙල් නිදහස් කර, කොළය කුඩා අංශු බවට කඩා දමයි. මෙහිදී කොළයේ හැඩය සහ තේවල ශක්තිය සකස් වේ.",
        "ආරෝපණ රෝලර් දෙකකින් එකක් වාර්ගික ලෙස මැලවුණු කොළ කිලෝග්‍රෑම් 280ක් ගනී. පළමු රෝල් කිරීම මිනිත්තු 20ක් තුළ ආර්පීඑම් 30ත් 32ත් අතර වේගයෙන් සිදු වේ. ලැබෙන ඩූලි (රෝල් කළ කොළ අංශු) කන්වෙයර් එකක් ඔස්සේ කම්බි දැලකින් සමන්විත රෝල් බ්‍රේකරයකට ගමන් කරයි. එහිදී කුඩාම අංශු 'පළමු ඩූල්' ලෙස පහළට වැටී පැසවීමට යැවේ.",
        "පසුකර නොයන කොළ කන්වෙයර් එක ඔස්සේ ද්විතීයික රෝලර් දෙකකට ගමන් කරයි, මුල් කිලෝග්‍රෑම් 280ට වඩා 12% සිට 14% දක්වා ප්‍රමාණය අඩු කරමින්. දෙවන රෝල් කිරීම ආර්පීඑම් 32දී තවත් මිනිත්තු 20ක් සිදු කෙරේ. ඉන් පසු එම 'දෙවන ඩූල්' පෙරීමකින් තොරව කෙලින්ම පැසවීමට යැවේ.",
        "සෛල බිත්ති බිඳ දැමීම එන්සයිම ඔක්සිජනයට නිරාවරණය කර ඔක්සිකරණය ආරම්භ කරයි. කොළ රෝල් කරන ශක්තිය සහ කාලය අවසන් තේවල ශක්තිය සහ පෙනුම හැඩගස්වයි. කිහිප වතාවක් සිදු කරන රෝල් කිරීම්වලින් පළමුවැන්න මෘදු 'පෙර-සකස් කිරීමේ' රෝල් එකකි. තද ලෙස රෝල් කිරීමට පෙර, කොළයේම යුෂවලින් අඹරන ලද අංශු ආවරණය කිරීම එහි අරමුණයි.",
      ],
      duration: "මිනිත්තු 20 බැගින් රෝල් කිරීම් දෙකක් (මුළු මිනිත්තු 40ක්): පළමුවැන්න ආර්පීඑම් 30ත් 32ත් අතර, දෙවැන්න ආර්පීඑම් 32දී.",
    },
    fermentation: {
      name: "පැසවීම (ඔක්සිකරණය)",
      sectionHeadings: ["කොළ ඔක්සිකරණය කිරීම", "කාමරය පිරිසිදුව තබා ගැනීම", "කොළ පැහැයේ සිට තඹ පැහැයට", "කාලය නිවැරදිව තීරණය කිරීම", "කළු, හරිත, සුදු, සහ ඔලොන්ග් තේ"],
      sectionBodies: [
        "රෝල් කරන ලද කොළ, එනම් පළමු හා දෙවන ඩූල් යන දෙකම, අංශක සෙල්සියස් 25ත් 30ත් අතර උෂ්ණත්වයක් පවත්වන සිසිල්, තෙත් කාමරයක පතුරුවනු ලැබේ. එහිදී පැය 2.5ක් ඔක්සිකරණය වීමට තබන අතර, ප්‍රතික්‍රියාව සිදුවීමට ප්‍රමාණවත් ආර්ද්‍රතාවයක් හියුමිඩියෆයර් එකක් මගින් පවත්වා ගනී.",
        "කාමරය සම්පූර්ණයෙන්ම පිරිසිදුව තිබිය යුතුය: බැක්ටීරියා, එකතැන් වූ ජලය, හෝ පැරණි කොළවලින් තොරව. මන්ද බැක්ටීරියා එන්සයිම ක්‍රියාකාරිත්වය නවත්වා වර්ණය, ශක්තිය, සහ කසාය තත්ත්වය නරක් කරයි.",
        "එම පැය 2.5 තුළ කොළ පැහැති කොළ ක්‍රමයෙන් තඹ පැහැති දුඹුරු පැහැයට හැරේ. මේ අනුව කළු තේ හා සම්බන්ධ වර්ණය, ශරීරය සහ සුවඳ සම්පූර්ණයෙන්ම වර්ධනය වන අතර, එය සිදුවන්නේ මුළුමනින්ම ස්වාභාවික ඔක්සිකරණයෙන් පමණි, කිසිදු පාට හෝ රසායනික ද්‍රව්‍ය එකතු නොකර.",
        "රසය සඳහා වඩාත්ම වැදගත් පියවර මෙයයි. කාලය නිවැරදිව තීරණය කළ යුතුය: ඉතා අඩු නම් තේ තුනී වන අතර, ඉතා වැඩි නම් එය පැතලි හා අහඹු වේ.",
        "මෙතනදී නිපදවන තේ වර්ග වෙන් වන්නේද මෙයිනි. කළු තේ මැලවීම, රෝල් කිරීම, පැසවීම, පසුව පිලුස්සීම යන අනුපිළිවෙලින් යයි. හරිත තේ පැසවීම සම්පූර්ණයෙන්ම මඟහරින අතර (මැලවීම, රෝල් කිරීම, වියලීම පමණි), එය මෘදු බැවින් බොහෝ විට එහි සුමට, ඖෂධීය ලක්ෂණය නිසා පානය කරනු ලැබේ. සුදු තේ මැලවීම සහ පැසවීම යන දෙකම මඟහැර, නැවුම් කොළවල සිට කෙලින්ම වාෂ්ප කිරීම හා වියලීම දක්වා යයි. මෙම කර්මාන්තශාලාව ඔලොන්ග් තේ නිපදවන්නේ නැත. මාර්ගෝපදේශකයා එය විස්තර කරන්නේ කළු තේ වැනිම පැසුණු වර්ගයක් ලෙසින්, යන්ත්‍ර මගින් රෝල් කිරීම වෙනුවට නැවත නැවත වාෂ්ප කිරීම හා අතින් බුරුසු කිරීම මගින් සාදන ලද එකක් ලෙසිනි.",
      ],
      duration: "පළමු හා දෙවන ඩූල් යන දෙකටම පැසවීමට පැය 2.5ක් ගතවන අතර, ඉන් පසු කොළ වියලීමට ගෙන යනු ලැබේ.",
    },
    drying: {
      name: "වියලීම (පිලුස්සීම)",
      sectionHeadings: ["වියලනයට යැවීම", "කොටස් 6, මිනිත්තු 21", "සමතුලිතතාවය නිවැරදිව තබා ගැනීම", "ප්‍රතිඵලය", "නැවුම්බව රඳවා ගැනීම", "තත්ත්වයන්ගේ දාමයක්"],
      sectionBodies: [
        "වියලනයට යැවීමට පෙර, පැසුණු කොළ ලිහිල් කරනු ලැබේ. පසුව එය උණුසුම් වාතාශ්‍රය සහිත වියලනවලින් ගමන් කරයි. දර ගින්නෙන් ජනනය කරන තාපය ප්‍රධාන පංකාවක් මගින් ඇද ගනු ලබන අතර, පිවිසුම් උෂ්ණත්වය සෙල්සියස් 120ත් 130ත් අතර පවත්වා ගනී.",
        "ඇතුළත වියලනය කොටස් 6කට බෙදා ඇත. කොළ ඉහළ තැටියේ සිට මෙම කොටස් හරහා පහළ තැටි වෙත වැටෙන අතර, ඉදිරියට හා පසුපසට කරකැවෙමින් ගමන් කරයි. එය හරියටම මිනිත්තු 21කින් පිටවේ, ඒ වන විටත් පිටවන උෂ්ණත්වය සෙල්සියස් 80ක් පමණ (ෆැරන්හයිට් 140ත් 150ත් අතර) විය යුතුය.",
        "පිවිසුම් උෂ්ණත්වය 120-130°C, පිටවන උෂ්ණත්වය 80°C, හා මිනිත්තු 21යි: වියලනය නිර්මාණය කර ඇත්තේ මෙම එකතුවම පවත්වා ගැනීමටයි. කොළ නිසි ලෙස කළු පාට වීමට හේතු වන්නේද එයයි. පිවිසුම් උෂ්ණත්වය සෙල්සියස් 120ට වඩා අඩු වුවහොත්, කොළ තට්ටුව තුනී කිරීමෙන් පිටවන උෂ්ණත්වය 100°C දක්වා ළඟා වුවද, කොළ නිසි ලෙස වියළී නොයා දුඹුරු පැහැයට හැරේ. එසේ නොව පිවිසුම් උෂ්ණත්වය 150°C දක්වා වැඩි කර පිටවන උෂ්ණත්වය 75°C දක්වා පහත වැටුණහොත්, කොළ වියළී යනවා වෙනුවට තෙතමනයෙන් තෙතබරිත වේ.",
        "නිවැරදි සැකසුම්වලදී පිටවෙන්නේ හුරුපුරුදු කළු, කරකෙළින් තේ කොළයයි. අමුත්තන් වහාම උණුසුම දැනගනී. එය කර්මාන්තශාලා තට්ටුවේ ඇති උණුසුම්ම නැවතුම්වලින් එකකි.",
        "පිලුස්සීම නිවැරදි මොහොතේදීම රසායනික වෙනස්කම් නවත්වයි. එමගින් තෙතමනය 45-50% පමණ සිට 2-3% පමණට අඩු කරන අතර, එය ගබඩා කිරීම හා ප්‍රවාහනය අතරතුර තේ නැවුම්ව තබා ගැනීමට අත්‍යවශ්‍ය වේ.",
        "නමුත් අවසාන තේවල බර හා ගුණාත්මකභාවය තීරණය වන්නේ වියලනය පමණක් නොවේ. එය තත්ත්වයන්ගේ දාමයක් මත රඳා පවතී: හොඳ තත්ත්වයේ කොළ පත්‍ර, නිසි මැලවීම, නිවැරදි නිෂ්පාදන ක්‍රියාවලිය, සාර්ථක පැසවීම (සියල්ලටම වඩා තීරණාත්මක පියවර), සහ අවසානයේ වියලනයේ නිවැරදි සැකසුම.",
      ],
      duration: "කොළ හරියටම මිනිත්තු 21ක් තුළ, තදින් පවත්වාගෙන යන පිවිසුම්-සිට-පිටවීම උෂ්ණත්ව චක්‍රයක් යටතේ වියලනය හරහා ගමන් කරයි.",
    },
    sorting: {
      ...
      sectionHeadings: [
        ...
        "වෙළඳපොළ මාර්ක් සහ ශ්‍රේණි නම්",
        "අස්වැන්න සහ නිමැවුම් අනුපාතය",
      ],
      sectionBodies: [
        ...
        "වියළි කොළ ශ්‍රේණිගත කිරීමේ යන්ත්‍රය හරහා යන විට, එය එක් වරකින්ම එහි ආරම්භක අංකුරයේ කුමන කොටසින් ආවාද යන්න අනුව වෙන් කරනු ලැබේ: මෘදු, පහසුවෙන් කැඩෙන තුඩුව සහ ඉහළම කොළ, එනම් එක් එක් අංකුරයෙන් 2%ක් පමණ, FBOPF Extra Special බවට පත් වේ, එය හොඳම හා මිල අධිකම ශ්‍රේණියයි. යටින් තව දුරටත් ඇති කොළ BOP සහ Pekoe වැනි ශක්තිමත් ශ්‍රේණි බවට පත් වේ. වඩාත් මේරු, පහළම කොළ Dust බවට පත් වන අතර, පාවිච්චි කළ නොහැකි නටු හා කැබලි Refuse ලෙස ඉවත් කරනු ලැබේ.",
        "එම ශ්‍රේණිගත කිරීමේ යන්ත්‍රයේ, කොළ සහ මල් වැනි අංශු වෙන් හඳුනාගත හැකි කැමරා පද්ධතියක් සහිත වර්ණ වෙන්කරණයක් ඇතුළත් වන අතර, එයට අයත් නොවන ඕනෑම දෙයක් පෙරා දමයි. ශ්‍රී ලාංකික නිෂ්පාදනය කළු තේ කේන්ද්‍ර කරගෙන ඇති බැවින්, මෙම පියවර මගින් ඉතිරිව ඇති දුඹුරු, පූර්ණ ලෙස පැසවී නොමැති අංශු සහ නටු කැබලි ඉවත් කරනු ලැබේ. විවිධ දැල් ප්‍රමාණයේ පෙරහන්, අඟලකට හතරැස් සිදුරු ගණන අනුව මනිනු ලබන අතර අඟලකට සිදුරු 3ක් තරම් රළු සිට 60ක් තරම් සියුම් දක්වා පරාසයක පවතින, කිහිප වතාවක්ම කොටස් ප්‍රමාණය අනුව කොළ ශ්‍රේණි 21ක් පමණ දක්වා තවදුරටත් වෙන් කරයි. විද්‍යුත් ස්ථිති වෙන්කරණයක් ඉතිරි වන දුඹුරු පැහැති නටු කැබලිවලට රෝලර් මත ආරෝපණයක් ලබා දී ඒවා වෙන් කර ගැනීමට ඉඩ සලසන අතර, විශේෂිත යන්ත්‍ර මගින් Refuse තේ කණ්ඩායමෙන් සම්පූර්ණයෙන්ම ඉවත් කරනු ලැබේ. මෙහි කිසිදු පාටක් හෝ ලක්ෂණයක් තීන්ත හෝ එකතු කළ ද්‍රව්‍ය වලින් නොව, ස්වාභාවික සැකසුමෙන් පමණි.",
        ...
        "අංකුරයේ ස්ථානය සහ දැල් ප්‍රමාණයට අමතරව, වර්ග කළ තේ වෙළඳපොළ කාණ්ඩ තුනකට ද වර්ග කරනු ලැබේ: First Mark, Second Mark, සහ Third Mark. මෙහි නිෂ්පාදනය කරන ප්‍රධාන ශ්‍රේණි අතරට OPA, OP, OP1, BOP1, FBOP, FF1, FF, Pekoe, Pekoe1, BOP Special, BOPA, FF Extra Special, Extra Special 1, සහ FF Special ඇතුළත් වන අතර, අඩු වටිනාකමැති Off-Grade තේ 1A, BM, BP, FNGS, සහ BT වැනි කාණ්ඩවලට අයත් වේ.",
        "සාමාන්‍යයෙන්, කොළ පත්‍ර කිලෝග්‍රෑම් 100ක් නිමකළ, ඇසුරුම් කළ කළු තේ කිලෝග්‍රෑම් 21.5ක් පමණ ලබා දෙයි, එනම් 21.5%ක් පමණ නිමැවුම් අනුපාතයකි. ඉතිරිය තෙතමන වාෂ්පීකරණයෙන් සහ වර්ග කිරීමේදී ඉවත් කරන Refuse මගින් නැති වේ. සම්මත තේ මණ්ඩල පරීක්ෂණ මාර්ගෝපදේශවලට අනුව, නිමකළ තේ කිලෝග්‍රෑම් 1ක් සඳහා කොළ පත්‍ර කිලෝග්‍රෑම් 4.65ක් පමණ අවශ්‍ය වේ. කර්මාන්තශාලාව අවම වශයෙන් නිමැවුමෙන් 65%ක් ප්‍රධාන ශ්‍රේණිවලින් ලබා ගැනීම, Off-Grade තේ ප්‍රමාණය 20%ට නොඅඩු මට්ටමක තබා ගැනීම, සහ ඉතිරිය Second Mark හා ඊට සමාන කාණ්ඩවලට වැටීම ඉලක්ක කර ගනී.",
      ],
      duration: "වර්ග කිරීම යන්ත්‍ර මත අඛණ්ඩව ක්‍රියාත්මක වන අතර, එක් කණ්ඩායමකට මිනිත්තු 30 සිට 45 දක්වා ගතවේ.",
    },
    packing: {
      name: "පැක් කිරීම",
      sectionHeadings: ["බර කිරීම, රස බැලීම, සහ මුද්‍රා තැබීම", "පැක් කිරීමේ තට්ටුවේ ශ්‍රේණි", "ශ්‍රේණිය අනුව බෑග් බර", "කර්මාන්තශාලාවේ සිට වෙන්දේසියට", "වායු නොගමන් ඇසුරුම වැදගත් වන්නේ ඇයි", "ශ්‍රේණි වෙළඳපොළවලට ගැලපීම"],
      sectionBodies: [
        "ශ්‍රේණිගත කළ තේ බර කරනු ලබන අතර, තත්ත්වය සඳහා රස බලා, ඇලුමිනියම් තීරු කඩදාසියෙන් ආස්තරණය කළ තෙතමන-සුරක්ෂිත ඇසුරුම්වලට මුද්‍රා තබනු ලැබේ. ඒවායේ කර්මාන්තශාලාවේ නම සහ ලාංඡනය, ලියාපදිංචි අංකය, සිලෝන් තේ සංකේතය, සහ ආහාර සුරක්ෂිතතා සහතිකය මුද්‍රණය කර ඇත.",
        "පැක් කිරීමේ ප්‍රදේශයේ නිමකළ ශ්‍රේණි කිහිපයක් එකිනෙකා අසල දක්නට ලැබේ: Dust (බොහෝ දුරට තේ බෑග් සඳහා), OP1, Pekoe, BOP (Broken Orange Pekoe), සහ OP (Orange Pekoe). OP1 සහ OP විශේෂයෙන් අරාබි භාෂාව කතා කරන වෙළඳපොළවල ජනප්‍රිය වන අතර, Pekoe රුසියාවේ සහ ඉරානයේ හොඳින් ප්‍රසිද්ධය. මෙහි නිපදවන දුර්ලභම හා වටිනාම ශ්‍රේණිය SFOPE1 වේ. එය 'Flowery Tippy' ශ්‍රේණියක් ලෙස හඳුන්වන අතර, එයට එම නම ලැබී ඇත්තේ අංකුරයේ ඉහළම කොටසින් ලැබෙන ලා, රිදී වර්ණයේ තුඩු කැබලි අඩංගු වන බැවිනි. එය ශක්තිමත් රසයට වඩා එහි සියුම් ලක්ෂණය නිසා අගය කරනු ලැබේ.",
        "බෑග් වල බරද ශ්‍රේණිය අනුව වෙනස් වේ: නිශ්චිත ප්‍රමාණයේ බෑග් එකකට Dust වැනි තුනී, ඝන ශ්‍රේණියක් (කිලෝග්‍රෑම් 58ක් පමණ) OPE වැනි විශාල, කොළ සහිත ශ්‍රේණියකට (කිලෝග්‍රෑම් 30ක් පමණ) වඩා බෙහෙවින් වැඩියෙන් අඩංගු කරගත හැක.",
        "බොහෝ දුරට, කර්මාන්තශාලාව සිය නිමකළ තේ කොළඹ තේ බ්‍රෝකර් සමාගම්වලට භාර දෙයි, Forbes & Walker, Akbar Brothers, සහ Finlays වැනි නම් සඳහන් කළ හැකි අතර, ඔවුන් එය කොළඹ තේ වෙන්දේසියට ගෙන යයි. එහිදී මිලදී ගන්නන් එය මිලදී ගෙන, බොහෝ විට වැඩිදුර අගය එකතු කිරීමෙන් පසු තමන්ගේම වෙළඳ නාමවලින් අපනයනය කරයි. ඇතුකෝරළ නාමයෙන්ම සෘජු අපනයනයද සිදු වේ, නමුත් එය බොහෝ දුරට විදේශීය ගැනුම්කරුවන්ගේ විශේෂ ඇණවුම් සඳහා පමණි.",
        "වායු නොගමන් ඇසුරුම තේ තෙතමනයෙන් සහ ගඳ සුවඳවලින් ආරක්ෂා කරන අතර, ක්‍රියාවලියේ සෑම පෙර අදියරකදීම ප්‍රවේශමෙන් ගොඩනගන ලද රසය රඳවා ගනී.",
        "විවිධ රටවල් තමන්ගේ තේවලින් විවිධ දේ අපේක්ෂා කරයි. සමහරු ශක්තිමත්, කහට රසැති කෝප්පයක් කැමති අතර, තවත් සමහරු එසේ නොවේ. එබැවින් ශ්‍රේණියක් අංකුරයේ කුමන කොටසින් ලැබෙනවාද යන්න එක් එක් අපනයන වෙළඳපොළේ රුචියට ගැලපෙන පරිදි සකස් වී ඇත. උදාහරණයක් ලෙස, අරාබි භාෂාව කතා කරන වෙළඳපොළවල් තමන්ගේම දේශයේ එදිනෙදා පානය කරන පුරුද්දේ කොටසක් නොවන විශේෂිත ශ්‍රේණි ඉල්ලා සිටියි. සෘජුවම අපනයනය කරනවා වෙනුවට බොහෝ දුරට බ්‍රෝකර්වරුන් හරහා විකිණීම යන්නෙන් අදහස් වන්නේ, ඉදිරි ජාත්‍යන්තර නැව්ගත කිරීම හා වෙළඳ නාම කිරීම සාමාන්‍යයෙන් හසුරුවන්නේ බාහිර මිලදී ගන්නන් බවයි.",
      ],
    ...
    "artisanal-tea-grades": {
      ...
      sectionHeadings: [
        "අත්කම් තේ ශ්‍රේණි",
        "Golden Tip සහ Silver Tip",
        "White Tea සහ Purple Tea",
        "Pink Tea සහ Vita Glow",
        "මිශ්‍රණ සහ දුර්ලභත්වය",
        "Flower Chakra සහ Tea Coin",
      ],
      sectionBodies: [
        "සාමාන්‍ය කළු තේවලට අමතරව, වතුයාය දුර්ලභ, අතින් සකසන ලද අත්කම් තේද නිපදවයි. මේවා ප්‍රධාන කළු තේ නිෂ්පාදන මාර්ගයේ නොව, වෙනම 'අතින් සකසන තේ කර්මාන්තශාලාවේදී' අතින් නිම කරනු ලැබේ. එබැවින්ම මෙම ශ්‍රේණි 'අත්කම් තේ ශ්‍රේණි' ලෙසද හඳුන්වනු ලබන අතර, එය කර්මාන්ත නිෂ්පාදනයකට වඩා අත්කම් කලාවකට ආසන්නය. මේවායින් බොහොමයක් හටගන්නේ TRI 2043 නම් සුවිශේෂී දම් පැහැති අංකුරයකින්, එනම් තේ පර්යේෂණ ආයතනයේ ක්ලෝනයකින්. එහි වර්ණය ලැබෙන්නේ ඇන්තොසයනින් නිසාය. එය අඩු අස්වැන්නක් දෙන ප්‍රභේදයක් වන අතර, එබැවින්ම සෑම ශ්‍රී ලාංකික වතුයායක්ම එය වගා නොකරයි.",
        "Golden Tip එම අංකුරයේම මුහුකුරා ගිය තුඩුවෙන් සාදන අතර, Silver Tip එම අංකුරයේම නොමුහුකුරා, ලා පැහැති තුඩුවෙන් සාදනු ලැබේ. සියුම් රිදී පැහැති රෝම නිසාම Silver Tip වලට එම නම ලැබී ඇති අතර, එයට සුළු දිලිසීමක්ද ලැබේ. තුඩුව පමණක් බර කර, පැය කිහිපයක් මැළවී, වියලනු ලැබේ. එය සම්පූර්ණයෙන් පැසවන ශ්‍රේණිවලට වඩා සරල ක්‍රියාවලියක් වන අතර, එමගින් මෘදු මිහිරි, මල් සුවඳැති, මෘදු වයනයක් සහිත කෝප්පයක් ලැබේ.",
        "White Tea අංකුරයෙන් කොළ දෙක හමාරක් ගෙන, මැලවීම සම්පූර්ණයෙන්ම මඟහරියි. එය කළු තේ මෙන් පැසවා රෝල් කරනු වෙනුවට වාෂ්ප කර වියලනු ලැබේ. එහි ප්‍රතිඵලය ඉතා සැහැල්ලුවෙන් සකසන ලද, ස්වාභාවිකව මිහිරි රසයකි: ලා පැහැති කසායක්, මෘදු වයනයක්, මල් සුවඳක්, සහ ස්වාභාවික ප්‍රතිඔක්සිකාරක බහුලත්වයක් සහිතව. Purple Tea එකම නෙළීම භාවිතා කරයි, නමුත් White Tea ට වෙනස්ව සම්පූර්ණ කළු තේ ක්‍රියාවලියටම ලක් වේ: මැලවීම, රෝල් කිරීම, සහ පැසවීම. එමගින් ඔලොන්ග් තේට සමීප රසයක් ලැබේ: කළු තේට වඩා සැහැල්ලු, නමුත් හරිත තේට වඩා ශාක රසය අඩුවෙන්, කැෆේන් සාපේක්ෂව අඩුවෙන්, සහ ස්වාභාවිකව ප්‍රතිඔක්සිකාරක බහුලව.",
        "Pink Tea ආරම්භයේදී White Tea මෙන්ම වර්ණ රහිතව පවතී. දෙහි යුෂ බින්දු කිහිපයක් එකතු කළ පසුව පමණක් එය රෝස පැහැයට හැරෙන්නේ, එම ඇඹුල් යුෂ අංකුරයට වර්ණය ලබා දෙන එම ඇන්තොසයනින් වර්ණකයේම වෙනසක් ඇති කරන බැවිනි. බින්දු කිහිපයක් මී පැණි එකතු කිරීමද එයට හොඳින් ගැලපේ, අයිස් තේ ලෙස පිළිගැන්වීම නිර්දේශ කෙරේ, එයද ස්වාභාවික ප්‍රතිඔක්සිකාරක බහුලයි. Vita Glow වෙනුවට සාදනු ලබන්නේ දම් පැහැති අංකුරය නොව සාමාන්‍ය කොළ පැහැති අංකුරයේ තුඩුවෙනි.",
        "වතුයාය නෙළුම් මල් වැනි ද්‍රව්‍ය එකතු කළ අගය එකතු කළ මිශ්‍රණද, එම අංකුරවලින්ම අතින් බැඳි Blooming Tea ද නිපදවයි. මෙම අත්කම් ශ්‍රේණි සාමාන්‍ය කළු තේට වඩා බෙහෙවින් අඩු ප්‍රමාණවලින් නෙළා සකසනු ලබන අතර, එය ඒවා දුර්ලභ හා අගය කරන කරුණ වේ. රස බැලීමේ කවුන්ටරයේ මෙය රස බැලීමට ඉල්ලා සිටින්න. මෙම ශ්‍රේණි බොහොමයක් සඳහා දම් පැහැති අංකුරය හෝ සාමාන්‍ය කොළ අංකුරය යන දෙකම භාවිතා කළ හැකි වුවත්, වතුයාය බොහෝ විට භාවිතා කරන්නේ දම් පැහැති අංකුරයයි.",
        "මෙහි නිපදවන වඩාත්ම ශ්‍රම-දැඩි අයිතම වන්නේ කුඩා-තොග විශේෂ භාණ්ඩ දෙකකි: Flower Chakra සහ Tea Coin. Flower Chakra යනු අංකුර දෙකක් පමණ භාවිතා කරන අතින් රෝල් කළ කළු තේ මුදුවකි. එක් පුද්ගලයෙකුට දිනකට එකකින් ග්‍රෑම් 25ක් පමණක් අතින් රෝල් කළ හැක, එබැවින් ඒවා මෙතරම් මිල අධික වේ. සාමාන්‍ය පානය සඳහා වඩා, පොහොසත් තේ රසිකයන් විසින් සමරු භාණ්ඩයක් ලෙසය ඒවා බොහෝ විට මිලදී ගන්නේ. Purple Tea ඇතුළුව, බොහෝ අත්කම් නිෂ්පාදන අඛණ්ඩ තොග නිෂ්පාදනයක් වෙනුවට කුඩා තොග වශයෙන් ඇණවුම් අනුව සාදනු ලැබේ.",
      ],
      duration: "මිනිත්තු 5",
    },
    "pahatharata-manufacturing": {
      name: "තේ නිෂ්පාදනය (පහත රට)",
      sectionHeadings: ["පහත රට එදිරිව උස් රට සැකසීම", "CTC සැකසුම"],
      sectionBodies: [
        "පහත රට ('පහත රට') වතු, මෙම වතුය මෙන්ම, දුර අභ්‍යන්තර උස් රට වතුවලට වඩා වෙනස් ලෙස තේ සකසයි. මෙම චාරිකාවේ මීට පෙර අදියරෙන් අදියර ආවරණය කළ වතුයායේම නිෂ්පාදන ක්‍රියාවලිය එය තහවුරු කරයි. මෙහි රෝල් කිරීම කොළය සම්පූර්ණ සාම්ප්‍රදායික කොළයක් ලෙස ඇඹරීම වෙනුවට කුඩා, ඝන අංශු බවට කඩා දමයි, එය CTC (Crush-Tear-Curl) සැකසීමේ ලාක්ෂණික ලක්ෂණයයි. මෙම නැවතුම එම පහත රට ප්‍රවේශය නෙළීම, සැකසීම, සහ ශ්‍රේණිගත කිරීම සඳහා අදහස් කරන්නේ කුමක්ද යන්න පිළිබඳ කෙටි සාරාංශයකි.",
        "CTC (Crush-Tear-Curl) කොළ යාන්ත්‍රිකව කුඩා, ඝන කැටිති බවට සකසන අතර, එමගින් පහත රට සිලෝන් තේවලට ලක්ෂණවත් ශක්තිමත් හා තියුණු රසය ලැබේ. මෙය මෙම චාරිකාවේ මීට පෙර රෝල් කිරීම හා ශ්‍රේණිගත කිරීමේ නැවතුම්වල දුටු එම කොළ අංශුමයි.",
      ],
    ...
    "clonal-tea": {
      name: "ක්ලෝන තේ",
      heroTagline: "වතුයායේම ලේබල් කළ ක්ලෝන කල්ටිවාර පරීක්ෂණ පැළ කිහිපයක්.",
      sectionHeadings: ["ක්ලෝන තේ", "ක්ලෝන තේ මිල අධික වන්නේ ඇයි", "තේ ප්‍රභේද", "TRI 3069", "TRI 3055", "TRI 4053", "TRI 2022", "TRI 2043", "TRI 4006", "TRI 3025"],
      sectionBodies: ["වඩාත්ම ඉල්ලුමක් ඇති තේ වර්ග කිහිපයක් ලැබෙන්නේ බීජවලින් නොව, එක් විශිෂ්ට මව් පඳුරකින් පැළැන්දෙන ලද ක්ලෝන පැළවලින්ය. මෙම නැවතුම ක්ලෝන තේ පැතිර යන ආකාරය සහ එහි වටිනාකම ඉහළ යාමට හේතුව ගැන කියා දෙයි. තේ පර්යේෂණ ආයතනය (TRI) විවිධ අවශ්‍යතා සඳහා ක්ලෝන කල්ටිවාර අභිජනනය කරයි. සමහරක් නියඟයට හෝ වැසි කාලයට ඔරොත්තු දීම සහ ඉහළ අස්වැන්න සඳහා වන අතර, තවත් සමහරක් අතින් සාදන විශේෂ තේ සඳහා අගය කරන රතු හෝ කොළ පැහැති තරුණ දලු සඳහාය. TRI 2023, TRI 2025, සහ TRI 2026 වැනි පුළුල් ලෙස වගා කරන, ඉහළ අස්වැන්නක් ලබා දෙන කල්ටිවාර වතු සහ ගෙවතු දෙකෙහිම සුලභ වන අතර, දුර්ලභ විශේෂ කල්ටිවාර බොහෝ විට හැම දිනකම වගාවට වඩා අතින් සාදන තේ සඳහා විශාල වතුවල වගා කරනු ලැබේ. මෙම TRI අංක වර්ෂ මෙන් පෙනුනද, ඒවා සරලව පවරන ලද අංක මිස, එක් එක් කල්ටිවාරය නිකුත් කළ වර්ෂය නොවේ.", "ක්ලෝන ව්‍යාප්තිය විශිෂ්ට පැළයක නිශ්චිත ලක්ෂණ රඳවා ගනී. හොඳම ක්ලෝන තේ බොහෝ විට ඉතා සීමිත ප්‍රමාණවලින් නිපදවනු ලැබේ. රතු හෝ කොළ පැහැති විශේෂ කල්ටිවාරවලින් ලැබෙන දලු, Golden Tip සහ Silver Tip වැනි දුර්ලභ ශ්‍රේණි සඳහා අතින් නෙළනු ලබන අතර, සාමාන්‍ය නෙළන ලද කොළවලට වඩා කිලෝග්‍රෑමයකට රුපියල් 100ක් පමණ වැඩිපුර වතු සාමාන්‍යයෙන් ගෙවයි.", "වතුයාය දේශීය පස හා කාලගුණය තුළ විශ්වාසදායක ලෙස කටයුතු කරන ආකාරය නිසා තේ පර්යේෂණ ආයතනය (TRI) විසින් අභිජනනය කරන ලද අධික අස්වැන්නක් ලබා දෙන, කෘමි-ප්‍රතිරෝධී ක්ලෝන කෙරෙහි ප්‍රිය කරයි, එනම් TRI 2000 මාලාවේ වර්ග සහ T65 ඇතුළුව. TRI 4049 යනු ශ්‍රී ලංකා තේ පර්යේෂණ ආයතනය විසින් සංවර්ධනය කරන ලද, මධ්‍යම සිට පහත් උස්මට වඩාත් සුදුසු, අධි අස්වැන්නක් ලබාදෙන තේ ප්‍රභේදයකි. එය Camellia sinensis හි ශාකීයව ප්‍රචාරණය කරන ලද ප්‍රභේදයක් වන අතර, ශක්තිමත් වර්ධනය, තද කොළ පැහැති කොළ සහ පොදු රෝගවලට ප්‍රතිරෝධය සඳහා ප්‍රසිද්ධය. TRI 4049 CTC සහ සම්ප්‍රදායික තේ යන දෙකටම සුදුසු දීප්තිමත්, සුවඳැති තේ රස නිපදවන අතර, එමගින් එය නැවත සිටුවීම සහ වාණිජ වගාව සඳහා ජනප්‍රිය තේරීමක් බවට පත් වේ.", "පේළි කිහිපයකින් ඔබ්බෙන් ලකුණු කර ඇති මෙම පැළය TRI 3069 වේ. එය තේ පර්යේෂණ ආයතනයේ ලියාපදිංචි වූ තවත් ප්‍රභේදයක් වන අතර, මෙම විශේෂිත පසෙහි එක් එක් ප්‍රභේදය කෙසේ වර්ධනය වේදැයි සංසන්දනය කිරීමට තවානේදී TRI 4049 සමඟ එකට සිටුවා ඇත.", "TRI 3055 ද එම පරීක්ෂණ පේළිවලම වේ. මෙහි සෑම ප්‍රභේද කොටසක්ම වෙන වෙනම ලකුණු කර නිරීක්ෂණය කරනු ලබන අතර, කීර්තියක් මත පමණක් රඳා නොසිට, ඇත්තටම පුළුල් කිරීමට වටින ප්‍රභේද මොනවාදැයි කර්මාන්තශාලාව තමන්ගේම සාක්ෂි ගොඩනගා ගන්නේ එලෙසිනි.", "මෙහි දැක්වෙන ප්‍රභේද පරීක්ෂණ කොටස් අවසන් වන්නේ TRI 4053 ප්‍රභේදයෙනි. ලියාපදිංචි ප්‍රභේද කිහිපයක් පැහැදිලිව ලකුණු කර එකට වගා කිරීම, කුඹුරක් සම්පූර්ණයෙන් එක් ප්‍රභේදයකට යොදවීමට පෙර වතුයායේ සම්මත පිළිවෙතකි.", "TRI 2022 ද මෙම එකම ප්‍රභේද පරීක්ෂණ කොටසේ සිටුවා ඇති අතර, එහි ලකුණු පුවරුව එහි අසල්වාසීන් මෙන්ම ලකුණු කර නිරීක්ෂණය කරනු ලැබේ. එකම බිමක ලියාපදිංචි ප්‍රභේද කිහිපයක් එකට වගා කිරීම, මෙහි කුමන ප්‍රභේද වඩාත් හොඳින් වර්ධනය වේදැයි තහවුරු කරන සාක්ෂි නාற්றුමේඩය ගොඩනගා ගන්නා ආකාරයයි.", "TRI 2043 පේළියේ ටිකක් ඉදිරියෙන් සිටී. මෙම පරීක්ෂණ කොටසේ අනෙකුත් අංකිත පැළ මෙන්ම, විශාල පරිමාණයෙන් සිටුවීමට තීරණයක් ගැනීමට පෙර එය වගා කර නිරීක්ෂණය කරනු ලැබේ.", "TRI 4006 එකම ප්‍රභේද පරීක්ෂණයේ තවත් එකකි. නාற්றුමේඩය සෑම ප්‍රභේදයක්ම එහි වෙන වෙනම පැහැදිලිව ලකුණු කළ ස්ථානයේ තබා ගන්නා අතර, එමගින් කාලයත් සමඟ වර්ධනය නිරීක්ෂණය කර සංසන්දනය කිරීම පහසු වේ.", "TRI 3025 පරීක්ෂණ කොටසේ මායිමේ, ද්විභාෂා 'තේ ප්‍රභේද' පුවරුවක් සහ දිනය සටහන් කළ සිටුවීමේ ඵලකයක් අසල පිහිටා ඇත. මෙහි ලකුණු කර ඇති පැළ එකට එකතු වී, කුමන ප්‍රභේද පුළුල් කිරීමට වටින්නේද යන්න පිළිබඳ නාற්றුමේඩයේ සැබෑ වාර්තාව සකසයි."],
      duration: "මිනිත්තු 5-10",
    },
    "shade-management": {
    ...
    "unique-fruits": {
      ...
      fruitItemBodies: [
        ...
        [
          ...
          ["නැවුම්ව අනුභව කරන හෝ ජෑම්, ජෙලි, යුෂ, සහ පේස්ට් බවට සකසයි. ඵලය විටමින් C සහ පෙක්ටින් වලින් අසාමාන්‍ය ලෙස පොහොසත් වේ.","පේර කොළ පාචනය හා ආහාර දිරවීමේ ගැටලු සඳහා තේ ලෙස පෙරන ලබයි. පොත්ත සහ ඵලයද ක්ෂුද්‍ර ජීවී විරෝධී හා ප්‍රතිඔක්සිකාරක ගුණාංග දරයි.","පේර ගස් විවිධ කුරුල්ලන්ට හා ක්ෂීරපායීන්ට ආහාර සපයන අතර, ඔවුන් බීජ විසිරීමට උපකාරී වන අතර, පිරිහුණු ඉඩම්වල පුරෝගාමී විශේෂයන් ලෙස සේවය කිරීමට තරම් ශක්තිමත් වේ."],
        ],
        [
          [],
    ...
    "tea-factory": {
      ...
      sectionBodies: ["ප්‍රධාන කර්මාන්තශාලා ගොඩනැගිල්ල, නෙළන ලද කොළ බර කිරීමේ සිට පැක් කිරීම දක්වා නිමි තේ බවට පත් වන ස්ථානයයි. මෙම නැවතුම ක්‍රියාත්මක වන කර්මාන්තශාලා තට්ටුව සහ එක් එක් අදියරේ භාවිතා කරන උපකරණ පිළිබඳ දළ විශ්ලේෂණයකි. එනම් බර කිරීම, මැලවීම, රෝල් කිරීම, පැසවීම (ඔක්සිකරණය), වියලීම, තේ වෙන් කිරීම (ශ්‍රේණිගත කිරීම), සහ අවසන් පැක් කිරීම, එම අනුපිළිවෙලින්ම. ක්‍රියාත්මක තේ කර්මාන්තශාලාවක්, සෑම දිනකම කොළ ප්‍රමාණයට සමීපව සමමුහුර්තව, බර කිරීමේ සිට පැක් කිරීම දක්වා සම්පූර්ණ අනුපිළිවෙල එකම වහලක් යටතේ ක්‍රියාත්මක කරයි. දිනපතා අස්වැන්න වාර්තා කිරීම සඳහා පැමිණෙන කොළ පළමුව බර කරනු ලැබේ. සනීපාරක්ෂක ප්‍රමිතීන්ට අනුකූලව, කර්මාන්තශාලා තට්ටුව නරඹන අමුත්තන්ගෙන් හිසකෙස් දැලක්, මුඛ ආවරණයක්, සහ ආරක්ෂිත පාවහන් පැළඳීමට ඉල්ලා සිටිනු ලැබේ."],
      duration: "මිනිත්තු 10-15",
    },
    "handmade-factory": {
      name: "අත්කම් තේ කර්මාන්තශාලාව",
      heroTagline: "වතුයායේම අතින් සාදන තේ කර්මාන්තශාලාවේ තැටි රාක්කයක්.",
      sectionHeadings: [
        "අත්කම් තේ කර්මාන්තශාලාව",
        "මැලවීම සහ රෝල් කිරීම",
        "හරිත තේ, වියලීම සහ ගොඩනැගිල්ල",
        "අත්කම් තේ වෙනස් වන්නේ කෙසේද",
      ],
      sectionBodies: [
        "යාන්ත්‍රික නිෂ්පාදනයට අමතරව, සමහර තේ සෑම අදියරකදීම යන්ත්‍රයක් නොව සම්පූර්ණයෙන් අතින් සකසනු ලැබේ. වතුයායේ අත්කම් විශේෂ ශ්‍රේණි බොහොමයක්, එනම් Golden Tip, Silver Tip, White Tea, Purple Tea, Pink Tea, Vita Glow, අතින් බැඳි Blooming Tea, සහ නෙළුම් තේ වැනි මල් එකතු කළ මිශ්‍රණ, ප්‍රධාන කළු තේ කර්මාන්තශාලාව වෙනුවට මෙහිදී නිම කරනු ලැබේ. (Vita Glow සාදනු ලබන්නේ Golden Tip සහ Silver Tip හි දම් පැහැති අංකුරයට වඩා වෙනස්ව, කොළ පැහැති අංකුරයේ තුඩුවෙනි.)",
        "මෙහි මැලවීම ස්වාභාවිකව සිදු වන්නේ කුඩා ද්‍රෝණිකාවක, හයිග්‍රෝමීටරයකින් නිරීක්ෂණය කරමිනි, ප්‍රධාන කර්මාන්තශාලාවේ බලෙන් වාතය එවන පංකා නොමැතිව. රෝල් කිරීමට පෙර තෙතමනය 50%ක් පමණට අඩු කරනු ලැබේ. පසුව කොළ තැටියක් මත අතින් රෝල් කරනු ලබන අතර, ඔරලෝසුවකට වඩා දැනීමෙන් තීරණය කරයි. විශාල තොග ඇණවුම් සඳහා කුඩා යාන්ත්‍රික රෝලර්ස් එම කාර්යයටම ආදේශ වේ, අතින් රෝල් කිරීම පමණක් ප්‍රමාණයට සරිලන්නට නොහැකි බැවිනි.",
        "විශේෂිත හරිත තේ යන්ත්‍රයක්, එනම් කොළ වාෂ්ප කර මිරිකන යන්ත්‍රයක්, එම ශ්‍රේණිය වෙන වශයෙන් හසුරුවයි, නමුත් එය දැනට කුඩා අලුත්වැඩියාවක් සඳහා ක්‍රියා විරහිතව පවතී. වාෂ්ප කිරීමෙන් පසු, කොළ එළියේ තබා මතුපිට තෙතමනය නැති කර ගනී, පසුව එහි වර්ණය, ශක්තිය, සුවඳ, සහ රසය ඉස්මතු කිරීමට එය දිගු කර අතින් රෝල් කරනු ලැබේ, අවසානයේ වියලනයක් හරහා යැවේ. වියලීමට ගතවන කාලය එක් කුඩා නිෂ්පාදකයෙකුගෙන් තවත් නිෂ්පාදකයෙකුට වෙනස් වේ. ගොඩනැගිල්ලේ කුඩා අවන් එක තේ පදනම් කරගත් බිස්කට් වැනි කෑම වර්ග පිසීමටද යොදා ගනී. රෝල් කිරීමෙන් පසු කරන වියලීම මෙහි වෙනම වියලන දෙකකින් සිදු කෙරේ: එකක් සාම්ප්‍රදායික දර ගින්නෙන්, අනෙක විදුලියෙන්. එක් එකකට තැටි 34ක් ඇති අතර, දෙකම එනසාල්, කරාබුනැටි, සහ ඉඟුරු වැනි වෙනත් කුඩා-පරිමාණ නිෂ්පාදන සඳහාද යොදා ගැනේ.",
        "මෙම නැවතුම අත්කම් තේ, කර්මාන්තශාලා රේඛා නිෂ්පාදනයෙන් වෙනස් වන්නේ කෙසේද යන්න ගැන කියා දෙයි. අතින් රෝල් කිරීම සහ අතින් සැකසීම කුඩා තොග නිපදවන අතර, කොළ අතර වැඩි විචලනයක් ඇති බැවින් ඒවායේ සුවිශේෂී ලක්ෂණය නිසා බොහෝ විට අගය කරනු ලැබේ. අත්කම් තේ පුළුල් කාණ්ඩ දෙකකට වැටේ. පළමු කාණ්ඩය නම් කළු තේ ආකාරයේ පැසුණු ඒවාය, කළු වර්ණය මගින් හඳුනාගත හැකි අතර, එයට Purple Teaද ඇතුළත් වේ, එහි සුවිශේෂී මූලික අංකුරය තිබියදීත්. දෙවන කාණ්ඩය නම් White Tea වැනි, පැසවීම සම්පූර්ණයෙන් මඟහැර කෙලින්ම වියලීමට යන නොපැසුණු ඒවාය.",
      ],
      duration: "මිනිත්තු 10",
    },
    "tea-center": {
      name: "තේ මධ්‍යස්ථානය",
      heroTagline: "වතුයායේම තේ රස බැලීමේ කවුන්ටරයේ රස බලන අමුත්තන්.",
      sectionHeadings: [
        "තේ මධ්‍යස්ථානය",
        "කසාය කිරීම සහ රස බැලීම",
        "එදිනෙදා ශ්‍රේණි සහ සුපිරි වෙළඳසැල් තේ",
        "හොඳම ශ්‍රේණි සහ අපනයන වෙළඳපොළවල්",
        "ඖෂධ පැළෑටි සහ කුළුබඩු මිශ්‍රණ",
      ],
      sectionBodies: [
        "සංචාරය අවසන් වන්නේ තේ මධ්‍යස්ථානයේදීය. එහිදී රස බැලීමේ කණ්ඩායමක් සෑම තොගයක්ම පරීක්ෂා කරයි, මෙහි පමණක් නොව තේ ප්‍රධාන කර්මාන්තශාලාවෙන් දේශීය වෙළඳපොළට යන විටත් හෝ පැක් කිරීම සඳහා පැමිණෙන විටත්, අධික පැසවීම, අඩු පැසවීම, හෝ දැවීම වැනි දෝෂ සඳහා. විකිණීමට පෙර අසමත් වන ඕනෑම දෙයක් ඉවත් කර, අනාගත නිෂ්පාදනයට උපකාරී වන පරිදි සුළු දෝෂ පවා සටහන් කරයි. මෙහි අත්කම් තේ කර්මාන්තශාලාවේ තවත් කුඩා-තොග විශේෂ භාණ්ඩ දෙකක් ප්‍රදර්ශනය කර ඇත: Flower Chakra සහ Tea Coin.",
        "මෙවැනි අත්කම් තේ උතුරන ලද ජලය වෙනුවට උපාධි 80ක් පමණ රත් ජලය භාවිතයෙන් තුන් වරක් දක්වා නැවත කසාය කළ හැක. නැවුම් උණු ජලය සමඟ එක් එක් වතාවේදී මිනිත්තු 2, 5, සහ 8ක් පමණ ගතවේ. සම්මත කළු තේ නම් උතුරන ලද ජලයෙන් එක් වරක් පමණක් සකසනු ලැබේ. රස බැලීම වයින් රස බැලීමට සමාන ක්‍රමයක් අනුගමනය කරයි. මුලින්ම සුවඳ ගැනීම, පසුව දිවේ පැති කොටස්වල රස මිද්දුම් හරහා තේ ඇදගැනීමට වාතය උරාගනිමින් උරුමීම. කණ්ඩායම දිනකට තේ 50ක් තෙක් රස බලන අතරතුර ගිලීම වෙනුවට ගසා දමයි, එහෙත් අමුත්තන්ට ඔවුන් රස බලන දේ සැබවින්ම පානය කිරීමට සාදරයෙන් පිළිගනී. නිසි ලෙස සකස් කළ කළු තේට ආවරණය කර මිනිත්තු 2.5 සිට 3 දක්වා කසාය කිරීම අවශ්‍ය වේ, ඉක්මන් කිරීමක් ප්‍රමාණවත් නොවේ.",
        "BOPF සහ BOPF Special බොහෝ ශ්‍රී ලාංකිකයන් එදිනෙදා පානය කරන ශ්‍රේණි වේ. Dust සහ Dust 1, අවම ශ්‍රේණි, කිසි විටෙකත් අපනයනයට යවනු නොලබන අතර, සාමාන්‍යයෙන් වර්ණකයක් එකතු කර මිල අඩු සුපිරි වෙළඳසැල් තේ බෑග්වල අවසන් වේ. ඒවා ගිල්වූ සැණින්ම, නිසි කසායකින් පසුව නොව, වර්ණය දිස්වන්නේ එබැවිනි. සුපිරි වෙළඳසැල්වල විකුණන බොහෝමයක් වෙනත් තැනකින් මිලදී ගත් වඩා මිල අඩු තේ සමඟ මිශ්‍ර කර ඇති අතර, එය වතුයායේ තනි-මූලයේ, මිශ්‍ර නොකළ තේවලට වෙනස්ය.",
        "හොඳම ශ්‍රේණි, එනම් FF Extra Special, FF Special, FBOP, ලැබෙන්නේ අස්වැන්නෙන් 1%ක් පමණින්, අංකුරයේ වඩාත්ම නොමුහුකුරා කොටසෙන්, සහ ග්‍රෑම් 100කට ඩොලර් 25ක් පමණට විකුණනු ලැබේ. OPA සහ OP වැනි විශාල ශ්‍රේණි ග්‍රෑම් 200කට ඩොලර් 5-6ක් පමණට විකිණේ. හොඳම ශ්‍රේණිවල දක්නට ලැබෙන සුදු තුඩු අංශු අමතර මිහිරි රසයක් එකතු කරන අතර බොහෝ දුරට යුරෝපීය ගැනුම්කරුවන් වෙත යයි, අරාබි වෙළඳපොළවල් වඩාත් ශක්තිමත් තේ රසයකට කැමති වෙයි.",
        "තේ හැරුණු විට, වතුයාය තමන්ගේම කොළ හා කළු තේ (ආසන්න වශයෙන් 90%) දේශීය ඖෂධ පැළෑටි (ආසන්න වශයෙන් 10%) වන ගොටුකොළ, හීන්බෝවිටිය, සහ මොරින්ගා සමඟ මිශ්‍ර කර, තමන්ගේම කුළුබඩුවලින් Masala, කුරුඳු, සහ ඉඟුරු කුළුබඩු තේ සාදයි. Masala වට්ටෝරුවේ ශ්‍රී ලාංකික කුළුබඩු හතක් භාවිතා වේ. ඉන් කරදමුංගු, කුරුඳු, ඉඟුරු, සහ ගම්මිරිස් නම් කර ඇති අතර ඉතිරිය රහසිගතව තබා ඇත. කිරි තේ සඳහා නිර්දේශ කරනු ලබන්නේ Masala හෝ BOP තේයි.",
      ],
      duration: "මිනිත්තු 10-15",
    },
    "coconut-field": {
    ...
    introduction: {
      ...
      sectionBodies: [
        ...
        "සිලෝන් තේවල කතාව ඇරඹුණේ 1867 දී මහනුවර ලූල්කඳුර වතුයායේ අක්කර 19ක ඉඩමකිනි. එය සිටෙව්වේ හිටපු ස්කොට්ලන්ත ජාතික කෝපි වගාකරුවෙකු වූ ජේම්ස් ටේලර්ය, විවිධාංගීකරණ පරීක්ෂණයක කොටසක් ලෙසිනි. වසර ගණනාවක් ගත වීමත් සමඟ, එය ශ්‍රී ලංකාවේ තේ වගා කරන ප්‍රදේශ හතක් දක්වා පැතිර ගියේය: මහනුවර, ඌව, රුහුණ (දකුණ), උඩපුස්සැල්ලාව, නුවරඑළිය, දිඹුල, සහ සබරගමුව.",
        ...
        "අමුත්තන් සාම්ප්‍රදායිකව පිළිගනු ලබන්නේ 'ආයුබෝවන්' යන වචනයෙනි, එනම් 'දීර්ඝායුෂ අත්වේවා' යන අර්ථය දෙන සිංහල ආචාර වචනයෙනි. සමහර විට එය වතුයායේම සභාපති අනුර අතුකෝරළ විසින් පෞද්ගලිකවම කරනු ලැබේ.",
      ],
      duration: "මිනිත්තු 5-10",
    },
    "jaggery-palm": {
    ...
    pruning: {
      ...
      sectionBodies: ["කප්පාදුව කිරීමට පෙර, වහාම කැපීම වෙනුවට, වතුයාය හිතාමතාම තේ පඳුර යම් කාලයක් වර්ධනය වීමට ඉඩ හරියි. එමගින් අලුත් වර්ධනය සම්පූර්ණයෙන් එන්නට ඉඩ දී, පඳුරට පෝෂණය රැස් කර ගැනීමට ඉඩ සලසයි. එම රැස්වීම සම්පූර්ණ වූ පසුව පමණක් කප්පාදුව ආරම්භ කරයි, එවිට පඳුර දණහිස උසට පමණ කපා දමනු ලැබේ. වතුයායේ දර්ශන ලකුණු මෙය නම් කරන ලද අදියර දෙකක් ලෙස පෙන්වයි. පළමු හරස් කැපුමේදී පඳුර අඟල් 9-12ක් (සෙ.මී. 22-30) දක්වා කපනු ලබන අතර, දෙවන හරස් කැපුමේදී එය නැවත අඟල් 16-18ක් (සෙ.මී. 40-45) දක්වා වර්ධනය වීමට ඉඩ හරිනු ලැබේ. තත්ත්වය නොසලකා නිශ්චිත කාලසටහනකට කප්පාදු කිරීම වෙනුවට, පළමුව පඳුරට වර්ධනය වීමට ඉඩ දීම, කැපීමට පෙර පෝෂණය උඩට ගෙන යාමට ශාකයට කාලය ලබා දෙයි."],
      duration: "මිනිත්තු 5",
    },
    polytunnel: {
      name: "පොලිටනල් සහ හරිතාගාරය",
      heroTagline: "වතුයායේම පොලිටනලය, දැනටමත් එහි පළමු බෝග වගා කරමින්.",
      sectionHeadings: ["පොලිටනලය හා හරිතාගාරය පිළිබඳ", "ජලය දීම හා වාරිමාර්ගය", "අස්වනු නෙළීමේ කාලසටහන", "පොහොර හා ජල පාලක ඒකකය", "මාලු මිරිස් (කැප්සිකම්)", "අමු මිරිස්", "නයි මිරිස්", "ගෙඩි ගෝවා", "අතු ගෝවා (රතු ගෝවා)", "බෝංචි", "රාබු", "කැරට්", "ලීක්ස්", "පිපිඤ්ඤා", "තක්කාලි පැළය"],
      sectionBodies: [
        "දැනට පවතින හරිතාගාරයට අමතරව, වතුයාය තමන්ගේම විවිධ එළවළු වගා කිරීම සඳහා පොලිටනලයක් ඉදි කරමින් සිටී. මේ අතර ස්කොච් බොනට් සහ කොළ මිරිස්, කැප්සිකම්, ගර්කින්, තක්කාලි, බෙල් පෙපර්, ගෝවා, රාබු, බෝංචි, ලීක්ස්, ස්ප්‍රින් ලූනු, සහ කැරට් ද ඇතුළත් වේ. එය සම්පූර්ණ වූ පසු, විලා හෝ කැබනාවේ නවාතැන් ගන්නා අමුත්තන්ට මෙහිම වගා කරන එළවළු තමන් විසින්ම නැවුම්ව නෙළා ගැනීමට හැකි වනු ඇත. පොලිටනලයක අර්ධ විනිවිද පෙනෙන ආවරණය හරිතාගාරයක වීදුරුවක් මෙන් ක්‍රියා කරයි. එය රශ්ණය රඳවා ගනිමින් ආලෝකය ඇතුළට යාමට ඉඩ දෙන අතරම, තද මෝසම් වර්ෂාව හා පොදු පළිබෝධ බොහොමයක් වළක්වයි. මෙම තත්ත්වයන් නිසා විවෘත බිමක නිසැකව වගා කළ නොහැකි එළවළු වර්ග පවා වතුයායට වගා කළ හැකි වන අතර, විවෘත වගාවකට වඩා වගා සමය දීර්ඝ කර ගැනීමටද හැකි වේ.",
        "සෑම බෝගයක්ම එහිම ජල සැපයුම් ක්‍රමයක් අනුගමනය කරයි, වර්ධන අවධිය අනුව මුල් මට්ටමේ බින්දු වාරිමාර්ග සහ ඉහළින් ඉසින මීදුම අතර මාරු වෙමින්. මීදුම ප්‍රධාන වශයෙන් කැරට් සහ රාබු වැනි බෝග සඳහා මුල් පැළ හටගැනීමට හා සිසිල් කිරීමට උපකාරී වන අතර, තක්කාලි වැනි බෝගවල ගෙඩි හටගැනීම ආරම්භ වූ පසු බින්දු වාරිමාර්ගය ප්‍රධාන ක්‍රමය බවට පත් වේ. මෙම ක්‍රම දෙක ම වෙනස් අරමුණු ඉටු කරයි. තරුණ බීජ හා පැළ මුල් බැසගැනීමට තරම් වාතය හා පස මතුපිට තෙතමනය මීදුම මගින් පවත්වා ගනී. ශාක වැඩුණු පසු බින්දු වාරිමාර්ගය ජලය කෙලින්ම මුල් කලාපයට ලබා දෙන අතරම, මුළුමනින්ම අඩු ජලයක් භාවිතා කරමින් හා පත්‍ර වියලිව තබා දිලීර රෝග අවදානම අඩු කරයි.",
        "පළමු අස්වැන්නට ගතවන කාලය බෝගයෙන් බෝගයට බෙහෙවින් වෙනස් වේ, රාබු සඳහා දින 25-35ක් තරම් අඩුවේ සිට ලීක්ස් සඳහා දින 90-120ක් දක්වා, එබැවින් සමයේ පාහේ සෑම අවස්ථාවකදීම පොලිටනලයේ නෙළා ගැනීමට යමක් තිබිය යුතුය. වේගයෙන් හා සෙමින් හමාරවන එළවළු වර්ග මෙසේ පසෙක්පසට වගා කිරීම එකම අවස්ථාවක සියල්ල එකවර සූදානම් වීම හෝ අවසන් වීම වළක්වා, අස්වනු ස්වභාවිකවම විවිධ අවස්ථාවලට බෙදී යාමට සැලසුම් කර ඇති තෝරාගැනීමකි.",
        "මෙම පාලක ඒකකය සෑම වගා බෑගයකටම යන බින්දු නල සපයයි. එහි අවසාදිතය ඉවත් කිරීමට පෙරහනක්, පීඩන මාපකයක්, පයිප්ප කොටස් වෙන් කිරීම සඳහා වැසුම් කපාට, සහ වාරිමාර්ග ජලය ගමන් කරන අතරතුර ද්‍රව පොහොර කෙලින්ම එයට එකතු කරන වෙන්ටියුරි එන්නත්කරණයකින් යුත් පොහොර ටැංකි දෙකක් ඇතුළත් වේ. මෙමගින් සෑම ශාකයකටම එකවර ජලය හා පෝෂණය ලැබේ.",
        "යුගල වශයෙන් වැඩෙන දිගටි, තුඩ තියුණු, දිලිසෙන පත්‍ර සහිත මෙම පැළය මාලු මිරිස්, කැප්සිකම් (Capsicum annuum) වර්ගයකි. එය පොල් නාරටි පිරවූ වගා බෑගයක වගා කරන අතර, එහි මුල වෙත බින්දු නලයක් දැනටමත් සම්බන්ධ කර ඇත.",
        "මෙම පැළය අමු මිරිස්, කැප්සිකම් annuum විශේෂයට අයත් මිරිස් වර්ගයකි. මෙම මුල් අවධියේදී එහි පත්‍ර කැප්සිකම් පැළයේ පත්‍රවලට වඩාත්ම වෙනස් කළ නොහැකි තරම් සමාන වේ, මන්ද දෙකම එකම විශේෂයේ, විවිධ භාවිත සඳහා වගා කරන ශාක බැවිනි.",
        "නයි මිරිස් යන නම, 'නයි' යනු සිංහලෙන් කෝබ්‍රා යන අර්ථය දෙන වචනයෙන් සෑදී ඇත. එය Capsicum chinense විශේෂයට අයත් වන අතර, හබනේරෝ හා ස්කොච් බොනට් වැනි වර්ග හා සමීප ඤාතිත්වයක් ඇති, වතුයායේ අනෙකුත් මිරිස් වර්ගවලට වඩා සාමාන්‍යයෙන් තියුණු විශේෂයකි.",
        "මෙම පැළයේ වඩාත් වටකුරු, තරමක් ඉටි ස්වභාවයක් සහිත පත්‍ර අයත් වන්නේ ගෙඩි ගෝවාට ය. එහි නම වචනාර්ථයෙන් 'බෝල ගෝවා' යන අර්ථය දෙන අතර, එය හිස සාදන සාමාන්‍ය ගෝවා වර්ගය (Brassica oleracea var. capitata) වේ. එහි හිස හටගැනීමට බොහෝ කලකට පෙර මෙහි සිටුවනු ලැබේ.",
        "මෙම පැළයේ රතු-දම් පැහැති පත්‍ර අයත් වන්නේ අතු ගෝවාට ය. එහි නම වචනාර්ථයෙන් 'අතු ගෝවා' යන අර්ථය දෙන අතර, එය හිස නොසාදන, බෝල හැඩැති වර්ගයට වඩා ලිහිල් ගෝවා වර්ගයකි (Brassica oleracea var. acephala).",
        "මෙම පළල්, හෘද හැඩැති පත්‍ර අයත් වන්නේ බෝංචිටය, බෝංචි (Phaseolus vulgaris) ය. එය වර්ධනය වීමට පටන් ගත් පසු, අවට වැඩෙන එළවළු වලට වඩා බෙහෙවින් වෙනස් වැඩුම් රටාවක් ඇති පදික බෝගයකි.",
        "තවමත් බීජාවරණ පත්‍ර අවධියේ පවතින මෙම කුඩා පැළය රාබු, රාබු (Raphanus sativus) වේ. එය පොලිටනලයේ අස්වැන්නට වැඩිම වේගයෙන් එළඹෙන බෝගවලින් එකකි, දින 25-35ක් තරම් ඉක්මනින් නෙළා ගැනීමට හැකි වේ.",
        "කැරට් (Daucus carota subsp. sativus) මෙම මුල් අවධියේදී පවා එහි තුනී, තෘණ හැඩැති පළමු පත්‍ර මගින් හඳුනාගත හැකිය, මෙහි වගා කරන අනෙකුත් එළවළු වල පළල් බීජාවරණ පත්‍රවලට හාත්පසින්ම වෙනස්ව.",
        "ලීක්ස් (Allium ampeloprasum) තනි තුනී තලයක් ලෙස මතු වන අතර, පොලිටනලයේ අස්වැන්නට වඩාත්ම සෙමින් එළඹෙන බෝගවලින් එකකි. සිටුවීමේ සිට එය දින 90-120ක් තරම් කාලයක් ගත විය හැක.",
        "මෙම පැළයේ පළල්, වටකුරු බීජාවරණ පත්‍ර දෙක වන්නේ සලාද පිපිඤ්ඤාවේ (Cucumis sativus) බීජාවරණ පත්‍ර වන අතර, එහි සැබෑ, වඩාත් හෘද හැඩැති පත්‍ර මතුවීමට පෙර දක්නට ලැබේ.",
        "මෙම තක්කාලි පැළය දැනටමත් තක්කාලි ශාකයේ විශේෂිත ලක්ෂණ වන කැපුම් සහිත, දාරයේ දැති සහිත, තරමක් රෝම සහිත පත්‍ර පෙන්වයි. තක්කාලි යනු ගෙඩි හටගැනීම ආරම්භ වූ පසු මීදුමෙන් බින්දු වාරිමාර්ගයට මාරු වන බෝගය ලෙස කලින් සඳහන් කළ බෝගයයි.",
      ],
      duration: "මිනිත්තු 5",
    },
    "mushroom-hut": {
      name: "බිම්මල් කුටිය",
      sectionHeadings: ["වෙනස් ආකාරයේ උද්‍යානයක්", "ඔයිස්ටර් බිම්මල් වර්ග", "වගා මාධ්‍යය සහ බීජ", "අස්වනු නෙළීමේ චක්‍රය", "පෝෂණය හා සෞඛ්‍ය ප්‍රතිලාභ"],
      sectionBodies: [
        "වතුයාය එහි තේ, කුළුබඩු, සහ පළතුරු බෝග අතරින්, විශේෂිත බිම්මල් කුටියක් පවත්වා ගෙන යයි. එය කුඩා, සෙවන සහිත ගොඩනැගිල්ලක් වන අතර, උයන්හි අනෙකුත් කොටස් වගා කරන සූර්යාලෝකය හා වායු සංචලනයට වඩා, බිම්මල් වර්ධනය වීමට අවශ්‍ය අඳුරු, සිසිල්, තෙත් තත්ත්වයන් පවත්වා ගැනීම සඳහා තැනිණි.",
        "මෙම කුටිය ඔයිස්ටර් බිම්මල් වර්ග කිහිපයක් වගා කිරීම සඳහා සකසා ඇත: ඇමරිකානු ඔයිස්ටර් (Pleurotus pulmonarius), භූතාන් ඔයිස්ටර්, කළු ඔයිස්ටර්, රෝස ඔයිස්ටර් (Pleurotus djamor), සහ ඇබලෝන් බිම්මල් (Pleurotus cystidiosus). සියල්ලම පසට වඩා පිදුරු හෝ මට්ටි කුඩු මාධ්‍යයක් මත වගා කරනු ලැබේ. මේ සියල්ලම Pleurotus ගණයට අයත් වේ. ඒවා හඳුනාගත හැක්කේ මාධ්‍යයෙන්ම කෙලින්ම වැඩෙන, තදින් එකිනෙක උඩින් තට්ටු වශයෙන් පිහිටි පංකා හැඩැති හෝ බෙල හැඩැති තොප්පි මගිනි. වර්ග අතර ප්‍රධාන වෙනස වර්ණයයි: සුදු හා අළු සිට රෝස, රන්වන්, හෝ කළුවට ආසන්න පැහැ දක්වා විහිදේ. එහෙත් මෘදු, තරමක් රසවත් රසය සියල්ලෙහිම එක හා සමානය.",
        "වගාව ආරම්භ වන්නේ මාධ්‍යයෙන්මය: පිදුරු හෝ මට්ටි කුඩු, එනම් වතුයායේ අඩු වියදම්, අපද්‍රව්‍ය පදනම් වූ ප්‍රවේශයට පදනම් වන එම ද්‍රව්‍යයම. එය තරඟකාරී දිලීර හා බැක්ටීරියා විනාශ කිරීම සඳහා තෙත් කර තාපයෙන් පැස්ටරීකරණය කරනු ලැබේ. ඉන් පසුව බිම්මල් මයිසීලියම් මගින් සම්පූර්ණයෙන් ව්‍යාප්ත වූ ධාන්‍ය වන 'බීජ' සමඟ මිශ්‍ර කර සිදුරු සහිත බෑග් හෝ බ්ලොක් තුළට පුරවනු ලැබේ. ඉන්පසු එම බ්ලොක් සති දෙක තුනක 'බීජ ව්‍යාප්ති' කාලයක් සඳහා අඳුරේ තබනු ලැබේ. එම කාලය තුළ සුදු මයිසීලියම් මුළු මාධ්‍යය පුරාම ව්‍යාප්ත වේ. ඉන් අනතුරුව කුටියේ අඳුරු, සිසිල්, තෙත් තත්ත්වයන් ක්‍රියාත්මක වී අස්වැන්න හට ගැනීම උත්තේජනය කරයි.",
        "මෙලෙස බිම්මල් වගා කිරීම ගොවිපලකට එහි ප්‍රධාන බෝගයෙන් ඔබ්බට විවිධාංගීකරණය වීමට අඩු වියදම්, තිරසාර ක්‍රමයකි. බොහෝ විට පිදුරු වැනි කෘෂිකාර්මික අපද්‍රව්‍ය වගා මාධ්‍යයක් ලෙස යොදාගනිමින්, එක් මාධ්‍ය කාණ්ඩයකින් සති කිහිපයක් තුළ අස්වනු කිහිපයක් ලබා ගත හැක. සාමාන්‍යයෙන් එක් බ්ලොක් එකකින් සති එකක් හෝ දෙකක පමණ පරතරයෙන් අස්වනු හතරක් තෙක් ලබා ගත හැකිය. පළමු අස්වැන්න වඩාත් විශාල වන අතර ඉන් පසු අස්වනු ප්‍රමාණය ක්‍රමයෙන් අඩු වේ. බිම්මල් කැපීමට වඩා මුළු පොකුරම ඇඹරී ගලවා ගනු ලබන්නේ, ඉතිරි වන කඳ කොටස් දිරාපත් වී ඊළඟ අස්වැන්නට හානි කළ හැකි දූෂණයකට තුඩු දිය හැකි බැවිනි.",
        "වතුයායේම සංඥා පුවරු මගින් එක් එක් වර්ගයේ පෝෂණ අගය පෙන්වා දෙයි: ඇමරිකානු ඔයිස්ටර්, රෝස ඔයිස්ටර්, සහ ඇබලෝන් බිම්මල් යන සියල්ලම ප්‍රෝටීන්, විටමින්, සහ ප්‍රතිඔක්සිකාරක වලින් පොහොසත් බවත්, ප්‍රතිශක්තිකරණ පද්ධතියට, හෘද හා ආහාර ජීර්ණ සෞඛ්‍යයට උපකාරී වන අතර, දැවිල්ල අඩු කිරීමට හා කොලෙස්ටරෝල් පාලනයට දායක වන බවත් විස්තර කරයි.",
      ],
      duration: "මිනිත්තු 5",
    },
  },
  ta: {
    ...
    withering: {
      name: "வதங்கல்",
      sectionHeadings: ["தொட்டிகளை நிரப்புதல்", "12 மணி நேர சுழற்சி", "ஈரப்பதம் நீக்கம்", "வதங்கல் ஏன் முக்கியம்"],
      sectionBodies: [
        "வயலிலிருந்து வரும் இலை முதலில் அன்றைய அறுவடையை பதிவு செய்ய எடைபோடப்படுகிறது. பின்னர் 85 அடி நீளமும் 6 அடி அகலமும் கொண்ட நீண்ட தொட்டிகளில் மெல்லிய அடுக்காக பரப்பப்படுகின்றன. ஒவ்வொரு தொட்டியிலும் 46 அங்குல அளவிலான ரசிகன் ஒன்று நிமிடத்திற்கு 960 சுழற்சிகள் வேகத்தில் இயங்கி, காற்றை வலுக்கட்டாயமாக இலை வழியாக செலுத்தி ஈரப்பதத்தை படிப்படியாக நீக்குகிறது. தோட்டம் இதை 'நவீன' வதங்கல் செயல்முறை என்று அழைக்கிறது. சதுர அடிக்கு சுமார் 2.5 கிலோகிராம் என்ற ஏற்றும் விகிதத்தில், ஒரு தொட்டி சுமார் 1,250 கிலோகிராம் பச்சை இலையை தாங்குகிறது.",
        "இலை தொட்டியில் 12 மணி நேரம் தங்கியிருக்கும். ஒவ்வொரு 4 மணி நேரத்திற்கும் கையால் தலைகீழாகத் திருப்பப்படும். அந்த சுழற்சியின் போது 3 முதல் 4 மணி நேரம் வரை தொட்டியின் அடியில் இருந்து வெப்பம் கொடுக்கப்படுகிறது. காலையில் இறக்குவதற்கு முன் இலை இறுதியாக தளர்த்தப்படுகிறது. வதங்கிய இலை உருட்டுதலுக்குச் செல்வதற்கு முன் மீண்டும் எடைபோடப்படுகிறது.",
        "ஒவ்வொரு 100 கிலோகிராம் பச்சை இலையிலிருந்தும் சுமார் 43 கிலோகிராம் நீர் அகற்றப்படுகிறது. ஒரு சதுர அடி பெட்டியில் 2.5 கிலோகிராம் இலையை சோதிப்பதன் மூலம் இது உறுதிப்படுத்தப்படுகிறது. இதன் விளைவாக இலை மென்மையாகவும் நெகிழக்கூடியதாகவும் மாறி, உடையாமல் உருட்டுவதற்குத் தயாரான நிலையில் ஈரப்பதம் 57% முதல் 58% வரை இருக்கும்.",
        "வதங்கல் இலை முழுவதும் ஈரப்பதத்தை சமன்படுத்துகிறது, மேலும் இயற்கை நொதிகள் பின்னர் தேயிலையின் நறுமணத்தையும் நிறத்தையும் உருவாக்கும் வேதியியல் மாற்றங்களைத் தொடங்க அனுமதிக்கிறது.",
      ],
      duration: "இந்த நிலை முழு 12 மணி நேரமும் நடைபெறும், இலை ஒவ்வொரு 4 மணி நேரத்திற்கும் தலைகீழாகத் திருப்பப்படும்.",
    },
    rolling: {
      name: "உருட்டுதல்",
      sectionHeadings: ["இலையை முறுக்கி உடைத்தல்", "முதல் உருட்டல்", "இரண்டாவது உருட்டல்", "உருட்டுதல் ஏன் முக்கியம்"],
      sectionBodies: [
        "வதங்கிய இலைகள் உருட்டும் இயந்திரங்கள் வழியாகச் செல்கின்றன. அவை இலையின் செல் சுவர்களை முறுக்கி உடைத்து, அதனுள் பூட்டப்பட்ட சாறுகளையும் எண்ணெய்களையும் வெளியிட்டு, இலையை சிறிய துகள்களாக உடைக்கின்றன. இங்குதான் இலையின் வடிவமும் தேயிலையின் வலிமையும் உருவாகின்றன.",
        "இரண்டு ஏற்றும் உருளைகளில் ஒவ்வொன்றும் 280 கிலோகிராம் வதங்கிய இலையை எடுக்கும். முதல் உருட்டல் 20 நிமிடங்கள் 30 முதல் 32 சுழற்சிகள் வேகத்தில் நடைபெறும். கிடைக்கும் டூலிகள் (உருட்டப்பட்ட இலைத் துகள்கள்) ஒரு கன்வேயர் வழியாக கம்பி வலை பொருத்தப்பட்ட ரோல் பிரேக்கருக்குச் செல்கின்றன. அங்கு மிகச் சிறிய துகள்கள் 'முதல் டூல்' ஆக கீழே விழுந்து நொதிப்பதற்கு அனுப்பப்படுகின்றன.",
        "கடந்து செல்லாத இலை கன்வேயர் வழியாக இரண்டு இரண்டாம் நிலை உருளைகளுக்குச் செல்கிறது, அசல் 280 கிலோகிராமிலிருந்து 12% முதல் 14% வரை அளவு குறைக்கப்பட்டு. இரண்டாவது உருட்டல் 32 சுழற்சிகள் வேகத்தில் மேலும் 20 நிமிடங்கள் நடைபெறுகிறது. அதன் பிறகு அந்த 'இரண்டாம் டூல்' சலிக்காமல் நேரடியாக நொதிப்பதற்குச் செல்கிறது.",
        "செல் சுவர்களை உடைப்பது நொதிகளை ஆக்ஸிஜனுக்கு வெளிப்படுத்தி, ஆக்சிஜனேற்றத்தைத் தொடங்குகிறது. இலை எவ்வளவு கடினமாகவும் நீண்ட நேரமும் உருட்டப்படுகிறது என்பது இறுதி தேயிலையின் வலிமையையும் தோற்றத்தையும் தீர்மானிக்கிறது. பல சுற்றுகளில் முதலாவது, மென்மையான 'முன்-தயாரிப்பு' உருட்டல், இறுக்கமாக உருட்டுவதற்கு முன் முறுக்கப்பட்ட துகள்களை இலையின் சொந்த சாற்றால் மூடுவதை நோக்கமாகக் கொண்டது.",
      ],
      duration: "20 நிமிடங்கள் வீதம் இரண்டு உருட்டல்கள் (மொத்தம் 40 நிமிடங்கள்): முதலாவது 30 முதல் 32 சுழற்சிகள், இரண்டாவது 32 சுழற்சிகள் வேகத்தில்.",
    },
    fermentation: {
      name: "நொதித்தல் (ஆக்சிஜனேற்றம்)",
      sectionHeadings: ["இலையை ஆக்ஸிஜனேற்றுதல்", "அறையை சுத்தமாக வைத்தல்", "பச்சையிலிருந்து செம்பு நிறத்திற்கு", "நேரத்தை சரியாக மதிப்பிடுதல்", "கருப்பு, பச்சை, வெள்ளை, மற்றும் ஊலோங் தேயிலை"],
      sectionBodies: [
        "உருட்டப்பட்ட இலை, அதாவது முதல் மற்றும் இரண்டாம் டூல் இரண்டும், 25 முதல் 30°C வரை வெப்பநிலையில் வைக்கப்பட்ட குளிர்ந்த, ஈரப்பதமான அறையில் பரப்பப்படுகிறது. அங்கு 2.5 மணி நேரம் ஆக்சிஜனேற்றமடைய விடப்படும், வினை நடைபெற போதுமான ஈரப்பதத்தை பராமரிக்க ஈரப்பதமூட்டி பயன்படுத்தப்படுகிறது.",
        "அறை முழுமையாக சுத்தமாக இருக்க வேண்டும்: பாக்டீரியா, தேங்கிய நீர், அல்லது பழைய இலைகள் இல்லாமல். ஏனெனில் பாக்டீரியா நொதி செயல்பாட்டை நிறுத்தி நிறம், வலிமை, மற்றும் காடி தரத்தை கெடுக்கும்.",
        "அந்த 2.5 மணி நேரத்தில் பச்சை இலை படிப்படியாக செம்பு நிற பழுப்பு நிறமாக மாறுகிறது. இதன் மூலம் கருப்பு தேயிலையுடன் தொடர்புடைய நிறம், வலிமை மற்றும் நறுமணத்தை முழுமையாக வளர்த்துக்கொள்கிறது, முற்றிலும் இயற்கையான ஆக்சிஜனேற்றத்தால் மட்டுமே, எந்த சாயமும் சேர்க்கப்படுவதில்லை.",
        "சுவைக்கு இது மிக முக்கியமான படி. நேரம் துல்லியமாக மதிப்பிடப்பட வேண்டும்: மிகக் குறைவாக இருந்தால் தேயிலை மெலிதாகவும், அதிகமாக இருந்தால் தட்டையாகவும் மந்தமாகவும் மாறும்.",
        "இங்கு தயாரிக்கப்படும் தேயிலை வகைகளை வேறுபடுத்துவதும் இதுவே. கருப்பு தேயிலை வதங்கல், உருட்டுதல், நொதித்தல், பின்னர் சுடுதல் என்ற வரிசையில் செல்கிறது. பச்சை தேயிலை நொதித்தலை முழுவதுமாகத் தவிர்க்கிறது (வதங்கல், உருட்டுதல், உலர்த்துதல் மட்டும்), அது மென்மையானது என்பதால் பெரும்பாலும் அதன் மென்மையான, மருத்துவ குணத்திற்காகவே அருந்தப்படுகிறது. வெள்ளை தேயிலை வதங்கலையும் நொதித்தலையும் இரண்டையும் தவிர்த்து, புதிய இலையிலிருந்து நேரடியாக ஆவியில் வேகவைத்து உலர்த்துவதற்குச் செல்கிறது. இந்த தொழிற்சாலை ஊலோங் தேயிலையை தயாரிப்பதில்லை. அதை வழிகாட்டி விவரிப்பது கருப்பு தேயிலையைப் போன்ற ஒரு நொதிக்கப்பட்ட வகையாகவே, இயந்திர உருட்டலுக்குப் பதிலாக மீண்டும் மீண்டும் ஆவியில் வேகவைத்தல் மற்றும் கையால் தேய்த்தல் மூலம் தயாரிக்கப்படுவதாகும்.",
      ],
      duration: "முதல் மற்றும் இரண்டாம் டூல் இரண்டிற்கும் நொதிக்க 2.5 மணி நேரம் ஆகும், அதன் பிறகு இலை உலர்த்தும் நிலைக்குச் செல்கிறது.",
    },
    drying: {
      name: "உலர்த்துதல் (சுடுதல்)",
      sectionHeadings: ["உலர்த்தியினுள்", "6 பிரிவுகள், 21 நிமிடங்கள்", "சமநிலையை சரியாக பராமரித்தல்", "விளைவு", "புத்துணர்ச்சியை பாதுகாத்தல்", "நிலைமைகளின் சங்கிலி"],
      sectionBodies: [
        "உலர்த்திக்குள் செல்வதற்கு முன், நொதித்த இலை தளர்த்தப்படுகிறது. பின்னர் அது சூடான காற்று உலர்த்திகள் வழியாகச் செல்கிறது. விறகு எரிபொருளால் உருவாக்கப்படும் வெப்பம் ஒரு முதன்மை ரசிகன் மூலம் இழுக்கப்பட்டு, 120°C முதல் 130°C வரையிலான உள்வரும் வெப்பநிலையில் பராமரிக்கப்படுகிறது.",
        "உள்ளே, உலர்த்தி 6 பிரிவுகளாகப் பிரிக்கப்பட்டுள்ளது. இலை மேல் தட்டிலிருந்து இந்தப் பிரிவுகள் வழியாக கீழ் தட்டுகளுக்கு விழுந்து, முன்னும் பின்னுமாக உருண்டு நகர்கிறது. இது சரியாக 21 நிமிடங்களில் வெளியேறுகிறது. அந்த நேரத்தில் வெளியேறும் வெப்பநிலை சுமார் 80°C (சுமார் 140°F முதல் 150°F வரை) இருக்க வேண்டும்.",
        "120-130°C உள்வரும் வெப்பநிலை, 80°C வெளியேறும் வெப்பநிலை, மற்றும் 21 நிமிடங்கள் என்ற இந்த சேர்க்கையே உலர்த்தி பராமரிக்க வடிவமைக்கப்பட்டுள்ளது. இதுவே இலையை சரியாக கருப்பாக மாற்றுகிறது. உள்வரும் வெப்பநிலை 120°C க்கும் கீழே சென்றால், இலையின் அடுக்கை மெலிதாக்குவதன் மூலம் வெளியேறும் வெப்பநிலை 100°C ஐ அடைந்தாலும், இலை சரியாக உலராமல் பழுப்பு நிறமாக மாறும். உள்வரும் வெப்பநிலை 150°C ஆக உயர்த்தப்பட்டு வெளியேறும் வெப்பநிலை 75°C ஆகக் குறைந்தால், இலை உலராமல் ஈரமாகவும் வேகவைக்கப்பட்டதுபோலவும் மாறிவிடும்.",
        "சரியான அமைப்புகளில் மறுமுனையில் வெளிவருவது நமக்குப் பரிச்சயமான கருப்பு, மொறுமொறுப்பான தேயிலை இலையாகும். பார்வையாளர்கள் உடனடியாக வெப்பத்தை உணர்கிறார்கள். இது தொழிற்சாலை தளத்தில் மிக வெப்பமான இடங்களில் ஒன்று.",
        "சுடுதல் சரியான நேரத்தில் வேதியியல் மாற்றங்களை நிறுத்துகிறது. இது ஈரப்பதத்தை சுமார் 45-50% இலிருந்து 2-3% ஆகக் குறைக்கிறது, இது சேமிப்பு மற்றும் ஏற்றுமதியின் போது தேயிலை புத்துணர்ச்சியாக இருக்க அவசியமானது.",
        "ஆனால் இறுதி தேயிலையின் எடையும் தரமும் உலர்த்தியை மட்டும் சார்ந்தில்லை. அது ஒரு தொடர் நிலைமைகளின் சங்கிலியைச் சார்ந்துள்ளது: உயர்தர பச்சை இலை, சரியான வதங்கல், சரியான உற்பத்தி செயல்முறை, வெற்றிகரமான நொதித்தல் (இதுவே மிக முக்கியமான படி), மற்றும் இறுதியாக உலர்த்தியில் சரியான செயலாக்கம்.",
      ],
      duration: "இலை சரியாக 21 நிமிடங்களில், நெருக்கமாகக் கட்டுப்படுத்தப்பட்ட உள்வரும்-வெளியேறும் வெப்பநிலைச் சுழற்சியில் உலர்த்தியின் வழியாகச் செல்கிறது.",
    },
    sorting: {
      ...
      sectionHeadings: [
        ...
        "சந்தை மார்க்குகள் மற்றும் தர பெயர்கள்",
        "விளைச்சல் மற்றும் உற்பத்தி விகிதம்",
      ],
      sectionBodies: [
        ...
        "உலர்ந்த இலை தரப்படுத்தும் இயந்திரம் வழியாக செல்லும் போது, அது ஒரே கடத்தலில் அதன் மூல தளிரின் எந்த பகுதியிலிருந்து வந்தது என்பதன் அடிப்படையில் பிரிக்கப்படுகிறது: மென்மையான, எளிதில் முறியக்கூடிய நுனியும் மேல் இலைகளும், அதாவது ஒவ்வொரு தளிரிலும் சுமார் 2% மட்டுமே, FBOPF Extra Special ஆக மாறுகிறது, இது சிறந்த மற்றும் அதிக விலை கொண்ட தரம். கீழே உள்ள இலைகள் BOP மற்றும் Pekoe போன்ற வலிமையான தரங்களாக மாறுகின்றன. அதிக முதிர்ந்த, கீழ்மட்ட இலை Dust ஆக மாறுகிறது, பயன்படுத்த முடியாத தண்டுகளும் துண்டுகளும் குப்பையாக நிராகரிக்கப்படுகின்றன.",
        "அந்த தரப்படுத்தும் இயந்திரத்தில், இலை மற்றும் மலர் போன்ற துகள்களை வேறுபடுத்தி அறியக்கூடிய கேமரா அமைப்புடன் கூடிய ஒரு நிற பிரிப்பான் அடங்கும், அதற்கு சொந்தமில்லாத எதையும் வடிகட்டுகிறது. இலங்கை உற்பத்தி கருப்பு தேயிலையை மையமாகக் கொண்டிருப்பதால், மீதமுள்ள பழுப்பு நிற, முழுமையாக நொதிக்காத துகள்களையும் தண்டு துண்டுகளையும் இந்த படி நீக்குகிறது. வெவ்வேறு வலை அளவுள்ள சல்லடைகள், அங்குலத்திற்கு சதுர துளைகளின் எண்ணிக்கையால் அளவிடப்படும், அங்குலத்திற்கு 3 துளைகள் என்ற கரடுமுரடான அளவிலிருந்து 60 வரையிலான நுண்ணிய அளவு வரை பரவியுள்ள, பல முறை துகள் அளவின்படி தேயிலையை சுமார் 21 தனித்துவமான தரங்களாக மேலும் பிரிக்கின்றன. மீதமுள்ள பழுப்பு நிற தண்டு துண்டுகளுக்கு ரோலர்களில் நிலைமின் ஏற்றம் தந்து அவற்றை பிரித்தெடுக்க ஒரு நிலைமின் பிரிப்பான் உதவுகிறது, மேலும் தனி இயந்திரங்கள் குப்பை தேயிலையை தொகுதியிலிருந்து முழுமையாக அகற்றுகின்றன. இங்கு எந்த நிறமும் தன்மையும் சாயம் அல்லது சேர்க்கைப் பொருட்களிலிருந்து அல்ல, இயற்கையான செயலாக்கத்திலிருந்தே கிடைக்கின்றன.",
        ...
        "தளிர் நிலை மற்றும் வலை அளவுக்கு அப்பால், வகைப்படுத்தப்பட்ட தேயிலை மூன்று சந்தை பிரிவுகளாகவும் வகைப்படுத்தப்படுகிறது: First Mark, Second Mark, மற்றும் Third Mark. இங்கு உற்பத்தி செய்யப்படும் முக்கிய தரங்களில் OPA, OP, OP1, BOP1, FBOP, FF1, FF, Pekoe, Pekoe1, BOP Special, BOPA, FF Extra Special, Extra Special 1, மற்றும் FF Special ஆகியவை அடங்கும், அதே நேரம் குறைந்த மதிப்புள்ள Off-Grade தேயிலை 1A, BM, BP, FNGS, மற்றும் BT போன்ற பிரிவுகளில் அடங்கும்.",
        "சராசரியாக, 100 கிலோ பச்சை இலை சுமார் 21.5 கிலோ முடிக்கப்பட்ட, பேக் செய்யப்பட்ட கருப்பு தேயிலையை உற்பத்தி செய்கிறது, இது சுமார் 21.5% உற்பத்தி விகிதமாகும். மீதமுள்ளவை ஈரப்பத ஆவியாதல் மற்றும் வகைப்படுத்தலின் போது அகற்றப்படும் குப்பை தேயிலையால் இழக்கப்படுகின்றன. நிலையான தேயிலை வாரிய சோதனை வழிகாட்டுதல்களின்படி, 1 கிலோ தயாரிக்கப்பட்ட தேயிலைக்கு சுமார் 4.65 கிலோ பச்சை இலை தேவைப்படுகிறது. தொழிற்சாலை குறைந்தபட்சம் 65% உற்பத்தியை முக்கிய தரங்களில் இருந்து பெறவும், Off-Grade தேயிலையை 20% க்கு மிகாமல் வைத்திருக்கவும், மீதமுள்ளதை Second Mark மற்றும் அது போன்ற பிரிவுகளில் விழச் செய்யவும் இலக்கு வைத்துள்ளது.",
      ],
      duration: "வகைப்படுத்தல் இயந்திரங்களில் தொடர்ச்சியாக நடைபெறுகிறது, ஒரு தொகுதிக்கு சுமார் 30 முதல் 45 நிமிடங்கள் ஆகும்.",
    },
    packing: {
      name: "பொதிசெய்தல்",
      sectionHeadings: ["எடைபோடுதல், சுவைபார்த்தல், முத்திரையிடுதல்", "பொதிசெய்யும் தளத்தில் உள்ள தரங்கள்", "தரத்திற்கேற்ப பை எடைகள்", "தொழிற்சாலையிலிருந்து ஏலத்திற்கு", "காற்றுப்புகா பேக்கிங் ஏன் முக்கியம்", "தரங்களை சந்தைகளுக்கு பொருத்துதல்"],
      sectionBodies: [
        "தரப்படுத்தப்பட்ட தேயிலை எடைபோடப்பட்டு, தரத்திற்காக சுவைபார்க்கப்பட்டு, அலுமினியம் இழை படலம் இட்ட ஈரப்பதம் தடுக்கும் பேக்கேஜிங்கில் முத்திரையிடப்படுகிறது. இதில் தொழிற்சாலையின் பெயரும் சின்னமும், பதிவு எண், சிலோன் தேயிலை சின்னம், மற்றும் உணவு பாதுகாப்பு சான்றிதழ் அச்சிடப்பட்டுள்ளன.",
        ...
        "பைகளின் எடையும் தரத்திற்கேற்ப மாறுபடும்: நிலையான அளவு பையில் Dust போன்ற மெல்லிய, அடர்த்தியான தரத்தை (சுமார் 58 கிலோ) OPE போன்ற பெரிய, இலை நிறைந்த தரத்தை (சுமார் 30 கிலோ) விட மிக அதிகமாக கொள்ள முடியும்.",
        "பெரும்பாலும், தொழிற்சாலை தனது முடிக்கப்பட்ட தேயிலையை கொழும்பு தேயிலை தரகர்களிடம் ஒப்படைக்கிறது, Forbes & Walker, Akbar Brothers, Finlays போன்ற பெயர்களைக் குறிப்பிடலாம், அவர்கள் அதை கொழும்பு தேயிலை ஏலத்திற்கு கொண்டு செல்கிறார்கள். அங்கு வாங்குபவர்கள் அதை வாங்கி, பெரும்பாலும் மேலும் மதிப்பு சேர்த்த பிறகு தங்கள் சொந்த பிராண்டில் ஏற்றுமதி செய்கிறார்கள். அத்துகோரள பெயரிலேயே நேரடி ஏற்றுமதியும் நடைபெறுகிறது, ஆனால் இது பெரும்பாலும் வெளிநாட்டு வாங்குபவர்களின் சிறப்பு ஆர்டர்களுக்கு மட்டுமே.",
        "காற்றுப்புகா பேக்கிங் தேயிலையை ஈரப்பதம் மற்றும் வாசனைகளிலிருந்து பாதுகாக்கிறது, செயல்முறையின் ஒவ்வொரு முந்தைய கட்டத்திலும் கவனமாக கட்டமைக்கப்பட்ட சுவையை பாதுகாக்கிறது.",
        "வெவ்வேறு நாடுகள் தங்கள் தேயிலையிலிருந்து வெவ்வேறு விஷயங்களை எதிர்பார்க்கின்றன. சிலர் வலிமையான, கடுமையான கோப்பையை விரும்புகிறார்கள், மற்றவர்கள் விரும்புவதில்லை. எனவே ஒரு தரம் தளிரின் எந்த பகுதியிலிருந்து வருகிறது என்பது ஒவ்வொரு ஏற்றுமதி சந்தையின் ருசிக்கும் ஏற்ப பொருத்தப்பட்டுள்ளது. உதாரணமாக, அரபு மொழி பேசும் சந்தைகள் தங்கள் சொந்த நாட்டில் அன்றாட குடிப்பழக்கத்தின் பகுதியாக இல்லாத குறிப்பிட்ட தரங்களை கோருகின்றன. நேரடியாக ஏற்றுமதி செய்வதற்கு பதிலாக பெரும்பாலும் தரகர்கள் மூலம் விற்பனை செய்வது என்பதன் பொருள், தொடர்ந்து சர்வதேச கப்பல் போக்குவரத்தும் பிராண்டிங்கும் பொதுவாக வெளி வாங்குபவர்களால் கையாளப்படுகிறது.",
      ],
    ...
    "artisanal-tea-grades": {
      ...
      sectionHeadings: [
        ...
        "Golden Tip மற்றும் Silver Tip",
        "White Tea மற்றும் Purple Tea",
        "Pink Tea மற்றும் Vita Glow",
        "கலவைகள் மற்றும் அபூர்வத்தன்மை",
        "Flower Chakra மற்றும் Tea Coin",
      ],
      sectionBodies: [
        "சாதாரண கருப்பு தேயிலைக்கு கூடுதலாக, இந்த தோட்டம் அரிதான, கைவினைத் தேயிலைகளையும் தயாரிக்கிறது. இவை பெரும்பாலும் முதன்மை கருப்பு தேயிலை உற்பத்தி வரிசையில் அல்ல, தனியான 'கைவினை தேயிலை தொழிற்சாலையில்' கையால் நிறைவு செய்யப்படுகின்றன. இதனாலேயே இந்த தரங்கள் 'கைவினை தேயிலை தரங்கள்' என்றும் அழைக்கப்படுகின்றன. இது பேரளவு உற்பத்தியை விட ஒரு கைவினைக்கு நெருக்கமானது. இவற்றில் பெரும்பாலானவை TRI 2043 எனப்படும் தனித்துவமான ஊதா நிற தளிரிலிருந்து வருகின்றன, அதாவது தேயிலை ஆராய்ச்சி நிறுவனத்தின் ஒரு குளோனிலிருந்து. அதன் நிறம் ஆந்தோசயனினிலிருந்து வருகிறது. இது குறைந்த விளைச்சல் தரும் இனமாகும், எனவேதான் ஒவ்வொரு இலங்கை தோட்டமும் இதை வளர்ப்பதில்லை.",
        "Golden Tip அந்த தளிரின் முழுமையாக முதிர்ந்த மொட்டிலிருந்து தயாரிக்கப்படுகிறது, அதே நேரம் Silver Tip அதே தளிரின் இன்னும் முதிராத, வெளிர் நிற மொட்டிலிருந்து தயாரிக்கப்படுகிறது. அதன் மெல்லிய வெள்ளி நிற முடிகளே இந்த தரத்திற்கு அதன் பெயரையும் ஒரு மெல்லிய பொலிவையும் தருகின்றன. மொட்டு மட்டும் எடைபோடப்பட்டு, சில மணி நேரம் வாடவைக்கப்பட்டு, உலர்த்தப்படுகிறது. இது முழுமையாக நொதிக்கப்படும் தரங்களை விட எளிமையான செயல்முறை, இதன் விளைவாக மென்மையான இனிப்பு, மலர் மணம், மென்மையான அமைப்புடைய ஒரு கோப்பை கிடைக்கிறது.",
        "White Tea தளிரிலிருந்து இரண்டரை இலைகளை எடுத்து, வாடவைத்தலை முழுவதுமாக தவிர்க்கிறது. இது கருப்பு தேயிலை போல நொதிக்கப்பட்டு உருட்டப்படுவதற்கு பதிலாக, ஆவியில் வேகவைத்து உலர்த்தப்படுகிறது. இதன் விளைவு மிக இலகுவாக செயலாக்கப்பட்ட, இயற்கையாக இனிப்பான சுவை: வெளிர் நிற காடி, மென்மையான அமைப்பு, மலர் வாசனை, இயற்கையான ஆக்ஸிஜனேற்ற எதிர்ப்பு நிறைவு ஆகியவற்றுடன். Purple Tea அதே பறிப்பைப் பயன்படுத்துகிறது, ஆனால் White Tea இலிருந்து வேறுபட்டு, முழு கருப்பு தேயிலை செயல்முறையின் வழியாக செல்கிறது: வாடவைத்தல், உருட்டுதல், நொதித்தல். இதன் விளைவாக ஊலாங் தேயிலைக்கு நெருக்கமான சுவை கிடைக்கிறது: கருப்பு தேயிலையை விட இலகுவானது, ஆனால் பச்சை தேயிலையை விட தாவர சுவை குறைவானது, ஒப்பீட்டளவில் காஃபின் குறைவானது, இயற்கையாக ஆக்ஸிஜனேற்ற எதிர்ப்பு நிறைந்தது.",
        "Pink Tea ஆரம்பத்தில் White Tea போலவே நிறமற்றதாக இருக்கும். எலுமிச்சை சாற்றின் சில துளிகள் சேர்க்கப்பட்ட பின்னரே இது இளஞ்சிவப்பு நிறமாக மாறுகிறது. அந்த சிட்ரஸ், தளிருக்கு அதன் நிறத்தைத் தரும் அதே ஆந்தோசயனின் நிறமியில் ஒரு நிற மாற்றத்தைத் தூண்டுகிறது. சில துளிகள் தேன் சேர்ப்பதும் இதற்கு நன்றாகப் பொருந்துகிறது, பனிக்கட்டி தேநீராக பரிமாறுவது பரிந்துரைக்கப்படுகிறது, இதுவும் இயற்கையாக ஆக்ஸிஜனேற்ற எதிர்ப்பு நிறைந்தது. Vita Glow ஊதா நிற தளிருக்கு பதிலாக, சாதாரண பச்சை நிற தளிரின் மொட்டிலிருந்து தயாரிக்கப்படுகிறது.",
        "தோட்டம் மதிப்புக்கூட்டப்பட்ட கலவைகளையும் தயாரிக்கிறது, தாமரை மலர்களுடன் கலந்த தேயிலை போன்றவை, மற்றும் அதே தளிர்களிலிருந்து கையால் கட்டப்பட்ட Blooming Teaவையும். இந்த கைவினைத் தரங்கள் நிலையான கருப்பு தேயிலையை விட மிகக் குறைந்த அளவில் பறிக்கப்பட்டு செயலாக்கப்படுகின்றன, இதுவே அவற்றை அரிதானதாகவும் மதிப்புமிக்கதாகவும் ஆக்குகிறது. சுவை பார்க்கும் கவுண்டரில் இவற்றை சுவைக்க கேளுங்கள். இந்த தரங்களில் பெரும்பாலானவற்றிற்கு ஊதா நிற தளிர் அல்லது சாதாரண பச்சை தளிர் இரண்டையும் பயன்படுத்தலாம் என்றாலும், தோட்டம் பெரும்பாலும் ஊதா நிற தளிரையே பயன்படுத்துகிறது.",
        "இங்கு தயாரிக்கப்படும் மிகவும் உழைப்பு மிக்க பொருட்கள் இரண்டு சிறு-தொகுதி சிறப்பு பொருட்களே: Flower Chakra மற்றும் Tea Coin. Flower Chakra என்பது சுமார் இரண்டு தளிர்களைப் பயன்படுத்தும் கையால் உருட்டப்பட்ட கருப்பு தேயிலை வளையம். ஒரு நபர் ஒரு நாளைக்கு இவற்றில் ஒன்றை மட்டுமே சுமார் 25 கிராம் கையால் உருட்ட முடியும், அதனால்தான் இவை மிகவும் விலை உயர்ந்தவை. இவை அன்றாட குடிப்பதற்கு பதிலாக, செல்வந்த தேயிலை ஆர்வலர்களால் ஒரு நினைவுப் பொருளாக பெரும்பாலும் வாங்கப்படுகின்றன. Purple Tea உட்பட, பெரும்பாலான கைவினை உற்பத்தி தொடர்ச்சியாக மொத்தமாக உற்பத்தி செய்யப்படுவதற்கு பதிலாக ஆர்டர் அடிப்படையில் சிறிய தொகுதிகளாக தயாரிக்கப்படுகிறது.",
      ],
      duration: "5 நிமிடங்கள்",
    },
    "pahatharata-manufacturing": {
      name: "தேயிலை உற்பத்தி (பஹத்தரட)",
      sectionHeadings: ["தாழ்நில எதிராக உயர்நில பதப்படுத்தல்", "CTC செயலாக்கம்"],
      sectionBodies: [
        "தாழ்வான நாட்டு ('பஹத்தரட') தோட்டங்கள், இந்த தோட்டத்தைப் போலவே, உள்நாட்டு உயர் தோட்டங்களிலிருந்து வேறுபட்ட முறையில் தேயிலையை செயலாக்குகின்றன. இந்த சுற்றுலாவில் முன்னதாக படிப்படியாக விளக்கப்பட்ட தோட்டத்தின் சொந்த உற்பத்தி செயல்முறையே இதை உறுதிப்படுத்துகிறது. இங்கு உருட்டுதல் இலையை முழுமையான பாரம்பரிய இலையாக முறுக்குவதற்குப் பதிலாக சிறிய, அடர்த்தியான துகள்களாக உடைக்கிறது, இதுவே CTC (நசுக்கு-கிழி-சுருட்டு) செயலாக்கத்தின் அடையாள அம்சமாகும். தாழ்வான நாட்டு அணுகுமுறை பறித்தல், செயலாக்கம், மற்றும் தரம் பிரித்தலுக்கு என்ன அர்த்தம் என்பதற்கான ஒரு சுருக்கமான மறுபார்வையே இந்த நிறுத்தம்.",
        "CTC (நசுக்கு-கிழி-சுருட்டு) இலையை இயந்திர முறையில் சிறிய, அடர்த்தியான துகள்களாக செயலாக்குகிறது, இது தாழ்வான நாட்டு இலங்கை தேயிலைக்கு பிரத்தியேகமான வலிமையான, தெளிவான சுவையை உருவாக்குகிறது. இந்த சுற்றுலாவில் முன்னதாக உருட்டுதல் மற்றும் தரப்படுத்தல் நிறுத்தங்களில் கண்ட அதே இலைத் துகள்கள்தான் இவை.",
      ],
    ...
    "clonal-tea": {
      name: "குளோன் தேயிலை",
      heroTagline: "தோட்டத்தின் சொந்த லேபிள் செய்யப்பட்ட குளோன் வகை சோதனை செடிகள் சில.",
      sectionHeadings: ["குளோன் தேயிலை", "குளோன் தேயிலை அதிக விலை கொண்டிருப்பது ஏன்", "தேயிலை வகைகள்", "TRI 3069", "TRI 3055", "TRI 4053", "TRI 2022", "TRI 2043", "TRI 4006", "TRI 3025"],
      sectionBodies: ["மிகவும் விரும்பப்படும் சில தேயிலைகள், விதையிலிருந்து வளர்க்கப்படுவதற்குப் பதிலாக, ஒரு விதிவிலக்கான தாய் புதரிலிருந்து பெருக்கப்பட்ட கிளைநடவுகளிலிருந்து குளோன் செடிகளாக வருகின்றன. குளோன் தேயிலை எவ்வாறு பெருக்கப்படுகிறது, ஏன் அதிக விலைக்கு விற்கப்படுகிறது என்பதை இந்த நிறுத்தம் விளக்குகிறது. தேயிலை ஆராய்ச்சி நிறுவனம் (TRI) வெவ்வேறு தேவைகளுக்காக குளோன் வகைகளை உருவாக்குகிறது. சில வறட்சி அல்லது மழைக்கு தாங்கும் தன்மைக்காகவும் அதிக விளைச்சலுக்காகவும், மற்றவை கைவினைச் சிறப்பு தேயிலைகளுக்குப் பயன்படும் சிவப்பு அல்லது பச்சை நுனி இளம் தளிர்களுக்காகவும். TRI 2023, TRI 2025, TRI 2026 போன்ற பரவலாக நடப்பட்ட, அதிக விளைச்சல் தரும் வகைகள் தோட்டங்களிலும் வீட்டுத் தோட்டங்களிலும் பொதுவானவை, அதே சமயம் அரிதான சிறப்பு வகைகள் பெரும்பாலும் தினசரி சாகுபடிக்குப் பதிலாக கைவினை தேயிலைக்காக பெரிய தோட்டங்களில் மட்டுமே வளர்க்கப்படுகின்றன. இந்த TRI எண்கள் ஆண்டுகளைப் போல தோன்றினாலும், அவை வெறும் ஒதுக்கப்பட்ட எண்களே, ஒவ்வொரு வகையும் வெளியிடப்பட்ட ஆண்டு அல்ல.", "குளோன் பெருக்கம் ஒரு சிறந்த செடியின் சரியான பண்புகளை பாதுகாக்கிறது. சிறந்த குளோன் தேயிலைகள் பெரும்பாலும் மிகக் குறைந்த அளவில் உற்பத்தி செய்யப்படுகின்றன. சிவப்பு அல்லது பச்சை நுனி சிறப்பு வகைகளிலிருந்து பெறப்படும் தளிர்கள், Golden Tips மற்றும் Silver Tips போன்ற அரிய தரங்களுக்காக கையால் பறிக்கப்படுகின்றன, மேலும் தோட்டங்கள் பொதுவாக சாதாரண பறிக்கப்பட்ட இலைகளை விட கிலோவுக்கு சுமார் 100 ரூபாய் அதிகமாக செலுத்துகின்றன.", "தோட்டம், உள்ளூர் மண் மற்றும் வானிலையில் நம்பகமாக செயல்படும் விதத்தால், தேயிலை ஆராய்ச்சி நிறுவனத்தால் (TRI) வளர்க்கப்பட்ட அதிக விளைச்சல் தரும், பூச்சி-எதிர்ப்புத் திறன் கொண்ட குளோன்களை விரும்புகிறது, அதாவது TRI 2000 தொடர் மற்றும் T65 உட்பட. TRI 4049 என்பது இலங்கை தேயிலை ஆராய்ச்சி நிறுவனத்தால் உருவாக்கப்பட்ட அதிக மகசூல் தரும் தேயிலை வகையாகும், இது நடுத்தர முதல் தாழ்வான உயரங்களுக்கு ஏற்றது. இது Camellia sinensis இன் தாவர இனப்பெருக்கம் செய்யப்பட்ட வகையாகும், வலுவான வளர்ச்சி, அடர் பச்சை இலைகள் மற்றும் பொதுவான நோய்களுக்கு எதிர்ப்புத்திறன் கொண்டதாக அறியப்படுகிறது. TRI 4049 CTC மற்றும் பாரம்பரிய தேயிலை இரண்டிற்கும் ஏற்ற பிரகாசமான, நறுமணமிக்க தேநீரை உற்பத்தி செய்கிறது, இது மறுநடவு மற்றும் வணிக சாகுபடிக்கு பிரபலமான தேர்வாக அமைகிறது.", "சில வரிசைகளுக்கு அப்பால், அடையாளப்படுத்தப்பட்ட இந்தச் செடி TRI 3069 ஆகும். இது தேயிலை ஆராய்ச்சி நிறுவனத்தில் பதிவு செய்யப்பட்ட மற்றொரு வகையாகும், இந்த குறிப்பிட்ட மண்ணில் ஒவ்வொன்றும் எவ்வாறு செயல்படுகிறது என்பதை ஒப்பிட நாற்றுமேடையில் TRI 4049 உடன் இணைந்து வளர்க்கப்படுகிறது.", "TRI 3055 அதே சோதனை வரிசைகளில் உள்ளது. இங்குள்ள ஒவ்வொரு வகை நிலமும் தனித்தனியாக அடையாளமிடப்பட்டு கண்காணிக்கப்படுகிறது, இதன் மூலமே புகழை மட்டும் நம்பாமல், உண்மையிலேயே விரிவாக்கத்திற்கு தகுதியான வகைகள் எவை என்பதற்கான ஆதாரங்களை தொழிற்சாலை தானே உருவாக்குகிறது.", "இங்கே காட்டப்பட்டுள்ள வகை சோதனை நிலங்களை TRI 4053 நிறைவு செய்கிறது. பதிவு செய்யப்பட்ட பல வகைகளை தெளிவாக அடையாளமிட்டு அருகருகே வளர்ப்பது, ஒரு வகையை முழு வயலுக்கும் ஒதுக்குவதற்கு முன் இந்த தோட்டத்தில் வழக்கமான நடைமுறையாகும்.", "TRI 2022 அதே வகை சோதனைப் பிரிவில் நடப்பட்டுள்ளது, அதன் அடையாளப் பலகை அதன் அண்டை செடிகளைப் போலவே குறியிடப்பட்டு கண்காணிக்கப்படுகிறது. ஒரே மண்ணில் பதிவு செய்யப்பட்ட பல வகைகளை அருகருகே வளர்ப்பது, இங்கு எது சிறப்பாக செயல்படுகிறது என்பதற்கான உண்மையான ஆதாரங்களை நாற்றுமேடை உருவாக்கும் விதமாகும்.", "TRI 2043 வரிசையில் இன்னும் சற்று முன்னே நிற்கிறது. இந்த சோதனை நிலத்தில் உள்ள மற்ற எண்ணிடப்பட்ட செடிகளைப் போலவே, பெரிய அளவில் நடுவதற்கான முடிவு எடுக்கப்படுவதற்கு முன் இது வளர்க்கப்பட்டு கண்காணிக்கப்படுகிறது.", "TRI 4006 அதே வகை சோதனையில் மற்றொரு உறுப்பினராகும். நாற்றுமேடை ஒவ்வொரு வகையையும் அதன் சொந்த தெளிவாக குறியிடப்பட்ட இடத்தில் வைத்திருக்கிறது, இது காலப்போக்கில் வளர்ச்சியை கண்காணித்து ஒப்பிடுவதை எளிதாக்குகிறது.", "TRI 3025 சோதனை நிலத்தின் ஓரத்தில், இருமொழி 'தேயிலை வகைகள்' பலகை மற்றும் தேதியிடப்பட்ட நடவு நினைவுப் பலகைக்கு அருகில் உள்ளது. இங்கு குறியிடப்பட்ட செடிகள் அனைத்தும் சேர்ந்து, எந்த வகைகள் விரிவாக்கத்திற்கு தகுதியானவை என்பதற்கான நாற்றுமேடையின் உண்மையான பதிவை உருவாக்குகின்றன."],
      duration: "5-10 நிமிடங்கள்",
    },
    "shade-management": {
    ...
    "tea-factory": {
      ...
      sectionBodies: ["பறிக்கப்பட்ட இலை எடைபோடுதல் முதல் பொதிசெய்தல் வரை முடிக்கப்பட்ட தேயிலையாக மாற்றப்படும் இடம் முதன்மை தொழிற்சாலை கட்டிடமாகும். இயங்கும் தொழிற்சாலை தளத்தையும் ஒவ்வொரு கட்டத்திலும் பயன்படுத்தப்படும் உபகரணங்களையும் இந்த நிறுத்தம் மேலோட்டமாக விளக்குகிறது. அதாவது எடைபோடுதல், வாடுதல், உருட்டுதல், நொதித்தல் (ஆக்சிஜனேற்றம்), உலர்த்துதல், தேயிலை பிரித்தல் (தரப்படுத்தல்), மற்றும் இறுதி பொதிசெய்தல், அந்த வரிசையில். இயங்கும் தேயிலை தொழிற்சாலை ஒரு கூரையின் கீழ் எடைபோடுதல் முதல் பொதிசெய்தல் வரையிலான முழு வரிசையையும் இயக்குகிறது, தினசரி இலை வரவுக்கு நெருக்கமாக நேரம் ஒத்திசைக்கப்படுகிறது. அன்றைய அறுவடையை பதிவு செய்ய வரும் இலை முதலில் எடைபோடப்படுகிறது. ஆலை தளத்தை பார்வையிடும் விருந்தினர்கள் சுகாதார தரநிலைகளுக்கு இணங்க தலைமுடி வலை, முகக்கவசம், மற்றும் பாதுகாப்பு காலணிகள் அணிய வேண்டும் என்று கேட்டுக்கொள்ளப்படுகிறார்கள்."],
      duration: "10-15 நிமிடங்கள்",
    },
    "handmade-factory": {
      name: "கைவினை தேயிலை தொழிற்சாலை",
      heroTagline: "தோட்டத்தின் சொந்த கையால் செய்யப்படும் தேயிலை தொழிற்சாலையிலிருந்து ஒரு தட்டு அடுக்கு.",
      sectionHeadings: [
        "கைவினை தேயிலை தொழிற்சாலை",
        "வாடவைத்தலும் உருட்டுதலும்",
        "பச்சை தேயிலை, உலர்த்துதல் மற்றும் கட்டிடம்",
        "கைவினை தேயிலையை வேறுபடுத்துவது என்ன",
      ],
      sectionBodies: [
        "இயந்திரமயமாக்கப்பட்ட உற்பத்திக்கு கூடுதலாக, சில தேயிலைகள் ஒவ்வொரு கட்டத்திலும் இயந்திரமின்றி முழுவதுமாக கையால் செயலாக்கப்படுகின்றன. தோட்டத்தின் பெரும்பாலான கைவினை சிறப்பு தரங்கள், அதாவது Golden Tip, Silver Tip, White Tea, Purple Tea, Pink Tea, Vita Glow, கையால் கட்டப்பட்ட Blooming Tea, மற்றும் தாமரை தேநீர் போன்ற மலர் கலந்த கலவைகள், முதன்மை கருப்பு தேயிலை தொழிற்சாலைக்கு பதிலாக இங்கேயே நிறைவு செய்யப்படுகின்றன. (Vita Glow, Golden Tip மற்றும் Silver Tip இன் ஊதா நிற தளிரிலிருந்து அல்லாமல், பச்சை நிற தளிரின் மொட்டிலிருந்து தயாரிக்கப்படுகிறது.)",
        "இங்கு வாடவைத்தல் இயற்கையாக நடைபெறுகிறது, ஒரு சிறிய தொட்டியில், ஈரப்பதமானியால் கண்காணிக்கப்படுகிறது, முதன்மை தொழிற்சாலையின் காற்று அடிக்கும் மின்விசிறிகள் இல்லாமல். உருட்டுவதற்கு முன் ஈரப்பதம் சுமார் 50% ஆக குறைக்கப்படுகிறது. பின்னர் இலை ஒரு தட்டில் கையால் உருட்டப்படுகிறது, கடிகாரத்தை விட உணர்வின் அடிப்படையில் தீர்மானிக்கப்படுகிறது. பெரிய மொத்த ஆர்டர்களுக்கு, சிறிய இயந்திர உருளைகள் அதே நோக்கத்திற்காக பயன்படுத்தப்படுகின்றன, ஏனெனில் கையால் உருட்டுவது மட்டும் அளவுக்கு ஈடுகொடுக்க முடியாது.",
        "ஒரு தனி பச்சை தேயிலை இயந்திரம், அதாவது இலையை ஆவியில் வேகவைத்து அழுத்தும் இயந்திரம், அந்த தரத்தை தனியாக கையாளுகிறது, ஆனால் இது தற்போது சிறிய பழுதுபார்ப்புக்காக செயலிழந்துள்ளது. ஆவியில் வேகவைத்த பிறகு, இலை வெளியே வைக்கப்பட்டு மேற்பரப்பு ஈரப்பதத்தை இழக்கிறது. பின்னர் அதன் நிறம், வலிமை, மணம், சுவையை வெளிக்கொணர நீட்டி கையால் உருட்டப்பட்டு, இறுதியாக உலர்த்தியின் வழியாக செல்கிறது. உலர்த்தும் நேரம் ஒவ்வொரு சிறு உற்பத்தியாளருக்கும் வேறுபடும். கட்டிடத்தின் சிறிய அடுப்பு தேயிலை அடிப்படையிலான பிஸ்கட் போன்ற தின்பண்டங்களை சுடவும் பயன்படுகிறது. உருட்டியபின் செய்யப்படும் உலர்த்துதல் இங்கு இரண்டு தனி உலர்த்திகளில் நடைபெறுகிறது: ஒன்று பாரம்பரிய விறகு எரிபொருள் மூலம், மற்றொன்று மின்சாரம் மூலம். ஒவ்வொன்றிலும் 34 தட்டுகள் கொள்ளும், இரண்டும் ஏலக்காய், கிராம்பு, இஞ்சி போன்ற பிற சிறு-அளவு தயாரிப்புகளுக்கும் பயன்படுத்தப்படுகின்றன.",
        "கைவினை தேயிலையை தொழிற்சாலை வரிசை உற்பத்தியிலிருந்து வேறுபடுத்துவது என்ன என்பதை இந்த நிறுத்தம் விளக்குகிறது. கையால் உருட்டுதலும் கையால் செயலாக்கமும் சிறிய தொகுதிகளை உருவாக்குகின்றன, இலைகளுக்கிடையே அதிக மாறுபாட்டுடன், அவற்றின் தனித்துவமான தன்மைக்காக பெரும்பாலும் மதிக்கப்படுகின்றன. கைவினை தேயிலைகள் இரண்டு பரந்த வகைகளாக பிரிக்கப்படுகின்றன. முதல் வகை கருப்பு தேயிலை பாணியில் நொதிக்கப்பட்டவை, கருப்பு நிறத்தால் அடையாளம் காணக்கூடியவை, இதில் Purple Teaவும் அடங்கும், அதன் அசாதாரண மூல தளிர் இருந்தபோதிலும். இரண்டாவது வகை White Tea போன்ற, நொதித்தலை முழுவதுமாக தவிர்த்து நேரடியாக உலர்த்தலுக்கு செல்லும் நொதிக்கப்படாதவை.",
      ],
      duration: "10 நிமிடங்கள்",
    },
    "tea-center": {
      name: "தேயிலை மையம்",
      heroTagline: "தோட்டத்தின் சொந்த தேநீர் சுவைப்பு கவுன்ட்டரில் தேநீர் சுவைக்கும் பார்வையாளர்கள்.",
      sectionHeadings: [
        "தேயிலை மையம்",
        "தேநீர் தயாரிப்பும் சுவை பரிசோதனையும்",
        "அன்றாடத் தரங்களும் சூப்பர்மார்க்கெட் தேநீரும்",
        "சிறந்த தரங்களும் ஏற்றுமதி சந்தைகளும்",
        "மூலிகை மற்றும் மசாலா கலவைகள்",
      ],
      sectionBodies: [
        "சுற்றுலா தேயிலை மையத்தில் முடிவடைகிறது. இங்கு ஒரு சுவை பரிசோதனை குழு ஒவ்வொரு தொகுதியையும் சோதிக்கிறது, இங்கு மட்டும் அல்ல, தேயிலை முதன்மை தொழிற்சாலையிலிருந்து உள்ளூர் சந்தைக்கு செல்லும் போதும் அல்லது பேக்கிங்கிற்கு வரும் போதும், அதிக நொதித்தல், குறைந்த நொதித்தல், அல்லது கருகுதல் போன்ற குறைபாடுகளுக்காக. விற்பனைக்கு முன் தோல்வியடையும் எதையும் நிராகரித்து, எதிர்கால உற்பத்திக்கு உதவும் வகையில் சிறிய பிரச்சினைகளை கூட குறிப்பிடுகிறது. கைவினை தொழிற்சாலையின் மேலும் இரண்டு சிறு-தொகுதி சிறப்பு பொருட்கள் இங்கு காட்சிக்கு வைக்கப்பட்டுள்ளன: Flower Chakra மற்றும் Tea Coin.",
        "இதுபோன்ற கைவினை தேயிலைகளை கொதிநிலைக்கு பதிலாக சுமார் 80°C சூடான நீரைப் பயன்படுத்தி மூன்று முறை வரை மீண்டும் ஊற வைக்கலாம். ஒவ்வொரு முறையும் புதிய சூடான நீருடன் சுமார் 2, 5, மற்றும் 8 நிமிடங்கள் ஆகும். சாதாரண கருப்பு தேயிலை கொதிநிலையில் ஒரே முறை மட்டும் தயாரிக்கப்படுகிறது. சுவை பரிசோதனை மது சுவை பரிசோதனையை ஒத்த ஒரு முறையை பின்பற்றுகிறது. முதலில் முகர்ந்து பார்த்தல், பின்னர் நாக்கின் பக்கவாட்டு சுவை மொட்டுகள் வழியாக தேயிலையை இழுக்க காற்றை உறிஞ்சி சுருட்டுதல். குழு ஒரு நாளைக்கு 50 தேயிலைகள் வரை சுவைக்கும் போது விழுங்குவதற்கு பதிலாக துப்புகிறது, எனினும் பார்வையாளர்கள் தாங்கள் சுவைப்பதை உண்மையில் குடிக்க வரவேற்கப்படுகிறார்கள். சரியாக தயாரிக்கப்பட்ட கருப்பு தேயிலைக்கு மூடப்பட்ட நிலையில் சுமார் 2.5 முதல் 3 நிமிடங்கள் ஊற வைக்க வேண்டும், விரைவான ஒரு தோய்த்தல் போதாது.",
        "BOPF மற்றும் BOPF Special ஆகியவை பெரும்பாலான இலங்கையர்கள் அன்றாடம் குடிக்கும் தரங்கள். Dust மற்றும் Dust 1, மிகக் குறைந்த தரங்கள், ஒருபோதும் ஏற்றுமதி சந்தைகளுக்கு அனுப்பப்படுவதில்லை, மேலும் பொதுவாக நிறமூட்டி சேர்க்கப்பட்டு மலிவான சூப்பர்மார்க்கெட் தேநீர் பைகளில் முடிவடைகின்றன. அதனால்தான் சரியாக ஊறவைத்த பிறகு அல்ல, அமிழ்த்தியவுடனேயே நிறம் தோன்றுகிறது. சூப்பர்மார்க்கெட்டுகளில் விற்பனையாகும் பெரும்பாலானவை வேறு இடங்களிலிருந்து வாங்கப்பட்ட மலிவான தேயிலையுடன் கலக்கப்படுகின்றன, தோட்டத்தின் சொந்த கலக்கப்படாத, ஒரே மூலத்தேயிலைகளிலிருந்து வேறுபட்டது.",
        "மிகச் சிறந்த தரங்கள், அதாவது FF Extra Special, FF Special, FBOP, அறுவடையில் சுமார் 1% மட்டுமே, தளிரின் மிக முதிராத பகுதியிலிருந்து கிடைக்கின்றன, 100 கிராமுக்கு சுமார் $25 விற்கப்படுகின்றன. OPA மற்றும் OP போன்ற பெரிய தரங்கள் 200 கிராமுக்கு $5-6 வரை விற்கப்படுகின்றன. சிறந்த தரங்களில் காணப்படும் வெள்ளை நுனித் துகள்கள் கூடுதல் இனிப்பைச் சேர்க்கின்றன, பெரும்பாலும் ஐரோப்பிய வாங்குபவர்களுக்கு செல்கின்றன, அரபு சந்தைகள் வலிமையான கோப்பையை விரும்புகின்றன.",
        "தேயிலை தவிர, தோட்டம் தனது சொந்த பச்சை மற்றும் கருப்பு தேயிலையை (சுமார் 90%) உள்ளூர் மூலிகைகளான (சுமார் 10%) கோதுகொலா, ஹீன்போவிட்டியா, மொரிங்கா ஆகியவற்றுடன் கலந்து, தனது சொந்த மசாலாக்களிலிருந்து Masala, இலவங்கப்பட்டை, மற்றும் இஞ்சி மசாலா தேநீர்களை தயாரிக்கிறது. Masala செய்முறையில் ஏழு இலங்கை மசாலாப் பொருட்கள் பயன்படுத்தப்படுகின்றன. அவற்றில் ஏலக்காய், இலவங்கப்பட்டை, இஞ்சி, மிளகு ஆகியவை பெயரிடப்பட்டுள்ளன, மீதமுள்ளவை ரகசியமாக வைக்கப்பட்டுள்ளன. பால் தேநீருக்கு Masala அல்லது BOP தேநீர் பரிந்துரைக்கப்படுகிறது.",
      ],
      duration: "10-15 நிமிடங்கள்",
    },
    "coconut-field": {
    ...
    introduction: {
      ...
      sectionBodies: [
        ...
        "விருந்தினர்கள் பாரம்பரியமாக 'ஆயுபோவன்' என்ற வார்த்தையால் வரவேற்கப்படுகிறார்கள், அதாவது 'நீடூழி வாழ்க' என்று பொருள்படும் சிங்கள வாழ்த்துச் சொல்லால். சில வேளைகளில் தோட்டத்தின் சொந்த தலைவர் அனுர அதுகோரளவே நேரடியாக இதைச் செய்கிறார்.",
      ],
      duration: "5-10 நிமிடங்கள்",
    },
    "jaggery-palm": {
    ...
    pruning: {
      ...
      sectionBodies: ["கத்தரிப்பதற்கு முன், உடனடியாக வெட்டுவதற்குப் பதிலாக, தோட்டம் வேண்டுமென்றே தேயிலைப் புதரை ஒரு காலத்திற்கு வளர விடுகிறது. இதனால் புதிய வளர்ச்சி முழுமையாக வர அனுமதித்து, புதர் ஊட்டச்சத்தை சேகரிக்க வைக்கிறது. அந்த சேகரிப்பு முழுமையானதும் மட்டுமே கத்தரித்தல் தொடங்குகிறது, புதரை தோராயமாக முழங்கால் உயரத்திற்கு வெட்டுகிறது. இடத்திலேயே உள்ள அறிவிப்புப் பலகைகள் இதை பெயரிடப்பட்ட இரண்டு நிலைகளாகக் காட்டுகின்றன. முதல் குறுக்கு வெட்டின் (பலமு ஹரஸ் கபும) போது புதர் சுமார் 9-12 அங்குலம் (22-30 செ.மீ.) அளவுக்கு வெட்டப்படுகிறது, இரண்டாவது குறுக்கு வெட்டின் (தேவன ஹரஸ் கபும) போது அது மீண்டும் சுமார் 16-18 அங்குலம் (40-45 செ.மீ.) அளவுக்கு வளர விடப்படுகிறது. நிலைமையைப் பொருட்படுத்தாமல் ஒரு நிலையான அட்டவணையில் கத்தரிப்பதற்குப் பதிலாக, முதலில் புதரை வளர விடுவது, வெட்டப்படுவதற்கு முன் ஊட்டச்சத்தை மேலே செலுத்த தாவரத்திற்கு நேரம் அளிக்கிறது."],
      duration: "5 நிமிடங்கள்",
    },
    polytunnel: {
      name: "பாலிடன்னல் மற்றும் பசுமை இல்லம்",
      heroTagline: "தோட்டத்தின் சொந்த பாலிடன்னல், ஏற்கனவே அதன் முதல் பயிர்களை வளர்த்து வருகிறது.",
      sectionHeadings: ["பாலிடன்னல் மற்றும் பசுமை இல்லம் பற்றி", "நீர்ப்பாசனமும் நீர் மேலாண்மையும்", "அறுவடை காலவரிசை", "உரம் மற்றும் நீர் கட்டுப்பாட்டு தலைமை", "மாலு மிரிஸ் (கேப்சிகம்)", "அமு மிரிஸ்", "நை மிரிஸ்", "கெடி கோவா (முட்டைக்கோஸ்)", "அது கோவா (சிவப்பு முட்டைக்கோஸ்)", "பொஞ்சி (பீன்ஸ்)", "ராபு (முள்ளங்கி)", "கேரட்", "லீக்ஸ்", "வெள்ளரி", "தக்காளி நாற்று"],
      sectionBodies: [
        "தற்போதுள்ள பசுமையக அறைக்கு கூடுதலாக, தோட்டம் தனது சொந்த பல்வேறு காய்கறிகளை வளர்ப்பதற்காக ஒரு பாலிடன்னலை கட்டி வருகிறது. இதில் ஸ்காட்ச் பொனட் மற்றும் பச்சை மிளகாய், கேப்சிகம், கெர்கின், தக்காளி, மணி மிளகாய், முட்டைக்கோஸ், முள்ளங்கி, பீன்ஸ், லீக்ஸ், கீரை வெங்காயம், மற்றும் கேரட் ஆகியவை அடங்கும். அது முடிந்தவுடன், வில்லா அல்லது கேபனாவில் தங்கும் விருந்தினர்கள் இங்கேயே வளர்க்கப்படும் காய்கறிகளை நேரடியாக பறித்துக்கொள்ள முடியும். பாலிடன்னலின் அரை-ஒளிபுகும் மேலோடு பசுமையக கண்ணாடி போலவே செயல்படுகிறது. இது வெப்பத்தை தக்கவைத்து ஒளியை உள்ளே அனுமதிக்கும் அதே வேளையில், கனமான பருவமழையையும் பொதுவான பூச்சிகளையும் தடுக்கிறது. இந்த நிலைமைகள் திறந்த வெளியில் நம்பகமாக வளர்க்க முடியாத காய்கறிகளையும் தோட்டம் வளர்க்க உதவுவதோடு, திறந்தவெளி பயிரிடலை விட வளர்ச்சிக் காலத்தை நீட்டிக்கவும் உதவுகிறது.",
        "ஒவ்வொரு பயிரும் அதன் சொந்த நீர்ப்பாசன முறையைப் பின்பற்றுகிறது, வளர்ச்சி நிலைக்கு ஏற்ப வேர் மட்ட சொட்டு நீர்ப்பாசனத்திற்கும் மேலிருந்து தெளிக்கும் நீராவிக்கும் இடையே மாறி மாறி. கேரட் மற்றும் முள்ளங்கி போன்ற பயிர்களுக்கு நீராவி முக்கியமாக முளைப்பதற்கும் குளிர்விப்பதற்கும் உதவுகிறது, அதே சமயம் தக்காளி போன்ற பயிர்களில் காய் பிடிக்கத் தொடங்கியதும் சொட்டு நீர்ப்பாசனம் முதன்மையான முறையாக மாறுகிறது. இந்த இரு முறைகளும் வெவ்வேறு நோக்கங்களுக்காகச் செயல்படுகின்றன. இளம் விதைகளும் நாற்றுகளும் வேரூன்றுவதற்குத் தேவையான ஈரப்பதத்தை காற்றிலும் மண் மேற்பரப்பிலும் நீராவி தக்கவைக்கிறது. செடிகள் வளர்ந்த பிறகு சொட்டு நீர்ப்பாசனம் நேரடியாக வேர் பகுதிக்கு நீரை வழங்குகிறது, இது மொத்த நீர் பயன்பாட்டைக் குறைப்பதோடு இலைகளை உலர்ந்த நிலையில் வைத்து பூஞ்சை நோய் அபாயத்தையும் குறைக்கிறது.",
        "முதல் அறுவடைக்கு ஆகும் காலம் பயிருக்குப் பயிர் வெகுவாக வேறுபடுகிறது, முள்ளங்கிக்கு 25-35 நாட்கள் என்ற குறைந்த காலத்திலிருந்து லீக்ஸுக்கு 90-120 நாட்கள் வரை, எனவே பருவத்தின் எந்த நேரத்திலும் பாலிடன்னலில் அறுவடை செய்ய ஏதாவது இருக்கும். வேகமாகவும் மெதுவாகவும் முதிர்ச்சியடையும் காய்கறிகளை இவ்வாறு பக்கத்துக்குப் பக்கம் வளர்ப்பதே ஒரு வேண்டுமென்றே எடுக்கப்பட்ட முடிவாகும், ஏனெனில் இது அனைத்தும் ஒரே நேரத்தில் தயாராகவோ அல்லது முடிந்துவிடவோ காரணமாகாமல், அறுவடையை இயற்கையாகவே பல கட்டங்களாகப் பரப்புகிறது.",
        "இந்த கட்டுப்பாட்டு தலைமை ஒவ்வொரு வளர்ப்பு பைக்கும் செல்லும் சொட்டு நீர்க் குழாய்களுக்கு நீர் வழங்குகிறது. படிவுகளை நீக்க ஒரு வடிகட்டி, அழுத்தமானி ஒன்று, குழாய்ப் பிரிவுகளைத் தனித்தனியாகக் கட்டுப்படுத்த மூடு வால்வுகள், மற்றும் நீர்ப்பாசன நீர் கடந்து செல்லும்போது திரவ உரத்தை நேரடியாக அதனுள் செலுத்தும் வென்சூரி ஊசி அமைப்புடன் கூடிய இரண்டு உர தொட்டிகள் ஆகியவை இதில் அடங்கும். இதனால் ஒவ்வொரு செடிக்கும் ஒரே நேரத்தில் நீரும் உரமும் கிடைக்கின்றன.",
        "ஜோடி ஜோடியாக வளரும் நீளமான, கூர்மையான, மெருகூட்டப்பட்ட இலைகளைக் கொண்ட இந்த நாற்று மாலு மிரிஸ், ஒரு கேப்சிகம் (Capsicum annuum) வகையாகும். இது தேங்காய் நார் நிரப்பப்பட்ட வளர்ப்பு பையில் வளர்க்கப்படுகிறது, அதன் வேர் பகுதிக்கு ஏற்கனவே ஒரு சொட்டு நீர்ப்பாசனக் குழாய் இணைக்கப்பட்டுள்ளது.",
        "இந்த நாற்று அமு மிரிஸ், கேப்சிகம் annuum இனத்தைச் சேர்ந்த ஒரு மிளகாய் வகையாகும். இந்த ஆரம்ப நிலையில் அதன் இலைகள் கேப்சிகத்தின் இலைகளிலிருந்து கிட்டத்தட்ட வேறுபடுத்த முடியாத அளவு ஒத்திருக்கின்றன, ஏனெனில் இரண்டும் ஒரே இனத்தைச் சேர்ந்தவை, வெவ்வேறு பயன்பாடுகளுக்காக வளர்க்கப்படுகின்றன.",
        "நை மிரிஸ் என்ற பெயர், சிங்களத்தில் 'நாக மிளகாய்' என்று பொருள்படும் சொல்லிலிருந்து வந்தது. இது Capsicum chinense இனத்தைச் சேர்ந்தது, தோட்டத்தின் மற்ற மிளகாய் வகைகளை விட வேறுபட்டதும் பொதுவாக காரமானதுமான ஒரு இனம், ஹபனேரோ மற்றும் ஸ்காட்ச் பொனட் போன்ற வகைகளுடன் நெருங்கிய தொடர்புடையது.",
        "இந்த நாற்றின் மேலும் வட்டமான, சற்று மெழுகு தன்மை கொண்ட இலைகள் கெடி கோவாவைச் சேர்ந்தவை. இதன் பெயர் சொல் நேர் பொருளில் 'உருண்டை முட்டைக்கோஸ்' என்று பொருள்படும், மேலும் இது தலை உருவாகும் பொதுவான முட்டைக்கோஸ் வகை (Brassica oleracea var. capitata) ஆகும். இதன் தலை உருவாகத் தொடங்குவதற்கு நீண்ட காலத்திற்கு முன்பே இங்கே நடப்பட்டுள்ளது.",
        "இந்த நாற்றின் சிவப்பு-ஊதா நிற இலைகள் அது கோவாவைச் சேர்ந்தவை. இதன் பெயர் சொல் நேர் பொருளில் 'கிளை முட்டைக்கோஸ்' என்று பொருள்படும், மேலும் இது உருண்டை தலை உருவாகும் வகையை விட தளர்வான, தலை உருவாகாத முட்டைக்கோஸ் வகை (Brassica oleracea var. acephala) ஆகும்.",
        "இந்த அகன்ற, இதய வடிவ இலைகள் பொஞ்சியைச் சேர்ந்தவை, பீன்ஸ் (Phaseolus vulgaris) ஆகும். வளரத் தொடங்கியதும், இது சுற்றி வளரும் காய்கறிகளிலிருந்து முற்றிலும் வேறுபட்ட ஏறும் தன்மை கொண்ட பயிராகிறது.",
        "இன்னும் விதையிலை நிலையில் மட்டுமே உள்ள இந்த சிறிய நாற்று ராபு, முள்ளங்கி (Raphanus sativus) ஆகும். இது பாலிடன்னலில் அறுவடைக்கு வேகமாக வரும் பயிர்களில் ஒன்று, வெறும் 25-35 நாட்களில் அறுவடை செய்யக்கூடியது.",
        "கேரட் (Daucus carota subsp. sativus) இந்த ஆரம்ப நிலையிலேயே அதன் மெல்லிய, புல் போன்ற முதல் இலைகளால் அடையாளம் காணக்கூடியது, இங்கு வளர்க்கப்படும் மற்ற காய்கறிகளின் அகன்ற விதையிலைகளிலிருந்து முற்றிலும் வேறுபட்டது.",
        "லீக்ஸ் (Allium ampeloprasum) ஒரு மெல்லிய இலையாக மட்டும் தோன்றும், பாலிடன்னலில் அறுவடைக்கு மிக மெதுவாக வரும் பயிர்களில் ஒன்று. நடவு செய்ததிலிருந்து இது 90-120 நாட்கள் வரை ஆகலாம்.",
        "இந்த நாற்றின் அகன்ற, வட்டமான இரு விதையிலைகள் ஒரு சாலட் வெள்ளரியின் (Cucumis sativus) விதையிலைகள் ஆகும், அதன் உண்மையான, மேலும் இதய வடிவ இலைகள் தோன்றுவதற்கு முன்பே.",
        "இந்த தக்காளி நாற்று ஏற்கனவே தக்காளி செடிக்கே உரிய பிரிந்த, பல் போன்ற விளிம்புகளுடைய, சற்று மயிர் போன்ற இலைகளைக் காட்டுகிறது. காய் பிடிக்கத் தொடங்கியதும் நீராவியிலிருந்து சொட்டு நீர்ப்பாசனத்திற்கு மாறும் பயிராக முன்பே குறிப்பிடப்பட்டது தக்காளிதான்.",
      ],
      duration: "5 நிமிடங்கள்",
    },
    "mushroom-hut": {
      name: "காளான் குடில்",
      sectionHeadings: ["வேறுபட்ட ஒரு தோட்டம்", "ஆய்ஸ்டர் காளான் வகைகள்", "வளர்ப்பு ஊடகமும் விதைப்பொருளும்", "அறுவடை சுழற்சி", "ஊட்டச்சத்து மற்றும் ஆரோக்கிய நன்மைகள்"],
      sectionBodies: [
        "தோட்டம் தனது தேயிலை, மசாலா, மற்றும் பழப் பயிர்களுடன், ஒரு தனி காளான் குடிலையும் பராமரிக்கிறது. இது ஒரு சிறிய, நிழல் நிறைந்த கட்டமைப்பாகும், தோட்டத்தின் மற்ற பகுதிகள் வளர்க்கப்படும் சூரிய ஒளி மற்றும் காற்றோட்டத்திற்கு பதிலாக, காளான்கள் காய்க்க தேவையான இருண்ட, குளிர்ந்த, ஈரப்பதமான நிலைமைகளை பராமரிக்க கட்டப்பட்டுள்ளது.",
        "இந்த குடில் பல ஆய்ஸ்டர் காளான் வகைகளை வளர்ப்பதற்காக அமைக்கப்பட்டுள்ளது: அமெரிக்கன் ஆய்ஸ்டர் (Pleurotus pulmonarius), பூட்டான் ஆய்ஸ்டர், பிளாக் ஆய்ஸ்டர், பிங்க் ஆய்ஸ்டர் (Pleurotus djamor), மற்றும் அபலோன் காளான் (Pleurotus cystidiosus). அனைத்தும் மண்ணுக்கு பதிலாக வைக்கோல் அல்லது மர அறுக்கல் தூள் ஊடகத்தில் பயிரிடப்படுகின்றன. இவை அனைத்தும் Pleurotus இனத்தைச் சேர்ந்தவை. தனித்தனி தண்டுகளில் அல்லாமல் ஊடகத்திலிருந்தே நேரடியாக வளர்ந்து, அடர்த்தியாக ஒன்றன்மேல் ஒன்றாக அடுக்கப்பட்ட விசிறி வடிவ அல்லது ஆய்ஸ்டர் வடிவ தொப்பிகளால் இவற்றை அடையாளம் காணலாம். வெள்ளை மற்றும் சாம்பல் நிறத்திலிருந்து இளஞ்சிவப்பு, தங்க நிறம், அல்லது கருப்புக்கு அருகிலான நிறம் வரை வேறுபடும் நிறமே ஒரு வகையையும் மற்றொன்றையும் பிரிக்கும் முக்கிய அம்சமாகும். ஆனால் லேசான, சற்று சுவையான ருசி அனைத்திலும் கிட்டத்தட்ட ஒரே மாதிரியாகவே இருக்கிறது.",
        "பயிரிடல் தொடங்குவது ஊடகத்திலிருந்தே: வைக்கோல் அல்லது மர அறுக்கல் தூள், தோட்டத்தின் குறைந்த செலவு, கழிவு அடிப்படையிலான அணுகுமுறையின் பின்னணியில் உள்ள அதே பொருள். இது போட்டியிடும் பூஞ்சைகளையும் பாக்டீரியாக்களையும் அழிக்க ஊறவைத்து வெப்பத்தால் பாஸ்டரைஸ் செய்யப்படுகிறது. பின்னர் காளான் மைசீலியத்தால் முழுமையாக காலனித்துவப்படுத்தப்பட்ட தானியமான 'விதைப்பொருளுடன்' கலக்கப்பட்டு, துளையிடப்பட்ட பைகளில் அல்லது தொகுதிகளில் அடைக்கப்படுகிறது. பின்னர் அந்த தொகுதிகள் இரண்டு முதல் மூன்று வாரங்கள் வரையிலான 'விதைப்பொருள் ஓட்டத்திற்காக' இருட்டில் வைக்கப்படுகின்றன. அந்த காலத்தில் வெள்ளை மைசீலியம் ஊடகம் முழுவதும் பரவுகிறது. அதன் பிறகே குடிலின் இருண்ட, குளிர்ந்த, ஈரப்பதமான நிலைமைகள் காய்ப்பைத் தூண்ட செயல்படுகின்றன.",
        "இப்படி காளான்கள் வளர்ப்பது ஒரு பண்ணை தனது முதன்மை பயிரைத் தாண்டி பல்வகைப்படுத்த குறைந்த செலவில், நிலைத்தன்மையான வழியாகும். பெரும்பாலும் வைக்கோல் போன்ற விவசாய கழிவுகளை வளர்ப்பு ஊடகமாகப் பயன்படுத்துகிறது, ஒரே ஒரு தொகுதி ஊடகம் பல வாரங்களில் பல அறுவடைகளை தருகிறது. பொதுவாக ஒரு தொகுதியிலிருந்து ஒன்று முதல் இரண்டு வாரங்கள் இடைவெளியில் இரண்டு முதல் நான்கு அறுவடைகள் வரை கிடைக்கும். முதல் அறுவடை மிகப் பெரியதாக இருந்து, அதன் பின் விளைச்சல் படிப்படியாகக் குறையும். காளான்கள் வெட்டப்படுவதற்குப் பதிலாக முழு கொத்தாக முறுக்கி எடுக்கப்படுகின்றன, ஏனெனில் மீதமிருக்கும் தண்டுத் துண்டுகள் அழுகி, அடுத்த அறுவடையைக் கெடுக்கும் அசுத்தத்தை உண்டாக்கக்கூடும்.",
        "தோட்டத்தின் சொந்த அறிவிப்புப் பலகைகள் ஒவ்வொரு வகையின் ஊட்டச்சத்து அம்சத்தையும் எடுத்துக்காட்டுகின்றன: அமெரிக்கன் ஆய்ஸ்டர், பிங்க் ஆய்ஸ்டர், மற்றும் அபலோன் காளான்கள் ஒவ்வொன்றும் புரதம், வைட்டமின்கள், மற்றும் ஆன்டிஆக்ஸிடன்ட்களால் நிறைந்தவை என விவரிக்கப்படுகின்றன, நோய் எதிர்ப்பு சக்தி, இதயம் மற்றும் செரிமான ஆரோக்கியத்திற்கு உதவுதல், அழற்சியைக் குறைத்தல், மற்றும் கொலஸ்ட்ரால் அளவைக் குறைக்க உதவுதல் ஆகியவற்றை மையமாகக் கொண்டு.",
      ],
      duration: "5 நிமிடங்கள்",
    },
  },
  it: {
    ...
    withering: {
      name: "Appassimento",
      sectionHeadings: ["Caricamento delle Vasche", "Il Ciclo di 12 Ore", "Perdita di Umidità", "Perché l'Appassimento è Importante"],
      sectionBodies: [
        "La foglia appena arrivata dal campo viene prima pesata per registrare il raccolto del giorno. Viene poi stesa in strati sottili su lunghe vasche larghe 85 piedi e profonde 6 piedi. Ciascuna vasca è dotata di una ventola da 46 pollici che gira a 960 giri al minuto, spingendo l'aria attraverso la foglia ed eliminando gradualmente l'umidità: è ciò che la tenuta chiama il processo di appassimento 'moderno'. Con un carico di circa 2,5 kg per piede quadrato, una vasca contiene circa 1.250 kg di foglia verde.",
        "La foglia resta nella vasca per 12 ore ed è girata a mano ogni 4 ore. Durante quel ciclo riceve calore dal basso per 3-4 ore, e viene smossa un'ultima volta prima di essere scaricata al mattino presto. La foglia appassita viene pesata di nuovo prima di passare all'arrotolamento.",
        "Da ogni 100 kg di foglia verde vengono eliminati circa 43 kg di acqua, una misura verificata testando 2,5 kg di foglia in una scatola di un piede quadrato. La foglia resta così con un'umidità del 57-58%, diventando flessibile e malleabile e pronta per l'arrotolamento senza rompersi.",
        "L'appassimento uniforma l'umidità in tutta la foglia e permette agli enzimi naturali di avviare i cambiamenti chimici che in seguito creeranno l'aroma e il colore del tè.",
      ],
      duration: "Questa fase dura per intere 12 ore, con la foglia girata ogni 4 ore.",
    },
    rolling: {
      name: "Arrotolamento",
      sectionHeadings: ["Torcere e Rompere la Foglia", "Il Primo Arrotolamento", "Il Secondo Arrotolamento", "Perché l'Arrotolamento è Importante"],
      sectionBodies: [
        "Le foglie appassite passano attraverso macchine arrotolatrici che torcono e rompono le pareti cellulari della foglia, rilasciando i succhi e gli oli racchiusi al loro interno e riducendola in piccole particelle. È qui che si formano la forma della foglia e la forza del tè.",
        "Due rulli di carico ricevono ciascuno 280 kg di foglia appassita. Il primo arrotolamento dura 20 minuti a 30-32 giri al minuto. Le 'doolie' risultanti (particelle di foglia arrotolata) passano poi su un nastro trasportatore fino a un separatore dotato di rete metallica, che lascia cadere le particelle più piccole come 'primo dool', inviato alla fermentazione.",
        "La foglia che non passa risale il nastro fino a due rulli secondari, con il carico ridotto del 12-14% rispetto ai 280 kg originali. Qui avviene un secondo arrotolamento di altri 20 minuti a 32 giri al minuto, dopo il quale il 'secondo dool' va direttamente alla fermentazione senza essere setacciato.",
      ...
      duration: "Due arrotolamenti da 20 minuti ciascuno (40 minuti totali): il primo a 30-32 giri al minuto, il secondo a 32 giri al minuto.",
    },
    fermentation: {
      name: "Fermentazione (Ossidazione)",
      sectionHeadings: ["Ossidare la Foglia", "Mantenere Pulita la Stanza", "Dal Verde al Ramato", "La Giusta Tempistica", "Tè Nero, Verde, Bianco e Oolong"],
      sectionBodies: [
        "La foglia arrotolata, sia il primo che il secondo dool, viene stesa in una stanza fresca e umida mantenuta a circa 25-30°C e lasciata ossidare per 2,5 ore, con un umidificatore che mantiene l'aria abbastanza umida perché la reazione avvenga.",
        "La stanza deve rimanere assolutamente pulita, priva di batteri, acqua stagnante o foglie vecchie, poiché i batteri arrestano l'attività enzimatica e rovinerebbero il colore, la forza e la qualità del liquore.",
        "Nel corso di quelle 2,5 ore la foglia verde diventa gradualmente di un colore bruno ramato, sviluppando pienamente il colore, il corpo e l'aroma associati al tè nero. Il cambiamento avviene interamente grazie all'ossidazione naturale, senza coloranti o additivi.",
        "Questo è il passaggio più importante per il sapore. Il tempo deve essere valutato con precisione: troppo poco e il tè risulta debole, troppo e diventa piatto e opaco.",
        "È anche ciò che distingue i tipi di tè prodotti qui. Il tè nero segue appassimento, arrotolamento, fermentazione e poi tostatura. Il tè verde salta completamente la fermentazione, passando solo per appassimento, arrotolamento ed essiccazione, ed è più delicato: viene bevuto soprattutto per il suo carattere più mite e quasi medicinale. Il tè bianco salta sia l'appassimento sia la fermentazione, passando direttamente dalla foglia fresca alla cottura a vapore e all'essiccazione. La fabbrica non produce tè Oolong, che la guida descrive come uno stile fermentato simile al tè nero, ottenuto con ripetute cotture a vapore e spazzolatura a mano anziché con l'arrotolamento meccanico.",
      ],
      duration: "La fermentazione dura 2,5 ore sia per il primo che per il secondo dool, prima che la foglia passi all'essiccazione.",
    },
    drying: {
      name: "Essiccazione (Tostatura)",
      sectionHeadings: ["Nell'Essiccatore", "Sei Sezioni, 21 Minuti", "Trovare il Giusto Equilibrio", "Il Risultato", "Preservare la Freschezza", "Una Catena di Condizioni"],
      sectionBodies: [
        "Prima di entrare nell'essiccatore, la foglia fermentata viene smossa. Passa poi attraverso essiccatori ad aria calda, con calore generato da legna da ardere e aspirato da una ventola principale, che mantengono una temperatura di ingresso tra 120°C e 130°C. Questo arresta istantaneamente l'ossidazione e fissa il carattere del tè.",
        "All'interno, l'essiccatore è diviso in 6 sezioni. La foglia cade dal vassoio superiore attraverso queste sezioni fino ai vassoi inferiori, rigirandosi avanti e indietro mentre avanza, ed esce dopo esattamente 21 minuti. A quel punto la temperatura in uscita dovrebbe essere di circa 80°C (all'incirca 140-150°F).",
        "È questa combinazione, 120-130°C in ingresso, 80°C in uscita e 21 minuti, che l'essiccatore è progettato per mantenere, ed è ciò che fa diventare la foglia correttamente nera. Se la temperatura di ingresso scende sotto i 120°C, la foglia esce poco essiccata e brunastra, anche assottigliando lo strato di foglia per far salire quella di uscita a 100°C. Se invece la temperatura di ingresso sale a 150°C mentre quella di uscita scende a 75°C, la foglia risulta molliccia e stufata.",
        "Ciò che esce con le impostazioni corrette è la nota foglia di tè nera e croccante. I visitatori notano subito il calore: è una delle zone più calde di tutto il reparto produttivo.",
        "La tostatura ferma i cambiamenti chimici nel momento esatto e riduce l'umidità da circa il 45-50% a circa il 2-3%, il che è essenziale affinché il tè rimanga fresco durante la conservazione e la spedizione.",
        "Il peso e la qualità finali del tè, però, non dipendono solo dall'essiccatore. Derivano da un'intera catena di condizioni: foglia verde di buona qualità, appassimento corretto, lavorazione corretta, fermentazione riuscita (il passaggio più cruciale di tutti) e, infine, una lavorazione corretta nell'essiccatore.",
      ],
      duration: "La foglia attraversa l'essiccatore in esattamente 21 minuti, secondo una rotazione di temperature di ingresso e uscita mantenuta con precisione.",
    },
    sorting: {
      ...
      sectionHeadings: [
        ...
        "Marchi di Mercato e Nomi delle Qualità",
        "Resa e Rendimento",
      ],
      sectionBodies: [
        ...
        "Quel macchinario di classificazione include un separatore di colore con un sistema a telecamera in grado di distinguere particelle come foglia e fiore, filtrando tutto ciò che non appartiene. Poiché la produzione dello Sri Lanka è incentrata sul tè nero, questo passaggio elimina eventuali particelle brune non completamente fermentate e frammenti di stelo rimanenti. Setacci di diverse dimensioni di maglia, misurate in numero di fori quadrati per pollice, da un minimo di 3 fori per pollice fino a un massimo di 60, separano ulteriormente la foglia in circa 21 qualità distinte per dimensione delle particelle, in più passaggi. Un separatore elettrostatico dà una carica statica sui rulli ai frammenti di stelo bruno rimanenti così da poterli scartare, e macchine dedicate rimuovono completamente il tè di scarto dal lotto. Nessuno di questo colore o carattere deriva da coloranti o additivi, solo dalla lavorazione naturale.",
        ...
        "Oltre alla posizione sul germoglio e alla dimensione della maglia, il tè selezionato viene classificato anche in tre categorie di mercato: First Mark, Second Mark e Third Mark. Le qualità principali prodotte qui includono OPA, OP, OP1, BOP1, FBOP, FF1, FF, Pekoe, Pekoe1, BOP Special, BOPA, FF Extra Special, Extra Special 1 e FF Special, mentre il tè di qualità inferiore rientra in categorie come 1A, BM, BP, FNGS e BT.",
        "In media, 100 kg di foglia verde producono circa 21,5 kg di tè nero finito e confezionato, una resa di circa il 21,5%, con il resto perso per evaporazione dell'umidità e per lo scarto rimosso durante la selezione. Le linee guida standard del Tea Board indicano che servono circa 4,65 kg di foglia verde per ogni kg di tè lavorato. La fabbrica punta ad almeno il 65% della produzione nelle qualità principali, mantiene il tè di qualità inferiore entro il 20%, e il resto rientra nel Second Mark e in categorie simili.",
      ],
    ...
    packing: {
      ...
      sectionHeadings: ["Pesatura, Assaggio e Sigillatura", "Le Qualità sul Piano di Confezionamento", "Il Peso dei Sacchi per Qualità", "Dalla Fabbrica all'Asta", "Perché la Sigillatura Ermetica è Importante", "Abbinare le Qualità ai Mercati"],
      sectionBodies: [
        "Il tè classificato viene pesato, assaggiato per la qualità, e sigillato in confezioni a prova di umidità rivestite di foglio di alluminio, stampate con il nome e il logo della fabbrica, il suo numero di registrazione, il Simbolo del Tè di Ceylon e il certificato di sicurezza alimentare.",
        "Nell'area di confezionamento diverse qualità finite si trovano fianco a fianco: Dust (destinato per lo più alle bustine), OP1, Pekoe, BOP (Broken Orange Pekoe) e OP (Orange Pekoe). OP1 e OP sono particolarmente popolari nei mercati di lingua araba, e il Pekoe è ben conosciuto in Russia e Iran. La qualità più rara e preziosa prodotta qui è la SFOPE1, una qualità 'Flowery Tippy' chiamata così per i pallidi frammenti argentati che contiene, provenienti dalla parte più alta del germoglio. È apprezzata più per il suo carattere delicato che per la forza.",
        "Anche il peso dei sacchi varia a seconda della qualità: un sacco di dimensioni fisse contiene molto più di una qualità fine e densa come il Dust (circa 58kg) rispetto a una qualità più voluminosa e fogliosa come l'OPE (circa 30kg).",
        "Per lo più, la fabbrica consegna il suo tè finito ai broker di tè di Colombo, tra cui nomi come Forbes & Walker, Akbar Brothers e Finlays, che lo portano all'Asta del Tè di Colombo, dove gli acquirenti lo comprano e spesso lo esportano sotto il proprio marchio dopo averlo ulteriormente valorizzato. Esiste anche l'esportazione diretta sotto il nome Athukorala stesso, ma soprattutto per ordini speciali occasionali di acquirenti esteri.",
        "Il confezionamento ermetico protegge il tè dall'umidità e dagli odori, preservando il gusto costruito con cura in ogni fase precedente del processo.",
        "Paesi diversi vogliono cose diverse dal loro tè. Alcuni preferiscono una tazza forte e astringente, altri no, quindi quale parte del germoglio dà origine a una qualità è di fatto abbinato al palato di ogni mercato di esportazione. I mercati di lingua araba, ad esempio, richiedono specificamente qualità che non fanno parte delle abitudini quotidiane di consumo nei loro paesi. Vendere principalmente tramite broker anziché esportare direttamente significa che gli acquirenti esterni gestiscono di solito la spedizione internazionale successiva e il branding.",
      ],
    ...
    "ginger-turmeric": {
      ...
      sectionBodies: [
        "Lo zenzero (Zingiber officinale) è un'erba perenne della famiglia delle Zingiberacee. Viene coltivato diffusamente nelle regioni tropicali e subtropicali, tra cui Sri Lanka e India. Ciò che conta di più è il rizoma sotterraneo, usato come spezia e medicina da secoli. Lo zenzero prospera in climi caldi e umidi con terreno ricco e ben drenato. Svolge inoltre un ruolo centrale nella cucina tradizionale e nella medicina erboristica di molte culture.",
    ...
    "artisanal-tea-grades": {
      ...
      sectionHeadings: [
        ...
        "White Tea e Purple Tea",
        "Pink Tea e Vita Glow",
        "Miscele e Rarità",
        "Flower Chakra e Tea Coin",
      ],
      sectionBodies: [
        "Oltre al tè nero standard, la tenuta produce anche rari tè artigianali lavorati a mano. Questi vengono per lo più rifiniti a mano nella Fabbrica del Tè Fatto a Mano separata, non sulla linea principale del tè nero. Ecco perché queste qualità sono chiamate anche 'Tè Artigianali': è più vicino a un'arte che a una produzione di massa. Molti di essi provengono da un germoglio color viola distintivo, il TRI 2043, un clone del Tea Research Institute il cui colore deriva dall'antociano. È una cultivar a basso rendimento, motivo per cui non tutte le tenute dello Sri Lanka la coltivano.",
        "Il Golden Tip usa la gemma di quel germoglio ormai matura, mentre il Silver Tip usa la gemma più chiara e ancora immatura dello stesso germoglio, i cui sottili peli argentati danno alla qualità il suo nome e una lieve luccichio. Solo la gemma viene pesata, appassita per qualche ora ed essiccata: un processo più semplice rispetto alle qualità completamente fermentate, che dà una tazza delicatamente dolce, floreale, dalla consistenza vellutata.",
        "Il White Tea prende due foglie e mezzo dal germoglio e salta completamente l'appassimento: viene cotto a vapore ed essiccato anziché fermentato e arrotolato come il tè nero. Il risultato è un gusto naturalmente dolce e molto leggermente lavorato, con un liquore chiaro, una consistenza vellutata, un aroma floreale e una naturale ricchezza di antiossidanti. Il Purple Tea usa la stessa raccolta ma, a differenza del White Tea, attraversa l'intero processo del tè nero: appassimento, arrotolamento e fermentazione. Il risultato è un gusto più vicino all'oolong: più leggero del tè nero ma meno vegetale del tè verde, relativamente povero di caffeina e naturalmente ricco di antiossidanti.",
        "Il Pink Tea parte praticamente incolore come il White Tea, e diventa rosa solo dopo l'aggiunta di un paio di gocce di lime. L'agrume innesca un cambiamento di colore nello stesso pigmento antociano che dà al germoglio la sua tonalità. Si abbina bene anche a un paio di gocce di miele, si consiglia servito come tè freddo, ed è naturalmente ricco di antiossidanti anch'esso. Il Vita Glow viene invece prodotto dalla gemma del germoglio verde ordinario anziché da quello viola.",
        "La tenuta crea anche miscele a valore aggiunto, come il tè infuso con fiori di loto, oltre al Blooming Tea legato a mano dagli stessi germogli. Questi pregiati tè artigianali vengono raccolti e lavorati in quantità molto più ridotte rispetto al tè nero standard, ed è proprio questo a renderli più rari e pregiati. Chiedete al banco di degustazione per assaggiarli. Per la maggior parte di queste qualità si può usare sia il germoglio dalle sfumature viola sia il normale germoglio verde, anche se la tenuta utilizza per lo più quello viola.",
        "Gli articoli più laboriosi prodotti qui sono due specialità in piccoli lotti: il Flower Chakra, un anello di tè nero arrotolato a mano che usa circa due germogli ciascuno, e il Tea Coin. Una sola persona può arrotolare a mano solo circa 25 grammi al giorno di uno dei due, motivo per cui sono così costosi e vengono acquistati per lo più come souvenir da appassionati di tè facoltosi piuttosto che per il consumo quotidiano. Gran parte della produzione fatta a mano, compreso il Purple Tea, viene realizzata su ordinazione in piccoli lotti anziché prodotta continuamente in grandi quantità.",
      ],
      duration: "5 minuti",
    },
    "pahatharata-manufacturing": {
      name: "Lavorazione del Tè (Pahatharata)",
      sectionHeadings: ["Lavorazione Bassa Quota vs. Alta Quota", "CTC: Crush-Tear-Curl"],
      sectionBodies: [
        "Le tenute delle pianure ('Pahatharata') come questa lavorano il tè in modo diverso rispetto alle tenute d'alta quota più nell'entroterra. Il processo di lavorazione della tenuta stessa, illustrato fase per fase in precedenza in questo tour, lo conferma: qui l'arrotolamento riduce la foglia in piccole particelle dense anziché arrotolarla in foglie intere in stile ortodosso, il tratto distintivo della lavorazione CTC (Crush-Tear-Curl). Questa tappa è un rapido riepilogo di cosa significhi questo approccio delle pianure per la raccolta, la lavorazione e la classificazione.",
        "Il CTC (Crush-Tear-Curl) lavora meccanicamente la foglia in granuli piccoli e densi, producendo il carattere forte e vivace per cui è noto il tè di Ceylon delle pianure. Sono le stesse particelle di foglia viste prima alle tappe di arrotolamento e selezione di questo tour.",
      ],
    ...
    "clonal-tea": {
      name: "Tè Clonale",
      heroTagline: "Alcune delle piante di prova per varietà clonali, segnalate dalla tenuta stessa.",
      sectionHeadings: ["Tè Clonale", "Perché il Tè Clonale Costa di Più", "Cultivar di Tè", "TRI 3069", "TRI 3055", "TRI 4053", "TRI 2022", "TRI 2043", "TRI 4006", "TRI 3025"],
      sectionBodies: ["Alcuni dei tè più ricercati provengono da piante clonali: talee propagate da un unico arbusto madre eccezionale anziché coltivate da seme. Questa tappa spiega come viene propagato il tè clonale e perché può raggiungere un prezzo elevato. Il Tea Research Institute (TRI) seleziona varietà clonali per esigenze diverse. Alcune per la tolleranza alla siccità o alle piogge e per la resa elevata, altre per le caratteristiche gemme dalla punta rossa o verde apprezzate nei tè artigianali. Le varietà ad alta resa ampiamente coltivate, come TRI 2023, TRI 2025 e TRI 2026, sono comuni sia nelle piantagioni che negli orti domestici, mentre le varietà speciali più rare vengono coltivate soprattutto nelle grandi piantagioni per il tè lavorato a mano, anziché per la coltivazione quotidiana. Nonostante sembrino anni, questi numeri TRI sono semplicemente identificativi assegnati, non l'anno in cui ciascuna varietà è stata rilasciata.", "La propagazione clonale preserva le caratteristiche esatte di una pianta pregiata. I migliori tè clonali sono spesso prodotti in quantità molto limitate. Le gemme delle varietà speciali dalla punta rossa o verde vengono raccolte a mano per qualità rare come i Golden Tips e i Silver Tips, e le piantagioni pagano in genere circa 100 rupie in più al chilo rispetto alle foglie raccolte normalmente.", "La tenuta privilegia cloni ad alta resa e resistenti ai parassiti selezionati dal Tea Research Institute (TRI), comprese varietà della serie TRI 2000 e T65, per l'affidabilità con cui si comportano nel suolo e nel clima locali. TRI 4049 è una cultivar di tè ad alta resa sviluppata dal Tea Research Institute dello Sri Lanka, ideale per altitudini medio-basse. È una varietà di Camellia sinensis propagata vegetativamente, nota per la sua crescita vigorosa, le foglie verde scuro e la resistenza alle malattie comuni. TRI 4049 produce infusi brillanti e aromatici, adatti sia al tè CTC che a quello ortodosso, il che lo rende una scelta popolare per il reimpianto e la coltivazione commerciale.", "Qualche fila più in là, questa pianta contrassegnata è il TRI 3069. È un'altra cultivar registrata presso l'Istituto di Ricerca sul Tè, coltivata accanto al TRI 4049 in modo che il vivaio possa confrontare come si comporta ciascuna su questo particolare terreno prima di piantarle su larga scala.", "Il TRI 3055 si trova nelle stesse file di prova. Ogni appezzamento di cultivar qui è etichettato e monitorato singolarmente, ed è così che la fabbrica costruisce le proprie prove su quali varietà valga davvero la pena espandere, invece di affidarsi solo alla reputazione.", "Il TRI 4053 completa gli appezzamenti di prova delle cultivar mostrati qui. Tenere diverse varietà registrate a crescere fianco a fianco, ciascuna chiaramente etichettata, è prassi comune nella tenuta prima di destinare una cultivar a un intero campo.", "Il TRI 2022 è piantato nella stessa sezione di prova delle cultivar, con il suo cartello etichettato e monitorato come quello dei suoi vicini. Coltivare diverse varietà registrate fianco a fianco sullo stesso terreno è il modo in cui il vivaio raccoglie prove reali su quali rendano meglio qui.", "Il TRI 2043 si trova un po' più avanti nella fila. Come le altre piante numerate in questo appezzamento di prova, viene coltivato e monitorato prima che venga presa qualsiasi decisione sulla sua piantumazione su larga scala.", "Il TRI 4006 è un'altra voce nella stessa prova di cultivar. Il vivaio mantiene ogni varietà nel proprio punto chiaramente contrassegnato, il che rende facile monitorare e confrontare la crescita nel tempo.", "Il TRI 3025 segna il bordo dell'appezzamento di prova, accanto a un cartello bilingue 'Cultivar di Tè' e una targa di piantumazione datata. Insieme, le piante etichettate qui costituiscono il registro concreto del vivaio su quali cultivar valga la pena espandere."],
      duration: "5-10 minuti",
    },
    "shade-management": {
    ...
    "handmade-factory": {
      ...
      heroTagline: "Uno scaffale a vassoi della Fabbrica di Tè Fatto a Mano della tenuta stessa.",
      sectionHeadings: [
        "Fabbrica di Tè Artigianale",
        "Appassimento e Arrotolamento",
        "Tè Verde, Essiccazione e l'Edificio",
        "Cosa Rende Diverso il Tè Fatto a Mano",
      ],
      sectionBodies: [
        "Oltre alla produzione meccanizzata, parte del tè viene lavorato interamente a mano, in ogni fase, senza macchine. La maggior parte delle qualità artigianali speciali della tenuta, tra cui Golden Tip, Silver Tip, White Tea, Purple Tea, Pink Tea, Vita Glow, il Blooming Tea legato a mano, e miscele infuse di fiori come il tè al loto, vengono rifinite qui anziché nella fabbrica principale del tè nero. (Il Vita Glow è fatto dalla gemma del germoglio verde, a differenza del germoglio viola di Golden e Silver Tip.)",
        "Qui l'appassimento avviene in modo naturale, in una piccola vasca monitorata con un igrometro, senza le ventole ad aria forzata della fabbrica principale, portando l'umidità intorno al 50% prima dell'arrotolamento. La foglia viene poi arrotolata a mano su un vassoio, giudicata al tatto più che con l'orologio. Per gli ordini all'ingrosso più grandi, piccoli rulli meccanici subentrano per lo stesso scopo, dato che l'arrotolamento a mano da solo non riesce a tenere il passo con i volumi.",
        "Una macchina dedicata al tè verde, che cuoce a vapore e pressa la foglia, gestisce quella qualità separatamente, anche se al momento è fuori servizio per una piccola riparazione. Una volta cotta a vapore, la foglia riposa all'esterno per perdere l'umidità superficiale, poi viene arrotolata a mano per farne uscire colore, forza, aroma e gusto, prima di un ultimo passaggio nell'essiccatore. Il tempo di essiccazione varia da un piccolo produttore all'altro. Il piccolo forno dell'edificio funge anche da forno per snack a base di tè come i biscotti. L'essiccazione, fatta subito dopo l'arrotolamento, avviene qui su due essiccatori separati, uno tradizionale a legna, uno elettrico, ciascuno con 34 vassoi, entrambi usati anche per altri prodotti in piccoli lotti, dal cardamomo ai chiodi di garofano, allo zenzero.",
        "Questa tappa spiega cosa distingue il tè fatto a mano dalla produzione di linea in fabbrica. L'arrotolamento e la lavorazione a mano producono lotti più piccoli con più variazione tra le foglie, spesso apprezzati per il loro carattere distintivo. I tè fatti a mano rientrano in due grandi categorie. La prima comprende quelli fermentati in stile tè nero (riconoscibili dal colore nero), che includono il Purple Tea nonostante il suo germoglio d'origine insolito. La seconda comprende quelli non fermentati come il White Tea, che saltano completamente la fermentazione e vanno direttamente all'essiccazione.",
      ],
      duration: "10 minuti",
    },
    "tea-center": {
      name: "Centro del Tè",
      heroTagline: "Visitatori che degustano il tè al banco di degustazione della tenuta.",
      sectionHeadings: [
        "Centro del Tè",
        "Preparazione e Degustazione",
        "Qualità Quotidiane e Tè da Supermercato",
        "Le Qualità più Pregiate e i Mercati di Esportazione",
        "Miscele di Erbe e Spezie",
      ],
      sectionBodies: [
        "Il tour si conclude al centro del tè, dove un panel di degustazione controlla ogni lotto, non solo qui ma anche prima, ogni volta che il tè lascia la fabbrica principale per il mercato locale o arriva per il confezionamento, alla ricerca di difetti come sovra-fermentazione, sotto-fermentazione o bruciature. Scarta tutto ciò che non supera il controllo prima della vendita e annota anche i problemi minori per informare la produzione futura. Qui sono esposte altre due specialità in piccoli lotti della fabbrica artigianale: il Flower Chakra e il Tea Coin.",
        "Questi tè artigianali possono essere re-infusi fino a tre volte usando acqua riscaldata a circa 80°C anziché bollente. Ogni infusione dura circa 2, 5 e 8 minuti, con acqua calda fresca ogni volta, a differenza del tè nero standard, che si prepara una sola volta a piena ebollizione. La degustazione segue un metodo simile a quello del vino: prima si annusa, poi si sorseggia rumorosamente per far passare il tè sulle papille gustative ai lati della lingua con un sorso d'aria. Il panel sputa anziché deglutire mentre assaggia fino a 50 tè al giorno, anche se i visitatori sono invitati a bere davvero ciò che assaggiano. Il tè nero preparato correttamente richiede circa 2,5-3 minuti di infusione coperta, non una semplice immersione veloce.",
        "Il BOPF e il BOPF Special sono le qualità che la maggior parte degli sri-lankesi beve quotidianamente. Il Dust e il Dust 1, le qualità più basse, non vengono mai inviate ai mercati di esportazione, e sono ciò che tipicamente finisce, con l'aggiunta di colorante, nelle bustine di tè economiche del supermercato: ecco perché il colore appare non appena vengono immerse, invece che dopo un'infusione corretta. Gran parte di ciò che si vende nei supermercati è anche mescolato con tè più economico acquistato altrove, a differenza dei tè non miscelati e a origine unica della tenuta.",
        "Le qualità più pregiate, cioè FF Extra Special, FF Special, FBOP, provengono solo dall'1% circa del raccolto, la parte più immatura della gemma, e si vendono a circa 25 dollari per 100g. Qualità più abbondanti come OPA e OP costano circa 5-6 dollari per 200g. Le particelle bianche visibili nelle qualità più pregiate aggiungono dolcezza extra e vanno per lo più agli acquirenti europei, mentre i mercati arabi tendono a preferire una tazza più forte.",
    ...
    "jaggery-palm": {
      ...
      sectionBodies: [
        "La palma kithul (Caryota urens), o palma dalle foglie a coda di pesce, è una palma alta ed elegante presente in Sri Lanka, India e altri paesi tropicali asiatici, apprezzata per i suoi molti usi. Il suo prodotto più noto è la linfa dolce raccolta dagli steli fiorali, fatta bollire per produrre il tradizionale sciroppo e jaggery di kithul. L'albero è noto anche per il suo tronco robusto e fibroso e per le sue belle foglie a forma di coda di pesce.",
        "Uno sguardo rapido alla palma stessa:",
    ...
    pruning: {
      ...
      sectionBodies: ["Prima della potatura, la tenuta lascia deliberatamente crescere la pianta di tè per un periodo anziché tagliarla subito. Questo lascia che la nuova crescita si sviluppi completamente, così la pianta accumula nutrimento. Solo una volta completato questo accumulo inizia la potatura, che taglia la pianta fino a un'altezza di circa il ginocchio. I cartelli in loco indicano questo come due fasi denominate: il primo taglio incrociato (Palamu Haras Kapuma) porta il cespuglio a circa 9-12 pollici (22-30 cm), e il secondo taglio incrociato (Dewana Haras Kapuma) lo riporta a circa 16-18 pollici (40-45 cm). Lasciare prima crescere la pianta, anziché potarla secondo un programma fisso indipendentemente dalle condizioni, dà alla pianta il tempo di convogliare il nutrimento verso l'alto prima di essere tagliata."],
      duration: "5 minuti",
    },
    polytunnel: {
      name: "Serra e Tunnel Coltivazione",
      heroTagline: "Il tunnel di coltivazione della tenuta, già al lavoro con le sue prime colture.",
      sectionHeadings: ["Informazioni sulla Serra e sul Tunnel di Coltivazione", "Irrigazione", "Tempi di Raccolta", "Testa di Controllo Fertirrigazione", "Malu Miris (Peperone)", "Amu Miris (Peperoncino)", "Nai Miris", "Gedi Gowa (Cavolo)", "Athu Gowa (Cavolo Rosso)", "Bonchi (Fagiolini)", "Raabu (Ravanello)", "Carota", "Porri", "Cetriolo", "Piantina di Pomodoro"],
      sectionBodies: [
        "Accanto a una serra già esistente, la tenuta sta costruendo un tunnel per coltivare una varietà di ortaggi propri. Tra questi ci sono peperoncini Scotch Bonnet e peperoncini verdi, peperoni, cetriolini, pomodori, peperoni dolci, cavoli, ravanelli, fagiolini, porri, cipollotti e carote. Una volta completato, gli ospiti del villa o della cabana potranno raccogliere di persona, freschi, gli ortaggi coltivati proprio qui. La copertura semitrasparente di un tunnel di coltivazione funziona un po' come il vetro di una serra, trattenendo il calore e lasciando passare la luce mentre tiene fuori le piogge monsoniche intense e molti parassiti comuni. Queste condizioni permettono alla tenuta di coltivare ortaggi che altrimenti non riuscirebbe a gestire in modo affidabile in pieno campo, ed estendere la stagione di coltivazione oltre quanto consentirebbero le aiuole all'aperto.",
        "Ogni coltura segue il proprio programma di irrigazione, alternando l'irrigazione a goccia a livello delle radici con la nebulizzazione dall'alto a seconda della fase di crescita. La nebulizzazione aiuta soprattutto la germinazione precoce e il raffreddamento per colture come carote e ravanelli, mentre l'irrigazione a goccia diventa il metodo principale una volta iniziata la fruttificazione per colture come il pomodoro. I due metodi servono a scopi diversi. La nebulizzazione mantiene l'aria e la superficie del terreno abbastanza umide da far attecchire semi giovani e piantine, mentre l'irrigazione a goccia porta l'acqua direttamente alla zona radicale una volta che le piante sono più grandi, usando meno acqua in totale e mantenendo le foglie asciutte per ridurre il rischio di malattie fungine.",
        "Il tempo per il primo raccolto varia molto da coltura a coltura, da appena 25-35 giorni per i ravanelli fino a 90-120 giorni per i porri, quindi il tunnel dovrebbe avere sempre qualcosa pronto da raccogliere in quasi ogni momento della stagione. Coltivare fianco a fianco una gamma così ampia di ortaggi a maturazione rapida e lenta è di per sé una scelta deliberata, poiché scagliona naturalmente i raccolti invece di far sì che tutto sia pronto, o finito, nello stesso momento.",
        "Questa testa di controllo alimenta le linee a goccia che arrivano a ogni sacco di coltivazione. Comprende un filtro per trattenere i sedimenti, un manometro, valvole di intercettazione per isolare le sezioni di tubo, e due serbatoi di fertilizzante dotati di un iniettore venturi che dosa il fertilizzante liquido direttamente nell'acqua di irrigazione mentre la attraversa, così ogni pianta viene irrigata e nutrita nello stesso passaggio.",
        "Con le sue foglie ovali lucide e appuntite disposte in coppie opposte, questa piantina è Malu Miris, un peperone (Capsicum annuum). È coltivato qui in un sacco di coltivazione riempito di fibra di cocco, con un tubicino a goccia già portato alla sua base.",
        "Questa piantina è Amu Miris, un peperoncino (Capsicum annuum). In questa fase iniziale le sue foglie sono quasi indistinguibili da quelle del peperone, poiché entrambi appartengono alla stessa specie coltivata per usi diversi.",
        "Il nome Nai Miris significa 'peperoncino cobra' in singalese. È Capsicum chinense, una specie diversa e generalmente più piccante degli altri peperoncini della tenuta, imparentata con varietà come l'habanero e lo Scotch Bonnet.",
        "Le foglie più tonde e dall'aspetto leggermente ceroso di questa piantina appartengono a Gedi Gowa, che significa letteralmente 'cavolo a palla': è il comune cavolo a testa (Brassica oleracea var. capitata), messo a dimora qui molto prima che inizi a formarsi la testa.",
        "Le foglie rosso-violacee di questa piantina appartengono ad Athu Gowa, che significa letteralmente 'cavolo a ramo': è una varietà di cavolo più sciolta e priva di testa (Brassica oleracea var. acephala), anziché del tipo a palla.",
        "Queste foglie larghe, a forma di cuore, appartengono a Bonchi, fagiolini (Phaseolus vulgaris). Una volta avviata è una coltura rampicante, con un portamento ben diverso dagli ortaggi bassi che la circondano.",
        "Questa piccola piantina, ancora allo stadio di cotiledone, è Raabu, ravanello (Raphanus sativus). È una delle colture più rapide del tunnel a raggiungere il raccolto, pronta in appena 25-35 giorni.",
        "La carota (Daucus carota subsp. sativus) si riconosce già a questo stadio dalle sue prime foglie sottili, simili a fili d'erba, ben diverse dalle larghe foglie cotiledonari degli altri ortaggi coltivati qui.",
        "I porri (Allium ampeloprasum) emergono come una singola lama sottile, tra le colture più lente del tunnel a raggiungere il raccolto. Può richiedere fino a 90-120 giorni dalla semina.",
        "Le due larghe foglie cotiledonari tonde di questa piantina sono quelle di un cetriolo da insalata (Cucumis sativus), prima che compaiano le sue foglie vere, più simili a un cuore.",
        "Questa piantina di pomodoro mostra già le foglie lobate, dentate e leggermente pelose tipiche della pianta. Il pomodoro è la coltura citata in precedenza come esempio di quelle che passano dalla nebulizzazione all'irrigazione a goccia una volta iniziata la fruttificazione.",
      ],
      duration: "5 minuti",
    },
    "mushroom-hut": {
      name: "Capanna dei Funghi",
      sectionHeadings: ["Un Giardino Diverso", "Le Varietà di Funghi Ostrica", "Substrato e Micelio", "Ciclo di Raccolta", "Nutrizione e Benefici per la Salute"],
      sectionBodies: [
        "Accanto alle sue colture di tè, spezie e frutta, la tenuta mantiene una capanna dedicata ai funghi. È una piccola struttura ombreggiata costruita per mantenere le condizioni buie, fresche e umide di cui i funghi hanno bisogno per fruttificare, anziché il sole e la circolazione d'aria per cui vengono coltivati il resto dei giardini.",
        "La capanna è allestita per coltivare diverse varietà di funghi ostrica: l'Ostrica Americana (Pleurotus pulmonarius), l'Ostrica del Bhutan, l'Ostrica Nera, l'Ostrica Rosa (Pleurotus djamor), e il Fungo Abalone (Pleurotus cystidiosus). Tutti sono coltivati su un substrato di paglia o segatura anziché sul terreno. Appartengono tutti al genere Pleurotus, riconoscibili dai loro cappelli a forma di ventaglio o di ostrica che crescono in mensole fitte e sovrapposte direttamente dal substrato anziché su steli singoli. Il colore è il principale elemento che distingue una varietà dall'altra, dal bianco e grigio al rosa, all'oro, o quasi al nero, mentre il sapore delicato e leggermente sapido resta pressoché lo stesso in tutte.",
        "La coltivazione parte dal substrato stesso: paglia o segatura, lo stesso materiale alla base dell'approccio economico e basato sugli scarti della tenuta. Questo viene inzuppato e pastorizzato con il calore per eliminare muffe e batteri concorrenti, prima di essere mescolato con il micelio innesto, grano completamente colonizzato dal micelio del fungo, e confezionato in sacchi o blocchi forati. Quei blocchi vengono poi lasciati al buio per un 'periodo di colonizzazione' di due o tre settimane, durante il quale il micelio bianco si diffonde in tutto il substrato. Solo dopo, le condizioni buie, fresche e umide della capanna subentrano per innescare la fruttificazione.",
        "Coltivare funghi in questo modo è un modo economico e sostenibile per una fattoria di diversificarsi oltre la sua coltura principale, spesso utilizzando rifiuti agricoli come la paglia come substrato di crescita. Un solo lotto di substrato produce diversi raccolti nell'arco di diverse settimane. Ogni blocco produce in genere da due a quattro di queste ondate di raccolto, distanziate all'incirca di una o due settimane l'una dall'altra, con la prima ondata la più abbondante e le rese che poi calano gradualmente. I funghi vengono staccati ruotandoli via in grappoli interi anziché tagliati, poiché i monconi rimasti possono marcire e favorire una contaminazione che rovinerebbe l'ondata successiva.",
        "La segnaletica della tenuta stessa evidenzia il lato nutrizionale di ogni varietà: l'Ostrica Americana, l'Ostrica Rosa e il Fungo Abalone sono descritti come ricchi di proteine, vitamine e antiossidanti, con benefici legati al supporto immunitario, alla salute cardiaca e digestiva, alla riduzione dell'infiammazione e all'abbassamento del colesterolo.",
      ],
      duration: "5 minuti",
    },
  },
  ar: {
    ...
    withering: {
      name: "الذبول",
      sectionHeadings: ["تحميل الأحواض", "دورة الـ12 ساعة", "فقدان الرطوبة", "لماذا يُعد الذبول مهمًا"],
      sectionBodies: [
        "تُوزن الأوراق القادمة من الحقل أولاً لتسجيل محصول اليوم. ثم تُفرش بطبقة رقيقة على أحواض طويلة يبلغ طولها 85 قدمًا وعرضها 6 أقدام. كل حوض مزود بمروحة قطرها 46 بوصة تعمل بسرعة 960 دورة في الدقيقة لدفع الهواء عبر الأوراق وإزالة الرطوبة تدريجيًا، وهو ما تسميه المزرعة عملية الذبول 'الحديثة'. وبمعدل تحميل يبلغ نحو 2.5 كجم لكل قدم مربع، يستوعب الحوض الواحد نحو 1,250 كجم من الأوراق الخضراء.",
        "تبقى الأوراق في الحوض لمدة 12 ساعة، وتُقلَب يدويًا كل 4 ساعات. وخلال تلك الدورة تُسخَّن من الأسفل لمدة 3 إلى 4 ساعات، وتُرخى مرة أخيرة قبل تفريغها في الصباح الباكر. تُوزن الأوراق الذابلة مرة أخرى قبل الانتقال إلى مرحلة اللف.",
        "من كل 100 كجم من الأوراق الخضراء، تُزال نحو 43 كجم من الماء، وهو رقم يُتحقق منه باختبار 2.5 كجم من الأوراق في صندوق مساحته قدم مربع واحد. وتبقى الأوراق برطوبة تتراوح بين 57% و58%، فتصبح مرنة وطيّعة وجاهزة للف دون أن تتكسر.",
        "يوازن الذبول الرطوبة عبر الورقة ويسمح للإنزيمات الطبيعية ببدء التغيرات الكيميائية التي ستشكل لاحقًا نكهة الشاي ولونه.",
      ],
      duration: "تستمر هذه المرحلة 12 ساعة كاملة، وتُقلَب الأوراق كل 4 ساعات.",
    },
    rolling: {
      name: "اللف",
      sectionHeadings: ["لَيّ الورقة وكسرها", "اللفة الأولى", "اللفة الثانية", "لماذا يُعد اللف مهمًا"],
      sectionBodies: [
        "تمر الأوراق الذابلة عبر آلات اللف التي تلوي وتكسر جدران خلايا الورقة، مما يحرر العصائر والزيوت المحبوسة بداخلها ويكسر الورقة إلى جزيئات صغيرة. هنا يتشكل شكل الورقة وقوة الشاي.",
        "تستقبل كل من أسطوانتي التغذية 280 كجم من الأوراق الذابلة. تستغرق اللفة الأولى 20 دقيقة بسرعة 30 إلى 32 دورة في الدقيقة. بعدها تنتقل جزيئات الأوراق الملفوفة (الدوليات) الناتجة عبر ناقل إلى كاسر ملفوف بشبكة سلكية، يُسقط أصغر الجزيئات كـ'الدول الأول' الذي يُرسل إلى التخمير.",
        "أما الأوراق التي لا تمر فتصعد عبر الناقل إلى أسطوانتين ثانويتين، بتغذية مخفّضة بنسبة 12% إلى 14% من الـ280 كجم الأصلية. وهنا تجري لفة ثانية تستغرق 20 دقيقة أخرى بسرعة 32 دورة في الدقيقة، وبعدها يذهب 'الدول الثاني' مباشرة إلى التخمير دون غربلة.",
        "كسر جدران الخلايا يعرّض الإنزيمات للأكسجين، مما يبدأ عملية الأكسدة. تحدد شدة ومدة اللف قوة الشاي النهائي ومظهره. أولى هذه المراحل المتعددة هي لفة تمهيدية لطيفة، والغرض منها تغليف الجزيئات الملتوية بعصارة الورقة نفسها قبل اللف الأقوى الذي يليها.",
      ],
      duration: "لفتان مدة كل منهما 20 دقيقة (40 دقيقة إجمالًا): الأولى بسرعة 30 إلى 32 دورة في الدقيقة، والثانية بسرعة 32 دورة في الدقيقة.",
    },
    fermentation: {
      name: "التخمير (الأكسدة)",
      sectionHeadings: ["أكسدة الورقة", "الحفاظ على نظافة الغرفة", "من الأخضر إلى النحاسي", "ضبط التوقيت بدقة", "الشاي الأسود والأخضر والأبيض والأولونغ"],
      sectionBodies: [
        "تُفرش الأوراق الملفوفة، الدول الأول والثاني معًا، في غرفة باردة ورطبة تُحفظ عند درجة حرارة تتراوح بين 25 و30 درجة مئوية، وتُترك لتتأكسد لمدة 2.5 ساعة. ويُستخدم جهاز ترطيب للحفاظ على رطوبة الهواء الكافية لحدوث التفاعل.",
        "ويجب أن تبقى الغرفة نظيفة تمامًا، خالية من البكتيريا والمياه الراكدة والأوراق القديمة، لأن البكتيريا توقف نشاط الإنزيمات وتفسد اللون والقوة وجودة السائل.",
        "وخلال تلك الساعات الـ2.5 تتحول الورقة الخضراء تدريجيًا إلى لون بني نحاسي، مع تطور كامل للون والقوام والنكهة المرتبطة بالشاي الأسود. ويحدث ذلك كله من الأكسدة الطبيعية فقط، دون أي أصباغ أو إضافات.",
        "هذه هي الخطوة الأهم لتحديد النكهة. يجب ضبط التوقيت بدقة: فإذا كان قصيرًا جدًا يصبح الشاي خفيفًا، وإذا طال أكثر من اللازم يصبح باهتًا وباردًا.",
        "وهذا أيضًا ما يفصل بين أنواع الشاي المصنوعة هنا. يمر الشاي الأسود بمراحل الذبول ثم اللف ثم التخمير ثم التحميص. أما الشاي الأخضر فيتجاوز التخمير تمامًا، مارًّا فقط بالذبول واللف والتجفيف، ويكون أكثر اعتدالًا: يُشرب غالبًا لطابعه الألطف وشبه العلاجي. أما الشاي الأبيض فيتجاوز كلًا من الذبول والتخمير، منتقلاً مباشرة من الورقة الطازجة إلى التبخير والتجفيف. لا ينتج هذا المصنع شاي الأولونغ، الذي يصفه المرشد بأنه نوع مخمّر يشبه الشاي الأسود، يُصنع عبر تبخير وتفريش متكررين يدويًا بدلاً من اللف الآلي.",
      ],
      duration: "يستغرق التخمير 2.5 ساعة لكل من الدول الأول والثاني، قبل أن تنتقل الورقة إلى مرحلة التجفيف.",
    },
    drying: {
      name: "التجفيف (التحميص)",
      sectionHeadings: ["دخول المجفف", "6 أقسام و21 دقيقة", "إيجاد التوازن الصحيح", "النتيجة", "الحفاظ على النضارة", "سلسلة من الشروط"],
      sectionBodies: [
        "قبل الدخول إلى المجفف، تُرخى الأوراق المخمّرة. ثم تمر عبر مجففات الهواء الساخن، بحرارة ناتجة عن حرق الحطب تُسحب بواسطة مروحة رئيسية، وتحافظ هذه المجففات على درجة حرارة دخول تتراوح بين 120 و130 درجة مئوية. وهذا يوقف الأكسدة فورًا ويحافظ على طابع الشاي.",
        "وفي الداخل، يُقسَّم المجفف إلى 6 أقسام. تسقط الأوراق من الدرج العلوي عبر هذه الأقسام إلى الأدراج السفلية، وتتقلب إلى الأمام والخلف أثناء تقدمها، وتخرج بعد 21 دقيقة بالضبط. وعندها ينبغي أن تكون درجة حرارة العادم نحو 80 درجة مئوية (حوالي 140 إلى 150 درجة فهرنهايت).",
        "وهذا المزيج، 120-130 درجة مئوية عند الدخول، و80 درجة مئوية عند العادم، و21 دقيقة، هو ما صُمم المجفف للحفاظ عليه، وهو ما يجعل الأوراق تتحول إلى اللون الأسود بشكل صحيح. فإذا انخفضت درجة حرارة الدخول عن 120 درجة مئوية، تخرج الأوراق غير مجففة جيدًا ومائلة إلى البني حتى لو بلغت درجة حرارة العادم 100 درجة مئوية عن طريق تقليل سماكة طبقة الأوراق. وإذا ارتفعت درجة حرارة الدخول إلى 150 درجة مئوية بينما انخفضت درجة حرارة العادم إلى 75 درجة مئوية، تنتهي الأوراق طرية ومسلوقة بدلاً من أن تكون مجففة بشكل صحيح.",
        "وما يخرج عند الإعدادات الصحيحة هو ورقة الشاي السوداء المقرمشة المعروفة. يلاحظ الزوار الحرارة فورًا، فهذه من أكثر المناطق سخونة في أرضية المصنع.",
        "يوقف التحميص التغيرات الكيميائية في اللحظة المناسبة تمامًا ويقلل الرطوبة من حوالي 45-50% إلى حوالي 2-3%، وهو أمر أساسي لبقاء الشاي طازجًا أثناء التخزين والشحن.",
        "ومع ذلك، فإن الوزن والجودة النهائيين للشاي لا يعتمدان على المجفف وحده. بل يعتمدان على سلسلة من الشروط: أوراق خضراء عالية الجودة، ذبول صحيح، عملية تصنيع صحيحة، تخمير ناجح (وهو الخطوة الأكثر أهمية من بينها جميعًا)، وأخيرًا معالجة صحيحة في المجفف.",
      ],
      duration: "تمر الورقة عبر المجفف في 21 دقيقة بالضبط، ضمن دورة محكمة لدرجات حرارة الدخول والعادم.",
    },
    sorting: {
      name: "الفرز والتصنيف",
      ...
      sectionHeadings: [
        ...
        "علامات السوق وأسماء الدرجات",
        "المردود ونسبة الإنتاج",
      ],
      sectionBodies: [
        ...
        "عندما تمر الأوراق المجففة عبر آلة التصنيف، تُفصَل في مرور واحد حسب الجزء الذي أتت منه من البرعم الأصلي: الطرف الطري سهل الكسر والأوراق العلوية، أي نحو 2% فقط من كل برعم، تصبح FBOPF Extra Special، وهي أرقى الدرجات وأغلاها. تصبح الأوراق الأسفل درجات قوية مثل BOP وPekoe. تصبح الأوراق الأكثر نضجًا والأدنى موقعًا درجة Dust، وتُستبعد السيقان والبقايا غير القابلة للاستخدام كنفايات (Refuse).",
        "تشمل آلة التصنيف تلك فاصلًا لونيًا بنظام كاميرا يمكنه التمييز بين جزيئات مثل الأوراق والزهور، فيُصفّي كل ما لا ينتمي إليها. وبما أن الإنتاج في سريلانكا يتركز على الشاي الأسود، فإن هذه الخطوة هي ما يزيل ما تبقى من جزيئات بنية غير مكتملة التخمير وبقايا السيقان. تفصل مناخل بأحجام شبكية مختلفة، تُقاس بعدد الثقوب المربعة لكل بوصة وتتراوح من 3 ثقوب لكل بوصة كأخشن مقاس إلى 60 ثقبًا كأدق مقاس، الأوراق أكثر إلى نحو 21 درجة متمايزة حسب حجم الجزيئات عبر عدة مراحل. يمنح فاصل كهروستاتيكي شحنة ساكنة على بكرات لبقايا السيقان البنية المتبقية بحيث يمكن سحبها جانبًا، وتزيل آلات مخصصة الشاي التالف من الدفعة بالكامل. لا يأتي أي من هذا اللون أو الطابع من أصباغ أو إضافات، بل من المعالجة الطبيعية فقط.",
        ...
        "إضافة إلى موقع الورقة على البرعم وحجم الشبكة، يُصنَّف الشاي المفروز أيضًا إلى ثلاث فئات سوقية: First Mark وSecond Mark وThird Mark. تشمل الدرجات الرئيسية المنتَجة هنا OPA وOP وOP1 وBOP1 وFBOP وFF1 وFF وPekoe وPekoe1 وBOP Special وBOPA وFF Extra Special وExtra Special 1 وFF Special، بينما يقع الشاي الأقل قيمة (Off-Grade) ضمن فئات مثل 1A وBM وBP وFNGS وBT.",
        "في المتوسط، تنتج 100 كجم من الأوراق الخضراء نحو 21.5 كجم من الشاي الأسود النهائي المعبأ، أي بنسبة إنتاج تبلغ حوالي 21.5%، بينما يُفقد الباقي بسبب تبخر الرطوبة والنفايات التي تُزال أثناء الفرز. وتشير إرشادات اختبار مجلس الشاي القياسية إلى أن الأمر يتطلب نحو 4.65 كجم من الأوراق الخضراء لإنتاج كجم واحد من الشاي الجاهز. ويهدف المصنع إلى أن يشكل الإنتاج من الدرجات الرئيسية 65% على الأقل، مع إبقاء الشاي الأقل قيمة عند 20% كحد أقصى، على أن يقع الباقي ضمن Second Mark وفئات مماثلة.",
      ],
      duration: "يعمل الفرز باستمرار على الآلات ويستغرق حوالي 30 إلى 45 دقيقة لكل دفعة.",
    },
    packing: {
      name: "التعبئة",
      sectionHeadings: ["الوزن والتذوق والختم", "الدرجات على أرضية التعبئة", "وزن الأكياس حسب الدرجة", "من المصنع إلى المزاد", "لماذا تُعد التعبئة المُحكَمة مهمة", "مطابقة الدرجات مع الأسواق"],
      sectionBodies: [
        "يُوزَن الشاي المُصنَّف، ويُتذوَّق للتأكد من جودته، ويُختَم في عبوات مقاومة للرطوبة مبطنة برقائق الألومنيوم، مطبوع عليها اسم المصنع وشعاره، ورقم تسجيله، ورمز شاي سيلان، وشهادة سلامة الأغذية.",
        "تتجاور عدة درجات جاهزة في منطقة التعبئة: Dust (المخصص أساسًا لأكياس الشاي)، وOP1، وPekoe، وBOP (Broken Orange Pekoe)، وOP (Orange Pekoe). ويحظى OP1 وOP برواج خاص في الأسواق الناطقة بالعربية، بينما يشتهر Pekoe في روسيا وإيران. أندر وأثمن درجة تُصنَع هنا هي SFOPE1، وهي درجة 'Flowery Tippy' سُمّيت بذلك لاحتوائها على قطع طرفية شاحبة فضية اللون من أعلى جزء في البرعم. وتُقدَّر أكثر لطابعها الرقيق منه لقوتها.",
        "يختلف وزن الأكياس أيضًا حسب الدرجة: يستوعب الكيس ذو الحجم الثابت كمية أكبر بكثير من درجة رقيقة وكثيفة مثل Dust (نحو 58 كجم) مقارنة بدرجة أكبر حجمًا وأكثر ورقية مثل OPE (نحو 30 كجم).",
        "تسلّم المصنع في الغالب شايه الجاهز لسماسرة الشاي في كولومبو، ومن بينهم أسماء مثل Forbes & Walker وAkbar Brothers وFinlays، الذين ينقلونه إلى مزاد شاي كولومبو، حيث يشتريه المشترون وغالبًا ما يصدّرونه تحت علامتهم التجارية الخاصة بعد إضافة قيمة أخرى. يحدث أيضًا تصدير مباشر تحت اسم أثوكورالا نفسه، لكن ذلك يكون بشكل رئيسي لطلبات خاصة عرضية من مشترين في الخارج.",
        "تحمي التعبئة المُحكَمة الشاي من الرطوبة والروائح، وتحافظ على النكهة التي بُنيت بعناية في كل مرحلة سابقة من العملية.",
        "تريد الدول المختلفة أشياء مختلفة من شايها. يفضل البعض كوبًا قويًا وقابضًا، بينما لا يفضل آخرون ذلك، لذا فإن أي جزء من البرعم تأتي منه درجة معينة يُوائَم فعليًا مع ذوق كل سوق تصدير. تطلب الأسواق الناطقة بالعربية، على سبيل المثال، درجات محددة ليست جزءًا من عادات الشرب اليومية في بلدانها. البيع في الغالب عبر السماسرة بدلاً من التصدير المباشر يعني أن المشترين الخارجيين هم من يتولون عادة الشحن الدولي اللاحق والعلامة التجارية.",
      ],
    ...
    "ginger-turmeric": {
      ...
      sectionBodies: [
        "الزنجبيل (Zingiber officinale) عشبة معمرة من الفصيلة الزنجبيلية. يُزرع على نطاق واسع في المناطق الاستوائية وشبه الاستوائية، بما في ذلك سريلانكا والهند. الأهم من ذلك هو الجذمور الموجود تحت الأرض، الذي يُستخدم كتوابل ودواء منذ قرون. يزدهر الزنجبيل في المناخات الدافئة والرطبة ذات التربة الغنية الجيدة الصرف. ويؤدي دورًا محوريًا في الطهي التقليدي والطب العشبي في العديد من الثقافات.",
    ...
    "artisanal-tea-grades": {
      ...
      sectionBodies: [
        ...
        "يأخذ White Tea ورقتين ونصف من البرعم، ويتخطى الذبول تمامًا. يُطهى بالبخار ويُجفف بدلاً من التخمير واللف كما في الشاي الأسود. والنتيجة مذاق حلو طبيعيًا ومعالج بخفة شديدة، بسائل فاتح اللون، وقوام مخملي، ورائحة زهرية، وغنى طبيعي بمضادات الأكسدة. يستخدم Purple Tea نفس القطاف، لكنه على عكس White Tea يمر بعملية الشاي الأسود الكاملة: الذبول واللف والتخمير. وينتج عن ذلك نكهة أقرب إلى الأولونغ: أخف من الشاي الأسود لكن أقل نباتية من الشاي الأخضر، وأقل نسبيًا في الكافيين، وغني طبيعيًا بمضادات الأكسدة.",
        "يبدأ Pink Tea عديم اللون تقريبًا مثل White Tea، ولا يتحول إلى اللون الوردي إلا بعد إضافة بضع قطرات من الليمون الحامض. إذ يُحدث الحمض تغيرًا لونيًا في نفس صبغة الأنثوسيانين التي تمنح البرعم لونه. كما يتناسب جيدًا مع بضع قطرات من العسل، ويُنصح بتقديمه كشاي مثلج، وهو أيضًا غني طبيعيًا بمضادات الأكسدة. أما Vita Glow فيُصنع بدلاً من ذلك من برعم الفرع الأخضر العادي وليس الأرجواني.",
        "تصنع المزرعة أيضًا مزيجات ذات قيمة مضافة، مثل الشاي المنقوع بأزهار اللوتس، إضافة إلى Blooming Tea المربوط يدويًا من نفس الفروع. تُقطف هذه الأنواع الحرفية وتُعالج بكميات أقل بكثير من الشاي الأسود القياسي، وهذا ما يجعلها أكثر ندرة وقيمة. اسأل عند طاولة التذوق لتجربتها. يمكن استخدام البرعم الأرجواني أو البرعم الأخضر العادي لمعظم هذه الدرجات، رغم أن المزرعة تستخدم في الغالب البرعم الأرجواني.",
        "أكثر الأصناف المُنتجة هنا كثافةً في العمل هما تخصصان صغيرا الدفعات: Flower Chakra، وهو حلقة شاي أسود ملفوفة يدويًا تستخدم فرعين تقريبًا لكل منها، وTea Coin. لا يستطيع الشخص الواحد أن يلف يدويًا سوى نحو 25 غرامًا من أي منهما في اليوم، ولهذا فهي باهظة الثمن، وتُشترى غالبًا كهدية تذكارية من قبل عشاق الشاي الأثرياء بدلاً من الشرب اليومي. يُصنع معظم الإنتاج اليدوي، بما في ذلك Purple Tea، حسب الطلب وبدفعات صغيرة بدلاً من الإنتاج المستمر بكميات كبيرة.",
      ],
      duration: "5 دقائق",
    },
    "pahatharata-manufacturing": {
      name: "تصنيع الشاي (باهاتارتا)",
      sectionHeadings: ["المعالجة في الأراضي المنخفضة مقابل المرتفعة", "CTC: السحق والتمزيق واللف"],
      sectionBodies: [
        "تعالج مزارع الأراضي المنخفضة ('باهاتارتا')، مثل هذه المزرعة، الشاي بشكل مختلف عن مزارع الأراضي المرتفعة الداخلية. وتؤكد ذلك عملية التصنيع الخاصة بالمزرعة نفسها، التي تناولتها المحطات السابقة في هذه الجولة مرحلةً تلو الأخرى: يُحوَّل اللف هنا الورقة إلى جزيئات صغيرة وكثيفة بدلاً من لفها إلى ورقة أرثوذكسية كاملة، وهي السمة المميزة لمعالجة CTC (السحق-التمزيق-اللف). هذه المحطة عبارة عن ملخص سريع لما يعنيه نهج الأراضي المنخفضة هذا بالنسبة للقطف والمعالجة والفرز.",
        "تعالج طريقة CTC (السحق-التمزيق-اللف) الأوراق آليًا إلى حبيبات صغيرة كثيفة، مما ينتج الطابع القوي والمنعش الذي يشتهر به شاي سيلان من الأراضي المنخفضة. وهي نفس جزيئات الأوراق التي شوهدت سابقًا في محطتي اللف والفرز في هذه الجولة.",
      ],
    ...
    "clonal-tea": {
      name: "الشاي المستنسخ",
      heroTagline: "بعض نباتات تجربة الأصناف المستنسخة الموسومة الخاصة بالمزرعة نفسها.",
      sectionHeadings: ["الشاي المستنسخ", "لماذا الشاي المستنسخ أغلى ثمنًا", "أصناف الشاي", "TRI 3069", "TRI 3055", "TRI 4053", "TRI 2022", "TRI 2043", "TRI 4006", "TRI 3025"],
      sectionBodies: ["تأتي بعض أكثر أنواع الشاي طلبًا من نباتات مستنسخة: عُقل مكاثرة من شجيرة أم واحدة استثنائية بدلاً من النمو من البذور. تتناول هذه المحطة كيفية إكثار الشاي المستنسخ ولماذا يمكن أن يحقق سعرًا مرتفعًا. يربي معهد أبحاث الشاي (TRI) أصنافًا مستنسخة لتلبية احتياجات مختلفة. بعضها لتحمل الجفاف أو الأمطار وزيادة الإنتاجية، وبعضها الآخر من أجل البراعم الصغيرة المميزة ذات الطرف الأحمر أو الأخضر التي تُستخدم في أنواع الشاي الفاخرة المصنوعة يدويًا. الأصناف عالية الإنتاجية المزروعة على نطاق واسع مثل TRI 2023 وTRI 2025 وTRI 2026 شائعة في المزارع والحدائق المنزلية على حد سواء، في حين تُزرع الأصناف النادرة الخاصة غالبًا في المزارع الكبيرة لإنتاج الشاي المصنوع يدويًا بدلاً من الزراعة اليومية. وعلى الرغم من أن هذه الأرقام تبدو وكأنها سنوات، إلا أنها مجرد معرّفات مخصصة، وليست سنة إصدار كل صنف.", "يحافظ الإكثار المستنسخ على الخصائص الدقيقة لنبات مميز. وغالبًا ما تُنتَج أفضل أنواع الشاي المستنسخ بكميات محدودة جدًا. تُقطف براعم الأصناف الخاصة ذات الطرف الأحمر أو الأخضر يدويًا لإنتاج درجات نادرة مثل Golden Tips وSilver Tips، وعادةً ما تدفع المزارع حوالي 100 روبية إضافية لكل كيلوغرام مقارنة بالأوراق العادية المقطوفة.", "تُفضّل المزرعة الاستنساخات عالية الإنتاجية والمقاومة للآفات التي طورها معهد أبحاث الشاي (TRI)، بما في ذلك أصناف من سلسلة TRI 2000 وT65، لموثوقية أدائها في التربة والمناخ المحليين. صنف TRI 4049 هو صنف شاي عالي الإنتاجية طوره معهد أبحاث الشاي في سريلانكا، وهو مثالي للارتفاعات المتوسطة إلى المنخفضة. وهو صنف من نبات الكاميليا الصينية (Camellia sinensis) يُكاثَر خضريًا، ويُعرف بنموه القوي وأوراقه الخضراء الداكنة ومقاومته للأمراض الشائعة. ينتج TRI 4049 نكهات زاهية وعطرية مناسبة لكل من شاي CTC والشاي التقليدي، مما يجعله خيارًا شائعًا لإعادة الزراعة والزراعة التجارية.", "على بعد بضعة صفوف، هذا النبات الموسوم هو TRI 3069. وهو صنف آخر مسجل لدى معهد أبحاث الشاي، يُزرع جنبًا إلى جنب مع TRI 4049 حتى يتمكن المشتل من مقارنة أداء كل منهما في هذه التربة تحديدًا قبل زراعته على نطاق واسع.", "يقع TRI 3055 في نفس صفوف التجربة. كل قطعة من الأصناف هنا موسومة ومتابَعة بشكل فردي، وهذه هي الطريقة التي يبني بها المصنع أدلته الخاصة على الأصناف التي تستحق التوسع فعلًا، بدلًا من الاعتماد على السمعة وحدها.", "يُكمل TRI 4053 قطع تجربة الأصناف الظاهرة هنا. الحفاظ على عدة أصناف مسجلة تنمو جنبًا إلى جنب، وكل منها موسوم بوضوح، ممارسة معتادة في المزرعة قبل تخصيص صنف لحقل كامل.", "TRI 2022 مزروع في نفس قسم تجربة الأصناف، ولوحته موسومة ومتابَعة تمامًا مثل جيرانها. زراعة عدة أصناف مسجلة جنبًا إلى جنب في نفس الأرض هي الطريقة التي يبني بها المشتل أدلة حقيقية حول أيها يؤدي أداءً أفضل هنا.", "يقف TRI 2043 على بعد مسافة أبعد قليلاً في الصف. مثل النباتات المرقمة الأخرى في قطعة التجربة هذه، يتم تنميته ومراقبته قبل اتخاذ أي قرار بشأن زراعته على نطاق واسع.", "TRI 4006 هو إضافة أخرى في نفس تجربة الأصناف. يحافظ المشتل على كل صنف في مكانه الموسوم بوضوح، مما يسهل تتبع نموه ومقارنته بمرور الوقت.", "يحدد TRI 3025 حافة قطعة التجربة، بجانب لوحة ثنائية اللغة تحمل عبارة 'أصناف الشاي' ولوحة تذكارية للزراعة مؤرخة. وتشكل النباتات الموسومة هنا معًا السجل الفعلي للمشتل حول الأصناف التي تستحق التوسع."],
      duration: "5-10 دقائق",
    },
    "shade-management": {
    ...
    "tea-factory": {
      ...
      sectionBodies: ["المصنع الرئيسي هو المكان الذي تتحول فيه الأوراق المقطوفة إلى شاي جاهز، من الوزن إلى التعبئة. تقدم هذه المحطة نظرة عامة على أرضية المصنع العاملة والمعدات المستخدمة في كل مرحلة: الوزن، الذبول، اللف، التخمير (الأكسدة)، التجفيف، فرز الشاي (التصنيف)، والتعبئة النهائية، بهذا الترتيب. يدير مصنع الشاي العامل التسلسل الكامل من الوزن إلى التعبئة تحت سقف واحد، بتوقيت دقيق مع كمية الأوراق الواردة يوميًا. وتُوزن الأوراق الواردة عند وصولها لتسجيل محصول اليوم قبل بدء المعالجة. يُطلب من الزوار الذين يجولون في أرضية المصنع ارتداء غطاء للشعر وكمامة وحذاء واقٍ، تماشيًا مع معايير النظافة في المنشأة."],
      duration: "10-15 دقيقة",
    },
    "handmade-factory": {
      name: "مصنع الشاي اليدوي",
      heroTagline: "رف صواني من مصنع الشاي المصنوع يدويًا الخاص بالمزرعة.",
      sectionHeadings: [
        "مصنع الشاي اليدوي",
        "الذبول واللف",
        "الشاي الأخضر والتجفيف والمبنى",
        "ما الذي يميز الشاي اليدوي",
      ],
      sectionBodies: [
        "إلى جانب الإنتاج الآلي، يُعالَج بعض الشاي يدويًا بالكامل في كل مرحلة دون استخدام آلات. تُنجَز معظم الدرجات الحرفية الخاصة بالمزرعة هنا بدلاً من مصنع الشاي الأسود الرئيسي: Golden Tip وSilver Tip وWhite Tea وPurple Tea وPink Tea وVita Glow ومزيج Blooming Tea المربوط يدويًا، ومزيجات الأزهار مثل شاي اللوتس. (يُصنع Vita Glow من برعم الفرع الأخضر، على عكس الفرع الأرجواني في Golden وSilver Tip.)",
        "يحدث الذبول هنا بشكل طبيعي، في حوض صغير تتم مراقبته بمقياس رطوبة، دون مراوح الهواء القسري في المصنع الرئيسي، مما يخفض الرطوبة إلى نحو 50% قبل اللف. تُلَف الأوراق بعد ذلك يدويًا على صينية، ويُحكم عليها باللمس أكثر من الوقت. بالنسبة للطلبات الكبيرة بالجملة، تحل محلها بكرات آلية صغيرة لنفس الغرض، لأن اللف اليدوي وحده لا يستطيع مواكبة الكميات الكبيرة.",
        "تتولى آلة مخصصة للشاي الأخضر، تُبخّر الأوراق وتضغطها، تلك الدرجة بشكل منفصل، رغم أنها حاليًا خارج الخدمة لإصلاح بسيط. بعد التبخير، تُترك الأوراق بالخارج لتفقد الرطوبة السطحية، ثم تُلَف يدويًا لإبراز لونها وقوتها ورائحتها ومذاقها، قبل تمريرها أخيرًا عبر المجفف. ويختلف وقت التجفيف من منتج صغير إلى آخر. يُستخدم الفرن الصغير في المبنى أيضًا كمخبز لوجبات خفيفة قائمة على الشاي مثل البسكويت. يتم التجفيف، الذي يتم مباشرة بعد اللف، هنا على مجففَين منفصلَين: أحدهما تقليدي يعمل بحطب الوقود، والآخر كهربائي، يتسع كل منهما لـ34 صينية، ويُستخدم كلاهما أيضًا لمنتجات أخرى صغيرة الحجم، من الهيل والقرنفل والزنجبيل.",
        "توضح هذه المحطة ما يميز الشاي المصنوع يدويًا عن الإنتاج بخط المصنع. ينتج اللف والتجهيز اليدويان دفعات أصغر بتفاوت أكبر بين الأوراق، وغالبًا ما تُقدَّر لطابعها المميز. تنقسم أنواع الشاي اليدوي إلى فئتين واسعتين. الفئة الأولى هي الأنواع المخمرة على طريقة الشاي الأسود (يمكن التعرف عليها من لونها الأسود)، والتي تشمل Purple Tea رغم برعمه الأصلي غير المعتاد. أما الفئة الثانية فهي الأنواع غير المخمرة مثل White Tea، التي تتخطى التخمير تمامًا وتنتقل مباشرة إلى التجفيف.",
      ],
      duration: "10 دقائق",
    },
    "tea-center": {
      name: "مركز الشاي",
      heroTagline: "زوار يتذوقون الشاي عند طاولة التذوق الخاصة بالمزرعة.",
      sectionHeadings: [
        "مركز الشاي",
        "التحضير والتذوق",
        "الدرجات اليومية وشاي السوبرماركت",
        "أرقى الدرجات وأسواق التصدير",
        "خلطات الأعشاب والتوابل",
      ],
      sectionBodies: [
        "تنتهي الجولة عند مركز الشاي، حيث يفحص فريق تذوق كل دفعة، ليس هنا فقط بل أيضًا في وقت سابق، كلما غادر الشاي المصنع الرئيسي إلى السوق المحلي أو وصل للتعبئة، بحثًا عن عيوب مثل الإفراط في التخمير أو نقصه أو الاحتراق. يستبعد الفريق كل ما لا يجتاز الفحص قبل بيعه، ويلاحظ حتى المشكلات الطفيفة لتوجيه الإنتاج المستقبلي. تُعرض هنا تخصصان آخران صغيرا الدفعات من المصنع اليدوي، هما Flower Chakra وTea Coin.",
        "يمكن إعادة نقع أنواع الشاي الحرفي هذه حتى ثلاث مرات باستخدام ماء ساخن بدرجة حرارة نحو 80°م بدلاً من الغليان. تستغرق كل نقعة نحو 2 و5 و8 دقائق بماء ساخن جديد في كل مرة، على عكس الشاي الأسود العادي الذي يُحضَّر مرة واحدة فقط عند نقطة الغليان الكاملة. يتبع التذوق طريقة تشبه تذوق النبيذ: الشم أولاً، ثم الرشف بصوت لسحب الشاي عبر براعم التذوق على جانبي اللسان بجرعة من الهواء. يبصق الفريق بدلاً من البلع أثناء تذوق ما يصل إلى 50 نوعًا من الشاي يوميًا، رغم أن الزوار مدعوون لشرب ما يتذوقونه فعليًا. يحتاج الشاي الأسود المحضَّر بشكل صحيح إلى نحو 2.5 إلى 3 دقائق من النقع المغطى، وليس مجرد غطسة سريعة.",
        "درجتا BOPF وBOPF Special هما ما يشربه معظم السريلانكيين يوميًا. أما Dust وDust 1، وهما أدنى الدرجات، فلا تُرسلان أبدًا إلى أسواق التصدير، وهما ما ينتهي به الأمر عادة، بعد إضافة لون، في أكياس الشاي الرخيصة في السوبرماركت، ولهذا يظهر اللون فور غمسها بدلاً من بعد نقع صحيح. كما يُمزَج الكثير مما يُباع في السوبرماركت بشاي أرخص مُشترى من مكان آخر، على عكس شاي المزرعة أحادي المصدر وغير الممزوج.",
        "تأتي أرقى الدرجات، وهي FF Extra Special وFF Special وFBOP، من نحو 1% فقط من المحصول، وهو الجزء الأقل نضجًا من البرعم، وتُباع بنحو 25 دولارًا لكل 100 غرام. بينما تُباع الدرجات الأكبر حجمًا مثل OPA وOP بنحو 5-6 دولارات لكل 200 غرام. تضيف جزيئات القمة البيضاء المرئية في أرقى الدرجات حلاوة إضافية، وتذهب في الغالب إلى المشترين الأوروبيين، بينما تميل الأسواق العربية إلى تفضيل كوب أقوى.",
        "إلى جانب الشاي، تمزج المزرعة شايها الأخضر والأسود الخاص (نحو 90%) بأعشاب محلية (نحو 10%)، وهي الغوتوكولا والهينبوفيتيا والمورينغا، وتصنع شاي التوابل من نوع Masala والقرفة والزنجبيل من توابلها الخاصة. تستخدم وصفة الماسالا سبعة توابل سريلانكية، منها الهيل والقرفة والزنجبيل والفلفل الأسود (والباقي يبقى سرًا). يُوصى بالماسالا أو BOP لشاي الحليب.",
      ],
      duration: "10-15 دقيقة",
    },
    "coconut-field": {
    ...
    "jaggery-palm": {
      ...
      sectionBodies: [
        "نخيل الكيثول (Caryota urens)، أو نخيل ذيل السمك، نخيل طويل وأنيق يوجد في سريلانكا والهند ودول آسيوية استوائية أخرى، ويُقدَّر لاستخداماته المتعددة. أبرز منتجاته النسغ الحلو المستخرج من سيقان أزهاره، والذي يُغلى لإنتاج دبس وجاجري الكيثول التقليديين. وتشتهر الشجرة أيضًا بجذعها القوي الليفي وأوراقها الجميلة الشبيهة بذيل السمك.",
        "نظرة سريعة على النخلة نفسها:",
    ...
    pruning: {
      ...
      sectionBodies: ["قبل التقليم، تترك المزرعة شجيرة الشاي عمدًا لتنمو لفترة بدلاً من تقليمها فورًا. وبذلك يكتمل النمو الجديد تمامًا وتتمكن الشجيرة من تجميع الغذاء. ولا يبدأ التقليم إلا بعد اكتمال هذا التجمع، حيث تُقلَّم الشجيرة حتى ارتفاع الركبة تقريبًا. تُظهر اللافتات الموجودة في الموقع هذا الأمر كمرحلتين مسمّاتين: يخفض القطع المتقاطع الأول (Palamu Haras Kapuma) الشجيرة إلى حوالي 9-12 بوصة (22-30 سم)، بينما يعيد القطع المتقاطع الثاني (Dewana Haras Kapuma) ارتفاعها إلى حوالي 16-18 بوصة (40-45 سم). ترك الشجيرة تنمو أولًا، بدلاً من تقليمها وفق جدول ثابت بغض النظر عن حالتها، يمنح النبات وقتًا لتوجيه الغذاء إلى الأعلى قبل تقليمه."],
      duration: "5 دقائق",
    },
    polytunnel: {
      name: "النفق البلاستيكي والبيت الزجاجي",
      heroTagline: "النفق البلاستيكي الخاص بالمزرعة، الذي يزرع بالفعل محاصيله الأولى.",
      sectionHeadings: ["عن النفق البلاستيكي والبيت الزجاجي", "الري وإدارة المياه", "توقيت الحصاد", "رأس التحكم بالري والتسميد", "مالو ميريس (الفلفل الرومي)", "أمو ميريس (الفلفل الحار)", "ناي ميريس", "غيدي غوا (الملفوف)", "أثو غوا (الملفوف الأحمر)", "بونتشي (الفاصولياء الخضراء)", "رابو (الفجل)", "الجزر", "الكراث", "الخيار", "شتلة الطماطم"],
      sectionBodies: [
        "إلى جانب دفيئة قائمة بالفعل، تعمل المزرعة على بناء نفق بلاستيكي لزراعة مجموعة متنوعة من الخضروات الخاصة بها. ومنها فلفل السكوتش بونيت والفلفل الأخضر الحار، والفلفل الحلو، والخيار المخلل، والطماطم، والفلفل الرومي، والملفوف، والفجل، والفاصولياء الخضراء، والكراث، والبصل الأخضر، والجزر. وبمجرد اكتماله، سيتمكن ضيوف الفيلا أو الكابانا من قطف الخضروات المزروعة هنا بأنفسهم طازجة. يعمل الغطاء شبه الشفاف للنفق البلاستيكي بطريقة مشابهة لزجاج البيت الزجاجي، إذ يحتفظ بالدفء ويسمح بمرور الضوء، بينما يمنع دخول أمطار الرياح الموسمية الغزيرة والعديد من الآفات الشائعة. وهذه ظروف تتيح للمزرعة زراعة خضروات لم تكن لتنجح بشكل موثوق في الحقل المفتوح، وتطيل موسم الزراعة إلى ما هو أبعد مما تسمح به الأحواض المكشوفة.",
        "يتبع كل محصول نظام ري خاصًا به، يتناوب بين الري بالتنقيط عند مستوى الجذور والرش العلوي حسب مرحلة النمو. ويساعد الرش بشكل أساسي على الإنبات المبكر والتبريد لمحاصيل مثل الجزر والفجل، بينما يصبح الري بالتنقيط هو الطريقة الرئيسية بمجرد بدء الإثمار في محاصيل مثل الطماطم. تخدم الطريقتان أغراضًا مختلفة. يحافظ الرش على رطوبة الهواء وسطح التربة بما يكفي لتثبيت البذور والشتلات الصغيرة، بينما يوصل الري بالتنقيط الماء مباشرة إلى منطقة الجذور بعد أن تكبر النباتات، مستخدمًا كمية أقل من المياه إجمالاً ومبقيًا الأوراق جافة لتقليل خطر الإصابة بالأمراض الفطرية.",
        "يختلف الوقت اللازم للحصاد الأول اختلافًا كبيرًا حسب المحصول، من 25 إلى 35 يومًا فقط للفجل إلى 90 إلى 120 يومًا للكراث، لذا من المفترض أن يكون هناك دائمًا شيء جاهز للقطف في النفق البلاستيكي في أي وقت تقريبًا من الموسم. زراعة هذا المزيج الواسع من الخضروات سريعة ومتأخرة النضج جنبًا إلى جنب هو في حد ذاته خيار مقصود، إذ يوزّع الحصاد بشكل طبيعي بدلاً من أن يصبح كل شيء جاهزًا، أو منتهيًا، في وقت واحد.",
        "يغذي رأس التحكم هذا أنابيب التنقيط الواصلة إلى كل كيس زراعة. وتشمل مرشح لإزالة الرواسب، ومقياس ضغط، وصمامات إغلاق لعزل أقسام الأنابيب، وخزانا سماد مزودان بحاقن فنتوري يضخ السماد السائل مباشرة في مياه الري أثناء مرورها، بحيث يحصل كل نبات على الماء والتغذية في المرور نفسه.",
        "بأوراقها البيضاوية اللامعة المدببة التي تنمو في أزواج متقابلة، هذه الشتلة هي مالو ميريس، نوع من الفلفل الرومي (Capsicum annuum). تُزرع هنا في كيس زراعة مملوء بألياف جوز الهند، مع أنبوب تنقيط موصول بالفعل إلى قاعدتها.",
        "هذه الشتلة هي أمو ميريس، نوع من الفلفل الحار (Capsicum annuum). وفي هذه المرحلة المبكرة تكاد أوراقها لا تُميَّز عن أوراق الفلفل الرومي، إذ ينتمي كلاهما إلى النوع نفسه، ويُزرعان لاستخدامات مختلفة.",
        "اسم ناي ميريس يعني بالسينهالية 'فلفل الكوبرا'. وهو Capsicum chinense، نوع مختلف وأكثر حرارة عادة من أنواع الفلفل الأخرى في المزرعة، وهو قريب لأصناف مثل الهابانيرو والسكوتش بونيت.",
        "الأوراق الأكثر استدارة وذات المظهر الشمعي الخفيف في هذه الشتلة تعود إلى غيدي غوا، ويعني هذا الاسم حرفيًا 'الملفوف الكروي'. وهو الملفوف الشائع المكوِّن للرأس (Brassica oleracea var. capitata)، يُزرع هنا قبل وقت طويل من بدء تكوّن رأسه.",
        "الأوراق ذات اللون الأحمر البنفسجي في هذه الشتلة تعود إلى أثو غوا، ويعني هذا الاسم حرفيًا 'الملفوف المتفرع'. وهو صنف ملفوف أكثر انفتاحًا ولا يُكوّن رأسًا (Brassica oleracea var. acephala)، بخلاف النوع الكروي.",
        "هذه الأوراق العريضة على شكل قلب تعود إلى بونتشي، الفاصولياء الخضراء (Phaseolus vulgaris). وهو محصول متسلق بمجرد أن يبدأ نموه، بعادة نمو مختلفة تمامًا عن الخضروات المنخفضة المحيطة به.",
        "هذه الشتلة الصغيرة، التي لا تزال في مرحلة الفلقة، هي رابو، الفجل (Raphanus sativus). وهي أحد أسرع المحاصيل في النفق البلاستيكي وصولاً إلى الحصاد، إذ يكون جاهزًا للقطف في 25 إلى 35 يومًا فقط.",
        "يمكن التعرف على الجزر (Daucus carota subsp. sativus) حتى في هذه المرحلة المبكرة من خلال أوراقه الأولى الرفيعة الشبيهة بالعشب، المختلفة تمامًا عن الأوراق الفلقية العريضة للخضروات الأخرى المزروعة هنا.",
        "يظهر الكراث (Allium ampeloprasum) على شكل شفرة رفيعة واحدة، وهو من أبطأ المحاصيل في النفق البلاستيكي وصولاً إلى الحصاد. وقد يستغرق من 90 إلى 120 يومًا من الزراعة.",
        "الورقتان الفلقيتان العريضتان المستديرتان في هذه الشتلة هما فلقتا خيار السلطة (Cucumis sativus)، قبل ظهور أوراقه الحقيقية الأكثر شبهًا بشكل القلب.",
        "تُظهر شتلة الطماطم هذه بالفعل الأوراق المفصصة المسننة الخفيفة الزغب المميزة للنبتة. الطماطم هي المحصول الذي ذُكر سابقًا كمثال على المحاصيل التي تتحول من الرش إلى الري بالتنقيط بمجرد بدء الإثمار.",
      ],
      duration: "5 دقائق",
    },
    "mushroom-hut": {
      name: "كوخ الفطر",
      sectionHeadings: ["حديقة من نوع مختلف", "أصناف فطر المحار", "الوسط الزراعي والبذار", "دورة الحصاد", "القيمة الغذائية والفوائد الصحية"],
      sectionBodies: [
        "إلى جانب محاصيلها من الشاي والتوابل والفواكه، تحتفظ المزرعة بكوخ مخصص لزراعة الفطر. وهو مبنى صغير مظلل بُني للحفاظ على الظروف المظلمة والباردة والرطبة التي يحتاجها الفطر لينمو ثماره، بدلاً من أشعة الشمس وتدفق الهواء الذي تُزرع من أجله بقية الحدائق.",
        "أُعد الكوخ لزراعة عدة أصناف من فطر المحار: المحار الأمريكي (Pleurotus pulmonarius)، ومحار بوتان، والمحار الأسود، والمحار الوردي (Pleurotus djamor)، وفطر الأذن البحرية (Pleurotus cystidiosus). وتُزرع جميعها على وسط من القش أو نشارة الخشب بدلاً من التربة. تنتمي هذه الأصناف جميعها إلى جنس Pleurotus، ويمكن التعرف عليها من خلال قبعاتها ذات الشكل المروحي أو الشبيه بالمحار، والتي تنمو في طبقات كثيفة ومتراكبة تخرج مباشرة من الوسط الزراعي بدلاً من سيقان منفردة. ويُعد اللون هو العامل الرئيسي الذي يميز صنفًا عن آخر، إذ يتراوح بين الأبيض والرمادي إلى الوردي والذهبي أو الأسود تقريبًا، في حين يبقى الطعم الخفيف الذي يميل قليلاً إلى الملوحة متشابهًا إلى حد كبير بين جميع الأصناف.",
        "تبدأ الزراعة بالوسط نفسه: القش أو نشارة الخشب، وهي نفس المادة التي يقوم عليها نهج المزرعة منخفض التكلفة القائم على المخلفات. يُنقع هذا الوسط ويُبستر بالحرارة للقضاء على العفن والبكتيريا المنافسة، قبل خلطه بـ'البذار' وهو حبوب استعمرها بالكامل ميسيليوم الفطر، ثم يُعبأ في أكياس أو كتل مثقوبة. تُترك هذه الكتل بعد ذلك في الظلام لمدة تتراوح بين أسبوعين وثلاثة أسابيع فيما يُعرف بـ'فترة انتشار البذار'، حيث ينتشر الميسيليوم الأبيض في الوسط بأكمله. بعد ذلك فقط، تتولى ظروف الكوخ المظلمة والباردة والرطبة مهمة تحفيز نمو الثمار.",
        "تُعد زراعة الفطر بهذه الطريقة وسيلة منخفضة التكلفة ومستدامة للمزرعة لتنويع إنتاجها إلى جانب محصولها الرئيسي، وغالبًا ما تستخدم مخلفات زراعية مثل القش كوسط للزراعة. وتنتج دفعة واحدة من الوسط عدة حصادات على مدى عدة أسابيع. وعادة ما تنتج الكتلة الواحدة ما بين دفعتين إلى أربع دفعات من هذا النوع، تفصل بينها أسبوع إلى أسبوعين تقريبًا، حيث تكون الدفعة الأولى هي الأكبر ثم يتناقص الإنتاج تدريجيًا بعدها. ويُلوى الفطر بلطف لفصله في مجموعات كاملة بدلاً من قطعه، إذ يمكن أن تتعفن بقايا السيقان وتتسبب في تلوث يفسد الدفعة التالية.",
        "وتُبرز لافتات المزرعة الخاصة الجانب الغذائي لكل صنف: يُوصف كل من المحار الأمريكي والمحار الوردي وفطر الأذن البحرية بأنه غني بالبروتين والفيتامينات ومضادات الأكسدة، مع فوائد تتمحور حول دعم المناعة وصحة القلب والجهاز الهضمي، والحد من الالتهاب، والمساعدة في خفض الكوليسترول.",
      ],
      duration: "5 دقائق",
    },
  },
  ru: {
    ...
    withering: {
      name: "Завяливание",
      sectionHeadings: ["Загрузка желобов", "12-часовой цикл", "Потеря влаги", "Почему завяливание важно"],
      sectionBodies: [
        "Лист, поступающий с плантации, сначала взвешивают, чтобы зафиксировать дневной сбор. Затем его тонким слоем раскладывают на длинных желобах длиной 85 футов и шириной 6 футов. Каждый желоб оснащён вентилятором диаметром 46 дюймов, работающим со скоростью 960 об/мин и прогоняющим воздух через лист, постепенно удаляя влагу. Это то, что на плантации называют «современным» способом завяливания. При норме загрузки около 2,5 кг на квадратный фут один желоб вмещает около 1250 кг свежесобранного листа.",
        "Лист остаётся в желобе 12 часов, его вручную переворачивают каждые 4 часа. В течение 3–4 часов этого цикла снизу подаётся тепло, а в последний раз лист рыхлят перед выгрузкой ранним утром. Увядший лист снова взвешивают перед тем, как отправить его на скручивание.",
        "Из каждых 100 кг зелёного листа удаляется около 43 кг воды. Эту цифру проверяют, взвешивая 2,5 кг листа в коробке площадью один квадратный фут. В итоге влажность листа составляет 57–58%: он становится мягким и податливым, готовым к скручиванию без повреждений.",
        "Завяливание выравнивает влажность по всему листу и позволяет природным ферментам начать химические изменения, которые впоследствии создадут аромат и цвет чая.",
      ],
      duration: "Этот этап длится полные 12 часов, лист переворачивают каждые 4 часа.",
    },
    rolling: {
      name: "Скручивание",
      sectionHeadings: ["Скручивание и разрушение листа", "Первое скручивание", "Второе скручивание", "Почему скручивание важно"],
      sectionBodies: [
        "Завяленный лист проходит через скручивающие машины, которые скручивают и разрушают клеточные стенки листа, высвобождая заключённые в них соки и масла и разбивая лист на мелкие частицы. Именно здесь формируются форма листа и крепость чая.",
        "Каждый из двух загрузочных роллеров принимает по 280 кг завяленного листа. Первое скручивание длится 20 минут при скорости 30–32 об/мин. Полученные «доли» (скрученные частицы листа) по конвейеру попадают в дробилку, оснащённую проволочной сеткой, которая пропускает самые мелкие частицы как «первую доль», отправляемую на ферментацию.",
        "Лист, не прошедший через сетку, поднимается по конвейеру к двум вторичным роллерам, с загрузкой, уменьшенной на 12–14% от первоначальных 280 кг. Здесь проходит второе скручивание ещё в течение 20 минут при 32 об/мин, после чего «вторая доль» направляется прямо на ферментацию без просеивания.",
        "Разрушение клеточных стенок открывает доступ кислорода к ферментам, запуская окисление. От силы и продолжительности скручивания зависят крепость и внешний вид готового чая. Первый из нескольких проходов, мягкое «предварительное» скручивание, призван покрыть скрученные частицы соком самого листа перед более интенсивным скручиванием.",
      ],
      duration: "Два скручивания по 20 минут (всего 40 минут): первое при 30–32 об/мин, второе при 32 об/мин.",
    },
    fermentation: {
      name: "Ферментация (окисление)",
      sectionHeadings: ["Окисление листа", "Чистота помещения", "От зелёного к медному", "Точность во времени", "Чёрный, зелёный, белый чай и улун"],
      sectionBodies: [
        "Скрученный лист, как первая, так и вторая доль, раскладывают в прохладном влажном помещении, поддерживаемом при температуре около 25–30°C, и оставляют окисляться на 2,5 часа. Увлажнитель поддерживает воздух достаточно влажным, чтобы реакция прошла как нужно.",
        "Помещение должно оставаться полностью чистым, без бактерий, застоявшейся воды или старого листа, поскольку бактерии останавливают ферментативную активность и портят цвет, крепость и качество настоя.",
        "За эти 2,5 часа зелёный лист постепенно приобретает медно-коричневый оттенок, полностью развивая цвет, насыщенность и аромат, характерные для чёрного чая. Всё это происходит исключительно за счёт естественного окисления, без красителей и добавок.",
        "Это самый важный этап для формирования вкуса. Время нужно определять точно: если недостаточно, чай получится слабым, а если слишком долго, плоским и невыразительным.",
        "Именно это и отличает виды чая, которые здесь производят. Чёрный чай проходит завяливание, скручивание, ферментацию, затем обжарку. Зелёный чай полностью пропускает ферментацию, проходя только завяливание, скручивание и сушку, и получается более мягким: его пьют в основном за более нежный, почти лечебный характер. Белый чай пропускает и завяливание, и ферментацию, отправляясь прямо со свежего листа на пропаривание и сушку. Улун на этой фабрике не производят. Экскурсовод описывает его как ферментированный сорт, похожий на чёрный чай, который делают путём многократного пропаривания и ручного растирания вместо машинного скручивания.",
      ],
      duration: "Ферментация занимает 2,5 часа как для первой, так и для второй доли, после чего лист отправляется на сушку.",
    },
    drying: {
      name: "Сушка (обжарка)",
      sectionHeadings: ["Подача в сушилку", "6 секций, 21 минута", "Поиск правильного баланса", "Результат", "Сохранение свежести", "Цепочка условий"],
      sectionBodies: [
        "Перед подачей в сушилку ферментированный лист рыхлят. Затем он проходит через сушилки горячего воздуха. Тепло от сжигания дров нагнетается главным вентилятором, а температура на входе поддерживается в диапазоне 120–130°C. Это мгновенно останавливает окисление и закрепляет характер чая.",
        "Внутри сушилка разделена на 6 секций. Лист падает с верхнего лотка через эти секции на нижние лотки, кувыркаясь вперёд и назад по мере продвижения, и выходит ровно через 21 минуту. К этому моменту температура на выходе должна составлять около 80°C (примерно 140–150°F).",
        "Именно это сочетание, 120–130°C на входе, 80°C на выходе, 21 минута, сушилка рассчитана поддерживать, и именно оно превращает лист в правильно почерневший. Если температура на входе опускается ниже 120°C, лист выходит недосушенным и буроватым, даже если температура на выходе достигает 100°C за счёт утончения слоя листа. Если же температуру на входе поднять до 150°C, а на выходе она упадёт до 75°C, лист получится сырым и как бы тушёным, а не правильно высушенным.",
        "При правильных настройках на выходе получается знакомый чёрный, хрустящий чайный лист. Посетители сразу же ощущают жар. Это один из самых горячих участков на всей фабрике.",
        "Обжарка останавливает химические изменения в нужный момент и снижает влажность примерно с 45–50% до 2–3%, что необходимо для сохранения свежести чая при хранении и транспортировке.",
        "Однако конечный вес и качество чая зависят не только от сушилки. Они определяются целой цепочкой условий: качественный зелёный лист, правильное завяливание, верный технологический процесс, успешная ферментация (самый важный из всех этапов) и, наконец, правильная обработка в сушилке.",
      ],
      duration: "Лист проходит через сушилку ровно за 21 минуту, следуя строго выдерживаемому циклу температур на входе и выходе.",
    },
    sorting: {
      ...
      sectionHeadings: [
        ...
        "Рыночные марки и названия сортов",
        "Выход и коэффициент готовой продукции",
      ],
      sectionBodies: [
        ...
        "Та же сортировочная машина включает цветовой сепаратор с камерной системой, способной отличать частицы вроде листа и цветка, отфильтровывая всё лишнее. Поскольку производство на Шри-Ланке сосредоточено на чёрном чае, именно этот этап устраняет оставшиеся коричневые, не до конца ферментированные частицы и фрагменты стеблей. Сита с разным размером ячеек, измеряемым числом квадратных отверстий на дюйм, от 3 отверстий на дюйм как самого крупного до 60 как самого мелкого, дополнительно разделяют лист примерно на 21 отдельный сорт по размеру частиц за несколько проходов. Электростатический сепаратор придаёт статический заряд на роликах оставшимся коричневым фрагментам стеблей, чтобы их можно было отделить, а специальные машины полностью удаляют из партии отбракованный чай. Ни цвет, ни характер здесь не связаны с красителями или добавками, только с естественной обработкой.",
        ...
        "Помимо положения на побеге и размера ячейки, отсортированный чай также делится на три рыночные категории: First Mark, Second Mark и Third Mark. Основные сорта, производимые здесь, включают OPA, OP, OP1, BOP1, FBOP, FF1, FF, Pekoe, Pekoe1, BOP Special, BOPA, FF Extra Special, Extra Special 1 и FF Special, тогда как менее ценный чай низших сортов относится к категориям вроде 1A, BM, BP, FNGS и BT.",
        "В среднем 100 кг зелёного листа дают около 21,5 кг готового упакованного чёрного чая, то есть выход составляет примерно 21,5%, а остальное теряется из-за испарения влаги и отбраковки в процессе сортировки. Согласно стандартным методическим указаниям Совета по чаю, на 1 кг готового чая требуется около 4,65 кг зелёного листа. Фабрика стремится к тому, чтобы не менее 65% продукции приходилось на основные сорта, доля низших сортов не превышала 20%, а остальное относилось к Second Mark и подобным категориям.",
      ],
      duration: "Сортировка проводится непрерывно на оборудовании и занимает примерно 30–45 минут на партию.",
    },
    packing: {
      name: "Упаковка",
      sectionHeadings: ["Взвешивание, дегустация и запечатывание", "Сорта на участке упаковки", "Вес мешков по сортам", "От фабрики до аукциона", "Почему герметичная упаковка важна", "Соответствие сортов рынкам"],
      sectionBodies: [
        "Отсортированный чай взвешивают, пробуют на качество и запечатывают во влагостойкую упаковку с алюминиевой фольгой внутри, с напечатанным названием и логотипом фабрики, её регистрационным номером, символом цейлонского чая и сертификатом безопасности пищевых продуктов.",
        "В зоне упаковки бок о бок хранятся несколько готовых сортов: Dust (в основном для чайных пакетиков), OP1, Pekoe, BOP (Broken Orange Pekoe) и OP (Orange Pekoe). OP1 и OP особенно популярны на арабоязычных рынках, а Pekoe хорошо известен в России и Иране. Самый редкий и ценный сорт, который здесь производят, — SFOPE1, сорт «Flowery Tippy», названный так за бледные серебристые кончиковые частицы с самой верхушки побега. Его ценят скорее за тонкий характер, чем за крепость.",
        "Вес мешков тоже различается в зависимости от сорта: мешок фиксированного размера вмещает намного больше тонкого, плотного сорта вроде Dust (около 58 кг), чем более объёмного, лиственного сорта вроде OPE (около 30 кг).",
        "В основном фабрика передаёт готовый чай брокерам в Коломбо, среди которых упоминались такие имена, как Forbes & Walker, Akbar Brothers и Finlays, которые доставляют его на чайный аукцион в Коломбо, где покупатели его приобретают и часто экспортируют под собственным брендом после дополнительной обработки. Прямой экспорт под именем Athukorala тоже случается, но в основном для отдельных специальных заказов от зарубежных покупателей.",
        "Герметичная упаковка защищает чай от влаги и посторонних запахов, сохраняя вкус, который тщательно формировался на каждом предыдущем этапе процесса.",
        "Разные страны хотят от своего чая разного. Одни предпочитают крепкий, терпкий настой, другие нет, поэтому то, из какой части побега получен сорт, фактически подбирается под вкус каждого экспортного рынка. Арабоязычные рынки, например, специально запрашивают сорта, которые не входят в повседневные привычки питья у себя дома. Продажа в основном через брокеров, а не прямой экспорт, означает, что дальнейшую международную доставку и брендинг обычно берут на себя внешние покупатели.",
      ],
    ...
    "artisanal-tea-grades": {
      ...
      sectionHeadings: [
        ...
        "White Tea и Purple Tea",
        "Pink Tea и Vita Glow",
        "Смеси и редкость",
        "Flower Chakra и Tea Coin",
      ],
      sectionBodies: [
        ...
        "Golden Tip делают из полностью созревшей почки этого побега, а Silver Tip — из более светлой, ещё незрелой почки того же побега. Именно тонкие серебристые волоски дают этому сорту его название и лёгкий блеск. Только почку взвешивают, вялят несколько часов и сушат. Это более простой процесс, чем для полностью ферментированных сортов, и в результате получается чуть сладковатый, цветочный настой с бархатистой текстурой.",
        "White Tea берёт с побега два с половиной листа и полностью пропускает вяление. Его пропаривают и сушат, а не ферментируют и скручивают, как чёрный чай. В итоге получается очень легко обработанный, естественно сладкий вкус со светлым настоем, бархатистой текстурой, цветочным ароматом и природным богатством антиоксидантов. Purple Tea использует тот же сбор, но, в отличие от White Tea, проходит полный процесс чёрного чая: вяление, скручивание и ферментацию. Это даёт вкус, близкий к улуну: легче чёрного чая, но менее травянистого, чем зелёный, сравнительно небольшое содержание кофеина и естественная насыщенность антиоксидантами.",
        "Pink Tea изначально практически бесцветен, как White Tea, и приобретает розовый оттенок только после добавления пары капель лайма. Цитрус вызывает изменение цвета того же пигмента антоциана, который придаёт побегу его окраску. Он также хорошо сочетается с парой капель мёда, его рекомендуют подавать как холодный чай, и он тоже естественно богат антиоксидантами. Vita Glow, напротив, делают из почки обычного зелёного побега, а не фиолетового.",
        "Поместье также создаёт смеси с добавленной ценностью, например чай с лотосом, а также связанный вручную Blooming Tea из тех же побегов. Эти ремесленные сорта собираются и обрабатываются в гораздо меньших количествах, чем стандартный чёрный чай, что и делает их более редкими и ценными. Спросите на дегустационной стойке, чтобы попробовать их. Для большинства этих сортов можно использовать как побег с фиолетовым оттенком, так и обычный зелёный побег, хотя поместье чаще использует именно фиолетовый.",
        "Самые трудоёмкие изделия, которые здесь делают, — это две мелкосерийные специальности: Flower Chakra, скрученное вручную кольцо из чёрного чая, на которое уходит примерно два побега, и Tea Coin. Один человек может скрутить вручную лишь около 25 граммов любого из них в день, поэтому они так дороги, и их чаще покупают как сувенир состоятельные любители чая, а не для повседневного питья. Большая часть ручного производства, включая Purple Tea, делается на заказ небольшими партиями, а не непрерывно в больших объёмах.",
      ],
      duration: "5 минут",
    },
    "pahatharata-manufacturing": {
      name: "Производство чая (Пахатарата)",
      sectionHeadings: ["Переработка на низинных и высокогорных плантациях", "CTC: дробление-разрыв-скручивание"],
      sectionBodies: [
        "Низинные («Пахатарата») плантации, как эта, обрабатывают чай иначе, чем высокогорные плантации дальше вглубь острова. Собственный производственный процесс поместья, показанный этап за этапом ранее в этом туре, подтверждает это: скручивание здесь превращает лист в мелкие плотные частицы, а не скручивает его в цельный ортодоксальный лист. Это отличительная черта CTC-обработки (Crush-Tear-Curl). Эта остановка — краткое резюме того, что означает такой низинный подход для сбора листа, обработки и сортировки.",
        "CTC (Crush-Tear-Curl — «дробление-разрыв-скручивание») механически превращает лист в мелкие плотные гранулы, придавая низинному цейлонскому чаю характерную крепость и резкость. Это те же самые частицы листа, что были видны ранее на остановках скручивания и сортировки в этом туре.",
      ],
    ...
    "clonal-tea": {
      name: "Клоновый чай",
      heroTagline: "Несколько собственных подписанных пробных растений клоновых сортов поместья.",
      sectionHeadings: ["Клоновый чай", "Почему клоновый чай стоит дороже", "Культивары Чая", "TRI 3069", "TRI 3055", "TRI 4053", "TRI 2022", "TRI 2043", "TRI 4006", "TRI 3025"],
      sectionBodies: ["Некоторые из самых востребованных сортов чая получают из клоновых растений: черенков, размноженных от одного исключительного маточного куста, а не выращенных из семян. Эта остановка рассказывает о том, как размножают клоновый чай и почему он может стоить дорого. Институт чайных исследований (TRI) выводит клоновые сорта для разных целей. Одни для устойчивости к засухе или дождям и высокой урожайности, другие ради характерных красно- или зеленоконечных молодых почек, ценимых в ручном производстве элитного чая. Широко высаживаемые высокоурожайные сорта, такие как TRI 2023, TRI 2025 и TRI 2026, распространены как на плантациях, так и в приусадебных садах, тогда как более редкие специальные сорта в основном выращивают на крупных плантациях для чая ручной работы, а не для повседневного возделывания. Хотя эти номера TRI похожи на годы, на самом деле это просто присвоенные идентификаторы, а не год выпуска каждого сорта.", "Клоновое размножение сохраняет точные характеристики выдающегося растения. Лучшие клоновые чаи часто производятся в очень ограниченных количествах. Почки редких сортов с красными или зелёными кончиками собирают вручную для таких редких сортов, как Golden Tips и Silver Tips, и плантации обычно платят за них примерно на 100 рупий больше за килограмм, чем за обычный сорванный лист.", "Поместье отдаёт предпочтение высокоурожайным, устойчивым к вредителям клонам, выведенным Институтом исследований чая (TRI), включая сорта серии TRI 2000 и T65, за их надёжность в местной почве и климате. TRI 4049 — высокоурожайный сорт чая, выведенный Институтом чая Шри-Ланки, идеально подходящий для средних и низких высот. Это вегетативно размножаемая разновидность Camellia sinensis, известная энергичным ростом, тёмно-зелёными листьями и устойчивостью к распространённым болезням. TRI 4049 даёт яркие, ароматные настои, подходящие как для чая CTC, так и для ортодоксального чая, что делает его популярным выбором для пересадки и промышленного выращивания.", "Через несколько рядов растёт помеченное табличкой растение — это TRI 3069, ещё один зарегистрированный Институтом чая сорт, посаженный рядом с TRI 4049, чтобы питомник мог сравнить, как каждый из них ведёт себя именно на этой почве, прежде чем высаживать в промышленных масштабах.", "TRI 3055 растёт в тех же испытательных рядах. Каждый участок с сортом здесь помечен и отслеживается отдельно. Именно так фабрика накапливает собственные данные о том, какие сорта действительно стоит расширять, а не полагается только на репутацию.", "TRI 4053 завершает показанные здесь испытательные участки сортов. Выращивание нескольких зарегистрированных сортов бок о бок, каждый с чёткой маркировкой, — стандартная практика на плантации перед тем, как отдать под один сорт целое поле.", "TRI 2022 посажен в том же испытательном участке сортов, его табличка помечена и отслеживается так же, как у соседей. Выращивание нескольких зарегистрированных сортов рядом на одной и той же почве позволяет питомнику собирать реальные доказательства того, какие из них показывают себя здесь лучше всего.", "TRI 2043 стоит чуть дальше по ряду. Как и другие пронумерованные растения на этом испытательном участке, он выращивается и отслеживается, прежде чем будет принято какое-либо решение о посадке в промышленных масштабах.", "TRI 4006 — ещё одно растение в том же испытании сортов. Питомник держит каждый сорт на своём чётко обозначенном месте, что упрощает отслеживание и сравнение роста со временем.", "TRI 3025 отмечает край испытательного участка, рядом с двуязычной табличкой «Сорта чая» и датированной памятной табличкой о посадке. Вместе помеченные здесь растения составляют реальную базу данных питомника о том, какие сорта стоит расширять."],
      duration: "5–10 минут",
    },
    "shade-management": {
    ...
    "unique-fruits": {
      ...
      fruitSectionBodies: [
        ...
        [
          "Averrhoa carambola, карамбола, также известная как звёздный фрукт, это дерево, родом из тропической Юго-Восточной Азии, теперь выращиваемое и здесь, в поместье. Названо так за характерные рёбра, образующие идеальную форму звезды при поперечном разрезе плода. Весь плод съедобен, включая кожицу, и обладает хрустящей текстурой со вкусом от терпкого до сладкого.",
          "Краткий обзор самого дерева:",
    ...
    "handmade-factory": {
      ...
      heroTagline: "Стеллаж с лотками с собственной Фабрики ручного чая поместья.",
      sectionHeadings: [
        "Фабрика ручного чая",
        "Вяление и скручивание",
        "Зелёный чай, сушка и здание",
        "Чем ручной чай отличается от фабричного",
      ],
      sectionBodies: [
        "Помимо механизированного производства, часть чая на каждом этапе обрабатывается полностью вручную, без машин. Большинство особых ремесленных сортов поместья, включая Golden Tip, Silver Tip, White Tea, Purple Tea, Pink Tea, Vita Glow, связанный вручную Blooming Tea и смеси с цветами, такие как чай с лотосом, дорабатываются здесь, а не на основной фабрике чёрного чая. (Vita Glow делают из почки зелёного побега, в отличие от фиолетового побега Golden и Silver Tip.)",
        "Вяление здесь происходит естественным путём, в небольшом лотке, под контролем гигрометра, без вентиляторов принудительной подачи воздуха основной фабрики, и влажность снижается примерно до 50% перед скручиванием. Затем лист скручивают вручную на подносе, оценивая на ощупь, а не по часам. Для крупных оптовых заказов ту же роль выполняют небольшие механические скручиватели, поскольку одно только ручное скручивание не поспевает за объёмом.",
        "Отдельная машина для зелёного чая, пропаривающая и прессующая лист, обрабатывает этот сорт отдельно, хотя сейчас она не работает из-за небольшого ремонта. После пропаривания лист отлёживается на воздухе, теряя поверхностную влагу, затем его вытягивают и скручивают вручную, чтобы раскрыть цвет, крепость, аромат и вкус, прежде чем отправить в сушилку. Время сушки варьируется от одного мелкого производителя к другому. Небольшая печь в здании также служит пекарней для чайных закусок вроде печенья. Сушка, которую проводят сразу после скручивания, здесь идёт на двух отдельных сушилках: одна традиционная, на дровах, другая электрическая. Каждая вмещает 34 подноса, и обе также используются для других мелкосерийных продуктов: кардамона, гвоздики, имбиря.",
        "Эта остановка рассказывает о том, что отличает ручной чай от фабричного поточного производства. Ручное скручивание и ручная обработка дают более мелкие партии с большей вариативностью между листьями, что часто ценится за неповторимый характер. Ручной чай делится на две большие категории. Первая — ферментированные, в стиле чёрного чая (их можно узнать по чёрному цвету), к которым относится и Purple Tea, несмотря на его необычный исходный побег. Вторая — неферментированные, как White Tea, которые полностью пропускают ферментацию и сразу идут на сушку.",
      ],
      duration: "10 минут",
    },
    "tea-center": {
      name: "Чайный центр",
      heroTagline: "Посетители дегустируют чай за собственной дегустационной стойкой поместья.",
      sectionHeadings: [
        "Чайный центр",
        "Заваривание и дегустация",
        "Повседневные сорта и супермаркетный чай",
        "Самые тонкие сорта и экспортные рынки",
        "Травяные и пряные смеси",
      ],
      sectionBodies: [
        "Экскурсия заканчивается в чайном центре, где дегустационная комиссия проверяет каждую партию, не только здесь, но и раньше, всякий раз когда чай покидает основную фабрику для местного рынка или поступает на упаковку, на дефекты вроде переферментации, недоферментации или подгорания. Всё, что не проходит проверку, отбраковывается ещё до продажи, а даже незначительные проблемы фиксируются, чтобы учесть их в будущем производстве. Здесь же выставлены ещё две мелкосерийные специальности с ручной фабрики: Flower Chakra и Tea Coin.",
        "Такой ремесленный чай можно заваривать повторно до трёх раз, используя воду с температурой около 80°C вместо кипятка. Каждая заварка занимает примерно 2, 5 и 8 минут, каждый раз со свежей горячей водой, в отличие от обычного чёрного чая, который заваривают один раз крутым кипятком. Дегустация проходит по методу, похожему на дегустацию вина: сначала нюхают, затем шумно втягивают чай вместе с воздухом, чтобы провести его по вкусовым рецепторам по бокам языка. Комиссия сплёвывает, а не глотает, пробуя до 50 сортов чая в день, хотя посетителям предлагают действительно выпить то, что они пробуют. Правильно заваренный чёрный чай требует около 2,5–3 минут настаивания под крышкой, а не просто быстрого погружения.",
        "BOPF и BOPF Special — это сорта, которые большинство шри-ланкийцев пьют каждый день. Dust и Dust 1, самые низкие сорта, никогда не отправляют на экспортные рынки. Именно они, обычно с добавлением красителя, оказываются в дешёвых супермаркетных чайных пакетиках, поэтому цвет появляется сразу при погружении пакетика, а не после нормального заваривания. Многое из того, что продаётся в супермаркетах, также смешано с более дешёвым чаем, закупленным на стороне, в отличие от несмешанного чая одного происхождения из этого поместья.",
        "Самые тонкие сорта, а именно FF Extra Special, FF Special, FBOP, получают лишь из примерно 1% урожая, самой незрелой части почки, и продают примерно по 25 долларов за 100 г. Более объёмные сорта вроде OPA и OP стоят около 5–6 долларов за 200 г. Белые кончики, заметные в лучших сортах, добавляют дополнительную сладость и в основном идут европейским покупателям, тогда как арабские рынки предпочитают более крепкий напиток.",
        "Помимо чая, поместье смешивает собственный зелёный и чёрный чай (около 90%) с местными травами (около 10%), готуколой, хинбовитией и морингой, и делает пряный чай Masala, коричный и имбирный из собственных специй. В рецепте Masala используются семь шри-ланкийских специй, включая кардамон, корицу, имбирь и чёрный перец (остальные держатся в секрете). Для чая с молоком рекомендуют Masala или BOP.",
      ],
      duration: "10–15 минут",
    },
    "coconut-field": {
    ...
    introduction: {
      ...
      sectionBodies: [
        ...
        "Гостей традиционно приветствуют словом «Аюбован», сингальским приветствием, означающим «долгих лет жизни». Иногда это делает лично сам председатель поместья, Анура Атукорала.",
      ],
      duration: "5–10 минут",
    },
    "jaggery-palm": {
      ...
      sectionBodies: [
        "Пальма китул (Caryota urens), или рыбохвостая пальма, — высокая, изящная пальма, встречающаяся в Шри-Ланке, Индии и других тропических странах Азии, ценимая за разностороннее применение. Её самый заметный продукт — сладкий сок, собираемый из цветоносов, который вываривают до традиционной китульской патоки и джаггери. Дерево также известно своим крепким, волокнистым стволом и красивыми, похожими на рыбий хвост листьями.",
        "Краткий обзор самой пальмы:",
    ...
    polytunnel: {
      ...
      heroTagline: "Собственный плёночный тоннель поместья, где уже растут его первые культуры.",
      sectionHeadings: ["О тоннеле и теплице", "Полив и орошение", "Сроки сбора урожая", "Блок управления поливом и удобрением", "Малу Мирис (болгарский перец)", "Аму Мирис (перец чили)", "Наи Мирис", "Геди Гова (капуста)", "Атху Гова (красная капуста)", "Бончи (стручковая фасоль)", "Раабу (редис)", "Морковь", "Лук-порей", "Огурец", "Рассада помидора"],
      sectionBodies: [
        "Рядом с уже существующей теплицей поместье строит плёночный тоннель для выращивания целого набора собственных овощей. Среди них перец чили сорта Scotch Bonnet и зелёного чили, болгарского перца, корнишонов, помидоров, сладкого перца, капусты, редиса, зелёной фасоли, лука-порея, зелёного лука и моркови. После завершения строительства гости, проживающие на вилле или в кабане, смогут сами собирать свежие овощи, выращенные прямо здесь. Полупрозрачное покрытие плёночного тоннеля работает почти как стекло теплицы: удерживает тепло и пропускает свет, но не пускает внутрь сильные муссонные дожди и многих распространённых вредителей. Эти условия позволяют поместью выращивать овощи, которые иначе не удалось бы надёжно вырастить в открытом грунте, и продлевают период выращивания по сравнению с открытыми грядками.",
        "Каждая культура выращивается по собственному графику полива, чередуя капельный полив у корней с верхним распылением в зависимости от стадии роста. Распыление в основном помогает прорастанию и охлаждению на ранних этапах для таких культур, как морковь и редис, тогда как капельный полив становится основным методом, как только начинается плодоношение, например у помидоров. Эти два способа полива служат разным целям. Распыление поддерживает достаточную влажность воздуха и поверхности почвы, чтобы молодые семена и рассада укоренились. Капельный полив, после того как растения подрастут, доставляет воду прямо к корневой зоне, расходуя меньше воды в целом и оставляя листья сухими, что снижает риск грибковых заболеваний.",
        "Время до первого урожая сильно варьируется в зависимости от культуры: от всего 25–35 дней для редиса до 90–120 дней для лука-порея, так что в тоннеле почти всегда должно найтись что собрать в любой момент сезона. Выращивание такого широкого набора быстро- и медленно созревающих овощей бок о бок само по себе является осознанным выбором: оно естественным образом растягивает урожай во времени, вместо того чтобы всё созревало или заканчивалось одновременно.",
        "Этот блок управления подаёт воду в капельные линии, идущие к каждому мешку для выращивания. В него входят фильтр для задержки осадка, манометр, запорные клапаны для отключения отдельных участков трубопровода и два бака с удобрением, оснащённые эжектором Вентури, который дозирует жидкое удобрение прямо в поливную воду по мере её прохождения, так что каждое растение получает воду и питание за один проход.",
        "С глянцевыми заострёнными овальными листьями, растущими супротивными парами, этот сеянец — Малу Мирис, сорт болгарского перца (Capsicum annuum). Он выращивается здесь в мешке, заполненном кокосовым волокном, с капельной трубкой, уже подведённой к его основанию.",
        "Этот сеянец — Аму Мирис, сорт перца чили (Capsicum annuum). На этой ранней стадии его листья почти неотличимы от листьев болгарского перца, поскольку оба относятся к одному виду, выращиваемому для разных целей.",
        "Название Наи Мирис на сингальском языке означает «кобра-перец». Это Capsicum chinense, отдельный и обычно более острый вид, чем другие перцы поместья, близкий к таким сортам, как хабанеро и Scotch Bonnet.",
        "Более округлые, слегка восковатые на вид листья этого сеянца принадлежат Геди Гова, что буквально означает «шаровидная капуста». Это обычная кочанная капуста (Brassica oleracea var. capitata), высаженная здесь задолго до того, как начнёт формироваться кочан.",
        "Красновато-фиолетовые листья этого сеянца принадлежат Атху Гова, что буквально означает «ветвистая капуста». Это более рыхлый, некочанный сорт капусты (Brassica oleracea var. acephala), в отличие от шаровидного типа.",
        "Эти широкие, сердцевидные листья принадлежат Бончи, стручковой фасоли (Phaseolus vulgaris). Как только она начинает расти, это вьющаяся культура, с совсем иной формой роста по сравнению с низкорослыми овощами вокруг неё.",
        "Этот крошечный сеянец, всё ещё на стадии семядолей, — Раабу, редис (Raphanus sativus). Это одна из самых быстрых культур в тоннеле, готовая к сбору всего за 25–35 дней.",
        "Морковь (Daucus carota subsp. sativus) можно узнать уже на этой ранней стадии по тонким, похожим на траву первым листьям, совершенно непохожим на широкие семядольные листья остальных овощей, выращиваемых здесь.",
        "Лук-порей (Allium ampeloprasum) появляется в виде одного тонкого листа-стрелки и относится к самым медленным культурам тоннеля по времени созревания. Ему требуется до 90–120 дней с момента посадки.",
        "Два широких округлых семядольных листа этого сеянца — семядоли салатного огурца (Cucumis sativus), которые появляются до его настоящих, более сердцевидных листьев.",
        "Этот сеянец помидора уже показывает характерные для растения лопастные, зубчатые, слегка опушённые листья. Помидор — именно та культура, которая упоминалась ранее как пример перехода с распыления на капельный полив после начала плодоношения.",
      ],
      duration: "5 минут",
    },
    "mushroom-hut": {
      name: "Грибной домик",
      sectionHeadings: ["Другой вид сада", "Сорта вёшенки", "Субстрат и мицелий", "Цикл сбора урожая", "Питательная ценность и польза для здоровья"],
      sectionBodies: [
        "Наряду с чаем, специями и фруктами поместье содержит отдельный грибной домик. Это небольшое затенённое строение, построенное для поддержания тёмных, прохладных и влажных условий, необходимых грибам для плодоношения, в отличие от солнца и циркуляции воздуха, для которых выращивается остальная часть садов.",
        "В домике выращивают несколько сортов вёшенки: американскую вёшенку (Pleurotus pulmonarius), бутанскую, чёрную и розовую вёшенку (Pleurotus djamor), а также гриб абалон (Pleurotus cystidiosus). Все они растут на субстрате из соломы или опилок, а не в почве, и относятся к роду Pleurotus. Узнать их можно по веерообразным или похожим на раковину устрицы шляпкам, растущим плотными, налегающими друг на друга гроздьями прямо из субстрата, а не на отдельных ножках. Главное, что отличает один сорт от другого, — это цвет: от белого и серого до розового, золотистого или почти чёрного. Мягкий, слегка пикантный вкус при этом остаётся практически одинаковым у всех сортов.",
        "Выращивание начинается с самого субстрата: соломы или опилок, того же материала, на котором основан недорогой подход поместья, использующий отходы. Его замачивают и пастеризуют горячим паром, чтобы уничтожить конкурирующие плесени и бактерии, а затем смешивают с посевным мицелием (зерном, полностью заселённым грибным мицелием) и упаковывают в перфорированные пакеты или блоки. Затем эти блоки на две-три недели оставляют в темноте для «периода обрастания», в течение которого белый мицелий распространяется по всему субстрату. Только после этого тёмные, прохладные и влажные условия домика запускают плодоношение.",
        "Выращивание грибов таким образом — недорогой и устойчивый способ для хозяйства диверсифицироваться за пределы основной культуры, часто с использованием сельскохозяйственных отходов, таких как солома, в качестве субстрата для выращивания. Одна партия субстрата даёт несколько урожаев в течение нескольких недель. Обычно один блок даёт от двух до четырёх таких волн плодоношения с интервалом примерно в одну-две недели, при этом первая волна оказывается самой обильной, а последующие постепенно становятся меньше. Грибы не срезают, а аккуратно откручивают целыми гроздьями, поскольку оставшиеся пеньки могут загнить и вызвать заражение, которое испортит следующую волну.",
        "На собственных табличках поместья отмечена питательная ценность каждого сорта: американская вёшенка, розовая вёшенка и гриб абалон описаны как богатые белком, витаминами и антиоксидантами, с пользой для иммунитета, здоровья сердца и пищеварения, снижения воспаления и уровня холестерина.",
      ],
      duration: "5 минут",
    },
  },
  de: {
    ...
    withering: {
      name: "Welken",
      sectionHeadings: ["Beladen der Tröge", "Der 12-Stunden-Zyklus", "Feuchtigkeitsverlust", "Warum Welken wichtig ist"],
      sectionBodies: [
        "Das vom Feld ankommende Blatt wird zunächst gewogen, um die Tagesernte zu erfassen. Dann wird es dünn auf langen Trögen ausgebreitet, die 85 Fuß lang und 6 Fuß breit sind. Jeder Trog ist mit einem 46-Zoll-Ventilator mit 960 U/min ausgestattet, der Luft durch das Blatt presst und so stetig Feuchtigkeit entzieht. Das Anwesen nennt dies den 'modernen' Welkeprozess. Bei einer Beladungsrate von etwa 2,5 kg pro Quadratfuß fasst ein Trog etwa 1.250 kg frisches Blatt.",
        "Das Blatt bleibt 12 Stunden im Trog und wird alle 4 Stunden von Hand gewendet. Während dieses Zyklus wird 3 bis 4 Stunden lang von unten Wärme zugeführt, und ein letztes Mal wird es aufgelockert, bevor es am frühen Morgen ausgeladen wird. Das gewelkte Blatt wird erneut gewogen, bevor es zum Rollen weitergeht.",
        "Aus je 100 kg frischem Blatt werden etwa 43 kg Wasser entfernt. Das wird überprüft, indem 2,5 kg Blatt in einer ein Quadratfuß großen Kiste getestet werden. Am Ende hat das Blatt eine Feuchtigkeit von etwa 57 bis 58 % und ist welk und geschmeidig, bereit zum Rollen, ohne zu zerbrechen.",
        "Das Welken gleicht die Feuchtigkeit im Blatt aus und lässt natürliche Enzyme die chemischen Veränderungen beginnen, die später Aroma und Farbe des Tees erzeugen.",
      ],
      duration: "Diese Phase dauert volle 12 Stunden, wobei das Blatt alle 4 Stunden gewendet wird.",
    },
    rolling: {
      name: "Rollen",
      sectionHeadings: ["Verdrehen und Aufbrechen des Blattes", "Der erste Rollgang", "Der zweite Rollgang", "Warum Rollen wichtig ist"],
      sectionBodies: [
        "Welkes Blatt durchläuft Rollmaschinen, die die Zellwände des Blattes verdrehen und aufbrechen, so die darin eingeschlossenen Säfte und Öle freisetzen und das Blatt in kleine Partikel zerlegen. Hier entstehen die Form des Blattes und die Kraft des Tees.",
        "Zwei Beschickungswalzen nehmen jeweils 280 kg welkes Blatt auf. Der erste Rollgang dauert 20 Minuten bei 30 bis 32 U/min. Die entstehenden Doolies (gerollte Blattpartikel) laufen über ein Förderband zu einem mit Drahtgeflecht ausgestatteten Rollbrecher, der die kleinsten Partikel als 'ersten Dool' durchfallen lässt, der zur Fermentation weitergeht.",
        "Blatt, das nicht durchfällt, wandert über das Förderband zu zwei sekundären Walzen, mit einer um 12 bis 14 % reduzierten Zuführung gegenüber den ursprünglichen 280 kg. Hier folgt ein zweiter, weitere 20 Minuten dauernder Rollgang bei 32 U/min, wonach der 'zweite Dool' ohne Siebung direkt zur Fermentation geht.",
        "Das Aufbrechen der Zellwände setzt Enzyme dem Sauerstoff aus und leitet die Oxidation ein. Wie stark und wie lange das Blatt gerollt wird, bestimmt Stärke und Aussehen des fertigen Tees. Der erste von mehreren Durchgängen, ein sanftes 'Vorroll', soll die verdrehten Partikel mit dem eigenen Saft des Blattes überziehen, bevor kräftigeres Rollen folgt.",
      ],
      duration: "Zwei Rollgänge zu je 20 Minuten (insgesamt 40 Minuten): der erste bei 30 bis 32 U/min, der zweite bei 32 U/min.",
    },
    fermentation: {
      name: "Fermentation (Oxidation)",
      sectionHeadings: ["Oxidation des Blattes", "Sauberkeit des Raumes", "Von Grün zu Kupferbraun", "Präzises Timing", "Schwarzer, grüner, weißer Tee und Oolong"],
      sectionBodies: [
        "Gerolltes Blatt, sowohl der erste als auch der zweite Dool, wird in einem kühlen, feuchten Raum ausgebreitet, der bei etwa 25 bis 30 °C gehalten wird, und 2,5 Stunden lang zur Oxidation stehen gelassen. Ein Luftbefeuchter hält die Luft dabei feucht genug, damit die Reaktion ablaufen kann.",
        "Der Raum muss vollständig sauber bleiben, frei von Bakterien, stehendem Wasser oder altem Blatt, da Bakterien die Enzymaktivität stoppen und Farbe, Stärke und Aufgussqualität verderben würden.",
        "Innerhalb dieser 2,5 Stunden verfärbt sich das grüne Blatt allmählich kupferbraun und entwickelt vollständig Farbe, Körper und Aroma des Schwarztees. Das geschieht ausschließlich durch natürliche Oxidation, ohne Farbstoffe oder Zusätze.",
        "Dies ist der wichtigste Schritt für den Geschmack. Das Timing muss präzise beurteilt werden: zu wenig, und der Tee ist dünn, zu viel, und er wird flach und fade.",
        "Genau das unterscheidet auch die hier hergestellten Teesorten. Schwarzer Tee durchläuft Welken, Rollen, Fermentation, dann Darren. Grüner Tee lässt die Fermentation vollständig aus, durchläuft nur Welken, Rollen und Trocknen, und ist milder: Er wird meist wegen seines sanfteren, fast medizinischen Charakters getrunken. Weißer Tee lässt sowohl Welken als auch Fermentation aus und geht direkt vom frischen Blatt zum Dämpfen und Trocknen über. Oolong-Tee wird in dieser Fabrik nicht hergestellt. Der Guide beschreibt ihn als eine dem Schwarztee ähnliche fermentierte Sorte, die durch wiederholtes Dämpfen und Handbürsten statt maschinelles Rollen entsteht.",
      ],
      duration: "Die Fermentation dauert für den ersten wie den zweiten Dool jeweils 2,5 Stunden, bevor das Blatt zum Trocknen weitergeht.",
    },
    drying: {
      name: "Trocknen (Darren)",
      sectionHeadings: ["Einfahrt in den Trockner", "6 Abschnitte, 21 Minuten", "Das richtige Gleichgewicht finden", "Das Ergebnis", "Frische bewahren", "Eine Kette von Bedingungen"],
      sectionBodies: [
        "Bevor es in den Trockner gelangt, wird das fermentierte Blatt aufgelockert. Danach durchläuft es Heißlufttrockner: durch Brennholz erzeugte Wärme wird von einem Hauptventilator angesaugt, wobei die Eintrittstemperatur bei 120 bis 130 °C gehalten wird. Diese Trockner stoppen die Oxidation sofort und halten den Charakter des Tees fest.",
        "Im Inneren ist der Trockner in 6 Abschnitte unterteilt. Das Blatt fällt vom obersten Tablett durch diese Abschnitte auf die unteren Tabletts, wobei es sich beim Vorwärts- und Rückwärtsbewegen umwälzt, und verlässt den Trockner nach genau 21 Minuten. Dann sollte die Austrittstemperatur etwa 80 °C (rund 140 bis 150 °F) betragen.",
        "Genau diese Kombination, 120–130 °C Eintritt, 80 °C Austritt, 21 Minuten, ist es, worauf der Trockner ausgelegt ist, und sie sorgt dafür, dass das Blatt richtig schwarz wird. Sinkt die Eintrittstemperatur unter 120 °C, kommt das Blatt unzureichend getrocknet und bräunlich heraus, selbst wenn die Austrittstemperatur durch eine dünnere Blattschicht 100 °C erreicht. Steigt dagegen die Eintrittstemperatur auf 150 °C, während die Austrittstemperatur auf 75 °C fällt, wird das Blatt stattdessen matschig und wie gedünstet.",
        "Bei den richtigen Einstellungen kommt am anderen Ende das bekannte schwarze, knackige Teeblatt heraus. Besucher spüren die Hitze sofort: Es ist einer der heißesten Bereiche auf dem Fabrikboden.",
        "Das Darren stoppt die chemischen Veränderungen genau im richtigen Moment und reduziert die Feuchtigkeit von etwa 45–50 % auf etwa 2–3 %, was für die Frischhaltung des Tees bei Lagerung und Versand unerlässlich ist.",
        "Gewicht und Qualität des fertigen Tees hängen aber nicht allein vom Trockner ab. Sie ergeben sich aus einer ganzen Kette von Bedingungen: hochwertiges grünes Blatt, richtiges Welken, korrekter Herstellungsprozess, erfolgreiche Fermentation (der entscheidendste Schritt von allen) und schließlich die richtige Verarbeitung im Trockner.",
      ],
      duration: "Das Blatt durchläuft den Trockner in genau 21 Minuten, nach einem eng eingehaltenen Zyklus aus Eintritts- und Austrittstemperatur.",
    },
    sorting: {
      ...
      sectionHeadings: [
        ...
        "Marktklassen und Sortennamen",
        "Ausbeute und Ertragsquote",
      ],
      sectionBodies: [
        ...
        "Diese Sortiermaschine umfasst einen Farbseparator mit einem Kamerasystem, das Partikel wie Blatt und Blüte unterscheiden kann und alles Fremde herausfiltert. Da sich die Produktion in Sri Lanka auf schwarzen Tee konzentriert, entfernt genau dieser Schritt verbleibende braune, nicht vollständig fermentierte Partikel und Stängelfragmente. Siebe mit unterschiedlicher Maschenweite, gemessen an der Anzahl quadratischer Löcher pro Zoll, von grob mit 3 Löchern pro Zoll bis fein mit 60, trennen das Blatt in mehreren Durchgängen weiter nach Partikelgröße in etwa 21 verschiedene Sorten. Ein elektrostatischer Separator gibt verbleibenden braunen Stängelfragmenten auf Walzen eine statische Ladung, sodass sie ausgesondert werden können, und eigene Maschinen entfernen Ausschusstee vollständig aus der Charge. Nichts von dieser Farbe oder diesem Charakter stammt von Farbstoffen oder Zusätzen, sondern allein von der natürlichen Verarbeitung.",
        ...
        "Neben der Position am Trieb und der Maschenweite wird sortierter Tee auch in drei Marktkategorien eingeteilt: First Mark, Second Mark und Third Mark. Zu den hier hergestellten Hauptsorten zählen OPA, OP, OP1, BOP1, FBOP, FF1, FF, Pekoe, Pekoe1, BOP Special, BOPA, FF Extra Special, Extra Special 1 und FF Special, während geringerwertiger Tee in Kategorien wie 1A, BM, BP, FNGS und BT fällt.",
        "Im Schnitt ergeben 100 kg grünes Blatt etwa 21,5 kg fertigen, verpackten schwarzen Tee, eine Ausbeute von rund 21,5 %, wobei der Rest durch Feuchtigkeitsverdunstung und beim Sortieren entfernten Ausschuss verloren geht. Nach den Standard-Testrichtlinien des Tea Board werden dafür etwa 4,65 kg grünes Blatt pro kg fertigem Tee benötigt. Die Fabrik strebt an, mindestens 65 % der Produktion in den Hauptsorten zu erzielen, Ausschusstee auf höchstens 20 % zu begrenzen, während der Rest auf Second Mark und ähnliche Kategorien entfällt.",
      ],
      duration: "Das Sortieren läuft kontinuierlich auf Maschinen und dauert etwa 30 bis 45 Minuten pro Charge.",
    },
    packing: {
      name: "Verpacken",
      sectionHeadings: ["Wiegen, Verkosten und Versiegeln", "Die Sorten im Verpackungsbereich", "Beutelgewicht nach Sorte", "Von der Fabrik zur Auktion", "Warum luftdichte Verpackung wichtig ist", "Sorten passend zu den Märkten"],
      sectionBodies: [
        "Sortierter Tee wird gewogen, zur Qualitätsprüfung verkostet und in feuchtigkeitsdichte, mit Aluminiumfolie ausgekleidete Verpackungen versiegelt, bedruckt mit dem Namen und Logo der Fabrik, ihrer Registrierungsnummer, dem Ceylon-Tee-Symbol und dem Lebensmittelsicherheitszertifikat.",
        "Im Verpackungsbereich stehen mehrere fertige Sorten nebeneinander: Dust (hauptsächlich für Teebeutel), OP1, Pekoe, BOP (Broken Orange Pekoe) und OP (Orange Pekoe). OP1 und OP sind besonders in arabischsprachigen Märkten beliebt, und Pekoe ist in Russland und im Iran gut bekannt. Die seltenste und wertvollste hier hergestellte Sorte ist SFOPE1, eine 'Flowery Tippy'-Sorte, benannt nach den blassen, silbrigen Spitzenstücken, die sie von ganz oben im Trieb enthält. Sie wird eher für ihren feinen Charakter als für ihre Stärke geschätzt.",
        "Auch das Beutelgewicht variiert je nach Sorte: Ein Beutel fester Größe fasst deutlich mehr von einer feinen, dichten Sorte wie Dust (etwa 58kg) als von einer voluminöseren, blattreicheren Sorte wie OPE (etwa 30kg).",
        "Meist übergibt die Fabrik ihren fertigen Tee an Teemakler in Colombo, darunter Namen wie Forbes & Walker, Akbar Brothers und Finlays, die ihn zur Colombo-Teeauktion bringen, wo Käufer ihn erwerben und oft nach weiterer Wertschöpfung unter eigener Marke exportieren. Direktexport unter dem Namen Athukorala selbst kommt ebenfalls vor, aber hauptsächlich für gelegentliche Sonderbestellungen von Käufern im Ausland.",
        "Luftdichte Verpackung schützt den Tee vor Feuchtigkeit und Gerüchen und bewahrt den Geschmack, der in jeder vorherigen Phase des Prozesses sorgfältig aufgebaut wurde.",
        "Verschiedene Länder wollen unterschiedliche Dinge von ihrem Tee. Manche bevorzugen eine kräftige, herbe Tasse, andere nicht, weshalb abgestimmt darauf ist, aus welchem Teil des Triebs eine Sorte stammt, passend zum Geschmack jedes Exportmarkts. Arabischsprachige Märkte etwa fragen gezielt Sorten nach, die zuhause nicht Teil der alltäglichen Trinkgewohnheiten sind. Der Verkauf hauptsächlich über Makler statt Direktexport bedeutet, dass externe Käufer üblicherweise den weiteren internationalen Versand und das Branding übernehmen.",
      ],
    ...
    "artisanal-tea-grades": {
      ...
      sectionBodies: [
        ...
        "Die arbeitsintensivsten hier hergestellten Artikel sind zwei Kleinserien-Spezialitäten: Flower Chakra, ein von Hand gerollter Ring aus schwarzem Tee, für den etwa zwei Triebe verwendet werden, und Tea Coin. Eine Person kann pro Tag nur etwa 25 Gramm von einem der beiden von Hand rollen, weshalb sie so teuer sind und meist als Souvenir von wohlhabenden Teeliebhabern gekauft werden statt für den Alltagsgebrauch. Der Großteil der handgemachten Produktion, einschließlich Purple Tea, wird auf Bestellung in kleinen Chargen hergestellt statt fortlaufend in großen Mengen.",
      ],
      duration: "5 Minuten",
    },
    "pahatharata-manufacturing": {
      name: "Teeherstellung (Pahatharata)",
      sectionHeadings: ["Verarbeitung im Tiefland gegenüber dem Hochland", "CTC: Crush-Tear-Curl"],
      sectionBodies: [
        "Tiefland-Plantagen ('Pahatharata') wie diese verarbeiten Tee anders als Hochlandplantagen weiter im Landesinneren. Der eigene Herstellungsprozess des Anwesens, der zuvor auf dieser Tour Schritt für Schritt gezeigt wurde, bestätigt das: Das Rollen zerkleinert das Blatt hier zu kleinen, dichten Partikeln, statt es zu einem ganzen orthodoxen Blatt zu rollen. Das ist das Kennzeichen der CTC-Verarbeitung (Crush-Tear-Curl). Diese Station fasst kurz zusammen, was dieser Tiefland-Ansatz für Pflücken, Verarbeitung und Sortierung bedeutet.",
        "CTC (Crush-Tear-Curl) verarbeitet das Blatt maschinell zu kleinen, dichten Granulaten und erzeugt den kräftigen, spritzigen Charakter, für den Tiefland-Ceylon-Tee bekannt ist. Es sind dieselben Blattpartikel, die zuvor an den Stationen Rollen und Sortieren auf dieser Tour zu sehen waren.",
      ],
    ...
    "clonal-tea": {
      name: "Klontee",
      heroTagline: "Einige der eigenen ausgeschilderten Klonsorten-Versuchspflanzen des Anwesens.",
      sectionHeadings: ["Klontee", "Warum Klontee Mehr Kostet", "Teekultursorten", "TRI 3069", "TRI 3055", "TRI 4053", "TRI 2022", "TRI 2043", "TRI 4006", "TRI 3025"],
      sectionBodies: ["Einige der begehrtesten Tees stammen von Klonpflanzen: Stecklingen, die von einem einzigen außergewöhnlichen Mutterstrauch vermehrt wurden, statt aus Samen gezogen zu werden. Diese Station erklärt, wie Klontee vermehrt wird und warum er einen Höchstpreis erzielen kann. Das Tea Research Institute (TRI) züchtet Klonsorten für unterschiedliche Zwecke. Manche für Trocken- oder Regentoleranz und höheren Ertrag, andere wegen der charakteristischen rot- oder grünspitzigen jungen Blattknospen, die für handgefertigte Spezialtees geschätzt werden. Weit verbreitete, ertragreiche Sorten wie TRI 2023, TRI 2025 und TRI 2026 sind sowohl auf Plantagen als auch in Hausgärten verbreitet, während die selteneren Spezialsorten meist auf größeren Plantagen für handgefertigten Tee statt für den alltäglichen Anbau wachsen. Auch wenn diese TRI-Nummern wie Jahreszahlen wirken, sind es lediglich zugewiesene Kennungen und nicht das Jahr, in dem die jeweilige Sorte veröffentlicht wurde.", "Die Klonvermehrung bewahrt die exakten Eigenschaften einer erlesenen Pflanze. Die besten Klontees werden oft nur in sehr begrenzten Mengen produziert. Knospen der speziellen rot- oder grünspitzigen Sorten werden von Hand für seltene Qualitäten wie Golden Tips und Silver Tips gepflückt, wofür Plantagen in der Regel rund 100 Rupien mehr pro Kilogramm zahlen als für normal gepflücktes Blatt.", "Das Anwesen bevorzugt ertragreiche, schädlingsresistente Klone, die vom Tea Research Institute (TRI) gezüchtet wurden, darunter Sorten der TRI-2000-Serie und T65, wegen ihrer zuverlässigen Leistung im lokalen Boden und Klima. TRI 4049 ist eine ertragsstarke Teesorte, die vom Tea Research Institute of Sri Lanka entwickelt wurde und sich ideal für mittlere bis niedrige Höhenlagen eignet. Es handelt sich um eine vegetativ vermehrte Sorte von Camellia sinensis, bekannt für kräftiges Wachstum, dunkelgrüne Blätter und Widerstandsfähigkeit gegen verbreitete Krankheiten. TRI 4049 liefert helle, aromatische Aufgüsse, die sowohl für CTC- als auch für orthodoxen Tee geeignet sind, was sie zu einer beliebten Wahl für Nachpflanzungen und den kommerziellen Anbau macht.", "Ein paar Reihen weiter steht diese beschilderte Pflanze: TRI 3069, eine weitere beim Tea Research Institute registrierte Sorte, die neben TRI 4049 gepflanzt wird, damit die Baumschule vergleichen kann, wie sich jede Sorte auf genau diesem Boden entwickelt, bevor eine davon großflächig ausgepflanzt wird.", "TRI 3055 steht in denselben Testreihen. Jede Sortenparzelle hier ist einzeln markiert und wird verfolgt. So sammelt die Fabrik ihre eigenen Belege dafür, welche Sorten sich wirklich lohnen auszuweiten, statt sich nur auf den Ruf zu verlassen.", "TRI 4053 rundet die hier gezeigten Sorten-Testparzellen ab. Mehrere registrierte Sorten nebeneinander wachsen zu lassen, jede deutlich beschildert, ist auf dem Anwesen gängige Praxis, bevor eine Sorte einem ganzen Feld zugewiesen wird.", "TRI 2022 steht im selben Sorten-Testabschnitt, sein Schild wird ebenso markiert und verfolgt wie das seiner Nachbarn. Mehrere registrierte Sorten nebeneinander auf demselben Boden anzubauen, ist die Art, wie die Baumschule echte Belege dafür sammelt, welche hier am besten abschneidet.", "TRI 2043 steht etwas weiter hinten in der Reihe. Wie die anderen nummerierten Pflanzen auf dieser Testparzelle wird es angebaut und beobachtet, bevor über eine großflächige Auspflanzung entschieden wird.", "TRI 4006 ist ein weiterer Eintrag im selben Sortenversuch. Die Baumschule hält jede Sorte an ihrem eigenen, klar markierten Platz, was es leicht macht, das Wachstum über die Zeit zu verfolgen und zu vergleichen.", "TRI 3025 markiert den Rand der Testparzelle, neben einem zweisprachigen Schild „Tee-Sorten\" und einer datierten Pflanzgedenktafel. Zusammen bilden die hier beschilderten Pflanzen die tatsächliche Aufzeichnung der Baumschule darüber, welche Sorten es wert sind, ausgeweitet zu werden."],
      duration: "5-10 Minuten",
    },
    "shade-management": {
    ...
    "handmade-factory": {
      ...
      heroTagline: "Ein Tablett-Gestell aus der eigenen Handgemachten Teefabrik des Anwesens.",
      sectionHeadings: [
        "Handgefertigte Teefabrik",
        "Welken und Rollen",
        "Grüntee, Trocknen und das Gebäude",
        "Was handgemachten Tee unterscheidet",
      ],
      sectionBodies: [
        "Neben der maschinellen Produktion wird ein Teil des Tees auf jeder Stufe komplett von Hand verarbeitet, ohne Maschinen. Die meisten der besonderen handwerklichen Sorten des Anwesens, darunter Golden Tip, Silver Tip, White Tea, Purple Tea, Pink Tea, Vita Glow, handgebundener Blooming Tea und Blütenmischungen wie Lotustee, werden hier statt in der Hauptfabrik für schwarzen Tee fertiggestellt. (Vita Glow wird aus der Knospe des grünen Triebs hergestellt, anders als der violette Trieb von Golden und Silver Tip.)",
        "Das Welken geschieht hier auf natürliche Weise, in einem kleinen Trog, überwacht mit einem Hygrometer, ohne die Zwangsbelüftungsventilatoren der Hauptfabrik, und die Feuchtigkeit sinkt vor dem Rollen auf etwa 50%. Das Blatt wird dann von Hand auf einem Tablett gerollt, beurteilt eher nach Gefühl als nach der Uhr. Für größere Großbestellungen übernehmen kleine mechanische Roller denselben Zweck, da das Handrollen allein mit der Menge nicht mithalten kann.",
        "Eine eigene Grünteemaschine, die das Blatt dämpft und presst, übernimmt diese Sorte separat, ist derzeit aber wegen einer kleinen Reparatur außer Betrieb. Nach dem Dämpfen ruht das Blatt draußen, um die Oberflächenfeuchtigkeit zu verlieren, wird dann von Hand gerollt, um Farbe, Kraft, Aroma und Geschmack herauszuarbeiten, bevor es abschließend durch den Trockner läuft. Die Trockenzeit variiert von einem kleinen Produzenten zum nächsten. Der kleine Ofen des Gebäudes dient auch als Backofen für teebasierte Snacks wie Kekse. Das Trocknen, das direkt nach dem Rollen erfolgt, läuft hier auf zwei separaten Trocknern: einer traditionell holzbefeuert, einer elektrisch. Beide haben jeweils 34 Tabletts und werden auch für andere Kleinserienprodukte wie Kardamom, Nelken und Ingwer genutzt.",
        "Diese Station zeigt, was handgemachten Tee von der Fabriklinienproduktion unterscheidet. Handrollen und Handverarbeitung ergeben kleinere Chargen mit mehr Variation zwischen den Blättern, oft geschätzt für ihren unverwechselbaren Charakter. Handgemachte Tees fallen in zwei große Kategorien. Die erste sind fermentierte, schwarztee-artige Sorten (erkennbar an ihrer schwarzen Farbe), zu denen trotz seines ungewöhnlichen Ausgangstriebs auch Purple Tea zählt. Die zweite sind nicht fermentierte wie White Tea, die die Fermentation vollständig überspringen und direkt zum Trocknen übergehen.",
      ],
      duration: "10 Minuten",
    },
    "tea-center": {
      name: "Teezentrum",
      heroTagline: "Besucher, die Tee an der eigenen Verkostungstheke des Anwesens probieren.",
      sectionHeadings: [
        "Teezentrum",
        "Aufbrühen und Verkostung",
        "Alltägliche Sorten und Supermarkt-Tee",
        "Die feinsten Sorten und Exportmärkte",
        "Kräuter- und Gewürzmischungen",
      ],
      sectionBodies: [
        "Die Tour endet im Teezentrum, wo ein Verkostungsteam jede Charge prüft, nicht nur hier, sondern auch schon früher, immer wenn Tee die Hauptfabrik in Richtung lokaler Markt verlässt oder zum Verpacken eintrifft, auf Fehler wie Über- oder Unterfermentierung oder Anbrennen. Alles, was vor dem Verkauf durchfällt, wird aussortiert, und selbst kleinere Probleme werden notiert, um die künftige Produktion zu informieren. Hier ausgestellt sind zwei weitere Kleinserien-Spezialitäten aus der Handwerksfabrik: Flower Chakra und Tea Coin.",
        "Solche handwerklichen Tees können bis zu dreimal mit etwa 80°C heißem statt kochendem Wasser erneut aufgegossen werden. Jeder Aufguss dauert etwa 2, 5 und 8 Minuten mit frischem heißem Wasser, anders als gewöhnlicher schwarzer Tee, der nur einmal mit sprudelnd kochendem Wasser aufgegossen wird. Die Verkostung folgt einer Methode ähnlich der Weinverkostung: zuerst riechen, dann schlürfen, um den Tee mit einem Luftzug über die Geschmacksknospen an den Zungenrändern zu ziehen. Das Team spuckt statt zu schlucken, während es bis zu 50 Tees am Tag verkostet, doch Besucher sind eingeladen, das Probierte tatsächlich zu trinken. Richtig aufgebrühter schwarzer Tee braucht etwa 2,5 bis 3 Minuten zugedeckte Ziehzeit, nicht nur ein kurzes Eintauchen.",
        "BOPF und BOPF Special sind die Sorten, die die meisten Sri Lanker alltäglich trinken. Dust und Dust 1, die niedrigsten Sorten, gehen nie in den Export und landen typischerweise, mit zugesetzter Farbe, in billigen Supermarkt-Teebeuteln, weshalb die Farbe sofort beim Eintauchen erscheint statt erst nach richtigem Ziehen. Ein Großteil dessen, was im Supermarkt verkauft wird, ist zudem mit billigerem, anderswo eingekauftem Tee gemischt, anders als die eigenen, unvermischten Herkunftstees des Anwesens.",
        "Die feinsten Sorten, also FF Extra Special, FF Special, FBOP, stammen aus nur etwa 1% der Ernte, dem unreifsten Teil der Knospe, und werden für rund 25 Dollar pro 100g verkauft. Voluminösere Sorten wie OPA und OP liegen eher bei 5-6 Dollar pro 200g. Die in den feinsten Sorten sichtbaren weißen Spitzenpartikel geben zusätzliche Süße und gehen meist an europäische Käufer, während arabische Märkte eher eine kräftigere Tasse bevorzugen.",
        "Neben Tee mischt das Anwesen seinen eigenen grünen und schwarzen Tee (etwa 90%) mit lokalen Kräutern (etwa 10%), Gotukola, Heenbovitiya und Moringa, und stellt aus eigenen Gewürzen Masala-, Zimt- und Ingwer-Gewürztees her. Das Masala-Rezept verwendet sieben sri-lankische Gewürze, darunter Kardamom, Zimt, Ingwer und schwarzen Pfeffer (der Rest bleibt geheim). Für Milchtee wird Masala oder BOP empfohlen.",
      ],
      duration: "10-15 Minuten",
    },
    "coconut-field": {
    ...
    introduction: {
      ...
      sectionBodies: [
        ...
        "Die Geschichte des Ceylon-Tees begann 1867 auf einer 19 Acres großen Fläche auf dem Loolecondera-Anwesen in Kandy. Gepflanzt wurde sie von dem schottischen ehemaligen Kaffeepflanzer James Taylor im Rahmen eines Diversifizierungsexperiments. Im Laufe der Jahre wuchs sie zu Sri Lankas sieben Tee-Anbauregionen heran: Kandy, Uva, Ruhuna (Süden), Udapussellawa, Nuwara Eliya, Dimbula und Sabaragamuwa.",
    ...
    "jaggery-palm": {
      ...
      sectionBodies: [
        "Die Kithul-Palme (Caryota urens), auch Fischschwanzpalme genannt, ist eine hohe, elegante Palme, die in Sri Lanka, Indien und anderen tropischen asiatischen Ländern vorkommt und für ihre vielseitige Nutzung geschätzt wird. Ihr bemerkenswertestes Produkt ist der süße Saft, der aus ihren Blütenstielen gewonnen und zu traditionellem Kithul-Sirup und Jaggery eingekocht wird. Der Baum ist außerdem für seinen kräftigen, faserigen Stamm und seine schönen fischschwanzförmigen Blätter bekannt.",
        "Ein kurzer Blick auf die Palme selbst:",
    ...
    pruning: {
      ...
      sectionBodies: ["Vor dem Beschneiden lässt das Anwesen den Teestrauch bewusst eine Zeit lang weiterwachsen, anstatt ihn sofort zurückzuschneiden. So darf sich der neue Trieb vollständig entwickeln, damit der Strauch Nährstoffe aufbaut. Erst wenn dieser Aufbau abgeschlossen ist, beginnt das Beschneiden, bei dem der Strauch auf etwa Kniehöhe zurückgeschnitten wird. Schilder vor Ort kennzeichnen dies als zwei benannte Stufen: Der erste Kreuzschnitt (Palamu Haras Kapuma) bringt den Strauch auf etwa 9-12 Zoll (22-30 cm) herunter, und der zweite Kreuzschnitt (Dewana Haras Kapuma) lässt ihn wieder auf etwa 16-18 Zoll (40-45 cm) heranwachsen. Den Strauch zunächst wachsen zu lassen, anstatt ihn nach einem festen Zeitplan unabhängig vom Zustand zu beschneiden, gibt der Pflanze Zeit, Nährstoffe nach oben zu leiten, bevor sie zurückgeschnitten wird."],
      duration: "5 Minuten",
    },
    polytunnel: {
      name: "Folientunnel & Gewächshaus",
      heroTagline: "Der eigene Folientunnel des Anwesens, in dem bereits die ersten Kulturen wachsen.",
      sectionHeadings: ["Über den Folientunnel & das Gewächshaus", "Bewässerung", "Erntezeiten", "Fertigations-Steuerkopf", "Malu Miris (Paprika)", "Amu Miris (Chili)", "Nai Miris", "Gedi Gowa (Kohl)", "Athu Gowa (Rotkohl)", "Bonchi (Grüne Bohnen)", "Raabu (Radieschen)", "Karotte", "Lauch", "Gurke", "Tomaten-Sämling"],
      sectionBodies: [
        "Neben einem bereits vorhandenen Gewächshaus baut das Anwesen einen Folientunnel für den Anbau einer eigenen Gemüsevielfalt. Darunter sind Scotch-Bonnet- und grüne Chilischoten, Paprika, Gewürzgurken, Tomaten, Gemüsepaprika, Kohl, Radieschen, grüne Bohnen, Lauch, Frühlingszwiebeln und Karotten. Sobald er fertiggestellt ist, können Gäste, die in der Villa oder Cabana wohnen, das hier angebaute Gemüse selbst frisch ernten. Die halbdurchlässige Abdeckung eines Folientunnels funktioniert ähnlich wie das Glas eines Gewächshauses: Sie hält die Wärme und lässt Licht durch, während sie starken Monsunregen und viele verbreitete Schädlinge fernhält. Diese Bedingungen ermöglichen es dem Anwesen, Gemüse anzubauen, das im offenen Feld nicht zuverlässig gedeihen würde, und die Anbausaison über das hinaus zu verlängern, was offene Beete erlauben.",
        "Jede Kultur folgt ihrem eigenen Bewässerungsplan, der je nach Wachstumsphase zwischen Tröpfchenbewässerung an der Wurzel und Beregnung von oben wechselt. Die Beregnung unterstützt vor allem die frühe Keimung und Kühlung bei Kulturen wie Karotten und Radieschen, während die Tröpfchenbewässerung zur Hauptmethode wird, sobald die Fruchtbildung einsetzt, etwa bei Tomaten. Die beiden Methoden erfüllen unterschiedliche Zwecke. Die Beregnung hält Luft und Bodenoberfläche feucht genug, damit junge Samen und Sämlinge anwachsen können. Sobald die Pflanzen größer sind, bringt die Tröpfchenbewässerung das Wasser direkt zur Wurzelzone, verbraucht insgesamt weniger Wasser und hält die Blätter trocken, um das Risiko von Pilzkrankheiten zu verringern.",
        "Die Zeit bis zur ersten Ernte variiert je nach Kultur stark, von nur 25-35 Tagen bei Radieschen bis zu 90-120 Tagen bei Lauch, sodass im Folientunnel fast zu jedem Zeitpunkt der Saison etwas erntereif sein sollte. Eine derart breite Mischung aus schnell und langsam reifendem Gemüse nebeneinander anzubauen, ist selbst eine bewusste Entscheidung, da sie die Ernten auf natürliche Weise staffelt, statt alles gleichzeitig reif oder abgeerntet werden zu lassen.",
        "Dieser Steuerkopf versorgt die Tropfleitungen, die zu jedem Anzuchtsack führen. Dazu gehören ein Filter zum Zurückhalten von Sediment, ein Manometer, Absperrventile zum Isolieren einzelner Leitungsabschnitte und zwei Düngertanks mit einem Venturi-Injektor, der flüssigen Dünger direkt in das durchfließende Bewässerungswasser dosiert, sodass jede Pflanze in einem Arbeitsgang gegossen und gedüngt wird.",
        "Mit seinen glänzenden, spitz zulaufenden, gegenständig wachsenden ovalen Blättern ist dieser Sämling Malu Miris, eine Paprikasorte (Capsicum annuum). Er wächst hier in einem mit Kokosfaser gefüllten Anzuchtsack, mit einem Tropfschlauch, der bereits bis zu seiner Basis verlegt ist.",
        "Dieser Sämling ist Amu Miris, eine Chilisorte (Capsicum annuum). In diesem frühen Stadium sind seine Blätter kaum von denen der Paprika zu unterscheiden, da beide zur selben Art gehören, die für unterschiedliche Zwecke angebaut wird.",
        "Der Name Nai Miris bedeutet auf Singhalesisch 'Kobra-Chili'. Es ist Capsicum chinense, eine andere und in der Regel schärfere Art als die übrigen Paprika- und Chilisorten des Anwesens, verwandt mit Sorten wie Habanero und Scotch Bonnet.",
        "Die runderen, leicht wachsig wirkenden Blätter dieses Sämlings gehören zu Gedi Gowa, was wörtlich 'Kugelkohl' bedeutet. Es ist der gewöhnliche kopfbildende Kohl (Brassica oleracea var. capitata), der hier lange gepflanzt wird, bevor sich überhaupt ein Kopf bildet.",
        "Die rötlich-violetten Blätter dieses Sämlings gehören zu Athu Gowa, was wörtlich 'Astkohl' bedeutet. Es ist eine lockerere, kopflose Kohlsorte (Brassica oleracea var. acephala) statt der kugelförmigen Art.",
        "Diese breiten, herzförmigen Blätter gehören zu Bonchi, grünen Bohnen (Phaseolus vulgaris). Sobald sie erst einmal in Fahrt kommt, ist es eine Kletterkultur, mit einem ganz anderen Wuchsverhalten als das niedrig wachsende Gemüse ringsum.",
        "Dieser winzige Sämling, noch im Keimblattstadium, ist Raabu, Radieschen (Raphanus sativus). Es ist eine der schnellsten Kulturen im Folientunnel bis zur Ernte, bereits nach nur 25-35 Tagen erntereif.",
        "Karotte (Daucus carota subsp. sativus) ist schon in diesem frühen Stadium an ihren dünnen, grasähnlichen ersten Blättern zu erkennen, die sich deutlich von den breiten Keimblättern der anderen hier angebauten Gemüsesorten unterscheiden.",
        "Lauch (Allium ampeloprasum) erscheint als einzelne dünne Klinge und zählt zu den langsamsten Kulturen im Folientunnel bis zur Ernte. Er benötigt bis zu 90-120 Tage ab der Pflanzung.",
        "Die beiden breiten, runden Keimblätter dieses Sämlings sind die Keimblätter einer Salatgurke (Cucumis sativus), bevor ihre echten, eher herzförmigen Blätter erscheinen.",
        "Dieser Tomaten-Sämling zeigt bereits die für die Pflanze typischen gelappten, gezähnten, leicht behaarten Blätter. Tomate ist genau die Kultur, die zuvor als Beispiel dafür genannt wurde, dass sie nach Beginn der Fruchtbildung von Beregnung auf Tröpfchenbewässerung umgestellt wird.",
      ],
      duration: "5 Minuten",
    },
    "mushroom-hut": {
      name: "Pilzhütte",
      sectionHeadings: ["Ein anderer Garten", "Die Austernpilzsorten", "Substrat und Pilzbrut", "Erntezyklus", "Nährwert und gesundheitliche Vorteile"],
      sectionBodies: [
        "Neben Tee-, Gewürz- und Obstkulturen unterhält das Anwesen eine eigene Pilzhütte. Es ist ein kleines, schattiges Gebäude, das gebaut wurde, um die dunklen, kühlen, feuchten Bedingungen zu halten, die Pilze zum Fruchten brauchen, statt der Sonne und Luftzirkulation, für die der Rest der Gärten angebaut wird.",
        "Die Hütte ist darauf ausgelegt, mehrere Austernpilzsorten anzubauen: Amerikanischer Austernpilz (Pleurotus pulmonarius), Bhutan-Austernpilz, Schwarzer Austernpilz, Rosa Austernpilz (Pleurotus djamor) und Abalonepilz (Pleurotus cystidiosus). Alle werden auf einem Stroh- oder Sägemehlsubstrat statt in Erde kultiviert. Alle gehören zur Gattung Pleurotus und sind an ihren fächer- oder austernförmigen Hüten zu erkennen, die dicht gedrängt und übereinanderliegend direkt aus dem Substrat wachsen statt an einzelnen Stielen. Die Farbe ist dabei das Hauptmerkmal, das eine Sorte von der anderen unterscheidet, von Weiß und Grau bis Rosa, Gold oder nahezu Schwarz, während der milde, leicht würzige Geschmack bei allen weitgehend gleich bleibt.",
        "Der Anbau beginnt beim Substrat selbst: Stroh oder Sägemehl, dasselbe Material, auf dem der kostengünstige, abfallbasierte Ansatz des Anwesens beruht. Dieses wird eingeweicht und mit Hitze pasteurisiert, um konkurrierende Schimmelpilze und Bakterien abzutöten, bevor es mit Pilzbrut vermischt wird, also Getreide, das vollständig vom Pilzmyzel besiedelt ist, und in perforierte Beutel oder Blöcke gepackt wird. Diese Blöcke bleiben dann zwei bis drei Wochen lang im Dunkeln für den sogenannten 'Durchwuchs', währenddessen sich das weiße Myzel über das gesamte Substrat ausbreitet. Erst danach lösen die dunklen, kühlen, feuchten Bedingungen der Hütte die Fruchtbildung aus.",
        "Pilze auf diese Weise anzubauen ist eine kostengünstige, nachhaltige Möglichkeit für einen Betrieb, sich über seine Hauptkultur hinaus zu diversifizieren, wobei oft landwirtschaftliche Abfälle wie Stroh als Anbaumedium genutzt werden. Eine einzige Substratcharge liefert über mehrere Wochen mehrere Ernten. Jeder Block liefert in der Regel zwei bis vier solcher Erntewellen im Abstand von etwa ein bis zwei Wochen, wobei die erste Welle am ergiebigsten ausfällt und der Ertrag danach abnimmt. Die Pilze werden im Ganzen als Büschel abgedreht statt geschnitten, da zurückbleibende Stümpfe faulen und eine Kontamination begünstigen können, die die nächste Welle verderben würde.",
        "Die eigenen Schilder des Anwesens heben den Nährwert jeder Sorte hervor: Amerikanischer Austernpilz, Rosa Austernpilz und Abalonepilz werden jeweils als reich an Protein, Vitaminen und Antioxidantien beschrieben, mit Vorteilen für die Immunfunktion, Herz- und Verdauungsgesundheit, die Verringerung von Entzündungen und die Senkung des Cholesterinspiegels.",
      ],
      duration: "5 Minuten",
    },
  },
  es: {
    ...
    withering: {
      name: "Marchitado",
      sectionHeadings: ["Carga de los Canales", "El Ciclo de 12 Horas", "Pérdida de Humedad", "Por Qué el Marchitado es Importante"],
      sectionBodies: [
        "La hoja recién llegada del campo se pesa primero para registrar la cosecha del día. Luego se extiende en capas finas sobre largos canales de 85 pies de largo y 6 pies de ancho. Cada canal está equipado con un ventilador de 46 pulgadas que gira a 960 RPM para forzar el paso del aire y eliminar gradualmente la humedad, lo que la finca llama el proceso de marchitado 'moderno'. Con una carga de aproximadamente 2,5 kg por pie cuadrado, un canal contiene alrededor de 1.250 kg de hoja verde.",
        "La hoja permanece en el canal durante 12 horas y se voltea a mano cada 4 horas. Dentro de ese ciclo recibe calor desde abajo durante 3 a 4 horas, y se afloja una última vez antes de descargarse a primera hora de la mañana. La hoja marchitada se pesa de nuevo antes de pasar al enrollado.",
        "Por cada 100 kg de hoja verde se eliminan cerca de 43 kg de agua, una cifra comprobada probando 2,5 kg de hoja en una caja de un pie cuadrado. La hoja queda así con una humedad de entre el 57% y el 58%, volviéndose flexible y maleable, lista para el enrollado sin quebrarse.",
        "El marchitado uniforma la humedad en toda la hoja y permite que las enzimas naturales comiencen los cambios químicos que más tarde crearán el aroma y el color del té.",
      ],
      duration: "Esta etapa dura 12 horas completas, con la hoja volteada cada 4 horas.",
    },
    rolling: {
      name: "Enrollado",
      sectionHeadings: ["Retorcer y Romper la Hoja", "El Primer Enrollado", "El Segundo Enrollado", "Por Qué el Enrollado es Importante"],
      sectionBodies: [
        "Las hojas marchitas pasan por máquinas enrolladoras que retuercen y rompen las paredes celulares de la hoja, liberando los jugos y aceites encerrados en su interior y reduciéndola a pequeñas partículas. Aquí es donde se forman la forma de la hoja y la fuerza del té.",
        "Dos rodillos de carga reciben cada uno 280 kg de hoja marchita. El primer enrollado dura 20 minutos a 30-32 RPM. Las 'doolies' resultantes (partículas de hoja enrollada) pasan por una cinta transportadora hasta un separador equipado con malla metálica, que deja caer las partículas más pequeñas como 'primer dool', enviado a fermentación.",
        "La hoja que no pasa sube por la cinta hasta dos rodillos secundarios, con la carga reducida entre un 12% y un 14% respecto a los 280 kg originales. Allí se realiza un segundo enrollado de otros 20 minutos a 32 RPM, tras el cual el 'segundo dool' va directo a fermentación sin cribar.",
        "Al romper las paredes celulares, las enzimas quedan expuestas al oxígeno, iniciando la oxidación. La intensidad y duración del enrollado determinan la fuerza y el aspecto final del té. La primera de varias pasadas, un suave enrollado de 'preacondicionamiento', busca cubrir las partículas retorcidas con el propio jugo de la hoja antes de un enrollado más firme.",
      ],
      duration: "Dos enrollados de 20 minutos cada uno (40 minutos en total): el primero a 30-32 RPM, el segundo a 32 RPM.",
    },
    fermentation: {
      name: "Fermentación (Oxidación)",
      sectionHeadings: ["Oxidación de la Hoja", "Manteniendo la Sala Limpia", "De Verde a Cobrizo", "Un Momento Preciso", "Té Negro, Verde, Blanco y Oolong"],
      sectionBodies: [
        "La hoja enrollada, tanto el primer como el segundo dool, se extiende en una sala fresca y húmeda, mantenida a unos 25-30°C, y se deja oxidar durante 2,5 horas. Un humidificador mantiene el aire lo bastante húmedo para que ocurra la reacción.",
        "La sala debe permanecer totalmente limpia, sin bacterias, agua estancada ni hoja vieja, ya que las bacterias detienen la actividad enzimática y arruinarían el color, la fuerza y la calidad del licor.",
        "En esas 2,5 horas, la hoja verde se va tornando de un color cobrizo a medida que desarrolla plenamente el color, cuerpo y aroma propios del té negro. Todo ello ocurre por oxidación natural, sin tintes ni aditivos.",
        "Este es el paso más importante para el sabor. El tiempo debe calcularse con precisión: muy poco y el té queda ligero, demasiado y se vuelve plano y sin vida.",
        "Esto es también lo que diferencia los tipos de té que se elaboran aquí. El té negro sigue marchitado, enrollado, fermentación y luego tostado. El té verde se salta la fermentación por completo, pasando solo por marchitado, enrollado y secado, y es más suave: se bebe sobre todo por su carácter más ligero y casi medicinal. El té blanco se salta tanto el marchitado como la fermentación, pasando directamente de la hoja fresca al vapor y al secado. Esta fábrica no produce té Oolong, que el guía describe como un tipo fermentado similar al té negro, elaborado mediante vaporizado y cepillado a mano repetidos en lugar de enrollado mecánico.",
      ],
      duration: "La fermentación dura 2,5 horas tanto para el primer como para el segundo dool, antes de que la hoja pase al secado.",
    },
    drying: {
      name: "Secado (Tostado)",
      sectionHeadings: ["Entrada al Secador", "Seis Secciones, 21 Minutos", "Encontrando el Equilibrio Correcto", "El Resultado", "Preservando la Frescura", "Una Cadena de Condiciones"],
      sectionBodies: [
        "Antes de entrar en el secador, la hoja fermentada se afloja. Luego pasa por secadoras de aire caliente, con calor generado por leña y aspirado por un ventilador principal, que mantienen una temperatura de entrada de entre 120 y 130 °C. Esto detiene la oxidación al instante y fija el carácter del té.",
        "En su interior, el secador está dividido en 6 secciones. La hoja cae de la bandeja superior a través de estas secciones hasta las bandejas inferiores, dando vueltas hacia adelante y hacia atrás a medida que avanza, y sale exactamente a los 21 minutos. En ese momento, la temperatura de salida debería rondar los 80 °C (aproximadamente 140-150 °F).",
        "Es esa combinación, 120-130 °C de entrada, 80 °C de salida, 21 minutos, la que el secador está diseñado para mantener, y la que hace que la hoja se vuelva correctamente negra. Si la temperatura de entrada baja de 120 °C, la hoja sale poco seca y parduzca, incluso si la de salida llega a 100 °C al adelgazar la capa de hoja. Si la de entrada sube a 150 °C mientras la de salida cae a 75 °C, la hoja termina blanda y como estofada en lugar de bien seca.",
        "Con los ajustes correctos, lo que sale al otro extremo es la conocida hoja de té negra y crujiente. Los visitantes notan el calor de inmediato: es una de las zonas más calurosas de toda la planta.",
        "El tostado detiene los cambios químicos en el momento exacto y reduce la humedad de aproximadamente un 45-50% a un 2-3%, algo esencial para que el té se mantenga fresco durante el almacenamiento y el transporte.",
        "Sin embargo, el peso y la calidad finales del té no dependen solo del secador. Dependen de toda una cadena de condiciones: hoja verde de buena calidad, un marchitado correcto, un proceso de fabricación adecuado, una fermentación exitosa (el paso más crucial de todos) y, por último, un procesamiento correcto en el secador.",
      ],
      duration: "La hoja pasa por el secador en exactamente 21 minutos, siguiendo un ciclo de temperaturas de entrada y salida estrictamente controlado.",
    },
    sorting: {
      ...
      sectionHeadings: [
        ...
        "Marcas de Mercado y Nombres de Grados",
      ...
      sectionBodies: [
        ...
        "Esa maquinaria de clasificación incluye un separador de color con un sistema de cámara capaz de distinguir partículas como hoja y flor, filtrando todo lo que no corresponde. Como la producción de Sri Lanka se centra en el té negro, este paso es lo que elimina las partículas marrones que no se fermentaron del todo y los fragmentos de tallo restantes. Tamices de distintos tamaños de malla, medidos por el número de agujeros cuadrados por pulgada, desde 3 agujeros por pulgada en el extremo más grueso hasta 60 en el más fino, separan aún más la hoja en unos 21 grados distintos por tamaño de partícula en varias pasadas. Un separador electrostático da una carga estática en rodillos a los fragmentos de tallo marrón restantes para poder apartarlos, y máquinas dedicadas retiran por completo el té de desecho del lote. Nada de este color o carácter proviene de tintes o aditivos, solo del procesamiento natural.",
        ...
        "Además de la posición en el brote y el tamaño de malla, el té clasificado también se divide en tres categorías de mercado: First Mark, Second Mark y Third Mark. Los grados principales producidos aquí incluyen OPA, OP, OP1, BOP1, FBOP, FF1, FF, Pekoe, Pekoe1, BOP Special, BOPA, FF Extra Special, Extra Special 1 y FF Special, mientras que el té de menor valor entra en categorías como 1A, BM, BP, FNGS y BT.",
        "En promedio, 100 kg de hoja verde producen unos 21,5 kg de té negro terminado y empacado, un rendimiento de aproximadamente el 21,5%, y el resto se pierde por evaporación de humedad y por el desecho eliminado durante la clasificación. Las pautas estándar de prueba del Tea Board sitúan esto en unos 4,65 kg de hoja verde necesarios por cada kg de té elaborado. La fábrica busca que al menos el 65% de la producción sea de los grados principales, mantener el té de grado inferior en un máximo del 20%, y que el resto quede dentro de Second Mark y categorías similares.",
      ],
      duration: "La clasificación se realiza de forma continua con maquinaria y tarda entre 30 y 45 minutos por lote.",
    },
    packing: {
      name: "Empaquetado",
      sectionHeadings: ["Pesaje, Cata y Sellado", "Los Grados en la Zona de Empaquetado", "Peso de las Bolsas por Grado", "De la Fábrica a la Subasta", "Por Qué el Sellado Hermético es Importante", "Ajustando los Grados a los Mercados"],
      sectionBodies: [
        "El té clasificado se pesa, se cata para comprobar su calidad, y se sella en envases a prueba de humedad forrados con papel de aluminio, impresos con el nombre y el logotipo de la fábrica, su número de registro, el Símbolo del Té de Ceilán y el certificado de seguridad alimentaria.",
        "En la zona de empaquetado hay varios grados terminados uno junto a otro: Dust (destinado sobre todo a bolsitas de té), OP1, Pekoe, BOP (Broken Orange Pekoe) y OP (Orange Pekoe). El OP1 y el OP son especialmente populares en los mercados de habla árabe, y el Pekoe es bien conocido en Rusia e Irán. El grado más raro y valioso que se produce aquí es el SFOPE1, un grado 'Flowery Tippy' llamado así por los pálidos fragmentos plateados de punta que contiene, procedentes de la parte más alta del brote. Se aprecia más por su carácter delicado que por su fuerza.",
        "El peso de las bolsas también varía según el grado: una bolsa de tamaño fijo contiene mucho más de un grado fino y denso como el Dust (unos 58kg) que de un grado más voluminoso y con más hoja como el OPE (unos 30kg).",
        "En su mayoría, la fábrica entrega su té terminado a corredores de té de Colombo, entre ellos nombres como Forbes & Walker, Akbar Brothers y Finlays, que lo llevan a la Subasta de Té de Colombo, donde los compradores lo adquieren y a menudo lo exportan bajo su propia marca tras añadirle más valor. También se da la exportación directa bajo el propio nombre Athukorala, pero sobre todo para pedidos especiales ocasionales de compradores extranjeros.",
        "El empaquetado hermético protege el té de la humedad y los olores, preservando el sabor construido con cuidado en cada etapa anterior del proceso.",
        "Los distintos países quieren cosas distintas de su té. Algunos prefieren una taza fuerte y astringente, otros no, así que qué parte del brote da origen a un grado se ajusta efectivamente al paladar de cada mercado de exportación. Los mercados de habla árabe, por ejemplo, solicitan específicamente grados que no forman parte de los hábitos cotidianos de consumo en sus países. Vender sobre todo a través de corredores en lugar de exportar directamente significa que los compradores externos suelen encargarse del envío internacional posterior y de la marca.",
      ],
    ...
    "ginger-turmeric": {
      ...
      sectionBodies: [
        "El jengibre (Zingiber officinale) es una hierba perenne de la familia de las Zingiberáceas. Se cultiva ampliamente en regiones tropicales y subtropicales, incluidas Sri Lanka e India. Lo más importante es el rizoma subterráneo, usado como especia y medicina durante siglos. El jengibre prospera en climas cálidos y húmedos con suelo rico y bien drenado. Desempeña además un papel central en la cocina tradicional y la medicina herbal de muchas culturas.",
    ...
    "artisanal-tea-grades": {
      ...
      sectionHeadings: [
        ...
        "White Tea y Purple Tea",
        "Pink Tea y Vita Glow",
        "Mezclas y Rareza",
        "Flower Chakra y Tea Coin",
      ],
      sectionBodies: [
        "Además del té negro estándar, la finca también produce tés artesanales raros, elaborados a mano. La mayoría se terminan a mano en la Fábrica de Té Hecho a Mano, separada de la línea principal de té negro. Por eso estos grados también se llaman 'tés artesanales': se acercan más a un oficio que a la producción en masa. Muchos de ellos provienen de un brote de color púrpura distintivo, el TRI 2043, un clon del Tea Research Institute. Su color proviene de la antocianina. Es una variedad de bajo rendimiento, por lo que no todas las fincas de Sri Lanka la cultivan.",
        "El Golden Tip se elabora con el brote ya completamente maduro de ese arbusto, mientras que el Silver Tip usa el brote más pálido y aún inmaduro del mismo arbusto. Sus finos pelillos plateados le dan al grado su nombre y un ligero brillo. Solo se pesa el brote, se marchita unas horas y se seca. Es un proceso más simple que el de los grados totalmente fermentados, y da como resultado una taza sutilmente dulce, floral y de textura aterciopelada.",
        "El White Tea toma dos hojas y media del brote y se salta el marchitado por completo. Se cuece al vapor y se seca en lugar de fermentarse y enrollarse como el té negro. El resultado es un sabor muy ligeramente procesado y naturalmente dulce, con un licor pálido, una textura aterciopelada, un aroma floral y una riqueza natural en antioxidantes. El Purple Tea usa la misma cosecha pero, a diferencia del White Tea, pasa por todo el proceso del té negro: marchitado, enrollado y fermentación. Esto da un sabor más cercano al oolong: más ligero que el té negro pero menos vegetal que el té verde, relativamente bajo en cafeína y naturalmente rico en antioxidantes.",
        "El Pink Tea comienza prácticamente incoloro como el White Tea, y solo se vuelve rosado tras añadir un par de gotas de lima. El cítrico desencadena un cambio de color en el mismo pigmento de antocianina que le da su tono al brote. También combina bien con un par de gotas de miel, se recomienda servido como té helado, y también es naturalmente rico en antioxidantes. El Vita Glow, en cambio, se elabora con el brote del arbusto verde ordinario en lugar del púrpura.",
        "La finca también crea mezclas de valor añadido, como té infusionado con flores de loto, además del Blooming Tea atado a mano a partir de los mismos brotes. Estos grados artesanales se recolectan y procesan en cantidades mucho menores que el té negro estándar, lo que los hace más raros y apreciados. Pregunte en el mostrador de degustación para probarlos. Para la mayoría de estos grados se puede usar tanto el brote de tono morado como el brote verde normal, aunque la finca suele usar el morado.",
        "Los artículos que más trabajo requieren aquí son dos especialidades de lote pequeño: el Flower Chakra, un anillo de té negro enrollado a mano que usa unos dos brotes cada uno, y el Tea Coin. Una sola persona puede enrollar a mano solo unos 25 gramos de cualquiera de los dos al día, por lo que son tan caros, y se compran sobre todo como recuerdo por parte de entusiastas del té adinerados, más que para el consumo diario. La mayor parte de la producción hecha a mano, incluido el Purple Tea, se elabora por encargo en lotes pequeños en lugar de producirse continuamente en grandes cantidades.",
      ],
      duration: "5 minutos",
    },
    "pahatharata-manufacturing": {
      name: "Fabricación de Té (Pahatharata)",
      sectionHeadings: ["Procesamiento en Tierras Bajas frente a Tierras Altas", "CTC: Crush-Tear-Curl"],
      sectionBodies: [
        "Las fincas de tierras bajas ('Pahatharata') como esta procesan el té de forma diferente a las fincas de tierras altas más hacia el interior. El propio proceso de fabricación de la finca, cubierto etapa por etapa anteriormente en este recorrido, lo confirma: aquí el enrollado reduce la hoja a partículas pequeñas y densas en lugar de enrollarla en hoja entera al estilo ortodoxo. Ese es el sello distintivo del procesamiento CTC (Crush-Tear-Curl). Esta parada es un resumen rápido de lo que ese enfoque de tierras bajas significa para la recolección, el procesamiento y la clasificación.",
        "El CTC (Crush-Tear-Curl, o triturar-desgarrar-enrollar) procesa mecánicamente la hoja en gránulos pequeños y densos, produciendo el carácter fuerte y vivo por el que se conoce el té ceilanés de tierras bajas. Son las mismas partículas de hoja vistas antes en las paradas de enrollado y clasificación de este recorrido.",
      ],
    ...
    "clonal-tea": {
      name: "Té Clonal",
      heroTagline: "Algunas de las propias plantas de prueba de variedades clonales, señalizadas por la finca.",
      sectionHeadings: ["Té Clonal", "Por Qué el Té Clonal Cuesta Más", "Cultivares de Té", "TRI 3069", "TRI 3055", "TRI 4053", "TRI 2022", "TRI 2043", "TRI 4006", "TRI 3025"],
      sectionBodies: ["Algunos de los tés más codiciados provienen de plantas clonales: esquejes propagados de un único arbusto madre excepcional en lugar de crecer a partir de semilla. Esta parada explica cómo se propaga el té clonal y por qué puede alcanzar un precio elevado. El Instituto de Investigación del Té (TRI) desarrolla variedades clonales para distintas necesidades. Algunas para la tolerancia a la sequía o a la lluvia y un mayor rendimiento, otras por los característicos brotes jóvenes de punta roja o verde, apreciados en los tés artesanales elaborados a mano. Las variedades de alto rendimiento y ampliamente plantadas, como TRI 2023, TRI 2025 y TRI 2026, son comunes tanto en las plantaciones como en los huertos domésticos, mientras que las variedades especiales más raras se cultivan sobre todo en grandes plantaciones para té artesanal en lugar de para el cultivo cotidiano. Aunque parezcan años, estos números TRI son simplemente identificadores asignados, no el año en que se lanzó cada variedad.", "La propagación clonal conserva las características exactas de una planta destacada. Los mejores tés clonales suelen producirse en cantidades muy limitadas. Los brotes de las variedades especiales de punta roja o verde se recogen a mano para grados raros como los Golden Tips y los Silver Tips, y las plantaciones suelen pagar unas 100 rupias más por kilogramo que por las hojas recogidas normalmente.", "La finca prefiere clones de alto rendimiento y resistentes a plagas desarrollados por el Instituto de Investigación del Té (TRI), incluidas variedades de la serie TRI 2000 y T65, por su desempeño fiable en el suelo y el clima locales. TRI 4049 es un cultivar de té de alto rendimiento desarrollado por el Instituto de Investigación del Té de Sri Lanka, ideal para elevaciones medias y bajas. Es una variedad de Camellia sinensis propagada vegetativamente, conocida por su crecimiento vigoroso, hojas de un verde intenso y resistencia a enfermedades comunes. TRI 4049 produce licores brillantes y aromáticos, adecuados tanto para té CTC como ortodoxo, lo que lo convierte en una opción popular para la replantación y el cultivo comercial.", "Unas filas más allá, esta planta señalizada es TRI 3069, otro cultivar registrado por el Instituto de Investigación del Té, sembrado junto a TRI 4049 para que el vivero pueda comparar cómo se comporta cada uno en este suelo en particular antes de plantarlos a gran escala.", "TRI 3055 se encuentra en las mismas filas de ensayo. Cada parcela de cultivar aquí está etiquetada y monitoreada individualmente, que es como la fábrica reúne sus propias pruebas sobre qué variedades realmente vale la pena expandir, en lugar de guiarse solo por la reputación.", "TRI 4053 completa las parcelas de ensayo de cultivares que se ven aquí. Mantener varias variedades registradas creciendo una junto a otra, cada una claramente etiquetada, es una práctica habitual en la finca antes de destinar un cultivar a un campo completo.", "TRI 2022 está plantado en la misma sección de ensayo de cultivares, con su señal etiquetada y monitoreada igual que sus vecinos. Cultivar varias variedades registradas una junto a otra en el mismo terreno es cómo el vivero construye pruebas reales de cuáles rinden mejor aquí.", "TRI 2043 se encuentra un poco más adelante en la fila. Como las demás plantas numeradas de esta parcela de ensayo, se está cultivando y monitoreando antes de tomar cualquier decisión sobre plantarla a gran escala.", "TRI 4006 es una entrada más en el mismo ensayo de cultivares. El vivero mantiene cada variedad en su propio lugar claramente marcado, lo que facilita seguir y comparar su crecimiento con el tiempo.", "TRI 3025 marca el borde de la parcela de ensayo, junto a un cartel bilingüe de 'Cultivares de Té' y una placa de plantación con fecha. Juntas, las plantas etiquetadas aquí forman el registro real del vivero sobre qué cultivares vale la pena expandir."],
      duration: "5-10 minutos",
    },
    "shade-management": {
    ...
    "handmade-factory": {
      ...
      heroTagline: "Un estante de bandejas de la propia Fábrica de Té Artesanal de la finca.",
      sectionHeadings: [
        "Fábrica de Té Artesanal",
        "Marchitado y Enrollado",
        "Té Verde, Secado y el Edificio",
        "Qué Distingue al Té Hecho a Mano",
      ],
      sectionBodies: [
        "Además de la producción mecanizada, parte del té se procesa por completo a mano en cada etapa, sin máquinas. La mayoría de los grados especiales artesanales de la finca, entre ellos el Golden Tip, Silver Tip, White Tea, Purple Tea, Pink Tea, Vita Glow, el Blooming Tea atado a mano y mezclas infusionadas con flores como el té de loto, se terminan aquí en lugar de en la fábrica principal de té negro. (El Vita Glow se hace con el brote del arbusto verde, a diferencia del brote púrpura del Golden y Silver Tip.)",
        "Aquí el marchitado ocurre de forma natural, en una pequeña artesa controlada con un higrómetro, sin los ventiladores de aire forzado de la fábrica principal, reduciendo la humedad a alrededor del 50% antes de enrollar. Luego la hoja se enrolla a mano sobre una bandeja, juzgando al tacto más que por el reloj. Para pedidos al por mayor más grandes, pequeños rodillos mecánicos cumplen la misma función, ya que el enrollado manual por sí solo no puede seguir el ritmo del volumen.",
        "Una máquina dedicada al té verde, que cuece al vapor y prensa la hoja, se encarga de ese grado por separado, aunque actualmente está fuera de servicio por una reparación menor. Tras el vapor, la hoja reposa afuera para perder la humedad superficial, luego se enrolla a mano para resaltar su color, fuerza, aroma y sabor, antes de un último paso por el secador. El tiempo de secado varía de un pequeño productor a otro. El pequeño horno del edificio también sirve como panadería para bocadillos a base de té, como galletas. El secado, que se hace justo después de enrollar, se realiza aquí en dos secadores independientes: uno tradicional a leña, otro eléctrico. Cada uno tiene capacidad para 34 bandejas, y ambos también se usan para otros productos de lotes pequeños, como cardamomo, clavo de olor y jengibre.",
        "Esta parada explica qué distingue al té hecho a mano de la producción en línea de fábrica. El enrollado y el procesamiento manuales producen lotes más pequeños con más variación entre hojas, a menudo apreciados por su carácter distintivo. Los tés hechos a mano se dividen en dos categorías amplias. La primera son los fermentados, al estilo del té negro (identificables por su color negro), que incluyen al Purple Tea pese a su inusual brote de origen. La segunda son los no fermentados como el White Tea, que se saltan la fermentación por completo y van directo al secado.",
      ],
      duration: "10 minutos",
    },
    "tea-center": {
      name: "Centro del Té",
      heroTagline: "Visitantes degustando té en el propio mostrador de cata de la finca.",
      sectionHeadings: [
        "Centro del Té",
        "Preparación y Cata",
        "Grados Cotidianos y Té de Supermercado",
        "Los Grados Más Finos y Mercados de Exportación",
        "Mezclas de Hierbas y Especias",
      ],
      sectionBodies: [
        "El recorrido termina en el centro de té, donde un panel de cata revisa cada lote, no solo aquí sino también antes, cada vez que el té sale de la fábrica principal hacia el mercado local o llega para el empaquetado, en busca de defectos como sobrefermentación, subfermentación o quemado. Descarta todo lo que no pasa la prueba antes de venderse, y anota incluso problemas menores para orientar la producción futura. Aquí se exhiben otras dos especialidades de lote pequeño de la fábrica artesanal, el Flower Chakra y el Tea Coin.",
        "Estos tés artesanales se pueden reinfusionar hasta tres veces usando agua calentada a unos 80°C en lugar de hirviendo. Cada infusión dura unos 2, 5 y 8 minutos respectivamente, con agua caliente fresca cada vez, a diferencia del té negro estándar, que se prepara una sola vez a punto de ebullición completo. La cata sigue un método parecido a la cata de vinos: primero se huele, luego se sorbe ruidosamente para llevar el té por las papilas gustativas en los laterales de la lengua con una bocanada de aire. El panel escupe en lugar de tragar mientras cata hasta 50 tés al día, aunque a los visitantes se les invita a beber de verdad lo que prueban. El té negro bien preparado necesita unos 2,5 a 3 minutos de infusión cubierta, no solo una inmersión rápida.",
        "El BOPF y el BOPF Special son los grados que la mayoría de los sri lankeses bebe a diario. El Dust y el Dust 1, los grados más bajos, nunca se envían a los mercados de exportación. Suelen terminar, con colorante añadido, en las bolsitas de té baratas de supermercado, por lo que el color aparece nada más sumergirlas, en vez de tras una infusión adecuada. Buena parte de lo que se vende en los supermercados también está mezclado con té más barato comprado en otro lugar, a diferencia de los tés de origen único y sin mezclar de la propia finca.",
        "Los grados más finos, FF Extra Special, FF Special, FBOP, provienen solo de cerca del 1% de la cosecha, la parte más inmadura del brote, y se venden por unos 25 dólares los 100g. Los grados más voluminosos, como OPA y OP, rondan en cambio los 5-6 dólares los 200g. Las partículas blancas de punta visibles en los mejores grados añaden dulzor extra y en su mayoría van a compradores europeos, mientras que los mercados árabes tienden a preferir una taza más fuerte.",
        "Además del té, la finca mezcla su propio té verde y negro (alrededor del 90%) con hierbas locales (alrededor del 10%): gotukola, heenbovitiya y moringa. También elabora tés especiados de Masala, Canela y Jengibre con sus propias especias. La receta del masala usa siete especias de Sri Lanka, entre ellas cardamomo, canela, jengibre y pimienta negra (el resto se mantiene en secreto). Para el té con leche se recomienda el masala o el BOP.",
      ],
      duration: "10-15 minutos",
    },
    "coconut-field": {
    ...
    "jaggery-palm": {
      ...
      sectionBodies: [
        "La palma kithul (Caryota urens), o palma de cola de pez, es una palma alta y elegante que se encuentra en Sri Lanka, India y otros países tropicales de Asia, apreciada por su uso multipropósito. Su producto más destacado es la savia dulce extraída de sus tallos florales, que se hierve para producir la tradicional melaza y jaggery de kithul. El árbol también es conocido por su tronco fuerte y fibroso y sus hermosas hojas con forma de cola de pez.",
    ...
    pruning: {
      ...
      sectionBodies: ["Antes de la poda, la finca deja deliberadamente que el arbusto de té crezca durante un tiempo en lugar de cortarlo de inmediato. Así el nuevo crecimiento se desarrolla por completo y el arbusto acumula nutrientes. Solo una vez completada esa acumulación comienza la poda, que corta el arbusto hasta una altura aproximada de la rodilla. Los carteles del propio lugar marcan esto como dos etapas con nombre: el primer corte cruzado (Palamu Haras Kapuma) baja el arbusto a unas 9-12 pulgadas (22-30 cm), y el segundo corte cruzado (Dewana Haras Kapuma) lo deja crecer de nuevo hasta unas 16-18 pulgadas (40-45 cm). Dejar que el arbusto crezca primero, en lugar de podarlo según un calendario fijo sin importar su estado, le da a la planta tiempo para dirigir los nutrientes hacia arriba antes de ser cortada."],
      duration: "5 minutos",
    },
    polytunnel: {
      name: "Túnel de Cultivo e Invernadero",
      heroTagline: "El propio túnel de cultivo de la finca, ya con sus primeras hortalizas creciendo.",
      sectionHeadings: ["Sobre el Túnel de Cultivo y el Invernadero", "Riego", "Tiempos de Cosecha", "Cabezal de Control de Fertirriego", "Malu Miris (Pimiento)", "Amu Miris (Chile)", "Nai Miris", "Gedi Gowa (Repollo)", "Athu Gowa (Repollo Rojo)", "Bonchi (Judías Verdes)", "Raabu (Rábano)", "Zanahoria", "Puerro", "Pepino", "Plántula de Tomate"],
      sectionBodies: [
        "Junto a un invernadero ya existente, la finca está construyendo un túnel de cultivo para una variedad de verduras propias. Entre ellas hay chiles Scotch Bonnet y chiles verdes, pimiento, pepinillos, tomates, pimientos morrones, repollo, rábano, judías verdes, puerros, cebolletas y zanahorias. Una vez terminado, los huéspedes alojados en la villa o la cabaña podrán recoger ellos mismos, frescas, las verduras cultivadas aquí. La cubierta semitransparente de un túnel de cultivo funciona de forma parecida al cristal de un invernadero, reteniendo el calor y dejando pasar la luz mientras impide la entrada de las fuertes lluvias monzónicas y muchas plagas comunes. Estas condiciones permiten a la finca cultivar hortalizas que de otro modo no podría manejar de forma fiable a cielo abierto, y ampliar la temporada de cultivo más allá de lo que permiten los bancales al aire libre.",
        "Cada cultivo sigue su propia rutina de riego, alternando el riego por goteo a nivel de raíz con la nebulización aérea según la etapa de crecimiento. La nebulización ayuda sobre todo a la germinación temprana y al enfriamiento en cultivos como la zanahoria y el rábano, mientras que el goteo se convierte en el método principal una vez que comienza la fructificación en cultivos como el tomate. Los dos métodos cumplen propósitos distintos. La nebulización mantiene el aire y la superficie del suelo lo bastante húmedos para que las semillas jóvenes y los plantones arraiguen. El goteo lleva el agua directamente a la zona de las raíces una vez que las plantas son más grandes, usando menos agua en total y manteniendo las hojas secas para reducir el riesgo de enfermedades fúngicas.",
        "El tiempo hasta la primera cosecha varía mucho según el cultivo, desde apenas 25-35 días para el rábano hasta 90-120 días para el puerro, por lo que el túnel debería tener siempre algo listo para recoger en casi cualquier momento de la temporada. Cultivar una mezcla tan amplia de hortalizas de maduración rápida y lenta una junto a otra es en sí misma una decisión deliberada, ya que escalona las cosechas de forma natural en lugar de que todo esté listo o se acabe al mismo tiempo.",
        "Este cabezal de control alimenta las líneas de goteo que llegan a cada bolsa de cultivo. Incluye un filtro para retener sedimentos, un manómetro, válvulas de cierre para aislar tramos de tubería, y dos tanques de fertilizante equipados con un inyector venturi que dosifica fertilizante líquido directamente en el agua de riego a su paso, de modo que cada planta recibe agua y nutrientes en el mismo momento.",
        "Con sus hojas ovaladas brillantes y puntiagudas que crecen en pares opuestos, esta plántula es Malu Miris, una variedad de pimiento (Capsicum annuum). Se cultiva aquí en una bolsa de cultivo rellena de fibra de coco, con una línea de goteo ya instalada hasta su base.",
        "Esta plántula es Amu Miris, una variedad de chile (Capsicum annuum). En esta etapa temprana sus hojas son casi indistinguibles de las del pimiento, ya que ambos pertenecen a la misma especie cultivada para usos distintos.",
        "El nombre Nai Miris significa 'chile cobra' en cingalés. Es Capsicum chinense, una especie distinta y generalmente más picante que los otros pimientos de la finca, emparentada con variedades como el habanero y el Scotch Bonnet.",
        "Las hojas más redondeadas y de aspecto ligeramente ceroso de esta plántula pertenecen a Gedi Gowa, que significa literalmente 'repollo bola'. Es el repollo común que forma cabeza (Brassica oleracea var. capitata), plantado aquí mucho antes de que empiece a formarse su cabeza.",
        "Las hojas de tono rojo-violáceo de esta plántula pertenecen a Athu Gowa, que significa literalmente 'repollo de rama'. Es una variedad de repollo más suelta y sin cabeza (Brassica oleracea var. acephala), a diferencia del tipo que forma bola.",
        "Estas hojas anchas en forma de corazón pertenecen a Bonchi, judías verdes (Phaseolus vulgaris). Una vez que arranca es un cultivo trepador, con un hábito de crecimiento muy distinto al de las hortalizas bajas que la rodean.",
        "Esta pequeña plántula, todavía en fase de cotiledón, es Raabu, rábano (Raphanus sativus). Es uno de los cultivos más rápidos del túnel en llegar a la cosecha, listo para recoger en apenas 25-35 días.",
        "La zanahoria (Daucus carota subsp. sativus) se reconoce ya en esta etapa temprana por sus primeras hojas finas, parecidas a briznas de hierba, muy distintas de las anchas hojas cotiledóneas de las demás hortalizas cultivadas aquí.",
        "El puerro (Allium ampeloprasum) emerge como una sola hoja fina, uno de los cultivos más lentos del túnel en llegar a la cosecha. Puede tardar hasta 90-120 días desde la siembra.",
        "Las dos anchas hojas cotiledóneas redondeadas de esta plántula son las de un pepino de ensalada (Cucumis sativus), antes de que aparezcan sus hojas verdaderas, más parecidas a un corazón.",
        "Esta plántula de tomate ya muestra las hojas lobuladas, dentadas y ligeramente vellosas características de la planta. El tomate es el cultivo mencionado antes como ejemplo de los que pasan de la nebulización al riego por goteo una vez que comienza la fructificación.",
      ],
      duration: "5 minutos",
    },
    "mushroom-hut": {
      name: "Cabaña de Setas",
      sectionHeadings: ["Un Jardín Diferente", "Las Variedades de Setas Ostra", "Sustrato y Micelio", "Ciclo de Cosecha", "Valor Nutricional y Beneficios para la Salud"],
      sectionBodies: [
        "Junto con sus cultivos de té, especias y frutas, la finca mantiene una cabaña dedicada a las setas. Es una pequeña estructura sombreada construida para mantener las condiciones oscuras, frescas y húmedas que las setas necesitan para fructificar, en lugar del sol y la circulación de aire para los que se cultiva el resto de los jardines.",
        "La cabaña está preparada para cultivar varias variedades de setas ostra: Ostra Americana (Pleurotus pulmonarius), Ostra de Bután, Ostra Negra, Ostra Rosa (Pleurotus djamor) y Seta Abalón (Pleurotus cystidiosus). Todas se cultivan en un sustrato de paja o serrín en lugar de tierra, y pertenecen al género Pleurotus. Se reconocen por sus sombreros en forma de abanico u ostra que crecen en racimos densos y superpuestos directamente del sustrato en lugar de sobre tallos individuales. El color es lo que principalmente distingue una variedad de otra, desde blanco y gris hasta rosa, dorado o casi negro, mientras que el sabor suave y ligeramente sabroso se mantiene bastante similar en todas ellas.",
        "El cultivo comienza con el propio sustrato: paja o serrín, el mismo material detrás del enfoque económico y basado en residuos de la finca. Este se remoja y pasteuriza con calor para eliminar mohos y bacterias competidoras antes de mezclarlo con micelio semilla, grano completamente colonizado por el micelio del hongo, y empaquetarlo en bolsas o bloques perforados. Esos bloques se dejan luego en la oscuridad durante dos o tres semanas para la 'carrera del micelio', durante la cual el micelio blanco se extiende por todo el sustrato. Solo después, las condiciones oscuras, frescas y húmedas de la cabaña entran en acción para desencadenar la fructificación.",
        "Cultivar setas de esta manera es una forma económica y sostenible de que una finca se diversifique más allá de su cultivo principal, a menudo usando residuos agrícolas como la paja como medio de cultivo. Un solo lote de sustrato produce varias cosechas a lo largo de varias semanas. Cada bloque produce normalmente entre dos y cuatro de estas oleadas, espaciadas aproximadamente una o dos semanas entre sí, siendo la primera oleada la más abundante y disminuyendo el rendimiento después. Las setas se retuercen y se sacan en racimos enteros en lugar de cortarse, ya que los tocones que quedan pueden pudrirse y provocar una contaminación que echaría a perder la siguiente oleada.",
        "Los propios carteles de la finca destacan el valor nutricional de cada variedad: la Ostra Americana, la Ostra Rosa y la Seta Abalón se describen como ricas en proteínas, vitaminas y antioxidantes, con beneficios centrados en el apoyo inmunológico, la salud cardíaca y digestiva, la reducción de la inflamación y la disminución del colesterol.",
      ],
      duration: "5 minutos",
    },
  },
  fr: {
    ...
    withering: {
      name: "Flétrissage",
      sectionHeadings: ["Chargement des Bacs", "Le Cycle de 12 Heures", "Perte d'Humidité", "Pourquoi le Flétrissage est Important"],
      sectionBodies: [
        "La feuille arrivant des champs est d'abord pesée pour consigner la récolte du jour. Elle est ensuite étalée en fine couche sur de longs bacs de 85 pieds de long et 6 pieds de large. Chaque bac est équipé d'un ventilateur de 46 pouces tournant à 960 tr/min pour forcer l'air à travers elle et éliminer progressivement l'humidité. C'est ce que le domaine appelle le flétrissage 'moderne'. À un taux de chargement d'environ 2,5 kg par pied carré, un bac contient environ 1 250 kg de feuille verte.",
        "La feuille reste dans le bac pendant 12 heures et est retournée à la main toutes les 4 heures. Durant ce cycle, elle reçoit de la chaleur par le dessous pendant 3 à 4 heures, et elle est ameublie une dernière fois avant d'être déchargée tôt le matin. La feuille flétrie est pesée à nouveau avant de passer au roulage.",
        "Sur chaque 100 kg de feuille verte, environ 43 kg d'eau sont éliminés, une mesure vérifiée en testant 2,5 kg de feuille dans une boîte d'un pied carré. La feuille se retrouve ainsi à environ 57 à 58 % d'humidité, devenant souple et malléable, prête pour le roulage sans se briser.",
        "Le flétrissage uniformise l'humidité dans la feuille et permet aux enzymes naturelles d'amorcer les changements chimiques qui créeront plus tard l'arôme et la couleur du thé.",
      ],
      duration: "Cette étape dure 12 heures complètes, la feuille étant retournée toutes les 4 heures.",
    },
    rolling: {
      name: "Roulage",
      sectionHeadings: ["Tordre et Briser la Feuille", "Le Premier Roulage", "Le Second Roulage", "Pourquoi le Roulage est Important"],
      sectionBodies: [
        "Les feuilles flétries passent dans des machines à rouler qui tordent et brisent les parois cellulaires de la feuille, libérant les jus et huiles qui y sont enfermés et réduisant la feuille en petites particules. C'est là que se forment la forme de la feuille et la force du thé.",
        "Deux rouleaux de chargement reçoivent chacun 280 kg de feuille flétrie. Le premier roulage dure 20 minutes à 30-32 tr/min. Les 'doolies' obtenues (particules de feuille roulée) passent ensuite sur un convoyeur jusqu'à un brise-rouleau équipé d'un tamis métallique, qui laisse tomber les plus petites particules comme 'premier dool', envoyé à la fermentation.",
        "La feuille qui ne passe pas remonte le convoyeur jusqu'à deux rouleaux secondaires, avec un chargement réduit de 12 à 14 % par rapport aux 280 kg d'origine. Un second roulage y a lieu, de 20 minutes supplémentaires à 32 tr/min, après quoi le 'second dool' va directement à la fermentation sans tamisage.",
        "La rupture des parois cellulaires expose les enzymes à l'oxygène, déclenchant l'oxydation. L'intensité et la durée du roulage déterminent la force et l'aspect final du thé. Le premier de plusieurs passages, un roulage de 'préconditionnement' tout en douceur, vise à enrober les particules torsadées du propre jus de la feuille avant un roulage plus ferme.",
      ],
      duration: "Deux roulages de 20 minutes chacun (40 minutes au total) : le premier à 30-32 tr/min, le second à 32 tr/min.",
    },
    fermentation: {
      name: "Fermentation (Oxydation)",
      sectionHeadings: ["Oxydation de la Feuille", "Garder la Pièce Propre", "Du Vert au Cuivré", "Un Minutage Précis", "Thé Noir, Vert, Blanc et Oolong"],
      sectionBodies: [
        "La feuille roulée, à la fois le premier et le second dool, est étalée dans une pièce fraîche et humide maintenue à environ 25-30°C et laissée à oxyder pendant 2,5 heures. Un humidificateur maintient l'air suffisamment humide pour que la réaction se produise.",
        "La pièce doit rester parfaitement propre, sans bactéries, eau stagnante ni vieille feuille, car les bactéries stoppent l'activité enzymatique et gâteraient la couleur, la force et la qualité de la liqueur.",
        "Au cours de ces 2,5 heures, la feuille verte devient progressivement brun cuivré tout en développant pleinement la couleur, le corps et l'arôme associés au thé noir. Ce changement se fait entièrement grâce à l'oxydation naturelle, sans colorant ni additif.",
        "C'est l'étape la plus importante pour la saveur. Le minutage doit être jugé avec précision : trop peu et le thé est fade, trop et il devient plat et terne.",
        "C'est aussi ce qui distingue les types de thé fabriqués ici. Le thé noir suit flétrissage, roulage, fermentation, puis torréfaction. Le thé vert évite entièrement la fermentation, ne passant que par le flétrissage, le roulage et le séchage, et il est plus doux : il est surtout bu pour son caractère plus léger, presque médicinal. Le thé blanc évite à la fois le flétrissage et la fermentation, passant directement de la feuille fraîche à la cuisson à la vapeur puis au séchage. L'usine ne produit pas de thé Oolong, que le guide décrit comme un style fermenté proche du thé noir, obtenu par des cuissons à la vapeur et des brossages à la main répétés plutôt que par un roulage mécanique.",
      ],
      duration: "La fermentation dure 2,5 heures aussi bien pour le premier que pour le second dool, avant que la feuille ne passe au séchage.",
    },
    drying: {
      name: "Séchage (Torréfaction)",
      sectionHeadings: ["Entrée dans le Séchoir", "Six Sections, 21 Minutes", "Trouver le Bon Équilibre", "Le Résultat", "Préserver la Fraîcheur", "Une Chaîne de Conditions"],
      sectionBodies: [
        "Avant d'entrer dans le séchoir, la feuille fermentée est ameublie. Elle passe ensuite dans des séchoirs à air chaud : une chaleur produite par du bois de chauffage est aspirée par un ventilateur principal, la température d'entrée étant maintenue entre 120 et 130 °C. Ces séchoirs arrêtent instantanément l'oxydation et figent le caractère du thé.",
        "À l'intérieur, le séchoir est divisé en 6 sections. La feuille tombe du plateau supérieur à travers ces sections jusqu'aux plateaux inférieurs, culbutant vers l'avant et l'arrière au fil de sa progression, et sort après exactement 21 minutes. À ce moment, la température de sortie doit avoisiner les 80 °C (environ 140 à 150 °F).",
        "C'est cette combinaison, 120-130 °C en entrée, 80 °C en sortie, 21 minutes, que le séchoir est conçu pour maintenir, et c'est elle qui fait noircir correctement la feuille. Si la température d'entrée descend sous 120 °C, la feuille ressort mal séchée et brunâtre, même si la sortie atteint 100 °C en amincissant la couche de feuilles. Si la température d'entrée monte à 150 °C tandis que la sortie chute à 75 °C, la feuille finit détrempée et comme cuite à l'étouffée au lieu d'être bien séchée.",
        "Avec les bons réglages, ce qui en ressort est la feuille de thé noire et croustillante bien connue. Les visiteurs remarquent immédiatement la chaleur : c'est l'une des zones les plus chaudes de tout l'atelier.",
        "La torréfaction arrête les changements chimiques au moment exact et réduit l'humidité d'environ 45-50 % à environ 2-3 %, ce qui est essentiel pour que le thé reste frais pendant le stockage et l'expédition.",
        "Le poids et la qualité finaux du thé ne dépendent toutefois pas du séchoir seul. Ils résultent de toute une chaîne de conditions : une feuille verte de bonne qualité, un flétrissage correct, un procédé de fabrication adéquat, une fermentation réussie (l'étape la plus cruciale de toutes) et, enfin, un traitement correct dans le séchoir.",
      ],
      duration: "La feuille traverse le séchoir en exactement 21 minutes, selon un cycle de températures d'entrée et de sortie rigoureusement maintenu.",
    },
    sorting: {
      ...
      sectionHeadings: [
        ...
        "Marques de Marché et Noms de Qualités",
        "Rendement et Taux de Production",
      ],
      sectionBodies: [
        ...
        "Cette machine de tri comprend un séparateur de couleur avec un système de caméra capable de distinguer des particules comme la feuille et la fleur, filtrant tout ce qui n'y appartient pas. La production sri-lankaise étant centrée sur le thé noir, c'est cette étape qui élimine les particules brunes pas totalement fermentées et les fragments de tige restants. Des tamis de différentes tailles de maille, mesurées en nombre de trous carrés par pouce, allant de 3 trous par pouce pour les plus grossières jusqu'à 60 pour les plus fines, séparent encore la feuille en environ 21 qualités distinctes selon la taille des particules, sur plusieurs passages. Un séparateur électrostatique donne une charge statique sur des rouleaux aux fragments de tige brune restants afin qu'ils puissent être écartés, et des machines dédiées retirent entièrement le thé de rebut du lot. Rien de cette couleur ou de ce caractère ne vient de colorants ou d'additifs, seulement du traitement naturel.",
        ...
        "Au-delà de la position sur la pousse et de la taille de maille, le thé trié est aussi classé en trois catégories de marché : First Mark, Second Mark et Third Mark. Les principales qualités produites ici comprennent l'OPA, l'OP, l'OP1, le BOP1, le FBOP, le FF1, le FF, le Pekoe, le Pekoe1, le BOP Special, le BOPA, le FF Extra Special, l'Extra Special 1 et le FF Special, tandis que le thé de moindre valeur relève de catégories comme le 1A, le BM, le BP, le FNGS et le BT.",
        "En moyenne, 100 kg de feuille verte donnent environ 21,5 kg de thé noir fini et emballé, un rendement d'environ 21,5 %, le reste étant perdu par évaporation de l'humidité et par le rebut retiré lors du triage. Les directives d'essai standard du Tea Board situent ce chiffre à environ 4,65 kg de feuille verte nécessaires par kg de thé fabriqué. La fabrique vise au moins 65 % de la production dans les qualités principales, limite le thé de rebut à 20 % maximum, le reste relevant du Second Mark et de catégories similaires.",
      ],
      duration: "Le triage s'effectue en continu sur des machines et prend environ 30 à 45 minutes par lot.",
    },
    packing: {
      name: "Emballage",
      sectionHeadings: ["Pesée, Dégustation et Scellage", "Les Qualités sur l'Aire d'Emballage", "Le Poids des Sacs par Qualité", "De la Fabrique à la Vente aux Enchères", "Pourquoi l'Emballage Hermétique est Important", "Adapter les Qualités aux Marchés"],
      sectionBodies: [
        "Le thé classé est pesé, dégusté pour en vérifier la qualité, et scellé dans des emballages étanches à l'humidité doublés de papier aluminium, imprimés avec le nom et le logo de la fabrique, son numéro d'enregistrement, le Symbole du Thé de Ceylan, et le certificat de sécurité alimentaire.",
        "Dans la zone d'emballage, plusieurs qualités finies se côtoient : Dust (destiné surtout aux sachets), OP1, Pekoe, BOP (Broken Orange Pekoe) et OP (Orange Pekoe). L'OP1 et l'OP sont particulièrement populaires sur les marchés arabophones, et le Pekoe est bien connu en Russie et en Iran. La qualité la plus rare et la plus précieuse produite ici est le SFOPE1, une qualité 'Flowery Tippy' ainsi nommée pour les fragments de pointe pâles et argentés qu'elle contient, provenant du tout haut du bourgeon. Elle est appréciée davantage pour son caractère délicat que pour sa force.",
        "Le poids des sacs varie aussi selon la qualité : un sac de taille fixe contient bien plus d'une qualité fine et dense comme le Dust (environ 58 kg) que d'une qualité plus volumineuse et feuillue comme l'OPE (environ 30 kg).",
        "La fabrique confie surtout son thé fini à des courtiers en thé de Colombo, parmi lesquels des noms comme Forbes & Walker, Akbar Brothers et Finlays, qui l'amènent à la Vente aux enchères de thé de Colombo, où des acheteurs l'acquièrent et l'exportent souvent sous leur propre marque après lui avoir ajouté de la valeur. L'exportation directe sous le nom Athukorala lui-même existe aussi, mais surtout pour des commandes spéciales occasionnelles d'acheteurs étrangers.",
        "L'emballage hermétique protège le thé de l'humidité et des odeurs, préservant le goût soigneusement construit à chaque étape antérieure du processus.",
        "Les pays veulent des choses différentes de leur thé. Certains préfèrent une tasse forte et astringente, d'autres non, donc la partie du bourgeon dont provient une qualité est en fait adaptée au palais de chaque marché d'exportation. Les marchés arabophones, par exemple, demandent spécifiquement des qualités qui ne font pas partie des habitudes quotidiennes de consommation chez eux. Vendre surtout via des courtiers plutôt qu'exporter directement signifie que des acheteurs extérieurs gèrent généralement l'expédition internationale ultérieure et le branding.",
      ],
    ...
    "ginger-turmeric": {
      ...
      sectionBodies: [
        "Le gingembre (Zingiber officinale) est une plante herbacée vivace de la famille des Zingibéracées. Il est largement cultivé dans les régions tropicales et subtropicales, notamment au Sri Lanka et en Inde. Ce qui compte le plus, c'est le rhizome souterrain, utilisé comme épice et remède depuis des siècles. Le gingembre prospère dans les climats chauds et humides avec un sol riche et bien drainé. Il joue en outre un rôle central dans la cuisine traditionnelle et la médecine par les plantes de nombreuses cultures.",
    ...
    "artisanal-tea-grades": {
      ...
      sectionHeadings: [
        ...
        "White Tea et Purple Tea",
        "Pink Tea et Vita Glow",
        "Mélanges et Rareté",
        "Flower Chakra et Tea Coin",
      ],
      sectionBodies: [
        ...
        "Le Golden Tip utilise le bourgeon pleinement mûr de ce théier, tandis que le Silver Tip utilise le bourgeon plus pâle et encore immature du même théier. Ses fins poils argentés donnent son nom à cette qualité et un léger scintillement. Seul le bourgeon est pesé, flétri quelques heures, puis séché. C'est un procédé plus simple que celui des qualités entièrement fermentées, pour une tasse subtilement sucrée, florale, à la texture veloutée.",
        "Le White Tea prélève deux feuilles et demie sur le bourgeon et saute complètement le flétrissage. Il est cuit à la vapeur et séché plutôt que fermenté et roulé comme le thé noir. Le résultat est un goût très légèrement transformé, naturellement sucré, avec une liqueur pâle, une texture veloutée, un arôme floral et une richesse naturelle en antioxydants. Le Purple Tea utilise la même cueillette mais, contrairement au White Tea, traverse le processus complet du thé noir : flétrissage, roulage et fermentation. Cela donne un goût plus proche de l'oolong : plus léger que le thé noir mais moins végétal que le thé vert, relativement pauvre en caféine et naturellement riche en antioxydants.",
        "Le Pink Tea commence pratiquement incolore comme le White Tea, et ne devient rose qu'après l'ajout de quelques gouttes de citron vert. L'agrume déclenche un changement de couleur dans le même pigment anthocyane qui donne sa teinte au bourgeon. Il se marie aussi bien avec quelques gouttes de miel, se recommande servi en thé glacé, et est lui aussi naturellement riche en antioxydants. Le Vita Glow, lui, est fabriqué à partir du bourgeon du théier vert ordinaire plutôt que du violet.",
        "Le domaine crée aussi des mélanges à valeur ajoutée, comme du thé infusé aux fleurs de lotus, ainsi que du Blooming Tea noué à la main à partir des mêmes bourgeons. Ces qualités artisanales sont cueillies et transformées en quantités bien plus faibles que le thé noir standard, ce qui les rend plus rares et plus prisées. Demandez au comptoir de dégustation pour les goûter. La pousse à teinte violette ou la pousse verte ordinaire peuvent être utilisées pour la plupart de ces qualités, bien que le domaine utilise le plus souvent la pousse violette.",
        "Les articles les plus exigeants en travail fabriqués ici sont deux spécialités en petites séries : le Flower Chakra, un anneau de thé noir roulé à la main utilisant environ deux bourgeons chacun, et le Tea Coin. Une seule personne ne peut rouler à la main qu'environ 25 grammes de l'un ou l'autre par jour, ce qui explique leur prix élevé. Ils sont surtout achetés comme souvenir par des amateurs de thé fortunés plutôt que pour la consommation quotidienne. La majeure partie de la production faite main, y compris le Purple Tea, est réalisée sur commande en petites séries plutôt que produite en continu en grande quantité.",
      ],
      duration: "5 minutes",
    },
    "pahatharata-manufacturing": {
      name: "Fabrication du Thé (Pahatharata)",
      sectionHeadings: ["Transformation en Basse Altitude vs. Haute Altitude", "CTC : Crush-Tear-Curl"],
      sectionBodies: [
        "Les plantations de basses terres (« Pahatharata ») comme celle-ci transforment le thé différemment des plantations de hautes terres situées plus à l'intérieur des terres. Le propre procédé de fabrication du domaine, présenté étape par étape plus tôt dans ce parcours, le confirme : ici, le roulage réduit la feuille en petites particules denses plutôt que de la rouler en feuille entière façon orthodoxe. C'est la marque distinctive du procédé CTC (Crush-Tear-Curl). Cette étape est un rapide résumé de ce que cette approche des basses terres signifie pour la cueillette, la transformation et le triage.",
        "Le CTC (Crush-Tear-Curl) transforme mécaniquement la feuille en granulés petits et denses, produisant le caractère fort et vif pour lequel le thé de Ceylan des basses terres est connu. Ce sont les mêmes particules de feuille observées plus tôt aux étapes du roulage et du triage de ce parcours.",
      ],
    ...
    "clonal-tea": {
      name: "Thé Clonal",
      heroTagline: "Quelques-uns des plants d'essai de variétés clonales du domaine, signalés par leurs propres panneaux.",
      sectionHeadings: ["Thé Clonal", "Pourquoi le Thé Clonal Coûte Plus Cher", "Cultivars de Thé", "TRI 3069", "TRI 3055", "TRI 4053", "TRI 2022", "TRI 2043", "TRI 4006", "TRI 3025"],
      sectionBodies: ["Certains des thés les plus recherchés proviennent de plants clonaux : des boutures propagées à partir d'un seul plant mère exceptionnel plutôt que cultivées à partir de graines. Cette étape explique comment le thé clonal est propagé et pourquoi il peut atteindre un prix élevé. Le Tea Research Institute (TRI) sélectionne des variétés clonales selon différents besoins. Certaines pour la tolérance à la sécheresse ou à la pluie et un rendement plus élevé, d'autres pour les jeunes bourgeons caractéristiques à pointe rouge ou verte, prisés pour les thés artisanaux faits main. Les variétés à haut rendement largement plantées, comme TRI 2023, TRI 2025 et TRI 2026, sont courantes aussi bien dans les plantations que dans les jardins particuliers, tandis que les variétés spéciales plus rares sont surtout cultivées dans les grandes plantations pour le thé fait à la main plutôt que pour la culture courante. Bien qu'ils ressemblent à des années, ces numéros TRI sont simplement des identifiants attribués, et non l'année de sortie de chaque variété.", "La propagation clonale préserve les caractéristiques exactes d'un plant remarquable. Les meilleurs thés clonaux sont souvent produits en quantités très limitées. Les bourgeons des variétés spéciales à pointe rouge ou verte sont cueillis à la main pour des qualités rares comme les Golden Tips et les Silver Tips, et les plantations paient généralement environ 100 roupies de plus par kilogramme que pour les feuilles cueillies normalement.", "Le domaine privilégie des clones à haut rendement et résistants aux ravageurs mis au point par le Tea Research Institute (TRI), dont des variétés de la série TRI 2000 et T65, pour leur fiabilité dans le sol et le climat locaux. Le TRI 4049 est un cultivar de thé à haut rendement développé par l'Institut de recherche sur le thé du Sri Lanka, idéal pour les altitudes moyennes à basses. Il s'agit d'une variété de Camellia sinensis multipliée végétativement, reconnue pour sa croissance vigoureuse, ses feuilles vert foncé et sa résistance aux maladies courantes. Le TRI 4049 produit des liqueurs vives et aromatiques, adaptées aussi bien au thé CTC qu'au thé orthodoxe, ce qui en fait un choix populaire pour la replantation et la culture commerciale.", "Quelques rangs plus loin, cette plante étiquetée est le TRI 3069, un autre cultivar enregistré auprès de l'Institut de Recherche sur le Thé, planté aux côtés du TRI 4049 afin que la pépinière puisse comparer le comportement de chacun sur ce sol particulier avant toute plantation à grande échelle.", "Le TRI 3055 se trouve dans les mêmes rangées d'essai. Chaque parcelle de cultivar y est étiquetée et suivie individuellement, ce qui permet à l'usine de constituer ses propres preuves sur les variétés qui méritent vraiment d'être développées, plutôt que de se fier à la seule réputation.", "Le TRI 4053 complète les parcelles d'essai de cultivars illustrées ici. Faire pousser côte à côte plusieurs variétés enregistrées, chacune clairement étiquetée, est une pratique courante sur le domaine avant d'engager un cultivar sur un champ entier.", "Le TRI 2022 est planté dans la même section d'essai de cultivars, son écriteau étiqueté et suivi comme ses voisins. Cultiver plusieurs variétés enregistrées côte à côte sur le même sol permet à la pépinière de rassembler des preuves réelles de celles qui se comportent le mieux ici.", "Le TRI 2043 se trouve un peu plus loin dans la rangée. Comme les autres plants numérotés de cette parcelle d'essai, il est cultivé et surveillé avant toute décision de le planter à grande échelle.", "Le TRI 4006 est une entrée de plus dans le même essai de cultivars. La pépinière garde chaque variété dans un emplacement clairement marqué, ce qui facilite le suivi et la comparaison de sa croissance au fil du temps.", "Le TRI 3025 marque la limite de la parcelle d'essai, à côté d'un panneau bilingue « Cultivars de thé » et d'une plaque de plantation datée. Ensemble, les plants étiquetés ici constituent le registre concret de la pépinière sur les cultivars qui méritent d'être développés."],
      duration: "5-10 minutes",
    },
    "shade-management": {
    ...
    "handmade-factory": {
      ...
      heroTagline: "Une étagère à plateaux de la propre Fabrique de Thé Artisanal du domaine.",
      sectionHeadings: [
        "Fabrique de Thé Artisanal",
        "Flétrissage et Roulage",
        "Thé Vert, Séchage et le Bâtiment",
        "Ce qui Distingue le Thé Fait Main",
      ],
      sectionBodies: [
        "En plus de la production mécanisée, une partie du thé est entièrement transformée à la main, à chaque étape, sans machine. La plupart des qualités artisanales spéciales du domaine, dont le Golden Tip, le Silver Tip, le White Tea, le Purple Tea, le Pink Tea, le Vita Glow, le Blooming Tea noué à la main, et des mélanges infusés de fleurs comme le thé au lotus, sont finies ici plutôt que dans la fabrique principale de thé noir. (Le Vita Glow est fait à partir du bourgeon du théier vert, contrairement au bourgeon violet du Golden et Silver Tip.)",
        "Ici, le flétrissage se fait naturellement, dans un petit bac surveillé avec un hygromètre, sans les ventilateurs à air forcé de la fabrique principale, faisant descendre l'humidité à environ 50 % avant le roulage. La feuille est ensuite roulée à la main sur un plateau, jugée au toucher plutôt qu'à l'horloge. Pour les grosses commandes en gros, de petits rouleaux mécaniques prennent le relais pour le même usage, le roulage manuel seul ne pouvant suivre le rythme des volumes.",
        "Une machine dédiée au thé vert, qui cuit la feuille à la vapeur et la presse, s'occupe séparément de cette qualité, bien qu'elle soit actuellement hors service pour une petite réparation. Une fois cuite à la vapeur, la feuille repose à l'extérieur pour perdre son humidité de surface, puis est étirée et roulée à la main pour en faire ressortir la couleur, la force, l'arôme et le goût, avant un dernier passage au séchoir. Le temps de séchage varie d'un petit producteur à l'autre. Le petit four du bâtiment sert aussi de fournil pour des en-cas à base de thé comme des biscuits. Le séchage, effectué juste après le roulage, se fait ici sur deux séchoirs distincts : l'un traditionnel au bois, l'autre électrique. Chacun peut contenir 34 plateaux, tous deux également utilisés pour d'autres produits en petites séries, de la cardamome au clou de girofle en passant par le gingembre.",
        "Cette étape explique ce qui distingue le thé fait main de la production en ligne de fabrique. Le roulage et la transformation à la main produisent des lots plus petits avec plus de variation entre les feuilles, souvent appréciés pour leur caractère distinctif. Les thés faits main se répartissent en deux grandes catégories. La première est celle des fermentés, façon thé noir (reconnaissables à leur couleur noire), qui incluent le Purple Tea malgré son bourgeon d'origine inhabituel. La seconde est celle des non fermentés comme le White Tea, qui sautent entièrement la fermentation et passent directement au séchage.",
      ],
      duration: "10 minutes",
    },
    "tea-center": {
      name: "Centre du Thé",
      heroTagline: "Des visiteurs dégustant du thé au comptoir de dégustation du domaine.",
      sectionHeadings: [
        "Centre du Thé",
        "Infusion et Dégustation",
        "Qualités Quotidiennes et Thé de Supermarché",
        "Les Qualités les Plus Fines et Marchés d'Exportation",
        "Mélanges d'Herbes et d'Épices",
      ],
      sectionBodies: [
        "La visite se termine au centre du thé, où un jury de dégustation contrôle chaque lot, pas seulement ici mais aussi plus tôt, chaque fois que le thé quitte la fabrique principale pour le marché local ou arrive pour l'emballage, à la recherche de défauts comme une sur-fermentation, une sous-fermentation ou un brûlage. Tout ce qui échoue avant la vente est écarté, et même les problèmes mineurs sont notés pour orienter la production future. Sont exposées ici deux autres spécialités en petites séries de la fabrique artisanale, le Flower Chakra et le Tea Coin.",
        "Ces thés artisanaux peuvent être réinfusés jusqu'à trois fois avec de l'eau chauffée à environ 80 °C plutôt que bouillante. Chaque infusion dure environ 2, 5 et 8 minutes respectivement, avec de l'eau chaude fraîche à chaque fois, contrairement au thé noir standard, infusé une seule fois à pleine ébullition. La dégustation suit une méthode proche de celle du vin : d'abord sentir, puis aspirer bruyamment pour faire passer le thé sur les papilles gustatives situées sur les côtés de la langue avec une bouffée d'air. Le jury crache plutôt que d'avaler en dégustant jusqu'à 50 thés par jour, même si les visiteurs sont invités à réellement boire ce qu'ils goûtent. Un thé noir correctement infusé nécessite environ 2,5 à 3 minutes d'infusion couverte, pas juste une immersion rapide.",
        "Le BOPF et le BOPF Special sont les qualités que la plupart des Sri-Lankais boivent au quotidien. Le Dust et le Dust 1, les qualités les plus basses, ne partent jamais vers les marchés d'exportation. Ils finissent typiquement, avec un colorant ajouté, dans les sachets de thé bon marché de supermarché, d'où la couleur qui apparaît dès l'immersion plutôt qu'après une infusion correcte. Une grande partie de ce qui se vend en supermarché est aussi mélangée à du thé moins cher acheté ailleurs, contrairement aux thés non mélangés et à origine unique du domaine.",
        "Les qualités les plus fines, FF Extra Special, FF Special, FBOP, ne proviennent que d'environ 1 % de la récolte, la partie la plus immature du bourgeon, et se vendent autour de 25 dollars les 100 g. Des qualités plus volumineuses comme l'OPA et l'OP tournent plutôt autour de 5-6 dollars les 200 g. Les particules blanches de pointe visibles dans les meilleures qualités ajoutent une douceur supplémentaire et vont surtout aux acheteurs européens, tandis que les marchés arabes préfèrent généralement une tasse plus forte.",
        "Au-delà du thé, le domaine mélange son propre thé vert et noir (environ 90 %) avec des herbes locales (environ 10 %) : gotukola, heenbovitiya et moringa. Il fabrique aussi des thés épicés Masala, Cannelle et Gingembre à partir de ses propres épices. La recette du masala utilise sept épices sri-lankaises, dont la cardamome, la cannelle, le gingembre et le poivre noir (le reste reste secret). Pour le thé au lait, le masala ou le BOP sont recommandés.",
      ],
      duration: "10-15 minutes",
    },
    "coconut-field": {
    ...
    "jaggery-palm": {
      ...
      sectionBodies: [
        "Le palmier kithul (Caryota urens), ou palmier à queue de poisson, est un palmier grand et élégant que l'on trouve au Sri Lanka, en Inde et dans d'autres pays tropicaux d'Asie, apprécié pour ses usages multiples. Son produit le plus notable est la sève sucrée récoltée sur ses tiges florales, bouillie pour produire le sirop et le jaggery de kithul traditionnels. L'arbre est également connu pour son tronc solide et fibreux et ses belles feuilles en forme de queue de poisson.",
        "Un aperçu rapide du palmier lui-même :",
    ...
    pruning: {
      ...
      sectionBodies: ["Avant la taille, le domaine laisse délibérément le théier pousser pendant un certain temps plutôt que de le couper immédiatement. Ainsi, la nouvelle pousse peut se développer pleinement et l'arbuste accumule des réserves nutritives. La taille ne commence qu'une fois cette accumulation terminée, en coupant l'arbuste jusqu'à une hauteur d'environ le genou. Des panneaux sur place identifient cela comme deux étapes nommées : la première coupe croisée (Palamu Haras Kapuma) ramène le théier à environ 9-12 pouces (22-30 cm), et la seconde coupe croisée (Dewana Haras Kapuma) le laisse repousser jusqu'à environ 16-18 pouces (40-45 cm). Laisser d'abord l'arbuste pousser, plutôt que de le tailler selon un calendrier fixe sans tenir compte de son état, donne à la plante le temps d'acheminer les nutriments vers le haut avant d'être coupée."],
      duration: "5 minutes",
    },
    polytunnel: {
      name: "Tunnel de Culture et Serre",
      heroTagline: "Le propre tunnel maraîcher du domaine, déjà en train de faire pousser ses premières cultures.",
      sectionHeadings: ["À propos du Tunnel de Culture et de la Serre", "Arrosage et Irrigation", "Calendrier des Récoltes", "Tête de Contrôle de Fertirrigation", "Malu Miris (Poivron)", "Amu Miris (Piment)", "Nai Miris", "Gedi Gowa (Chou)", "Athu Gowa (Chou Rouge)", "Bonchi (Haricots Verts)", "Raabu (Radis)", "Carotte", "Poireau", "Concombre", "Plant de Tomate"],
      sectionBodies: [
        "À côté d'une serre déjà existante, le domaine construit un tunnel maraîcher pour cultiver tout un assortiment de légumes qui lui sont propres. On y trouve des piments Scotch Bonnet et piments verts, poivrons, cornichons, tomates, poivrons doux, choux, radis, haricots verts, poireaux, oignons nouveaux et carottes, entre autres. Une fois terminé, les hôtes séjournant à la villa ou au cabana pourront cueillir eux-mêmes, frais, les légumes cultivés ici. La couverture semi-transparente d'un tunnel maraîcher fonctionne un peu comme le verre d'une serre, retenant la chaleur et laissant passer la lumière tout en empêchant les fortes pluies de mousson et de nombreux nuisibles courants d'entrer. Ces conditions permettent au domaine de cultiver des légumes qu'il ne pourrait autrement pas gérer de façon fiable en plein champ, et de prolonger la saison de culture au-delà de ce que permettent les planches en plein air.",
        "Chaque culture suit son propre programme d'arrosage, alternant l'irrigation goutte à goutte au niveau des racines et la brumisation par le haut selon le stade de croissance. La brumisation aide surtout à la germination précoce et au rafraîchissement pour des cultures comme la carotte et le radis, tandis que le goutte-à-goutte devient la méthode principale une fois la fructification commencée, comme pour la tomate. Les deux méthodes répondent à des besoins différents. La brumisation maintient l'air et la surface du sol suffisamment humides pour que les jeunes graines et les semis s'établissent. Le goutte-à-goutte, une fois les plantes plus grandes, apporte l'eau directement à la zone racinaire, utilisant moins d'eau au total et gardant les feuilles sèches pour réduire le risque de maladies fongiques.",
        "Le temps avant la première récolte varie fortement selon la culture, d'à peine 25 à 35 jours pour le radis jusqu'à 90 à 120 jours pour le poireau, si bien qu'il devrait toujours y avoir quelque chose à cueillir dans le tunnel, presque à tout moment de la saison. Cultiver côte à côte un aussi large éventail de légumes à maturation rapide et lente est en soi un choix délibéré, puisque cela échelonne naturellement les récoltes plutôt que d'avoir tout prêt ou terminé en même temps.",
        "Cette tête de contrôle alimente les lignes goutte-à-goutte qui desservent chaque sac de culture. Elle comprend un filtre pour retenir les sédiments, un manomètre, des vannes d'arrêt pour isoler des sections de tuyauterie, et deux réservoirs d'engrais équipés d'un injecteur venturi qui dose l'engrais liquide directement dans l'eau d'irrigation à son passage, de sorte que chaque plante reçoit eau et nutriments en un seul passage.",
        "Avec ses feuilles ovales luisantes et pointues poussant en paires opposées, ce plant est Malu Miris, une variété de poivron (Capsicum annuum). Il pousse ici dans un sac de culture rempli de fibre de coco, avec un tuyau goutte-à-goutte déjà amené jusqu'à sa base.",
        "Ce plant est Amu Miris, une variété de piment (Capsicum annuum). À ce stade précoce, ses feuilles sont presque impossibles à distinguer de celles du poivron, puisque les deux appartiennent à la même espèce cultivée pour des usages différents.",
        "Le nom Nai Miris signifie 'piment cobra' en cingalais. C'est Capsicum chinense, une espèce différente et généralement plus piquante que les autres piments du domaine, apparentée à des variétés comme l'habanero et le Scotch Bonnet.",
        "Les feuilles plus rondes, à l'aspect légèrement cireux, de ce plant appartiennent à Gedi Gowa, ce qui signifie littéralement 'chou boule'. C'est le chou pommé commun (Brassica oleracea var. capitata), mis en terre ici bien avant que sa pomme ne commence à se former.",
        "Les feuilles rouge-violacé de ce plant appartiennent à Athu Gowa, ce qui signifie littéralement 'chou branchu'. C'est une variété de chou plus lâche, sans pomme (Brassica oleracea var. acephala), contrairement au type pommé.",
        "Ces larges feuilles en forme de cœur appartiennent à Bonchi, des haricots verts (Phaseolus vulgaris). Une fois lancée, c'est une culture grimpante, avec un port de croissance bien différent des légumes bas qui l'entourent.",
        "Ce minuscule plant, encore au stade cotylédon, est Raabu, un radis (Raphanus sativus). C'est l'une des cultures les plus rapides du tunnel à atteindre la récolte, prête en à peine 25 à 35 jours.",
        "La carotte (Daucus carota subsp. sativus) se reconnaît déjà à ce stade précoce à ses premières feuilles fines, semblables à des brins d'herbe, bien différentes des larges feuilles cotylédonaires des autres légumes cultivés ici.",
        "Le poireau (Allium ampeloprasum) émerge sous la forme d'une seule feuille fine, parmi les cultures les plus lentes du tunnel à atteindre la récolte. Cela peut prendre jusqu'à 90 à 120 jours après la plantation.",
        "Les deux larges feuilles cotylédonaires arrondies de ce plant sont celles d'un concombre de salade (Cucumis sativus), avant l'apparition de ses vraies feuilles, plus proches d'une forme de cœur.",
        "Ce plant de tomate présente déjà les feuilles lobées, dentées et légèrement duveteuses caractéristiques de la plante. La tomate est justement la culture citée plus tôt comme exemple de celles qui passent de la brumisation au goutte-à-goutte une fois la fructification commencée.",
      ],
      duration: "5 minutes",
    },
    "mushroom-hut": {
      name: "Cabane à Champignons",
      sectionHeadings: ["Un Jardin Différent", "Les Variétés de Pleurotes", "Substrat et Mycélium", "Cycle de Récolte", "Valeur Nutritionnelle et Bienfaits pour la Santé"],
      sectionBodies: [
        "Aux côtés de ses cultures de thé, d'épices et de fruits, le domaine entretient une cabane dédiée aux champignons. C'est une petite structure ombragée construite pour maintenir les conditions sombres, fraîches et humides dont les champignons ont besoin pour fructifier, plutôt que le soleil et la circulation d'air pour lesquels le reste des jardins est cultivé.",
        "La cabane est aménagée pour cultiver plusieurs variétés de pleurotes : le Pleurote Américain (Pleurotus pulmonarius), le Pleurote du Bhoutan, le Pleurote Noir, le Pleurote Rose (Pleurotus djamor), et le Champignon Abalone (Pleurotus cystidiosus). Tous sont cultivés sur un substrat de paille ou de sciure plutôt que dans la terre, et appartiennent au genre Pleurotus. On les reconnaît à leurs chapeaux en forme d'éventail ou d'huître qui poussent en étagères denses et superposées directement à partir du substrat plutôt que sur des tiges individuelles. La couleur est le principal élément distinguant une variété d'une autre, du blanc et gris au rose, à l'or ou au presque noir, tandis que la saveur douce et légèrement savoureuse reste globalement la même pour toutes.",
        "La culture commence par le substrat lui-même : paille ou sciure, le même matériau à la base de l'approche économique et fondée sur les déchets du domaine. Celui-ci est trempé et pasteurisé à la chaleur pour éliminer les moisissures et bactéries concurrentes, avant d'être mélangé avec du mycélium semence, du grain entièrement colonisé par le mycélium du champignon, puis emballé dans des sacs ou des blocs perforés. Ces blocs sont ensuite laissés dans le noir pendant deux à trois semaines pour la 'course du mycélium', durant laquelle le mycélium blanc se propage dans tout le substrat. Ce n'est qu'ensuite que les conditions sombres, fraîches et humides de la cabane prennent le relais pour déclencher la fructification.",
        "Cultiver des champignons de cette manière est un moyen économique et durable pour une exploitation de se diversifier au-delà de sa culture principale, utilisant souvent des déchets agricoles comme la paille comme milieu de culture. Un seul lot de substrat produit plusieurs récoltes sur plusieurs semaines. Chaque bloc produit généralement deux à quatre de ces vagues de récolte, espacées d'environ une à deux semaines, la première vague étant la plus abondante et les rendements diminuant ensuite. Les champignons sont détachés en grappes entières par torsion plutôt que coupés, car les moignons restants peuvent pourrir et favoriser une contamination qui gâcherait la vague suivante.",
        "La signalétique du domaine met en avant l'aspect nutritionnel de chaque variété : le Pleurote Américain, le Pleurote Rose et le Champignon Abalone sont chacun décrits comme riches en protéines, vitamines et antioxydants, avec des bienfaits axés sur le soutien immunitaire, la santé cardiaque et digestive, la réduction de l'inflammation et la baisse du cholestérol.",
      ],
      duration: "5 minutes",
    },
  },
  zh: {
    ...
    withering: {
      name: "萎凋",
      sectionHeadings: ["装填萎凋槽", "12小时周期", "水分流失", "萎凋为何重要"],
      sectionBodies: [
        "从茶园运来的茶叶会先称重，以记录当日采收量。之后薄薄地摊放在长85英尺、宽6英尺的萎凋槽中。每条萎凋槽都配有一台46英寸的风扇，以每分钟960转的速度运转，将空气强行送穿茶叶，持续带走水分，这就是庄园所称的“现代”萎凋工艺。以每平方英尺约2.5公斤的装载量计算，一条萎凋槽可容纳约1,250公斤鲜叶。",
        "茶叶在槽中停留12小时，每4小时人工翻动一次。在此周期内槽底会加热3至4小时，清晨卸出之前还会再松动一次。萎凋后的茶叶在送去揉捻之前会再次称重。",
        "每100公斤鲜叶大约要去除43公斤水分，这一数字是通过在一平方英尺的箱子里测试2.5公斤茶叶来验证的。这样一来，茶叶的水分含量会保持在57%至58%之间，变得柔软有韧性，可以揉捻而不会破碎。",
        "萎凋使叶片水分均匀分布，并使天然酶开始引发日后形成茶叶香气和色泽的化学变化。",
      ],
      duration: "此阶段持续整整12小时，茶叶每4小时翻动一次。",
    },
    rolling: {
      name: "揉捻",
      sectionHeadings: ["扭转与破壁", "第一次揉捻", "第二次揉捻", "揉捻为何重要"],
      sectionBodies: [
        "萎凋后的茶叶通过揉捻机，扭转并破坏叶片的细胞壁，释放出锁在其中的汁液和精油，并将茶叶揉碎成细小的颗粒。茶叶的外形和茶汤的浓度就在此阶段形成。",
        "两台进料揉捻机各自处理280公斤萎凋叶。第一次揉捻以每分钟30至32转的速度进行20分钟。产生的“朵利”（揉捻后的茶叶颗粒）随后经传送带送到装有金属网的解块机，最细小的颗粒作为“头子茶”落下并送去发酵。",
        "未能通过筛网的茶叶则沿传送带送至两台二级揉捻机，进料量比原本的280公斤减少12%至14%。这里会再进行一次20分钟、转速为每分钟32转的揉捻，之后产生的“二子茶”不经筛分直接送去发酵。",
        "破坏细胞壁使酶暴露于氧气中，从而引发氧化。揉捻的力度和时长决定了成品茶的浓度和外观。多次揉捻中的第一次是轻柔的“预处理”揉捻，目的是让扭转成形的茶叶颗粒先裹上茶叶自身的汁液，再进行更用力的揉捻。",
      ],
      duration: "共两次揉捻，每次20分钟（总计40分钟）：第一次转速为每分钟30至32转，第二次为每分钟32转。",
    },
    fermentation: {
      name: "发酵（氧化）",
      sectionHeadings: ["茶叶氧化", "保持房间洁净", "从绿转铜", "精准把控时间", "红茶、绿茶、白茶与乌龙茶"],
      sectionBodies: [
        "揉捻后的茶叶，头子茶和二子茶均包括在内，被摊放在保持25至30°C的凉爽潮湿房间中氧化2.5小时。加湿器会维持足够的空气湿度，以使反应顺利进行。",
        "房间必须保持绝对清洁，不得有细菌、积水或陈叶，因为细菌会终止酶的活性，破坏茶汤的色泽、浓度和品质。",
        "在这2.5小时内，绿色的叶片逐渐变成古铜色，充分形成红茶特有的色泽、口感和香气。这一变化完全来自天然氧化，不含任何染色剂或添加剂。",
        "这是决定风味最关键的一步。时间把控必须精准：氧化不足茶汤会寡淡，氧化过度则会使茶汤变得平淡无味。",
        "这也是本厂各类茶叶的区别所在。红茶依次经过萎凋、揉捻、发酵，再到烘焙。绿茶则完全跳过发酵，只经过萎凋、揉捻、干燥，口感更温和，主要因其更柔和、近乎药用的特质而被饮用。白茶则跳过萎凋和发酵这两道工序，直接从鲜叶进入蒸制和干燥。本厂不生产乌龙茶，导游将其描述为一种类似红茶的发酵茶，通过反复蒸制和手工揉搓而非机械揉捻制成。",
      ],
      duration: "头子茶和二子茶的发酵均需2.5小时，之后茶叶便进入干燥工序。",
    },
    drying: {
      name: "干燥（烘焙）",
      sectionHeadings: ["进入干燥机", "6个区段，21分钟", "找到恰当的平衡", "成果", "锁住新鲜度", "环环相扣的条件链"],
      sectionBodies: [
        "进入干燥机之前，发酵后的茶叶会先被松动。随后茶叶通过热风干燥机，由主风扇抽入柴火加热产生的热风，进风温度保持在120°C至130°C之间。这会瞬间停止氧化，并锁住茶叶的特质。",
        "干燥机内部分为6个区段。茶叶从最上层的托盘经这些区段逐层落到下方托盘，在前后翻动中向前移动，恰好在21分钟后出料。此时出风温度应约为80°C（约140°F至150°F）。",
        "正是这一组合，进风120-130°C、出风80°C、耗时21分钟，才是干燥机设计要维持的状态，也正是它让茶叶恰当地转黑。若进风温度低于120°C，即便通过减薄茶叶厚度使出风温度达到100°C，茶叶仍会干燥不足并偏褐色。若进风温度升至150°C而出风温度却降至75°C，茶叶则会变得潮湿、如同被闷煮，而非正常干燥。",
        "在正确的设定下，出来的便是我们熟悉的乌黑、干脆的茶叶。参观者会立刻感受到高温，这是整个厂区中最热的区域之一。",
        "烘焙在恰当的时刻停止化学变化，并将水分从约45-50%降至约2-3%，这对茶叶在储存和运输过程中保持新鲜至关重要。",
        "不过，成品茶叶的最终重量和品质并非仅取决于干燥机。它取决于一整条环环相扣的条件链：优质鲜叶、恰当的萎凋、正确的制作工艺、成功的发酵（这是其中最关键的一步），以及最后干燥机中的正确处理。",
      ],
      duration: "茶叶在严格保持的进风与出风温度循环下，恰好用21分钟通过烘干机。",
    },
    sorting: {
      name: "分级筛选",
      ...
      sectionBodies: [
        ...
        "那台分级机还配备了一套带摄像系统的色选机，能够分辨出茶叶和花朵等颗粒，将不属于成品的东西过滤掉。由于斯里兰卡的产茶以红茶为主，这一步正是用来剔除残留的、发酵不完全的棕色颗粒和茎梗碎片。不同网目大小的筛网，以每英寸的方孔数量来衡量，从最粗的每英寸3孔到最细的每英寸60孔不等，会在多次筛选中进一步按颗粒大小把茶叶分成约21个不同的等级。一台静电分离器会给滚轴上残留的棕色茎梗碎片施加静电，以便将它们分拣出来，专用机械还会把废茶从批次中彻底剔除。这里的任何颜色或特性都不是来自染料或添加剂，而完全来自天然加工。",
        "茶叶的价值取决于它在新梢上的位置，而不仅仅是叶片大小。叶片位置越高、越嫩，等级和价格就越高。通常出现在加香茶包里的并不是优质茶叶，而是茶末（Dust）等级。像FBOPF Extra Special这样的顶级茶，由于在出口市场上价值极高，很少在本地销售。",
        "除了新梢位置和网目大小之外，分选后的茶叶还会被划分为三个市场类别：First Mark、Second Mark和Third Mark。这里生产的主要等级包括OPA、OP、OP1、BOP1、FBOP、FF1、FF、Pekoe、Pekoe1、BOP Special、BOPA、FF Extra Special、Extra Special 1和FF Special，而价值较低的次级茶则归入1A、BM、BP、FNGS和BT等类别。",
        "平均而言，100公斤鲜叶大约能产出21.5公斤成品、已包装的红茶，即约21.5%的产出率，其余部分则因水分蒸发和分选时剔除的废茶而损耗。根据标准茶叶局测试指南，每生产1公斤成品茶大约需要4.65公斤鲜叶。该厂力求主要等级至少占产量的65%，次级茶不超过20%，其余则归入Second Mark等类别。",
      ],
      duration: "分级在机器上连续进行，每批大约需要30至45分钟。",
    },
    packing: {
      name: "包装",
      sectionHeadings: ["称重、试饮与封装", "包装区的各个等级", "各等级的袋重", "从茶厂到拍卖会", "密封包装为何重要", "等级与市场的匹配"],
      sectionBodies: [
        "分级后的茶叶经过称重、品质试饮，然后密封进衬有铝箔的防潮包装中，包装上印有茶厂的名称和标志、注册编号、锡兰茶标志以及食品安全证书。",
        "包装区并排存放着多个成品等级：Dust（主要用于茶包）、OP1、Pekoe、BOP（Broken Orange Pekoe）和OP（Orange Pekoe）。其中OP1和OP在阿拉伯语市场尤其受欢迎，Pekoe则在俄罗斯和伊朗广为人知。这里生产的最稀有、最珍贵的等级是SFOPE1，一种被称为「Flowery Tippy」的等级，因其中含有取自嫩芽最顶端的淡银色毫尖碎片而得名。它更多是因其细腻的品质而非浓烈口感而受到推崇。",
        "袋子的重量也因等级而异：同样大小的袋子能装下比Dust这类细密紧实的茶（约58公斤）多得多的重量，而像OPE这种更蓬松、多叶的等级则只能装约30公斤。",
        "茶厂大多将成品茶交给科伦坡的茶叶经纪公司，其中包括Forbes & Walker、Akbar Brothers和Finlays等名字，由他们送往科伦坡茶叶拍卖会，买家在那里购入，往往在进一步加值后以自己的品牌出口。以阿图科拉拉自己的名义直接出口的情况也存在，但主要是应海外买家偶尔的特殊订单。",
        "密封包装能保护茶叶免受潮气和异味影响，保留住整个工序中每一个环节精心积累而成的风味。",
        "不同国家对茶叶的需求各不相同。有些人偏好浓烈涩口的茶汤，有些人则不然，因此一款等级取自嫩芽的哪个部位，实际上是根据各个出口市场的口味来匹配的。例如，阿拉伯语市场专门要求那些在他们本国日常饮用习惯之外的特定等级。以经纪商销售为主而非直接出口，意味着后续的国际运输和品牌事务通常由外部买家来处理。",
      ],
    ...
    "artisanal-tea-grades": {
      ...
      sectionHeadings: [
        "手工精品茶",
        "Golden Tip与Silver Tip",
        "White Tea与Purple Tea",
        "Pink Tea与Vita Glow",
        "混合茶与稀有性",
        "Flower Chakra与Tea Coin",
      ],
      sectionBodies: [
        "除了标准红茶之外，庄园还生产稀有的手工精制茶。这些茶大多在独立的手工茶厂手工完成，而不是在主要的红茶生产线上。因此这些等级也被称为「手工精品茶」：它更接近一门手艺，而非批量生产。其中许多来自一种独特的紫色嫩芽，即TRI 2043，这是茶叶研究所培育的一个品种。它的颜色来自花青素。这是一个低产品种，因此并非每个斯里兰卡庄园都种植它。",
        "Golden Tip取自这种嫩芽完全成熟的芽头，Silver Tip则取自同一嫩芽颜色更浅、尚未成熟的芽头。细密的银色绒毛让Silver Tip得名，也带来一丝微光。只需称重芽头，萎凋几个小时，然后烘干。这是比全发酵茶更简单的工艺，能得到一杯略带甜味、花香、口感丝滑的茶汤。",
        "White Tea取自嫩芽的两叶半，完全不经过萎凋。它以蒸青和烘干代替发酵和揉捻，不像红茶那样处理。成品是一种加工程度很轻、天然带甜的味道，汤色浅淡，口感丝滑，带有花香，并天然富含抗氧化物质。Purple Tea采摘方式相同，但与White Tea不同，要经过完整的红茶工艺：萎凋、揉捻、发酵。风味更接近乌龙茶：比红茶清淡，但比绿茶少了几分青草味，咖啡因含量相对较低，天然富含抗氧化物质。",
        "Pink Tea起初几乎无色，就像White Tea一样，只有加入几滴青柠汁后才会变成粉红色。柑橘会触发同一种赋予嫩芽颜色的花青素色素发生变色。它也很适合加几滴蜂蜜，建议做成冰茶饮用，同样天然富含抗氧化物质。而Vita Glow则不是用紫色嫩芽，而是用普通绿色嫩芽的芽头制成。",
        ...
        "这里制作最费工的两款产品是小批量特色茶：Flower Chakra，一种手工卷制的红茶环，每个约用两根嫩芽制成，以及Tea Coin。一个人一天只能手工卷制约25克其中一种，这也是它们如此昂贵的原因，它们大多被富裕的茶叶爱好者当作纪念品购买，而非日常饮用。包括Purple Tea在内，大部分手工产品都是按订单小批量制作，而非持续大批量生产。",
      ],
      duration: "5分钟",
    },
    "pahatharata-manufacturing": {
      name: "低地茶制作（Pahatharata）",
      sectionHeadings: ["低地与高地加工工艺对比", "CTC：压碎-撕裂-揉卷"],
      sectionBodies: [
        "像本庄园这样的低地（“Pahatharata”）庄园的制茶方式与内陆高地庄园不同。本次游览中此前逐一介绍过的庄园自身生产流程正印证了这一点：这里的揉捻是将茶叶揉碎成细小紧实的颗粒，而不是像正统工艺那样揉成完整的茶叶。这正是CTC（压碎-撕裂-揉卷）加工的标志性特征。本站是对低地这种方式在采茶、加工与分级方面所代表意义的简要回顾。",
        "CTC（压碎-撕裂-揉卷）通过机械方式将茶叶加工成细小紧实的颗粒，造就了低地锡兰红茶浓烈爽口的特点。这正是本次游览此前在揉捻和分级站点所见到的那种茶叶颗粒。",
      ],
      duration: "10-15分钟",
    },
    "organic-cultivation": {
      ...
      sectionBodies: ["这座庄园以有机方式种植的茶树品种是TRI 2043，一种由茶叶研究所培育的中国型品种，以紫色嫩芽著称，这种颜色来自花青素色素，而非通常的绿色。这里不使用合成农药或化肥种植的成熟TRI 2043茶树，已经在供应Golden Tip、Silver Tip、白茶（White Tea）、紫茶（Purple Tea）和粉茶（Pink Tea）等特色茶品。这与茶叶苗圃里较年幼的TRI 2043插条不同，那些插条仍在试种阶段，尚未获批大规模种植。TRI 2043天生产量偏低，这也是由它制成的特色茶品产量如此稀少的原因。有机认证通常需要连续数年无化学品种植才能获得认证，在此期间产量始终低于常规管理的茶园。"],
      duration: "5-10分钟",
    },
    "clonal-tea": {
      name: "克隆茶",
      heroTagline: "庄园自己挂牌标注的部分克隆品种试验茶树。",
      sectionHeadings: ["克隆茶", "克隆茶为何更贵", "茶树品种", "TRI 3069", "TRI 3055", "TRI 4053", "TRI 2022", "TRI 2043", "TRI 4006", "TRI 3025"],
      sectionBodies: ["一些备受追捧的茶叶来自克隆植株，即通过扦插繁殖自单一优良母株，而非种子繁殖。本站介绍克隆茶的繁殖方式，以及它为何能卖出高价。茶叶研究所（TRI）针对不同需求培育克隆品种。有些是为了耐旱或耐雨并提高产量，有些则是为了那些红尖或绿尖的嫩芽，这类嫩芽备受手工特色茶的青睐。像TRI 2023、TRI 2025、TRI 2026这样广泛种植、高产的品种，在茶园和家庭园圃中都很常见；而更稀有的特色品种则大多种植在大型茶园中，用于手工制茶，而非日常种植。尽管这些TRI编号看起来像年份，但它们其实只是分配的编号，并非各品种发布的年份。", "克隆繁殖能保留优良母株的确切特性。最优质的克隆茶往往产量非常有限。红尖或绿尖特色品种的嫩芽会被手工采摘，用于制作Golden Tips、Silver Tips等稀有茶品，茶园通常会为此多付约每公斤100卢比，高于普通采摘叶片的价格。", "庄园偏爱由茶叶研究所（TRI）培育的高产、抗病虫害克隆品种，包括TRI 2000系列和T65，因为它们在当地土壤和气候条件下表现稳定可靠。TRI 4049是由斯里兰卡茶叶研究所培育的高产茶树品种，适合中低海拔地区种植。它是山茶（Camellia sinensis）的无性繁殖品种，以生长旺盛、叶色深绿、抗病性强而著称。TRI 4049可制成滋味明亮、香气浓郁的茶汤，适用于CTC工艺和传统工艺茶叶生产，是补植和商业种植的热门选择。", "几行之外，这株挂牌的茶树是TRI 3069，另一个斯里兰卡茶叶研究所登记的品种，与TRI 4049种植在一起，以便苗圃比较两者在这片土壤上的表现，再决定是否大规模种植。", "TRI 3055也种植在同一片试验区内。这里每一株茶苗品种都单独挂牌记录，工厂正是借此积累自己的数据，判断哪些品种真正值得推广，而不仅仅依赖名声。", "TRI 4053是这片试验区中的最后一个品种。让多个登记品种并排种植、逐一挂牌，是茶园在正式大面积种植某一品种之前的标准做法。", "TRI 2022同样种植在这片品种试验区内，挂牌编号，和邻近的植株一样受到追踪记录。在同一块地上并排种植多个登记品种，正是苗圃借以积累哪些品种表现最好的真实证据的方法。", "TRI 2043 位于这一排稍靠后的位置。和这片试验地里的其他编号植株一样，它也在被培育和观察，尚未决定是否值得大面积种植。", "TRI 4006 是同一品种试验区中的又一株。苗圃为每个品种保留各自明确标记的位置，方便随时间追踪比较其生长情况。", "位于试验区边缘的TRI 3025，紧邻一块双语的“茶树品种”标牌和一块注明日期的种植纪念牌。这里挂牌标注的植株，共同构成了苗圃判断哪些品种值得推广的实际依据。"],
      duration: "5-10分钟",
    },
    "shade-management": {
      name: "遮荫树管理",
    ...
    "handmade-factory": {
      ...
      sectionBodies: [
        "除了机械化生产之外，部分茶叶在每个环节都完全依靠人工处理，不使用机器。庄园大多数手工特色茶品，包括Golden Tip、Silver Tip、White Tea、Purple Tea、Pink Tea、Vita Glow、手工绑扎的Blooming Tea，以及莲花茶等花香混合茶，都是在这里而非主要红茶厂完成的。（Vita Glow取自绿色嫩芽的芽头，与Golden Tip、Silver Tip所用的紫色嫩芽不同。）",
        "这里的萎凋是自然进行的，在一个小槽中用湿度计监测，不使用主厂那种强制通风的风扇，在揉捻前把水分降到约50%。之后茶叶在托盘上手工揉捻，凭手感判断，而不是靠时钟。对于较大的批量订单，会由小型机械揉捻机来完成同样的工作，因为单靠手工揉捻跟不上产量需求。",
        "一台专用的绿茶机，负责蒸青和压制茶叶，单独处理这个品类，不过目前因小故障暂停使用。蒸青之后，茶叶被放在室外散去表面水分，再经拉伸手工揉捻以激发其色泽、力度、香气和滋味，最后送入烘干机完成最后一道工序。烘干时间因小生产者而异。这座建筑里的小烤炉还兼作烘焙房，用来制作以茶为原料的小吃，比如饼干。揉捻后立即进行的烘干，在这里由两台独立的烘干机完成：一台传统柴火烘干机，一台电力烘干机。每台可容纳34个托盘，两台也都用于豆蔻、丁香、生姜等其他小批量产品。",
        "这一站介绍了手工茶与流水线生产茶的区别所在。手工揉捻和手工处理产出的批次更小，叶片之间差异更大，也因这种独特个性而常受青睐。手工茶大致分为两大类。一类是像红茶一样经过发酵的（可通过其黑色辨认），Purple Tea尽管原料嫩芽特殊，也属于这一类。另一类是不发酵的，例如White Tea，完全跳过发酵直接进入烘干。",
      ],
      duration: "10分钟",
    },
    "tea-center": {
      name: "茶叶中心",
      heroTagline: "游客在庄园自己的品茶柜台品尝茶叶。",
      sectionHeadings: [
        "茶叶中心",
        "冲泡与品鉴",
        "日常等级与超市茶",
      ...
      sectionBodies: [
        "行程的终点是茶叶中心，一支品茶小组会在这里检查每一批茶，此前也是如此：每当茶叶离开主厂运往本地市场，或送来包装时，都要接受检查，查找过度发酵、发酵不足或烘焦等缺陷。任何在出售前未通过检验的茶叶都会被剔除，即使是细微的问题也会被记录下来，以指导未来的生产。这里还展出了手工茶厂的另外两款小批量特色产品：Flower Chakra和Tea Coin。",
        "这类手工茶可以用约80°C的热水（而非沸水）冲泡多达三次，每次用新鲜热水，分别约2、5、8分钟。这不同于标准红茶，后者只用滚沸的水冲泡一次。品茶遵循一种与品酒类似的方法：先闻香，再吸气啜饮，让茶汤掠过舌头两侧的味蕾。品茶小组一天最多品鉴50款茶，会吐出而不是咽下，不过访客倒是可以真正喝下自己品尝的茶。正确冲泡的红茶需要加盖浸泡约2.5到3分钟，而不只是快速一蘸。",
        "BOPF和BOPF Special是大多数斯里兰卡人日常饮用的等级。而最低等级的Dust和Dust 1从不出口，它们通常会加入色素，最终出现在廉价的超市茶包里。这也是为什么茶包一浸入水中颜色就立刻显现，而不是等到充分浸泡之后。超市里出售的许多茶叶还会与从别处购入的更便宜茶叶混合，不同于庄园自家未经混合的单一原产地茶叶。",
        "最上乘的等级（FF Extra Special、FF Special、FBOP）仅取自约1%的收成，是嫩芽中最不成熟的部分，每100克售价约25美元。而OPA、OP等更粗大的等级，每200克售价约5到6美元。最上乘等级中可见的白色芽尖颗粒增添了额外的甜味，大多销往欧洲买家，而阿拉伯市场则更偏好口感更浓的茶。",
        "除了茶叶，庄园还将自家的绿茶和红茶（约90%）与本地草本（约10%：积雪草、灰莉叶和辣木叶）混合，并用自家香料制作Masala、肉桂和姜味香料茶。Masala配方使用七种斯里兰卡香料，其中包括豆蔻、肉桂、姜和黑胡椒（其余成分保密）。奶茶推荐使用Masala茶或BOP茶。",
      ],
      duration: "10-15分钟",
    },
    "coconut-field": {
    ...
    pruning: {
      ...
      sectionHeadings: ["修剪"],
      sectionBodies: ["在修剪之前，庄园会特意让茶树先生长一段时间，而不是立刻剪短。这样可以让新梢充分长出，使茶树积累养分。只有在这种养分积累完成之后，才会开始修剪，将茶树剪至大约膝盖的高度。现场的标牌将其标注为两个命名阶段：第一次十字修剪（Palamu Haras Kapuma）将茶丛剪至约9-12英寸（22-30厘米），第二次十字修剪（Dewana Haras Kapuma）则让它重新长到约16-18英寸（40-45厘米）。让茶树先自然生长，而不是不顾其状态按固定时间表修剪，可以让茶树有时间在被剪短之前把养分向上输送。"],
      duration: "5分钟",
    },
    polytunnel: {
      name: "塑料大棚与温室",
      heroTagline: "庄园自家的塑料大棚，已经种下了第一批作物。",
      sectionHeadings: ["关于塑料大棚与温室", "浇水与灌溉", "采收时间", "施肥灌溉控制头", "马鲁米里斯辣椒（灯笼椒）", "阿姆米里斯辣椒（青辣椒）", "奈米里斯辣椒", "格迪高瓦（卷心菜）", "阿图高瓦（红卷心菜）", "邦奇（四季豆）", "拉布（萝卜）", "胡萝卜", "韭葱", "黄瓜", "番茄幼苗"],
      sectionBodies: [
        "在现有温室之外，庄园正在搭建一座塑料大棚，用于种植自家的各种蔬菜。其中包括苏格兰帽椒和青辣椒、灯笼椒、小黄瓜、番茄、彩椒、卷心菜、萝卜、四季豆、韭葱、香葱以及胡萝卜等。建成后，入住别墅或小木屋的客人将能够亲手采摘这里新鲜种植的蔬菜。塑料大棚半透明的覆盖层的作用很像温室的玻璃，既能锁住温度、透进阳光，又能挡住强劲的季风雨水和许多常见害虫。正是这样的环境，让庄园得以种植那些在露天地里难以稳定生长的蔬菜，并比露天栽培延长了种植季节。",
        "每种作物都遵循自己的浇水方式，根据生长阶段在根部滴灌和上方喷雾之间交替进行。喷雾主要有助于胡萝卜、萝卜等作物的早期发芽和降温，而一旦番茄等作物开始结果，滴灌就会成为主要的浇水方式。这两种浇水方式各有用途。喷雾能让空气和土壤表面保持足够的湿度，帮助幼小的种子和秧苗扎根。滴灌则在植株长大后将水直接送到根部，既能整体节约用水，又能让叶片保持干燥，从而降低真菌病害的风险。",
        "不同作物达到首次采收所需的时间差异很大，短则萝卜只需25至35天，长则韭葱需要90至120天，因此大棚里几乎在整个种植季节的任何时候都应该有可采摘的蔬菜。将生长速度有快有慢的多种蔬菜混合种植在一起，本身就是一种有意的安排，因为这样可以让收成自然地错开，而不是所有作物同时成熟，或同时结束。",
        "这个控制头为通向每个种植袋的滴灌管供水，包括一个用于滤除沉积物的过滤器、一个压力表、用于隔离各段管道的截止阀，以及两个配备文丘里注入器的肥料罐，可在灌溉水流经时将液体肥料直接注入其中，让每株植物在同一次灌溉中同时获得水分和养分。",
        "这株幼苗叶片呈光滑的尖椭圆形，成对生长，是马鲁米里斯，一种灯笼椒（辣椒属，Capsicum annuum）。它种植在装满椰糠的种植袋中，根部已经接上了滴灌管。",
        "这株幼苗是阿姆米里斯，一种辣椒（Capsicum annuum）。在这个早期阶段，它的叶片与灯笼椒的叶片几乎无法区分，因为两者属于同一物种，只是种植用途不同。",
        "奈米里斯这个名字在僧伽罗语中意为'眼镜蛇辣椒'。它是Capsicum chinense，与庄园其他辣椒不同的一个物种，通常更为辛辣，与哈瓦那辣椒、苏格兰帽椒等品种关系密切。",
        "这株幼苗更圆、略带蜡质光泽的叶片属于格迪高瓦，字面意思是'球形卷心菜'。它就是常见的结球卷心菜（Brassica oleracea var. capitata），早在叶球开始形成之前很久就已经种下。",
        "这株幼苗红紫色的叶片属于阿图高瓦，字面意思是'分枝卷心菜'。它是一种更为松散、不结球的卷心菜品种（Brassica oleracea var. acephala），有别于结球型。",
        "这些宽大的心形叶片属于邦奇，即四季豆（Phaseolus vulgaris）。它一旦开始生长便会攀爬，生长习性与周围低矮的蔬菜截然不同。",
        "这株仍处于子叶阶段的小幼苗是拉布，即萝卜（Raphanus sativus）。它是大棚里生长最快、最早能采收的作物之一，最快只需25至35天就能采收。",
        "胡萝卜（Daucus carota subsp. sativus）即使在这个早期阶段也能通过其细长如草的最初叶片辨认出来，与这里种植的其他蔬菜宽大的子叶截然不同。",
        "韭葱（Allium ampeloprasum）只长出单片细长的叶片，是大棚里采收最慢的作物之一。从种下到采收可能需要90至120天。",
        "这株幼苗宽大、圆润的两片子叶是沙拉黄瓜（Cucumis sativus）的子叶，其真正的、更呈心形的叶片尚未长出。",
        "这株番茄幼苗已经长出了番茄植株特有的分裂、锯齿边缘、略带绒毛的叶片。番茄正是前文提到的、在开始结果后会从喷雾转为滴灌的那种作物。",
      ],
      duration: "5分钟",
    },
    "mushroom-hut": {
      name: "蘑菇棚",
      sectionHeadings: ["别具一格的花园", "平菇品种", "栽培基质与菌种", "收获周期", "营养与健康益处"],
      sectionBodies: [
        "除了茶叶、香料和水果作物外，庄园还设有一间专用的蘑菇棚。这是一座小型遮荫建筑，专为维持蘑菇长菇所需的阴暗、凉爽、潮湿环境而建，而不像庄园其他地方那样为了日照和通风而种植。",
        "这间蘑菇棚专门种植多个平菇品种：美国平菇（Pleurotus pulmonarius）、不丹平菇、黑平菇、粉红平菇（Pleurotus djamor），以及鲍鱼菇（Pleurotus cystidiosus）。全部种植在稻草或木屑基质上，而非土壤中，且都属于平菇属（Pleurotus）。其特征是呈扇形或蚝形的菌盖，密集地重叠成层，直接从基质中长出，而非依附于单独的菌柄。区分各品种的主要因素是颜色，从白色、灰色到粉红色、金色，乃至近乎黑色不等，而它们温和、略带鲜味的口感则大体相近。",
        "栽培从基质本身开始：稻草或木屑，正是庄园低成本、变废为宝理念背后所依赖的材料。先经过浸泡并用高温灭菌，以杀灭竞争性的霉菌和细菌，再与菌种（即已被蘑菇菌丝完全定植的谷粒）混合，装入带孔的袋子或菌包中。随后这些菌包会在黑暗中放置两到三周，进行所谓的“发菌”，白色菌丝会在这段时间里布满整个基质。之后，蘑菇棚阴暗、凉爽、潮湿的环境才会接管，促使菌菇出菇。",
        "以这种方式种植蘑菇，是农场在主要作物之外实现多元化经营的一种低成本、可持续方式，常利用稻草等农业废弃物作为栽培基质。一批基质可在数周内产出多次收成。每个菌包通常能产出两到四茬这样的收成，间隔大约一到两周，第一茬产量最大，之后逐渐减少。采摘时需整簇拧下而非切割，因为残留的菇柄断茬容易腐烂，招致污染，从而毁掉下一茬的收成。",
        "庄园自己的标牌上标明了每个品种的营养特点：美国平菇、粉红平菇和鲍鱼菇均富含蛋白质、维生素和抗氧化剂，有助于增强免疫力、促进心脏和消化系统健康、减轻炎症并帮助降低胆固醇。",
      ],
      duration: "5分钟",
    },
  },
  ja: {
    plucking: {
      name: "摘採",
      heroTagline: "農園自前の茶園で、自らの手で茶摘みを体験する訪問者たち。",
      sectionHeadings: ["摘採"],
      sectionBodies: ["すべての茶葉は茶園から始まります。摘み手はスタッフが「デカ・ハマラ」（葉2枚半）と呼ぶ基準まで、多くの場合は3枚半まで、厳格な「一芯二葉」のルールではなく、手摘みで各新芽を摘み取ります。この繊細な摘採基準こそがセイロンティーの繊細な個性を生み出しており、粗い葉は茶樹に残されます。摘み取る正確な葉の枚数は紅茶と緑茶で異なります。最も若い新芽のみが使用されます。手摘みは茶樹を保護し、風味成分の濃度が最も高い葉を選び取ります。これはこの品質レベルでは機械には再現できません。"],
      duration: "茶葉は一年を通して毎日摘まれ、鮮度を保つため数時間以内に工場へ運ばれます。",
    },
    withering: {
      name: "萎凋",
      sectionHeadings: ["萎凋槽への投入", "12時間のサイクル", "水分の減少", "萎凋が重要な理由"],
      sectionBodies: [
        "畑から届いた生葉はまずその日の収穫量を記録するために計量されます。その後、長さ85フィート、幅6フィートの長い萎凋槽に薄く広げられます。各槽には毎分960回転で回る46インチの送風機が備え付けられており、強制的に風を通すことで水分が徐々に取り除かれます。農園ではこれを「モダン」萎凋プロセスと呼んでいます。1平方フィートあたり約2.5kgという投入量で、1つの槽には約1,250kgの生葉が収容されます。",
        "葉は槽内に12時間置かれ、4時間ごとに手で裏返されます。その周期のうち3〜4時間は槽の下から熱が加えられ、早朝に取り出される前に最後にもう一度ほぐされます。萎凋を終えた葉は揉捻に送られる前に再び計量されます。",
        "生葉100kgごとに約43kgの水分が取り除かれます。これは1平方フィートの箱で2.5kgの葉をテストして確認された数値です。こうして葉は、崩れることなく揉捻できるしなやかな状態になる頃には水分量が57〜58%になります。",
        "萎凋は葉全体の水分を均一にし、天然の酵素が、後に紅茶の香りと色を生み出す化学変化を始めるきっかけとなります。",
      ],
      duration: "この工程は丸12時間続き、葉は4時間ごとに裏返されます。",
    },
    rolling: {
      name: "揉捻",
      sectionHeadings: ["葉をねじり破壊する", "最初の揉捻", "二回目の揉捻", "揉捻が重要な理由"],
      sectionBodies: [
        "萎凋した茶葉は揉捻機にかけられ、葉の細胞壁がねじられて破壊され、内部に閉じ込められていた液汁と油分が放出されるとともに、茶葉が細かい粒子に砕かれます。ここで茶葉の形と紅茶の濃さが形作られます。",
        "2基の投入ローラーはそれぞれ280kgの萎凋葉を受け入れます。最初の揉捻は毎分30〜32回転で20分間行われます。できあがった「ドゥーリー」（揉捻された茶葉粒子)はその後コンベアで金網付きのロールブレーカーへ送られ、最も細かい粒子が「一番ドール」として振るい落とされ、発酵工程へ送られます。",
        "ふるいを通らなかった葉はコンベアを上って2基の二次ローラーへ送られます。ここでは投入量が元の280kgから12〜14%減らされます。毎分32回転でさらに20分間の二回目の揉捻が行われた後、その「二番ドール」はふるい分けせずそのまま発酵工程へ送られます。",
        "細胞壁が破壊されることで酵素が酸素にさらされ、酸化が始まります。どれだけ強く、どれだけ長く揉むかが、最終的な紅茶の濃さと外観を決定します。複数回行われる揉捻のうち最初の一回は「予備揉捻」と呼ばれる優しい揉み方で、しっかりとした揉捻に入る前に、ねじれた茶葉の粒子を葉自身の液汁で覆うことを目的としています。",
      ],
      duration: "20分間の揉捻を2回（合計40分）: 最初は毎分30〜32回転、2回目は毎分32回転です。",
    },
    fermentation: {
      name: "発酵（酸化）",
      sectionHeadings: ["茶葉を酸化させる", "部屋を清潔に保つ", "緑から銅色へ", "正確なタイミング", "紅茶・緑茶・白茶・ウーロン茶"],
      sectionBodies: [
        "揉捻された茶葉は、一番ドールと二番ドールの両方とも、25〜30°Cに保たれた涼しく湿度の高い部屋に広げられ、2.5時間かけて酸化させられます。加湿器によって反応が進むのに十分な湿度が保たれます。",
        "部屋は完全に清潔でなければなりません。細菌、よどんだ水、古い茶葉があってはならず、細菌が酵素の働きを止め、色、濃さ、水色の品質を損なってしまうからです。",
        "この2.5時間のうちに、緑色の葉は次第に銅色がかった茶色に変わり、紅茶特有の色、コク、香りを十分に発達させます。これはすべて天然の酸化によるもので、着色料や添加物は一切使用されません。",
        "これは風味にとって最も重要な工程です。タイミングは正確に見極める必要があります。短すぎると水っぽくなり、長すぎると平板で鈍い味になります。",
        "これはまた、ここで作られる茶の種類を分けるものでもあります。紅茶は萎凋、揉捻、発酵、そして火入れという順序をたどります。緑茶は発酵を完全に省き、萎凋、揉捻、乾燥のみを経て、より穏やかな味わいとなり、主にその優しく薬用的な性質のために飲まれます。白茶は萎凋と発酵の両方を省き、生葉から直接蒸し、乾燥させます。この工場ではウーロン茶は作っていません。ガイドはこれを、機械による揉捻の代わりに蒸しと手もみを繰り返して作る、紅茶に似た発酵茶として説明しています。",
      ],
      duration: "一番ドールと二番ドールのいずれも発酵に2.5時間かかり、その後茶葉は乾燥工程へ進みます。",
    },
    drying: {
      name: "乾燥（火入れ）",
      sectionHeadings: ["乾燥機への投入", "6つの区画、21分間", "適切なバランスを見つける", "仕上がり", "鮮度を閉じ込める", "一連の条件の積み重ね"],
      sectionBodies: [
        "乾燥機に入る前に、発酵を終えた茶葉はほぐされます。その後、熱風乾燥機を通ります。薪を燃料とする熱源からメイン送風機で吸い込まれた熱風により、入口温度は120〜130°Cに保たれます。これにより酸化を瞬時に止め、紅茶の特性を閉じ込めます。",
        "内部では乾燥機は6つの区画に分かれています。茶葉は最上段のトレイからこれらの区画を通って下段のトレイへと落ち、前後にころがりながら進み、ちょうど21分後に排出されます。その時点での排気温度は約80°C（およそ140〜150°F）であるべきです。",
        "この入口120〜130°C、排気80°C、21分という組み合わせこそ、乾燥機が維持するよう設計されているものであり、茶葉を正しく黒く仕上げるものです。入口温度が120°Cを下回ると、たとえ茶葉の層を薄くして排気温度を100°Cまで上げても、茶葉は乾燥不足で茶色っぽく仕上がってしまいます。逆に入口温度を150°Cまで上げても排気温度が75°Cまで下がってしまうと、茶葉はしっかり乾燥せずべたついた蒸し煮のような状態になってしまいます。",
        "正しい設定であれば、出てくるのはおなじみの黒く、パリッとした茶葉です。見学者はすぐに熱気に気づきます。工場内でも屈指の暑さのエリアです。",
        "火入れはまさに適切な瞬間に化学変化を止め、水分を約45〜50%から約2〜3%まで減らします。これは保管・輸送中に紅茶の鮮度を保つために不可欠です。",
        "ただし、最終的な紅茶の重量と品質を決めるのは乾燥機だけではありません。良質な生葉、適切な萎凋、正しい製造工程、成功した発酵（すべての中で最も重要な工程）、そして最後に乾燥機での正しい処理という、一連の条件の積み重ねによって決まります。",
      ],
      duration: "茶葉は厳密に管理された入口・排気温度のサイクルのもと、ちょうど21分かけて乾燥機を通過します。",
    },
    sorting: {
      name: "選別・格付け",
      ...
      sectionBodies: [
        ...
        "乾燥した葉が選別機を通る際、元の新芽のどの部分から来たかによって一度の通過で分けられます。柔らかく折れやすい先端と一番上の葉、それぞれの新芽のわずか約2%は、FBOPF Extra Specialになります。これは最上級で最も高価な等級です。その下にある葉はBOPやPekoeのような力強い等級になります。最も成熟した一番下の葉はDustになり、使えない茎やくずはRefuseとして取り除かれます。",
        "その選別機には、葉や花のような粒子を見分けられるカメラシステムを備えた色選別機が組み込まれており、該当しないものはすべてろ過されます。スリランカの生産は紅茶中心であるため、この工程によって発酵が不十分な茶色の粒子や残った茎の破片が取り除かれます。異なる網目の篩は、1インチあたりの正方形の穴の数で測られ、最も粗いもので1インチあたり3穴、最も細かいもので60穴まで幅がありますが、複数回の工程でさらに粒子の大きさ別に葉を約21種類の等級に分けます。静電分離機はローラー上で残った茶色の茎の破片に静電気を帯びさせ、それらを取り除けるようにし、専用の機械がロット全体から廃棄茶をすべて取り除きます。ここでの色や個性はいずれも染料や添加物によるものではなく、天然の加工工程によるものだけです。",
        "価値は葉の大きさだけでなく、新芽のどの位置にあったかによって決まります。葉が上部にあり柔らかいほど、等級も価格も高くなります。香り付けした紅茶ティーバッグに通常入っているのは高級茶葉ではなく、Dust等級です。FBOPF Extra Specialのような最高級茶は、輸出市場での価値が非常に高いため、国内で販売されることはめったにありません。",
        "新芽の位置や網目の大きさに加えて、選別された茶葉はFirst Mark、Second Mark、Third Markという3つの市場区分にも分類されます。ここで生産される主要な等級にはOPA、OP、OP1、BOP1、FBOP、FF1、FF、Pekoe、Pekoe1、BOP Special、BOPA、FF Extra Special、Extra Special 1、FF Specialが含まれ、価値の低いオフグレード茶は1A、BM、BP、FNGS、BTといった区分に分類されます。",
        "平均すると、生葉100kgから完成・梱包された紅茶約21.5kgが得られます。歩留まりにしておよそ21.5%で、残りは水分の蒸発と選別時に取り除かれる廃棄茶によって失われます。標準的な茶業委員会の試験ガイドラインでは、仕上げ茶1kgあたり生葉約4.65kgが必要とされています。工場では、主要等級で少なくとも生産量の65%を確保し、オフグレード茶を最大20%に抑え、残りをSecond Markなどの区分に振り分けることを目標としています。",
      ],
      duration: "選別は機械で連続的に行われ、1バッチあたり約30〜45分かかります。",
    },
    packing: {
      name: "梱包",
      sectionHeadings: ["計量、試飲、密封", "梱包エリアの等級", "等級ごとの袋の重量", "工場からオークションへ", "気密包装が重要な理由", "等級と市場のすり合わせ"],
      sectionBodies: [
        "等級付けされた茶は計量され、品質のために試飲され、アルミ箔で裏打ちされた防湿包装に密封されます。そこには工場の名前とロゴ、登録番号、セイロンティーのシンボル、そして食品安全証明書が印刷されています。",
        "梱包エリアには複数の完成した等級が並んでいます。Dust（主にティーバッグ用）、OP1、Pekoe、BOP（Broken Orange Pekoe）、OP（Orange Pekoe）です。OP1とOPは特にアラビア語圏の市場で人気があり、Pekoeはロシアとイランでよく知られています。ここで作られる最も希少で価値の高い等級はSFOPE1で、「Flowery Tippy」等級と呼ばれています。新芽の最上部から得られる淡く銀色のチップの断片を含んでいることからこの名がつき、力強さよりもその繊細な個性のために評価されています。",
        "袋の重量も等級によって異なります。決まった大きさの袋には、Dustのような薄く密度の高い等級（約58キロ）の方が、OPEのようなよりかさばる葉の多い等級（約30キロ）よりもはるかに多く入ります。",
        "工場は主に完成した茶をコロンボの茶商に引き渡し、彼らがそれをコロンボ茶オークションに持ち込みます。挙げられる名前にはForbes & Walker、Akbar Brothers、Finlaysなどがあります。そこで買い手がそれを購入し、多くの場合さらに付加価値を加えた後、自社ブランドで輸出します。アトゥコララという名前自体での直接輸出も行われますが、それは主に海外の買い手からの臨時の特別注文向けです。",
        "気密包装は茶を湿気と匂いから守り、工程の各段階で丁寧に築き上げられた風味を保ちます。",
        "国によって茶に求めるものは異なります。強く渋みのある一杯を好む国もあれば、そうでない国もあります。そのため、ある等級が新芽のどの部分から得られるかは、実質的に各輸出市場の好みに合わせて調整されています。例えば、アラビア語圏の市場は、自国での日常的な飲用習慣には含まれない特定の等級を、名指しで求めます。直接輸出するのではなく主に仲介業者を通じて販売するということは、その後の国際輸送やブランディングを通常は外部の買い手が担うことを意味します。",
      ],
    ...
    "artisanal-tea-grades": {
      ...
      sectionHeadings: [
        "職人技の高級茶",
        "Golden TipとSilver Tip",
        "White TeaとPurple Tea",
        "Pink TeaとVita Glow",
        "ブレンドと希少性",
        "Flower ChakraとTea Coin",
      ],
      sectionBodies: [
        "標準的な紅茶に加えて、この農園では希少な手作りの職人技の茶も生産しています。その多くは主力の紅茶ラインではなく、別棟の手作り茶工場で手作業により仕上げられます。だからこそこれらの等級は「職人技の高級茶」とも呼ばれ、大量生産というより一つの手仕事に近いものです。その多くは特徴的な紫色の新芽、すなわち茶業研究所が開発したクローン品種TRI 2043に由来します。その色はアントシアニンによるものです。収量の少ない品種のため、スリランカのすべての農園がこれを栽培しているわけではありません。",
        "Golden Tipはこの新芽の完全に成熟した芽を使い、Silver Tipは同じ新芽のより淡く、まだ未熟な芽を使います。その細く銀色の産毛が、この等級に名前と淡い輝きを与えています。芽だけを計量し、数時間萎凋させてから乾燥させます。これは完全発酵させる等級よりも簡単な工程で、ほのかに甘く、花のような香り、なめらかな口当たりの一杯に仕上がります。",
        "White Teaは新芽から二枚半の葉を摘み、萎凋を完全に省きます。紅茶のように発酵させて揉むのではなく、蒸してから乾燥させます。その結果、非常に軽く加工された、自然な甘みのある味わいになり、淡い水色、なめらかな口当たり、花のような香り、そして天然の抗酸化物質の豊富さを備えています。Purple Teaは同じ茶摘みを使いますが、White Teaとは異なり、萎凋・揉捻・発酵という紅茶の全工程を経ます。その結果、ウーロン茶に近い風味になります。紅茶より軽く、緑茶より青臭さが少なく、カフェインは比較的少なく、天然の抗酸化物質を豊富に含みます。",
        "Pink Teaは最初はWhite Teaと同様にほぼ無色ですが、ライムを数滴加えると初めてピンク色に変わります。柑橘が、新芽に色を与えるのと同じアントシアニン色素に変色反応を起こすのです。蜂蜜を数滴加えるのにも合い、アイスティーとして飲むことが推奨されており、これも天然の抗酸化物質を豊富に含みます。Vita Glowはこれとは異なり、紫色の新芽ではなく、通常の緑色の新芽の芽から作られます。",
        "農園ではまた、蓮の花を使ったブレンドなど付加価値のあるブレンド茶や、同じ新芽から手で結んだBlooming Teaも作っています。これらの職人技の茶は、標準的な紅茶よりもはるかに少ない量で摘み取られ加工されるため、より希少で貴重なものとなっています。試飲カウンターでお尋ねください。これらの等級のほとんどには、紫がかった新芽と通常の緑の新芽のどちらも使用できますが、農園では主に紫がかった新芽を使用しています。",
        "ここで作られる最も手間のかかる品は二つの少量生産の特産品です。一つはFlower Chakraで、それぞれ約二本の新芽を使う手巻きの紅茶リングです。もう一つはTea Coinです。一人が一日に手で巻けるのはどちらか一方につきわずか約25グラムほどで、これが高価な理由であり、日常的に飲むためというより、裕福な茶愛好家によって記念品として購入されることがほとんどです。Purple Teaを含め、手作り生産の多くは継続的に大量生産されるのではなく、注文に応じて少量ずつ作られます。",
      ],
      duration: "5分",
    },
    "pahatharata-manufacturing": {
      name: "紅茶製造（パハタラタ）",
      sectionHeadings: ["低地と高地の製法の違い", "CTC：クラッシュ・ティア・カール"],
      sectionBodies: [
        "この農園のような低地（「パハタラタ」）の農園は、内陸の高地農園とは異なる方法で茶を加工します。このツアーで先ほど段階ごとに紹介した農園自体の製造工程が、まさにそれを裏付けています。ここでの揉捻は、正統派の紅茶のように葉を丸ごと巻き上げるのではなく、小さく密な粒状に砕きます。これはCTC（クラッシュ・ティア・カール）加工の特徴です。このストップでは、この低地方式が摘採、加工、選別にとって何を意味するのかを簡単に振り返ります。",
        "CTC（クラッシュ・ティア・カール）は茶葉を機械的に小さく密な粒状に加工し、低地セイロン紅茶特有の力強くキレのある風味を生み出します。このツアーで先ほど揉捻や選別のストップで見た、まさにあの茶葉の粒です。",
      ],
      duration: "10〜15分",
    },
    "organic-cultivation": {
      ...
      sectionBodies: ["この農園で有機栽培されている茶品種はTRI 2043です。茶業研究所によって育成された中国系の品種で、通常の緑色ではなく、アントシアニン色素によって紫色に染まった新芽で知られています。合成農薬や化学肥料を使わずに栽培されているここの成熟したTRI 2043の木は、すでにGolden Tip、Silver Tip、White Tea、Purple Tea、Pink Teaといった特選等級を供給しています。これは茶の苗床にあるより若いTRI 2043の挿し木とは異なり、そちらはまだ試験段階で、大規模な植え付けはまだ承認されていません。TRI 2043はもともと収量が少ない品種で、そのためこの品種から作られる特選等級の供給はこれほど限られています。有機認証を得るには通常、数年間の無化学栽培が必要で、その間ずっと収穫量は慣行栽培の農地より少なくなります。"],
      duration: "5〜10分",
    },
    "clonal-tea": {
      name: "クローン茶",
      heroTagline: "農園自前の看板が立つ、クローン品種の試験栽培株の数々。",
      sectionHeadings: ["クローン茶", "クローン茶が高価な理由", "茶の品種", "TRI 3069", "TRI 3055", "TRI 4053", "TRI 2022", "TRI 2043", "TRI 4006", "TRI 3025"],
      sectionBodies: ["最も人気のある茶の一部は、種からではなく、単一の優れた親株から挿し木で増やされたクローン株から作られます。このストップでは、クローン茶がどのように増殖され、なぜ高値がつくのかを紹介します。茶業研究所（TRI）は、目的に応じてさまざまなクローン品種を育成しています。干ばつや雨への耐性、高い収量を目的としたものもあれば、手作りの特選茶に珍重される赤色または緑色の芽先を持つ品種などです。TRI 2023、TRI 2025、TRI 2026のような広く植えられている高収量品種は、大農園でも家庭の庭でも一般的ですが、より希少な特選品種は主に大規模農園で、日常栽培ではなく手作り茶のために栽培されています。これらのTRI番号は年号のように見えますが、実際には単に割り当てられた識別番号であり、各品種が発表された年ではありません。", "クローン繁殖は優れた親株の特性をそのまま保持します。最高級のクローン茶は生産量が非常に限られていることが多いです。赤色または緑色の芽先を持つ特選品種の芽は、Golden TipやSilver Tipといった希少な等級のために手摘みされ、農園では通常、標準的に摘まれた葉よりも1キログラムあたり約100ルピー多く支払われます。", "この農園は、地元の土壤や気候で確実に育つことから、茶業研究所（TRI）が開発した高収量で耐病虫害性のクローン品種を好んで用いています。TRI 2000シリーズやT65もこれに含まれます。TRI 4049は、スリランカ茶業研究所が開発した高収量の茶品種で、中低標高地域に適しています。カメリア・シネンシス（Camellia sinensis）の栄養繁殖品種であり、旺盛な生育、濃い緑色の葉、そして一般的な病害への耐性で知られています。TRI 4049は明るく香り高い水色の紅茶を生み出し、CTC製法・オーソドックス製法のいずれにも適しているため、改植や商業栽培において人気の高い選択肢となっています。", "数列先に立てられた札にはTRI 3069とあります。これもスリランカ茶業研究所に登録された品種で、TRI 4049と並べて植えることで、この土壌でそれぞれがどう育つかを苗床で比較しています。", "TRI 3055も同じ試験列に植えられています。ここではすべての品種区画が個別に札を付けて記録されており、こうして工場は評判だけに頼らず、実際に拡大栽培する価値のある品種を自らのデータで見極めています。", "写真の試験区画の最後を飾るのがTRI 4053です。複数の登録品種をそれぞれ札を付けて並べて育てるのは、本格的に畑へ植える前にこの農園で行われている標準的なやり方です。", "TRI 2022も同じ品種試験区画に植えられており、隣の株と同様に札を付けて追跡されています。同じ土壌で複数の登録品種を並べて育てることが、ここでどの品種が一番よく育つかという実際の証拠を苗床が積み重ねる方法です。", "TRI 2043は列のもう少し先に立っています。この試験区画にある他の番号付きの株と同様、本格的に植える判断が下される前に、育成と観察が続けられています。", "TRI 4006も同じ品種試験のもう一株です。苗床では各品種をそれぞれ明確に印を付けた場所に保ち、時間の経過とともに成長を追跡・比較しやすくしています。", "TRI 3025は試験区画の端に位置し、バイリンガルの「茶品種」の看板と日付入りの植樹記念プレートのそばにあります。ここに札を付けられた株々が、どの品種を拡大する価値があるかという苗床の実際の記録を形作っています。"],
      duration: "5〜10分",
    },
    "shade-management": {
      name: "シェードツリー（日陰木）管理",
      ...
      sectionBodies: ["茶樹には均衡の取れた日光が必要です。強すぎれば葉が焼け、暗すぎれば育ちません。そのため農園では茶樹の間に2層の遮光樹を植えてこれを管理しています。低木遮光樹は、現地ではマクラタ（ラダッパ、ヴァタマラ）として知られており、茶樹の近くに植えられ、地表温度を和らげ、土壌水分を保持し、天然の防風林として機能します。アルビジアなどの高木遮光樹は、樹冠が広く高い木で、農園の広い範囲にわたって強い直射日光を遮る一方、落ち葉などの有機物で土壌を豊かにします。農園の管理者は季節と標高に応じて遮光の状態を微調整するため、両方の層を定期的に剪定し、生物多様性と作物の健全性を守るために各遮光樹種と茶の品種の植物学名や特徴を記録しています。"],
      duration: "5分",
    },
    "unique-fruits": {
      name: "スリランカ固有の果物",
    ...
    "tea-factory": {
      ...
      sectionBodies: ["本館の工場は、摘み取られた茶葉が計量から梱包まで経て仕上げの紅茶になる場所です。このストップでは、稼働中の工場フロアと各工程で使われる設備を概観します。順に計量、萎凋、揉捻、発酵（酸化）、乾燥、茶葉選別（格付け）、そして最終梱包です。稼働中の紅茶工場は、計量から梱包までの全工程を一つの屋根の下で行い、毎日の茶葉の搬入量に合わせて緊密にタイミングを調整します。搬入された茶葉はまず計量され、その日の収穫量が記録されてから加工が始まります。工場フロアを見学する訪問者は、施設の衛生基準に従い、ヘアネット、マスク、保護シューズの着用をお願いしています。"],
      duration: "10〜15分",
    },
    "handmade-factory": {
      name: "手作り紅茶工房",
      heroTagline: "この農園自前の手作り紅茶工場のトレイラック。",
      sectionHeadings: [
        "手作り紅茶工房",
        "萎凋と揉捻",
        "緑茶、乾燥、そして建物",
      ...
      sectionBodies: [
        "機械化された生産に加えて、一部の茶はすべての工程を機械を使わず完全に手作業で加工します。農園の職人技による特選等級の多く、Golden Tip、Silver Tip、White Tea、Purple Tea、Pink Tea、Vita Glow、手で結んだBlooming Tea、そして蓮茶のような花をブレンドした茶は、主力の紅茶工場ではなく、ここで仕上げられます。（Vita Glowは、Golden TipやSilver Tipの紫色の新芽とは異なり、緑色の新芽の芽から作られます。）",
        "ここでは萎凋は自然に行われ、小さな槽の中で湿度計を使って管理されます。主力工場の強制送風ファンは使わず、揉捻前に水分を約50%まで下げます。その後、葉はトレイの上で手作業により揉まれ、時計ではなく感触で判断されます。大量の卸売注文に対しては、小型の機械式ローラーが同じ役割を担います。手揉みだけでは量に追いつかないためです。",
        "専用の緑茶機械、葉を蒸して圧搾するものが、その等級を別途扱っていますが、現在は軽微な修理のため停止しています。蒸した後、葉は屋外に置かれて表面の水分を飛ばし、その後、色、力強さ、香り、味を引き出すために引き伸ばしながら手で揉まれ、最後に乾燥機を通ります。乾燥時間は小規模生産者ごとに異なります。この建物の小さなオーブンは、ビスケットのような茶を使った軽食を焼くパン焼き場としても使われています。揉捻の直後に行う乾燥は、ここでは二台の別々の乾燥機で行われます。一台は伝統的な薪式、もう一台は電気式で、それぞれ34枚のトレイを収容でき、両方ともカルダモン、クローブ、生姜といった他の少量生産品にも使われています。",
        "このストップでは、手作り茶が工場のライン生産とどう違うかを紹介しています。手揉みと手作業による加工は、葉ごとのばらつきが大きい、より小さなロットを生み出し、その独特な個性がしばしば高く評価されます。手作り茶は大きく二つのカテゴリーに分かれます。一つは紅茶タイプの発酵茶（黒い色で見分けられます）で、これには珍しい元となる新芽を持つPurple Teaも含まれます。もう一つは、White Teaのように発酵を完全に省いて直接乾燥に進む非発酵茶です。",
      ],
      duration: "10分",
    },
    "tea-center": {
      name: "ティーセンター",
      heroTagline: "農園自前のテイスティングカウンターでお茶を試飲する訪問者たち。",
      sectionHeadings: [
        "ティーセンター",
        "抽出とテイスティング",
        "日常の等級とスーパーの茶",
      ...
      sectionBodies: [
        "見学の最後は茶センターです。ここでは試飲チームがすべてのバッチを検査します。ここだけでなく、茶が主力工場から地元市場に向かう時や、包装のために届いた時にも、それ以前から検査されています。過発酵、発酵不足、焦げといった欠陥がないかを確認し、販売前に不合格となったものはすべて取り除き、些細な問題でも記録して今後の生産に活かします。ここには手作り工場のさらに二つの少量生産の特産品、Flower ChakraとTea Coinも展示されています。",
        "こうした職人技の茶は、沸騰したお湯ではなく約80°Cのお湯を使って最大三回まで淹れ直すことができます。毎回新しいお湯を使い、それぞれ約2分、5分、8分かけて淹れます。これは、完全な沸騰状態で一度だけ淹れる標準的な紅茶とは異なります。試飲はワインのテイスティングに似た方法で行われます。まず香りを嗅ぎ、次に空気を吸い込みながら音を立ててすすり、舌の両側にある味蕾に茶を通します。チームは一日に最大50種類の茶を試飲する間、飲み込まずに吐き出しますが、訪問者は実際に試飲したものを飲むことを歓迎されています。正しく淹れた紅茶には、覆いをして約2.5分から3分の抽出時間が必要で、単なる素早い浸漬では足りません。",
        "BOPFとBOPF Specialは、ほとんどのスリランカ人が日常的に飲んでいる等級です。DustとDust 1という最も低い等級は輸出市場に送られることは決してなく、通常は着色料を加えられて安価なスーパーの茶葉パックに使われます。だからこそ、正しく抽出した後ではなく、浸した瞬間に色が出るのです。スーパーで売られているものの多くも、農園自体の未ブレンド・単一産地の茶とは異なり、他所から仕入れた安価な茶とブレンドされています。",
        "最高級の等級（FF Extra Special、FF Special、FBOP）は収穫のわずか約1%、新芽の最も未熟な部分からしか得られず、100グラムあたり約25ドルで販売されます。OPAやOPのようなよりかさばる等級は200グラムあたり約5〜6ドルです。最高級の等級に見られる白いチップの粒は追加の甘みを加え、その多くはヨーロッパの買い手に渡り、一方でアラブ市場はより濃い味わいを好む傾向があります。",
        "茶以外にも、農園は自家製の緑茶と紅茶（約90%）を地元のハーブ（約10%：ゴツコラ、ヒーンボビティヤ、モリンガ）とブレンドし、自家製のスパイスからMasala、シナモン、ジンジャーのスパイスティーを作っています。Masalaのレシピにはカルダモン、シナモン、ジンジャー、黒胡椒を含む七種類のスリランカのスパイスが使われており（残りは秘密にされています）、ミルクティーにはMasalaかBOPが推奨されています。",
      ],
      duration: "10〜15分",
    },
    "coconut-field": {
    ...
    introduction: {
      ...
      sectionBodies: [
        ...
        "訪問者は伝統的に「アーユボーワン」で迎えられます。これは「長寿を」という意味のシンハラ語の挨拶で、時には農園の会長であるアヌラ・アトゥコララ氏自らが直接出迎えることもあります。",
      ],
      duration: "5〜10分",
    },
    "jaggery-palm": {
      ...
      sectionBodies: [
        "キトゥルヤシ（Caryota urens）は、キトゥルパーム、またはフィッシュテールパームとも呼ばれ、スリランカ、インド、その他の熱帯アジア諸国に分布する背が高く優美なヤシで、その多目的な利用価値で重宝されています。最も注目すべき産物は花茎から採取される甘い樹液で、これを煮詰めて伝統的なキトゥルシロップとジャガリーを作ります。この木はまた、頑丈な繊維質の幹と美しい魚の尾のような形の葉でも知られています。",
        "ヤシそのものを簡単に見てみましょう：",
    ...
    pruning: {
      ...
      sectionHeadings: ["剪定"],
      sectionBodies: ["剪定を行う前に、農園ではすぐに刈り込むのではなく、あえて一定期間茶樹を伸ばすままにしておきます。これにより新しい芽が十分に育ち、茶樹に養分が蓄えられます。この養分の蓄積が完了して初めて剪定が始まり、茶樹はおよそ膝の高さまで刈り込まれます。現地の看板では、これを2つの名前の付いた段階として示しています。第一次十字剪定（Palamu Haras Kapuma）で茶樹は約9〜12インチ（22〜30cm）まで切り詰められ、第二次十字剪定（Dewana Haras Kapuma）で再び約16〜18インチ（40〜45cm）まで伸びるのを待ちます。状態に関係なく決まった予定で剪定するのではなく、まず茶樹を伸ばすままにしておくことで、刈り込まれる前に養分を上へ送る時間を植物に与えます。"],
      duration: "5分",
    },
    polytunnel: {
      name: "ビニールハウスと温室",
      heroTagline: "農園自前のビニールハウスで、すでに最初の作物が育っています。",
      sectionHeadings: ["ビニールハウスと温室について", "水やりと灌漑", "収穫までの期間", "施肥灌漑コントロールヘッド", "マル・ミリス（パプリカ）", "アム・ミリス（唐辛子）", "ナイ・ミリス", "ゲディ・ゴワ（キャベツ）", "アトゥ・ゴワ（赤キャベツ）", "ボンチ（さやいんげん）", "ラーブ（ラディッシュ）", "ニンジン", "リーキ", "キュウリ", "トマトの苗"],
      sectionBodies: [
        "既存の温室に加えて、農園では自家製のさまざまな野菜を栽培するためのビニールハウスを建設中です。その中にはスコッチボネットや青唐辛子、ピーマン、ガーキン、トマト、パプリカ、キャベツ、ラディッシュ、さやいんげん、リーキ、青ネギ、ニンジンなどが含まれます。完成すれば、ヴィラやキャビンに宿泊するゲストは、ここで栽培された新鮮な野菜を自分の手で収穫できるようになります。ビニールハウスの半透明の覆いは温室のガラスとよく似た働きをし、暖かさを保ちながら光を通す一方で、激しいモンスーンの雨や一般的な害虫の多くを遮ります。こうした環境のおかげで、農園は露地では安定して育てられない野菜も栽培でき、屋外の畑よりも栽培期間を延ばすことができます。",
        "それぞれの作物は生育段階に応じて、根元への点滴灌漑と上からのミスト散布を切り替える独自の水やりスケジュールに従います。ミストは主にニンジンやラディッシュなどの作物の発芽初期と冷却に役立ちます。トマトなどの作物では、結実が始まると点滴灌漑が主な方法になります。この2つの方法にはそれぞれ異なる役割があります。ミスト散布は、若い種や苗が根付くのに十分な湿度を空気と土の表面に保ちます。一方、点滴灌漑は植物が大きくなった後、水を根の部分に直接届け、全体の使用水量を抑えるとともに葉を乾いた状態に保ち、カビによる病気のリスクを減らします。",
        "最初の収穫までにかかる時間は作物によって大きく異なり、ラディッシュのわずか25〜35日から、リーキの90〜120日まで幅があるため、ビニールハウスではシーズンのほぼどの時期にも収穫できるものがあるはずです。生育の早い野菜と遅い野菜をこのように並べて育てること自体が意図的な工夫であり、すべてが同時に収穫期を迎えたり、同時に終わったりするのではなく、自然と収穫時期をずらすことにつながっています。",
        "このコントロールヘッドは、各栽培袋につながる点滴チューブに水を供給します。堆積物を取り除くフィルター、圧力計、配管の区画を個別に遮断できる止水バルブ、そして灌漑水が通過する際に液体肥料を直接注入するベンチュリー式インジェクターを備えた2基の肥料タンクから成り、これにより各株が同じ工程で水やりと施肥を同時に受けられます。",
        "対になって生える光沢のある先の尖った楕円形の葉を持つこの苗はマル・ミリス、パプリカの一種（Capsicum annuum）です。ヤシガラ繊維を詰めた栽培袋で育てられており、根元にはすでに点滴チューブが取り付けられています。",
        "この苗はアム・ミリス、唐辛子の一種（Capsicum annuum）です。この初期段階では、パプリカの葉とほとんど見分けがつきません。両者は同じ種に属し、異なる用途のために栽培されているためです。",
        "ナイ・ミリスという名前は、シンハラ語で「コブラ唐辛子」を意味します。これはCapsicum chinenseで、農園の他の唐辛子とは異なる、一般的により辛味の強い種であり、ハバネロやスコッチボネットなどの品種と近縁です。",
        "この苗のより丸みを帯びた、やや蝋質のような葉はゲディ・ゴワ（文字通り「玉キャベツ」）に属します。これは結球する一般的なキャベツ（Brassica oleracea var. capitata）で、結球が始まるずっと前からここに植えられています。",
        "この苗の赤紫色の葉はアトゥ・ゴワ（文字通り「枝キャベツ」）に属します。これは玉を結ばない、より緩やかなキャベツの品種（Brassica oleracea var. acephala）で、結球するタイプとは異なります。",
        "この幅広いハート形の葉はボンチ、さやいんげん（Phaseolus vulgaris）に属します。一度育ち始めるとつる性になり、周囲の低く育つ野菜とはまったく異なる成長の仕方をします。",
        "まだ子葉の段階にあるこの小さな苗はラーブ、ラディッシュ（Raphanus sativus）です。ビニールハウスの中でも収穫までが最も早い作物の一つで、わずか25〜35日で収穫できます。",
        "ニンジン（Daucus carota subsp. sativus）は、この早い段階でも、草のように細い最初の葉によって見分けることができ、ここで栽培されている他の野菜の幅広い子葉とはまったく異なります。",
        "リーキ（Allium ampeloprasum）は一枚の細い葉として現れ、ビニールハウスの中でも収穫までが最も遅い作物の一つです。植え付けから収穫まで90〜120日かかることもあります。",
        "この苗の幅広く丸みを帯びた2枚の子葉は、サラダキュウリ（Cucumis sativus）の子葉であり、より本来のハート形に近い本葉が現れる前の姿です。",
        "このトマトの苗はすでに、トマトの木に特徴的な、切れ込みのある鋸歯状でやや産毛のある葉を見せています。トマトはまさに、結実が始まるとミスト散布から点滴灌漑に切り替わる例として先に挙げた作物です。",
      ],
      duration: "5分",
    },
    "mushroom-hut": {
      name: "きのこ小屋",
      sectionHeadings: ["一味違う庭", "ヒラタケの品種", "培地と種菌", "収穫サイクル", "栄養と健康効果"],
      sectionBodies: [
        "茶、スパイス、果物の作物に加えて、農園にはきのこ専用の小屋があります。これは小さな日陰の建物で、庭の他の部分が日光と通気のために育てられているのとは異なり、きのこが実をつけるために必要な暗く涼しく湿った環境を保つために建てられています。",
        "この小屋では、アメリカンヒラタケ（Pleurotus pulmonarius）、ブータンヒラタケ、ブラックヒラタケ、ピンクヒラタケ（Pleurotus djamor）、アワビタケ（Pleurotus cystidiosus）という複数の品種が栽培されています。いずれも土ではなく藁やおがくずを基質として育てられ、すべてヒラタケ属（Pleurotus）に属しています。個々の茎ではなく基質から直接、扇形または貝殻状の傘が密集して重なり合うように育つのが特徴です。品種を分ける主な違いは色で、白や灰色からピンク、金色、ほぼ黒に近いものまで幅がありますが、いずれも穏やかでかすかにうま味のある風味はほとんど変わりません。",
        "栽培は基質そのものから始まります。藁やおがくずは、農園の低コストで廃棄物を活用する方針を支える同じ素材です。これを水に浸して熱で殺菌し、競合するカビや細菌を除去したうえで、きのこの菌糸が完全に行き渡った穀粒である種菌と混ぜ合わせ、通気穴のある袋やブロックに詰めます。そのブロックは2〜3週間ほど「菌糸の伸長期間」として暗所に置かれ、その間に白い菌糸が基質全体に広がります。その後ようやく、小屋の暗く涼しく湿った環境が引き継いで、きのこの発生を促します。",
        "このようにきのこを育てることは、農園が主要作物以外に多角化するための低コストで持続可能な方法であり、藁のような農業廃棄物を栽培基質として利用することが多くあります。一つの基質のバッチから数週間かけて何度も収穫できます。1つのブロックからは通常、1〜2週間ほどの間隔をあけて2〜4回ほどこうした収穫の波が得られ、最初の収穫が最も多く、その後は徐々に減っていきます。きのこは切り取るのではなく、株ごとひねって収穫します。残った切り株が腐ると、次の収穫を台無しにしかねない汚染を招くおそれがあるためです。",
        "農園自身の看板には、それぞれの品種の栄養面についての説明があります。アメリカンヒラタケ、ピンクヒラタケ、アワビタケはいずれもタンパク質、ビタミン、抗酸化物質が豊富とされ、免疫力のサポート、心臓や消化器系の健康、炎症の軽減、コレステロールの低下といった効果が挙げられています。",
      ],
      duration: "5分",
    },
  },
